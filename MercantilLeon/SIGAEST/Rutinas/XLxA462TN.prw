...
User Function XLxA462TN(lUsaFiscal)
	...
	If lContinua
		...
		cQuery += " AND CP_FILIAL = '" + _xfilial + "' AND SCP.D_E_L_E_T_=' ' "   
	...
	If SCP->(!Eof())
		...
		For nCntFor := 1 To Len(aHeader)
			Do Case
				...
				aCols[nItem,nCntFor] := GETNEWPAR("MV_UTESSAL", "701") //SC7->C7_TES
				Case Trim(aHeader[nCntFor,2]) == "D2_TESENT" //.And. !Empty(SC7->C7_TESENT)
				aCols[nItem,nCntFor] := GETNEWPAR("MV_UTESENT", "301") //"301" //SC7->C7_TESENT
				...
				Case Trim(aHeader[nCntFor,2]) == "D2_UESPECI"
				aCols[nItem,nCntFor] := SB1->B1_ESPECIF
				Case Trim(aHeader[nCntFor,2]) == "D2_UESPEC2"
				aCols[nItem,nCntFor] := SB1->B1_UESPEC2
