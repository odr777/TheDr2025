...
±±Descripcion ³Impresion de Remitos de compra y venta       		      º±±
±±º          ³                                                            º±±
...
Private	aPre_Lot   		:= {} //solange
Private	aPre_Fel   		:= {} //solange
...
Else	
	...
	While !eof() .and. SF1->F1_DOC <= mv_par02 .and. SF1->F1_SERIE == mv_par03 .and. lContinua
		...
		While sd1->(!eof()) .and. SF1->F1_FILIAL+DTOS(SF1->F1_EMISSAO)+SF1->(F1_DOC+F1_SERIE+F1_FORNECE+F1_LOJA) ==;
			...
			AADD(aPedido   ,IIF(Empty(SD1->D1_PEDIDO),"",SD1->D1_PEDIDO))   // Ordem de Compra
...
IF(FUNNAME()=='MATA462N')
   @ 06, 050 PSAY "R E M I T O  D E  V E N T A"
   @ 07, 006 PSAY "Nota Factura: "
   @ 07, 020 PSAY cNum_NF               // Numero da Nota Fiscal 
else   
   @ 06, 006 PSAY PADC(cTitulo_cab,213)
   @ 07, 006 PSAY "Número de Remito: " + alltrim(cNum_NF) // Numero da Nota Fiscal
   @ 07, 035 PSAY "Serie: " +cSerie              // Serie
endif   
...
IF(FUNNAME()=='MATA462N')  
	@ 08, 006 PSAY "CLIENTE : "+cCod_Cli + " - " + cNome_Cli
else
	@ 08, 006 PSAY "Proveedor : "+cCod_Cli + " - " + cNome_Cli
end     
...
IF(FUNNAME()<>'MATA462N')    
  if mv_par04 == 2
	  @ 35, 088  PSAY "valor Mercaderia: " + TRANSFORM(nValor_Merc,"@E 9,999,999.99")
	  @ 36, 088  PSAY "valor Flete     : " + TRANSFORM(nFrete,"@E 9,999,999.99")
	  @ 37, 088  PSAY "valor Seguro    : " + TRANSFORM(nSeguro,"@E 9,999,999.99")
	  @ 38, 088  PSAY "Total Factura   : " + TRANSFORM(nTot_Fat,"@E 9,999,999.99") + Iif(nMoneda ==1,"  BOLIVIANOS","  DOLARES")
  Else    
      nLin:=nLin+1
	  @ nLin, 088  PSAY "valor Mercaderia: " + TRANSFORM(nValor_Merc,"@E 9,999,999.99")
      nLin:=nLin+1
	  @ nLin, 088  PSAY "valor Flete     : " + TRANSFORM(nFrete,"@E 9,999,999.99")
      nLin:=nLin+1
	  @ nLin, 088  PSAY "valor Seguro    : " + TRANSFORM(nSeguro,"@E 9,999,999.99")
      nLin:=nLin+1
	  @ nLin, 088  PSAY "Total Remito   : " + TRANSFORM(nTot_Fat,"@E 9,999,999.99") + Iif(nMoneda ==1,"  BOLIVIANOS","  DOLARES")
      nLin:=nLin+1
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
if(mv_par04!=2)
@ nLin, 00 PSAY "     CODIGO           DESCRIPCION                                     UNID.      CANTIDAD        VAL UNIT.     VALOR TOTAL     TES  PED.COMPRA     SOL.COMPRA"
else
@ nLin, 00 PSAY "     CODIGO           DESCRIPCION                                     UNID.      CANTIDAD        VAL UNIT.     VALOR TOTAL     TES         PED.VENTA      
endif
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  nLin := nLin+1
@ nLin, 00 PSAY REPLICATE('-',213) 
...
For _I:=1 to nTamDET
    IF _I <= Len(aCod_Pro)                                                                            
		...
		@ nLin, 005  PSAY Substr(aCod_Pro[_I],1,15)	Picture "@!"
		@ nLin, 022  PSAY  Substr(aDescricao[_I],1,45)	
//	   	@ nLin, 078  PSAY RTRIM (aCod_Trib[I])
		@ nLin, 071  PSAY aUnid_Pro[_I]
		@ nLin, 077  PSAY aQtd_Pro[_I]               Picture"@E 999,999.99"
		@ nLin, 091  PSAY aPre_Uni[_I]               Picture"@E 99,999,999.99"
		@ nLin, 108  PSAY aVal_Merc[_I]              Picture"@E 99,999,999.99"                  
//		@ nLin, 122  PSAY aPre_Lot[_I]      nahim     
//		@ nLin, 134  PSAY aPre_Fel[_I]      nahim 
		@ nLin, (145- 17)  PSAY aTES[_I]
		if(mv_par04==2 .and. aPed_Vend[_I] != "" )
			@ nLin, (160- 18)  PSAY aPed_Vend[_I]
//		    @ nLin, (165- 15)  PSAY POSICIONE('SC6',1,XFILIAL('SC6')+ aPed_Vend[_I] + aItem_Ped[_I] + aCod_Pro[_I] ,'C6_NUMSC')
		
		else
		if (aPedido[_I] != "")
				@ nLin, (150- 15)  PSAY aPedido[_I]
				@ nLin, (165- 15)  PSAY POSICIONE('SC7',3,XFILIAL('SC7')+ SF1->F1_FORNECE + SF1->F1_LOJA+ aPedido[_I],'C7_NUMSC')
			endif
		endif
		if len(alltrim(aDescricao[_I]))>45
		 Nlin++
		 @ nLin, 022 PSay alltrim(SubStr(aDescricao[_I],46,80))
		 endif
		if !Empty(aPre_Lot[_I])
			nLin++      // nahim
			@ nLin, 022  PSAY aPre_Lot[_I]   // nahim	
		endif
		J:=J+1 
    ELSE
       _i:=ntamdet
...
IF(FUNNAME()=='MATA462N')
	L++ 
	@ L, 088  PSAY "valor Mercaderia: " + TRANSFORM(nValor_Merc,"@E 9,999,999.99")
	L++
	@ L, 088  PSAY "valor Flete     : " + TRANSFORM(nFrete,"@E 9,999,999.99")
	L++
	@ L, 088  PSAY "valor Seguro    : " + TRANSFORM(nSeguro,"@E 9,999,999.99")
	L++
	@ L, 088  PSAY "Total Factura   : " + TRANSFORM(nTot_Fat,"@E 9,999,999.99") + Iif(nMoneda ==1,"  BOLIVIANOS","  DOLARES")
ENDIF
Return
