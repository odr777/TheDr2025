...
±±ºPrograma  ³  TRANS2SAL  ºAutor  ³Nahim Terrazas        Fecha 20/04/2016   º±±
...
Static  Function IMPREP(lEnd,WnRel,cString,cPerg,tamanho,titulo,cDesc1,cDesc2,cDesc3)
	//Transaccion de salida Mercantil B1_UCODFAB   15

	cQuery := "select F2_FILIAL,F2_DOC,F2_EMISSAO,F2_FILDEST,F2_UNROREC,F2_UOBSERV,B1_DESC,D2_LOTECTL,D2_CUSTO1,D2_UM,(D2_QUANT*D2_CUSTO1) as total,A1_NOME, max(A1_NOME) as cli1,min(A1_NOME) as cli2,"//c6_num,
	cQuery += "D2_ITEM,D2_COD,D2_QUANT,D2_PRCVEN,D2_TOTAL,D2_PEDIDO,D2_ITEMORI,D2_NFORI ,F2_MOEDA,NNR.NNR_DESCRI as almdes,D2_LOCDEST,F2_SERIE,D2_LOCAL,D2_LOTECTL,D2_LOCALIZ,D2_NUMSERI,F2_USRREG,B1_ESPECIF,B1_UESPEC2,B1_UDESCMA,B1_UMARCA,B1_UCODFAB,F2_DTDIGIT  "//B2_QATU,B2_CM1  "
	...
	cQuery += " group by F2_FILIAL,F2_DOC,F2_EMISSAO,F2_FILDEST,F2_UNROREC,F2_UOBSERV,B1_DESC,D2_CUSTO1,D2_UM,D2_ITEM,D2_COD,D2_QUANT,D2_PRCVEN,D2_TOTAL,D2_PEDIDO,D2_ITEMORI,D2_NFORI,F2_MOEDA,NNR.NNR_DESCRI,D2_PRCVEN,D2_TOTAL,A1_NOME,D2_LOCDEST,F2_SERIE,D2_LOCAL,D2_LOTECTL,D2_LOCALIZ,D2_NUMSERI,F2_USRREG,B1_ESPECIF,B1_UESPEC2,B1_UDESCMA,B1_UMARCA,B1_UCODFAB,F2_DTDIGIT     "//,c6_num,
...
Static Function fImpCab()
	...
	nNroLin 	:= 60
	oPrint:Say( nNroLin, nNroCol-10, "MERCANTIL LEON SRL", oFontSTitn)
	...
	nNroLin	+= 80
	nNroLin	+= 80;	oPrint:Say(nNroLin,0120,"DOCUMENTO: TRF - "	+ OrdenesCom->F2_DOC,oFont10n)
	oPrint:Say(nNroLin,1400,"FECHA: "+ ffechalatin(OrdenesCom->f2_EMISSAO) 	,oFont10n)
	nNroLin	+= 80;	oPrint:Say(nNroLin,0120,"SUCURSAL DE ORIGEN: " + OrdenesCom->F2_FILIAL,oFont10n)
	oPrint:Say(nNroLin,1400,"NUMERO RECIBO: " + OrdenesCom->F2_UNROREC	,oFont10n)
	nNroLin	+= 80;	oPrint:Say(nNroLin,0120,"SUCURSAL DE DESTINO: "	+ OrdenesCom->F2_FILDEST ,oFont10n)
	oPrint:Say(nNroLin,1400,"DEPOSITO DE ORIGEN: " + POSICIONE('NNR',1,XFILIAL('NNR')+ OrdenesCom->D2_LOCAL,'NNR_DESCRI')	,oFont10n)
	nNroLin	+= 80;	oPrint:Say(nNroLin,0120,"ELABORADO POR: " +	upper(OrdenesCom->F2_USRREG ),oFont10n)
	oPrint:Say(nNroLin,1400,"DEPOSITO DE DESTINO: " + 	POSICIONE('NNR',1,XFILIAL('NNR')+ OrdenesCom->D2_LOCDEST,'NNR_DESCRI'),oFont10n)
	
	nNroLin	+= 80;	oPrint:Say(nNroLin,0120,"FECHA DIGITACION: " + ffechalatin(OrdenesCom->F2_DTDIGIT) ,oFont10n)
	oPrint:Say(nNroLin,1400,"FECHA DE IMPRESI�N: " + ffechalatin(Dtos(date())),oFont10n)
	nNroLin	+= 80;oPrint:Say(nNroLin,0120,"PESO NETO: "+ TRANSFORM(pesototal,"@E 999,999.99")	,oFont10n)
	oPrint:Say(nNroLin,1400,"IMPRESO POR: " + SUBSTR(CUSERNAME,1,15),oFont10n)
	nNroLin	+= 80;oPrint:Say(nNroLin,0120,"OBSERVACION: " + substr(upper(OrdenesCom->F2_UOBSERV ),1,93),oFont10n)
	nNroLin	+= 80;oPrint:Say(nNroLin,0120, SUBSTR(upper(OrdenesCom->F2_UOBSERV ),93,107),oFont10n)
...
// Especifica	B1_ESPECIF B1_UESPEC2
	oPrint:Say(nNroLin,0130,OrdenesCom->D2_ITEM,oFont09)
	oPrint:Say(nNroLin,0235,OrdenesCom->D2_COD,oFont09)
	oPrint:Say(nNroLin,0560,OrdenesCom->B1_UCODFAB,oFont09)
//	oPrint:Say(nNroLin,1830,OrdenesCom->B1_UMARCA,oFont09)	
	oPrint:Say(nNroLin,1980,OrdenesCom->D2_UM,oFont09)
	oPrint:Say(nNroLin,2100, TRANSFORM(OrdenesCom->D2_QUANT,"@E 999,999.99"),oFont09)
	oPrint:Say(nNroLin,0900,SUBSTR(OrdenesCom->B1_ESPECIF,1,45),oFont09)
	if len(alltrim(OrdenesCom->B1_ESPECIF))>45  //OrdenesCom->B1_ESPECIF
		nNroLin+= 30
		oPrint:Say(nNroLin,0900,alltrim(SubStr(OrdenesCom->B1_ESPECIF,46,80)),oFont09)
	endif
// Especifica2
	if !Empty(OrdenesCom->B1_UESPEC2) //OrdenesCom->B1_UESPEC2
		nNroLin+= 40
		oPrint:Say(nNroLin,0900,alltrim(SubStr(OrdenesCom->B1_UESPEC2,1,45)),oFont09)
		if len(alltrim(OrdenesCom->B1_UESPEC2))>45
			nNroLin+= 30
			oPrint:Say(nNroLin,0900,alltrim(SubStr(OrdenesCom->B1_UESPEC2,46,80)),oFont09)
		endif
	endif
// lote
	if !Empty(OrdenesCom->D2_LOTECTL)//OrdenesCom->D2_LOTECTL
		nNroLin += 40
		oPrint:Say(nNroLin,0900,OrdenesCom->D2_LOTECTL,oFont09)
	endif
	if !Empty(OrdenesCom->B1_UMARCA)// marca
		if !Empty(OrdenesCom->B1_UDESCMA) // descripcion de la marca
			nNroLin += 40
			oPrint:Say(nNroLin,0900,OrdenesCom->B1_UMARCA + "   " + OrdenesCom->B1_UDESCMA,oFont09)
		endif
	endif
	nNroLin += 40
