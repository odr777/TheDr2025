...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
If lRastroCTB
	wnrel:=SetPrint(cString,wnrel,,Titulo,cDesc1,cDesc2,cDesc3,.F.,,.F.,,,.F.)
Else
...
If lRastroCTB .And. !Empty(CTL->CTL_KEY)
	cChave    := &(CTL->CTL_KEY)
	mv_par01  := Substr(cChave,3,TamSX3("EL_RECIBO")[1])
	mv_par02  := Substr(cChave,3,TamSX3("EL_RECIBO")[1])
EndIf
...
If cPaisLoc $ "ARG" .AND. lVersao
	...
	cChaveSEL := "EL_FILIAL+EL_SERIE+EL_RECIBO"
...
While !Eof() .And. ( xFilial("SEL")==SEL->EL_FILIAL ) .And. SEL->EL_RECIBO <= mv_par02 .And. SEL->EL_SERIE == cSerie;
	...
	If Len(aChqs) > 0
		For nA := 1  to Len(aChqs)
		    ...
		    If cPaisloc $ "PTG|ANG|MEX"  
				...
				@nLin+nP,018 PSAY aCHQS[nA][3] PICTURE TM(aCHQS[nA][3],16,MsDecimais(Val(aCHQS[nA][10]))) //"@R 999,999,999.99"  //Valor do Doc.
	...
	If Len(aNfs) > 0
		...
		For nA:=1  to Len(aNfs)                                         //6        7         8
			...
			If cPaisLoc $ "PTG|ANG|MEX"
			  	@nLin+nP,028 PSAY IIf(AllTrim(aNfs[nA][8])$ mv_crneg+"|"+mvrecant,aNfs[nA][3]*-1,aNfs[nA][3]) PICTURE TM(aNfs[nA][3],16,MsDecimais(Val(aNfs[nA][7]))) //"@R 99,999,999,999"  //Valor do Doc.
				@nLin+nP,046 PSAY aNfs[nA][4]  //Simbolo da moeda
				@nLin+nP,052 PSAY aNfs[nA][5]  //Data vencto.
			  	@nLin+nP,062 PSAY IIf(AllTrim(aNfs[nA][8])$ mv_crneg+"|"+mvrecant,aNfs[nA][6]*-1,aNfs[nA][6]) PICTURE TM(aNfs[nA][6],21,MsDecimais(Val(aNfs[nA][7]))) //"@R 999,999,999.99"  //Valor na moeda 1
			Else
			  	@nLin+nP,021 PSAY IIf(AllTrim(aNfs[nA][8])$ mv_crneg+"|"+mvrecant,aNfs[nA][3]*-1,aNfs[nA][3]) PICTURE TM(aNfs[nA][3],21,MsDecimais(Val(aNfs[nA][7]))) //"@R 99,999,999,999"  //Valor do Doc.
	...
	ElseIf Len(aChqs) > 0
		...
		@ 53,054 PSAY "|"+Transform(nTotal,TM(nTotal,21,MsDecimais(Val(aChqs[1][10]))))		// "Total en "
	ElseIf Len(aNFs) > 0
...
If cPaisLoc $ "ARG" .AND. lVersao
	@ 02,008 PSAY OemToAnsi(STR0030)+IIf(Empty(cSerRec),'    ',cSerRec+"-")+cRecibo + "/" + Str(nCntRcbo,2) + " Rev. " + cVersao //"Recibo Nro: "
	@ 02,003 PSAY "|"
Else
	@ 02,048 PSAY OemToAnsi(STR0030)+IIf(Empty(cSerRec),'    ',cSerRec+"-")+cRecibo + "/" + Str(nCntRcbo,2)  //"Recibo Nro: "
	@ 02,084 PSAY "|"
EndIf
