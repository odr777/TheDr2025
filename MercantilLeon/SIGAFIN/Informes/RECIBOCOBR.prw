...
#Define COL_TDA  0100
#Define COL_VENCTO  0130
#Define COL_VALOR	0195
#Define COL_MONEDA	0275
#Define COL_IMPORTE 0320
#Define COL_BANCO 0310
#Define COL_AGENCIA 0350
#Define COL_COBRADO 0410
#Define COL_SALDO 0495
...
User Function RECIBOCOBR(cReciboco,cSerieco)
	LOCAL cString		:= "SD3"
	Local titulo 		:= ""
...
Static Function MOVD3CONF(Titulo)
	...
	cArquivo  := MV_PAR01 + "_" + MV_PAR03 + "_" + dToS(date()) + "_" + StrTran(time(), ':', '-')
...
Static Function fImpPres(lEnd,WnRel,cString,nReg)
	...
	if ! VW_DBA->(EoF())
		...
		While ! VW_DBA->(EoF())
			...
			oPrint:SayAlign(nLinAtu, COL_SALDO - 30, "$US",     oFontDet, 0130, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_SALDO + 10, ALLTRIM(TRANSFORM(TOTALE,"@E 999,999,999.99")) ,     oFontDet, 0130, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			nLinAtu += (nTamLin * 2)
			...
			oPrint:SayAlign(nLinAtu, COL_SALDO -30 , "Bs.",     oFontDet, 0130, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_SALDO + 10 ,ALLTRIM(TRANSFORM(TOTALBS,"@E 999,999,999.99")),     oFontDet, 0130, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			...
			if ! VW_MED->(EoF())
				...
				oPrint:SayAlign(nLinAtu, COL_SALDO, "FECHA EMISION" ,     oFontDetn, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
				nLinAtu += nTamLin
				oPrint:Line(nLinAtu, nColIni, nLinAtu, nColFin, CLR_BLACK)
				While ! VW_MED->(EoF())
					If nLinAtu + nTamLin > nLinFin
					...
					cFecha	:= 		alltrim(VW_MED->EL_EMISSAO)
			...
			oPrint:SayAlign(nLinAtu, COL_TDA + 20, "$US: ",     oFontDet, 0130, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_TDA + 50, ALLTRIM(TRANSFORM(TCDOLAR,"@E 999,999,999.99")),     oFontDet, 0130, nTamLin, CLR_BLACK, PAD_LEFT, 0)//TCDOLAR
			...
			oPrint:SayAlign(nLinAtu, COL_AGENCIA, "Saldo deudor total $US: ",     oFontDet, 0130, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_SALDO + 35,ALLTRIM(TRANSFORM(SALDO,"@E 999,999,999.99")),     oFontDet, 0130, nTamLin, CLR_BLACK, PAD_LEFT, 0)//SALDO
...
Static Function fImpCab()
    ...
    OBSV := AllTrim(VW_DBA->EL_OBSBCO)
	cUSERLGI := GetAdvFVal("SAQ","AQ_NOME",xFilial("SAQ")+VW_DBA->EL_COBRAD,1,"")    //AllTrim(VW_DBA->EL_COBRAD) //UsrFullName(SUBSTR(EMBARALHA(VW_DBA->EL_USERLGI,1),3,6 ) )	//EDUAR 30/06/2020
	...
	oPrint:SayBitmap(040,10, cFLogo,170,030)
    ...
    aDtTemp := dtaCobr(VW_DBA->EL_RECIBO,VW_DBA->EL_SERIE)

	nLinCab += nTamLin
	oPrint:SayAlign(nLinCab, COL_COBRADO, "Digitacion:",     oFontDet, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
	oPrint:SayAlign(nLinCab, COL_COBRADO+070, DTOC(STOD(VW_DBA->EL_DTDIGIT)),     oFontDet, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
    ...
    if aDtTemp[2] != ''
        oPrint:SayAlign(nLinCab, COL_COBRADO, "Fecha Rev1:",     oFontDet, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
        oPrint:SayAlign(nLinCab, COL_COBRADO+070,DTOC(STOD(aDtTemp[2])) ,     oFontDet, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
    endif
	nLinCab += nTamLin ///TELEFONO
	oPrint:SayAlign(nLinCab, COL_SERIE, TELEMPR,     oFontDet, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
    if aDtTemp[3] != ''
        oPrint:SayAlign(nLinCab, COL_COBRADO, "Fecha Rev2:",     oFontDet, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
        oPrint:SayAlign(nLinCab, COL_COBRADO+070,DTOC(STOD(aDtTemp[3])),     oFontDet, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
    endif

	nLinCab += nTamLin
    if aDtTemp[4] != ''
        oPrint:SayAlign(nLinCab, COL_COBRADO, "Fecha Rev3:",     oFontDet, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
        oPrint:SayAlign(nLinCab, COL_COBRADO+070,DTOC(STOD(aDtTemp[4])),     oFontDet, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
    endif
	nLinCab += nTamLin
	...
	oPrint:SayAlign(nLinCab, COL_SERIE + 110, NOMBRECL ,     oFontDet, 0500, nTamLin, CLR_BLACK, PAD_LEFT, 0)//nomcli
    nLinCab += nTamLin
	oPrint:SayAlign(nLinCab, COL_SERIE, "Cliente: ",     oFontDet, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
	oPrint:SayAlign(nLinCab, COL_SERIE + 40, CODCL,     oFontDet, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)//CODCLI
	nLinCab += (nTamLin * 1.2)
    oPrint:SayAlign(nLinCab, COL_SERIE, "Nit/Ci: ",     oFontDet, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
	oPrint:SayAlign(nLinCab, COL_SERIE + 70, NIT ,     oFontDet, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)//NIT
    nLinCab += nTamLin
    ...
    nLinCab += nTamLin
	oPrint:SayAlign(nLinCab, COL_SERIE, "Localidad: ",     oFontDet, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
	oPrint:SayAlign(nLinCab, COL_SERIE + 47, LOCALIDAD ,     oFontDet, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)//CODCLI
...
Static Function fImpRod(lTot,nLinFir)//RODAPE PIE PAGINA
    ...
    if lTot
        nLinFir += 10
        oPrint:SayAlign(nLinFir, COL_SERIE, "Observacion:" + OBSV , oFontDet, 240, 20, , PAD_LEFT, 0)
		nLinFir += 45
		oPrint:Line(nLinFir, nColIni, nLinFir, COL_VENCTO, CLR_BLACK)
		oPrint:Line(nLinFir, COL_COBRADO, nLinFir, nColFin, CLR_BLACK)
		oPrint:SayAlign(nLinFir, COL_NUMERO, "Recibí Conforme", oFontDet, 240, 20, , PAD_LEFT, 0)
		...
		oPrint:SayAlign(nLinFir, COL_NUMERO, cUSERLGI  , oFontDet, 240, 20, , PAD_LEFT, 0)		//EDUAR 30/06/2020
		oPrint:SayAlign(nLinFir, COL_COBRADO+20 , SUBSTR(NOMCL,1,34), oFontDet, 240, 20, , PAD_LEFT, 0)
		nLinFir += 7
		oPrint:SayAlign(nLinFir, COL_COBRADO+20 , "NIT/CI: ", oFontDet, 240, 20, , PAD_LEFT, 0)
		oPrint:SayAlign(nLinFir, COL_COBRADO +45, NITCL, oFontDet, 240, 20, , PAD_LEFT, 0)
        nLinFir += 7
        oPrint:SayAlign(nLinFir, COL_COBRADO+20 , "NOMBRE: ________________________ ", oFontDet, 240, 20, , PAD_LEFT, 0)

    endif

    oPrint:SayAlign(nLinRod, nColIni,    "CH= CHEQUE", oFontRod, 400, 05, CLR_GRAY, PAD_LEFT,  0)
    nLinRod += 5.4
    oPrint:SayAlign(nLinRod, nColIni,    "TF= DEPOSITO", oFontRod, 400, 05, CLR_GRAY, PAD_LEFT,  0)
    nLinRod += 5.4
    oPrint:SayAlign(nLinRod, nColIni,    "EF= EFECTIVO", oFontRod, 400, 05, CLR_GRAY, PAD_LEFT,  0)
    nLinRod += 5.4
    oPrint:SayAlign(nLinRod, nColIni,    "CD= TARJETA DEBITO", oFontRod, 400, 05, CLR_GRAY, PAD_LEFT,  0)
    nLinRod += 5.4
    oPrint:SayAlign(nLinRod, nColIni,    "CC= TARJETA CREDITO", oFontRod, 400, 05, CLR_GRAY, PAD_LEFT,  0)
    nLinRod += 10
	
    //Dados da Esquerda e Direita
	cTextoEsq := PADC("Av.Viedma No 51 Z/Cementerio General - Telfs: 3 3326174 / 3 3331447 / 3 3364244 - Fax: 3 3368672",100) + "  " + ALLTRIM(cNomeUsr)
	cTextoDir := "Página " + cValToChar(nPagAtu)

	//Imprimindo os textos
	oPrint:Line(nLinRod, nColIni, nLinRod, nColFin, CLR_BLACK)
	oPrint:SayAlign(nLinRod, nColIni,    cTextoEsq, oFontRod, 500, 05, CLR_GRAY, PAD_LEFT,  0)
...
static function getCabec() //obtiene la cabecera del cobro
	...
	cQuery := "	SELECT EL_DTDIGIT,A1_FILIAL, A1_COD, A1_LOJA, A1_NOME,A1_UNITFAC, EL_SERIE, EL_RECIBO, EL_TXMOE02 ,EL_CANCEL ,MAX(EL_USERLGI) EL_USERLGI, EL_OBSBCO , EL_COBRAD, "
	cQuery += " SUM(CASE EL_MOEDA WHEN '1' THEN (CASE EL_BANCO WHEN ' ' THEN 0 ELSE EL_VALOR/EL_TXMOE02 END) WHEN '01' THEN (CASE EL_BANCO WHEN ' ' THEN 0 ELSE EL_VALOR/EL_TXMOE02 END) ELSE (CASE EL_BANCO WHEN ' ' THEN 0 ELSE EL_VALOR END) END) TOTAL "
	...
	cQuery += " AND (EL_TIPODOC IN ('CH ','TF ','EF ','CD ','CC ') OR EL_TIPODOC IN ('TB ') AND EL_TIPO in ('RA','NCC')) "
	cQuery += " AND SEL.D_E_L_E_T_ = ' ' AND EL_FILIAL = '" + xfilial("SEL") + "' "
	cQuery += " GROUP BY EL_DTDIGIT,A1_FILIAL, A1_COD, A1_LOJA, A1_NOME,A1_UNITFAC, EL_SERIE, EL_RECIBO, EL_TXMOE02, EL_CANCEL , EL_OBSBCO ,EL_COBRAD "
...
static function getPagos(cRecb,cSeris)
	...
	*/
	...
	cQuery := "	SELECT EL_TIPO, EL_NUMERO, EL_VALOR, EL_AGENCIA, EL_MOEDA,EL_BANCO, EL_FILIAL, EL_CONTA, EL_DTDIGIT, EL_EMISSAO "
	cQuery += " FROM " + RetSqlName("SEL") + " SEL "
	cQuery += " WHERE EL_FILIAL = '" + xfilial("SEL") + "' "
	cQuery += " AND EL_BANCO <> '   ' "
	cQuery += " AND EL_RECIBO BETWEEN '" + cRecb + "' AND '" + cRecb + "' "
...
static function dtaCobr(cRebco,cSerco)
	Local cQuery	:= ""
	Local nTotal := 0
	Local aArea  := GetArea()
	Local aDatsRet := {"","","","",""}
    ...
    /*SELECT EL_DTDIGIT,EL_RECIBO,EL_SERIE
FROM SEL010
WHERE EL_TIPO IN('TF',
                 'EF',
                 'CH','CC','CD')
  AND EL_PARCELA IN(' ',
                    '1 ')
 AND EL_RECIBO+EL_SERIE='024901CR'
ORDER BY R_E_C_N_O_
 */


	cQuery := "	SELECT EL_DTDIGIT "
	cQuery += " FROM " + RetSqlName("SEL") + " SEL "
	cQuery += " WHERE "
	cQuery += " EL_TIPO IN('TF', 'EF', 'CH','CC','CD') "
	cQuery += " AND EL_PARCELA IN(' ','1 ') "
	cQuery += " AND EL_RECIBO+EL_SERIE = '" + alltrim(cRebco) + alltrim(cSerco) + "' "
	...
	if !VW_DTEMB->(EoF())
		...
		while VW_DTEMB->(!EOF())
			if i < 5
				aDatsRet[i] = VW_DTEMB->EL_DTDIGIT
			else
				exit
			endif
			i++
