...
//	begin transaction
	...
	if empty(cBody)
		...
		if Type("oObj:C5_CONDPAG") <> "U"// caso exista
			aadd(aCabec,{"C5_NUM",u_GetNextNrPed(oObj:C5_CONDPAG),Nil})
		endif
		...
		if Type("oObj:C5_UNITCLI") <> "U"// caso exista
			aadd(aCabec,{"C5_UNITCLI",VAL(oObj:C5_UNITCLI),Nil})
		endif
		...
		For nX := 1 To nCanPro // este for deberia ser por cada producto
			...
			aadd(aLinha,{"C6_PRUNIT",oObj:productos[nX]:B1_PRV1,Nil}) // precio venta en un comienzo NAHIM COMENTANDO 12/03/2020
			aadd(aLinha,{"C6_PRCVEN",oObj:productos[nX]:B1_PRV1,Nil}) // precio venta en un comienzo NAHIM COMENTANDO 12/03/2020
...
WSMETHOD GET WSRECEIVE CODVEN WSSERVICE pedido // recibir por QueryString si hay codigo de usuario para filtrar
	...
	if !empty(cCodUsr)
		...
		BeginSQL Alias cNextAlias
			SELECT DISTINCT top 5000 C5_FILIAL,C5_NUM,C5_CLIENTE,C5_EMISSAO,C5_CONDPAG,C5_UNOMCLI,C5_LOJACLI,F2_EMISSAO,C5_DOCGER,
		CASE
	...
	,C6_UM, C6_QTDVEN,C6_PRUNIT,C6_VALOR, C6_LOTECTL,A1_NOME,A1_CGC,A1_END,B1_DESC C6_UESPECI,C5_SERIE,C5_NOTA,
	C5_TXMOEDA,C5_MOEDA,E4_DESCRI,A1_COD,A1_LOJA,B1_DESC ,(SELECT (DSCTO/TOTAL) * 100.00 PDESC
	...
	JOIN %Table:SA1% SA1 ON A1_COD=C5_CLIENTE AND A1_LOJA = C5_LOJACLI and SA1.D_E_L_E_T_<>'*'
...
END
	...
	,C6_TES,C6_UM, C6_QTDVEN,C6_PRUNIT,C6_VALOR, C6_LOTECTL,A1_NOME,A1_CGC,A1_END,B1_DESC C6_UESPECI,C5_SERIE,C5_NOTA,
	C5_TXMOEDA,C5_MOEDA,E4_DESCRI,A1_COD,A1_LOJA,B1_DESC ,(SELECT (DSCTO/TOTAL) * 100.00 PDESC
	FROM (SELECT SUM(C6_VALOR) + SUM(C6_VALDESC) TOTAL, SUM(C6_VALDESC) DSCTO
	FROM %Table:SC6% SC61
	WHERE C6_NUM=SC5.C5_NUM AND C6_CLI=SC5.C5_CLIENTE AND SC61.D_E_L_E_T_!='*') A ) C5_PDESC,
...
If (cNextAlias)->( !Eof() )
	...
	While !(cNextAlias)->(Eof() )
		...
		IF alltrim(oPedido['C5_NUM']) != alltrim((cNextAlias)->C5_NUM)
			...
			oPedido['C5_UNOMCLI']	:=     (cNextAlias)->C5_UNOMCLI
			if !empty(cCodUsr)
			...
			oPedido['A3_NOME']	:=     (cNextAlias)->A3_NOME
...
END
	...
	C6_UM, C6_QTDVEN,C6_PRUNIT,C6_VALOR, C6_LOTECTL,A1_NOME,A1_CGC,A1_END,B1_DESC C6_UESPECI,C6_VALDESC,C6_PRCVEN,
	C6_TES,C5_TXMOEDA,C5_MOEDA,E4_DESCRI,A1_COD,A1_LOJA,B1_DESC ,(SELECT (DSCTO/TOTAL) * 100.00 PDESC
	FROM (SELECT SUM(C6_VALOR) + SUM(C6_VALDESC) TOTAL, SUM(C6_VALDESC) DSCTO
	FROM %Table:SC6% SC61
	WHERE C6_NUM=SC5.C5_NUM AND C6_CLI=SC5.C5_CLIENTE AND SC61.D_E_L_E_T_!='*') A ) C5_PDESC,
	(SELECT (TOTAL)
