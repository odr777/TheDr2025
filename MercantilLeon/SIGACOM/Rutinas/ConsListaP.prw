...
User Function ConsListaP(_lSXB)
	...
	If lBrowse
		...
		oListProd := TWBrowse():New(  070,05,545,065,,{"Código","UM","Descr. Específica","Descr. Específica 2","Cod.Fábrica","Grupo","Marca"},{40,10,300,300,50,20,80},oDlg,,,,,{||IF(LEN(_aBuscaProd)>0,MostrarDatos(_aBuscaProd[oListProd:nAT,1],.T.),.t.)},,,,,,,.F.,,.T.,,.F.,,,)
	...
	IF BUSCAPROD->(!EOF()) .AND. BUSCAPROD->(!BOF())
		...
		WHILE BUSCAPROD->(!EOF())
			if _nCont==1
				...
				_aBuscaProd[1,2]:=BUSCAPROD->B1_UM
...
Static Function GrabaBusq(cProduto)
	...
	If( TRIM(FunName()) <> 'MATA410' )//Si NO es un Pedido de venta
		...
		Else
			U_GrabaZB1(cProduto, "Consulta Mercantil León")
		EndIf
