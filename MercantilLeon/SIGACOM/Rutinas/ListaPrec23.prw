...
User Function ListaPrec23(_lSXB)
	...
	If lBrowse
		...
		@ 010,370 BUTTON "&Buscar" SIZE 040,010 ACTION BuscaProd23() Object oBnt
		@ 010,500 BUTTON "&Conocimiento" SIZE 040,010 ACTION MSdoc(_cProdSel) Object oBnt
		@ 025,370 BUTTON "Foto Producto" SIZE 040,010 ACTION MostraFoto23(_cProdSel) Object oBnt
		...
		//@ 112,005 BUTTON "Cons Producto" SIZE 040,008 ACTION ConsProd23(_cProdSel) Object oBnt
		//	@ 112,060 BUTTON "Kardex por Dia" SIZE 040,008 ACTION ConsKard23(_cProdSel) Object oBnt

		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//³ Inicia montagem do browse (Busca Produtos).                             ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		oListProd := TWBrowse():New(  070,05,545,065,,{"Código","UM","Descr. Específica","Descr. Específica 2","Cod.Fábrica","Grupo","Marca"},{40,10,300,300,50,20,80},oDlg,,,,,{||IF(LEN(_aBuscaProd)>0,MostrarDatos23(_aBuscaProd[oListProd:nAT,1],.T.),.t.)},,,,,,,.F.,,.T.,,.F.,,,)
		oListProd :SetArray(_aBuscaProd)
		//	oListProd :bChange := { || if(Len(_aBuscaProd)>0,MostrarProducto23(_aBuscaProd[oListProd:nAT],,.F.),.t.)}
		oListProd :bLine := { || if(Len(_aBuscaProd)>0,_aBuscaProd[oListProd:nAT],.t.)}

		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//³ Inicia montagem do browse (Produtos Similares).                         ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		//	oListSim := TWBrowse():New( 072,105,280,060,,{"Codigo","Descripcion","Unid","Precio Venta"},{50,165,15,40},oDlg,,,,,{||If(Len(_aProdSim)>0,MostraEst23(_aProdSim[oListSim:nAT,1],.T.),.T.)},,,,,,,.F.,,.T.,,.F.,,,)
		...
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		oListBox := TWBrowse():New( 140,05,545,060,,{"Cod.Alm.","Almacén","Disponible","Saldo Actual","Pedido Ventas","Reservada","Prevista Entrada","Reservada S.A.","Reservada"},{25,65,42,42,42,42,42,42,42,42,42},oDlg,,,,,,,,,,,,.F.,,.T.,,.F.,,,)
		...
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		oListBox1 := TWBrowse():New( 205,05,545,060,,{"Empresa/Sucursal","Lista","Mon","Precio Venta","Desc 00%","Desc 05%","Desc 10%","Desc 12%","Desc 15%","Desc 17%","Desc 20%","Desc 23%"},{65,25,15,42,42,42,42,42,42,42,42,42},oDlg,,,,,,,,,,,,.F.,,.T.,,.F.,,,)
...
Static Function MostrarDatos23(cProduto,_lSim,cArmazem,lBrowse,aArmazem)

	GrabaBusq(cProduto)//Graba búsqueda

	MostraEst23(_aBuscaProd[oListProd:nAT,1],.T.)
	MostraLista23(_aBuscaProd[oListProd:nAT,1],.T.)
Return _cProdSel

Static Function MostrarProducto23(cProduto,_lSim,_lSXB)
	_cProdSel:= cProduto
...
Static Function MostraEst23(cProduto,_lSim,cArmazem,lBrowse,aArmazem)
	Local aStruSB2  := {}
...
*/
Static Function MontaSim23(_cProduto)
...
Static Function ConsProd23(_cProduto)
...
Static Function ConsKard23(_cProduto,_cAlm)
	Local _cArea:=GetArea()

	If !Empty(_cProduto)
		_cAlm:=aViewB2[oListBox:nAT,1]
		SB1->(DbSetOrder(1))
		If SB1->(DbSeek(xFilial("SB1")+_cProduto))
...
Static Function MostraFoto23(_cProduto)
	Local _cArea:=GetArea()
...
Static Function MostraLista23(cProduto,_lSim,cArmazem,lBrowse,aArmazem)
	Local aStruDA1  := {}
	Local oCursor	:= LoadBitMap(GetResources(),"LBNO")
	Local nTotDisp	:= 0
	Local nSaldo	:= 0
		...
		While !Eof()
			...
			If 	TCCANOPEN("DA1"+SM0->M0_CODIGO+"0")
				...
				If nTC > 0
					...
					IIF(funname() $ "MATA415",cQuery += "AND DA1_CODTAB = '" + M->CJ_TABELA+ "' ", " ")
...
Static Function GrabaBusq(cProduto)
	...
	If( TRIM(FunName()) <> 'MATA410' )//Si NO es un Pedido de venta
		...
		Else
			U_GrabaZB1(cProduto, "Consulta Material 23")
		EndIf
