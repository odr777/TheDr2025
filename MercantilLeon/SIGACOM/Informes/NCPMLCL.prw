...
*/
User Function NCPMLCL(cxNro1,cxNro2,cxSerie,cxClient)

	FatLocal('Nota Credito/Debito',.F.,cxNro1,cxNro2,cxSerie,cxClient)
...
static function FatMat(cDoc1,cDoc2,cSerie,cClient,cTitulo,bImprimir) //Datos Maestro de factura;
	...
	Local aDados
	Local i := 1
	Local cSql     := "SELECT F2_FILIAL,F2_SERIE,F2_DOC,F2_VALBRUT,F2_MOEDA,F2_NUMAUT,F2_EMISSAO,A2_NOME F2_UNOMCLI,A2_CGC F2_UNITCLI,F2_COND,F2_VALFAT,F2_DESCONT,F2_BASIMP1,F2_CODCTR,FP_DTAVAL,FP_ULEYE,F2_UNROIMP,F2_CLIENTE,F2_VEND1 "
	cSql           := cSql+" FROM " + RetSqlName("SF2 ") +" SF2 JOIN " + RetSqlName("SA2") +" SA2 ON (A2_COD=F2_CLIENTE AND A2_LOJA = F2_LOJA AND (F2_DOC BETWEEN '" + cDoc1 + "' AND '"+cDoc2+ "' ) AND F2_SERIE='"+cSerie+"' AND F2_CLIENTE='" + cClient + "' AND SF2.D_E_L_E_T_ =' ' AND SA2.D_E_L_E_T_ =' ' AND A2_LOJA=F2_LOJA AND F2_ESPECIE='NCP' ) "
	cSql           := cSql+" LEFT JOIN " + RetSqlName("SFP") +" SFP ON (FP_SERIE=F2_SERIE AND SFP.D_E_L_E_T_ =' ') "
	...
	While !Eof()
		...
		If ( bSw == .T.)
			...
			//Datos detalle de NCP
			aDetalle:=FatDet (F2_DOC,F2_SERIE,F2_CLIENTE)                	   	
...
Static Function CabFact(nNroPag,nNroTotPag,aMaestro,cTipo)
	...
	oPrn:Say( nLinIni - 030, nTCol2, "NOTA" ,oFont20N )
	oPrn:Say( nLinIni - 030, nTCol3 + 100, "NIT: 143953021", oFont16N )

	cxDep := GetAdvFVal("SD2", "D2_LOCAL", aMaestro[1] + aMaestro[3] + aMaestro[2] + aMaestro[16] ,3, "")
	cxLocal  := GETADVFVAL("NNR","NNR_DESCRI",aMaestro[1]+cxDep,1,"")//descripcion cnd pago

	nLinAdd += 10
	oPrn:Say( nLinIni + (1.0*nLinAdd), nTCol3, "NOTA CRED/DEBITO: " + alltrim(aMaestro[3]), oFont12N ) //Nro Factura
	oPrn:Say( nLinIni + (2.0*nLinAdd), nTCol3, "DEPOSITO: " + cxDep + "-" + cxLocal , oFont12N ) //Nro Autorizacion
...
Static Function PieFact(nNroPag,nNroTotPag,aMaestro)
	...
	nLargo := 85
	cTotalLiteral := ALLTRIM(Extenso(aMaestro[11],.F.,1))+ IIF(nMoedax == 2,"DOLARES","BOLIVIANOS")
	...
	if nMoedax == 1
		...
		//oPrn:Say( nLinFin + 150, nDCol4 + 20, "TOTAL $us." ,oFont08N )
		oPrn:Say( nLinFin + 90, nDCol4 + 20, "T/C" ,oFont08N )
		oPrn:Say( nLinFin + 20, nDCol4 + 20, "TOTAL $us." ,oFont08N )		
		//oPrn:Say( nLinFin + 150, nDCol5 + 250, TRANSFORM(aMaestro[11]/_nTipoCambio,"@A 99,999,999,999.99") ,oFont08N,,,,1)  //Total Dolares
		oPrn:Say( nLinFin + 90, nDCol5 + 250, TRANSFORM(_nTipoCambio,"@A 99,999,999,999.99") ,oFont08N,,,,1)  //Tipo Cambio
		oPrn:Say( nLinFin + 20, nDCol5 + 250, TRANSFORM(aMaestro[11],"@A 99,999,999,999.99") ,oFont08N,,,,1)  //Total Bolivianos				
	endif
	...
	If !Empty(aMaestro[16])
		oPrn:Say( nLinFin + nAddLin, nTCol1, "CLIENTE: " + aMaestro[16], oFont10 )  //Codigo Cliente
		nAddLin += 50
	EndIf
	If !Empty(aMaestro[8])
...
static function FatDet(cDoc,cSerie,cClient) // Datos detalle de factura
	...
	cSql				:= cSql+"FROM " + RetSqlName("SD2") +" SD2 JOIN	" + RetSqlName("SB1") +" SB1 ON (D2_DOC='"+cDoc+"' AND D2_SERIE='"+cSerie+"'  AND D2_CLIENTE='" +cClient + "' AND  D2_COD=B1_COD AND D2_ESPECIE='NCP' AND SD2.D_E_L_E_T_=' ' AND SB1.D_E_L_E_T_=' ' ) "
