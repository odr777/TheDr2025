...
±±³Fun‡…o    ³ REPORTBASE ³ Autor Erick etcheverry Query	: Erick etcheverry³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ Imprime despacho fob items() 		 	 	     			³±±
...
User Function IMDESPA()
	Local oReport
	PRIVATE cPerg   := "IMDESPA"   // elija el Nombre de la pregunta
...
Static Function ReportDef()
	...
	Local oReport	 := TReport():New("IMDESPA",NombreProg,,{|oReport| PrintReport(oReport)},"DESPACHO")
...
Static Function PrintReport(oReport)
	...
	/*
	SELECT D1_COD,B1_UESPEC2,B1_DESC, B1_UM, B1_GRUPO, B1_UMARCA,B1_UCODFAB, D1_QUANT,B1_ESPECIF, ZMA_DESCRI
	FROM DBA010 DBA
	JOIN DBB010 DBB ON DBA_HAWB = DBB_HAWB AND DBB_FILIAL = DBA_FILIAL AND DBB_TIPONF = '5' AND DBB.D_E_L_E_T_ =''
	JOIN SF1010 SF1 ON F1_DOC = DBB_DOC AND DBB_FORNEC = F1_FORNECE AND F1_LOJA = DBB_LOJA AND DBB_FILIAL = F1_FILIAL AND  F1_SERIE IN('IMP','CLO') AND F1_ESPECIE = 'NF' AND SF1.D_E_L_E_T_ =''
	JOIN SD1010 SD1 ON D1_DOC = F1_DOC AND D1_FORNECE = F1_FORNECE AND D1_LOJA = F1_LOJA AND D1_ESPECIE = 'NF' AND SD1.D_E_L_E_T_ =''
	JOIN SB1010 SB1 ON B1_COD = D1_COD AND SB1.D_E_L_E_T_ =''
	JOIN ZMA010 ZMA ON B1_UMARCA = ZMA_CODIGO  AND ZMA.D_E_L_E_T_ <> '*'
	WHERE
	DBA_HAWB ='T02469              '
	AND DBA_FILIAL = '0101'
	AND DBA.D_E_L_E_T_ = ''	
	*/
	...
	While (cAliasQry)->(! EOF())
		...
		oSection:Cell("D1_QUANT"):SetPicture(PesqPict("SD1","D1_TOTAL")) 
