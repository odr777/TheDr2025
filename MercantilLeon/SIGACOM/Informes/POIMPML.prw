...
#Define COL_UMED  0360
#Define	COL_QANTIDA	0390
...
±±ºPrograma  ³  POIMPML ºAutor ³ERICK ETCHEVERRY º   º Date 08/12/2019   º±±
...
User Function POIMPML()
	LOCAL cString		:= "SC7"
	Local titulo 		:= ""
	LOCAL wnrel		 	:= "Impresion del Purchase Order"
	LOCAL cDesc1	    := "Impresion del Purchase Order"
	LOCAL cDesc2	    := "conforme PE"
	LOCAL cDesc3	    := "Especifico ML"
	PRIVATE nomeProg 	:= "POIMPML"
	PRIVATE lEnd        := .F.
	Private oPrint
	...
	PRIVATE cNomFor     := ""
	Private nReg 		:= 0
	PRIVATE cPerg   := "POIMPML"   // elija el Nombre de la pregunta
	CriaSX1(cPerg)	// Si no esta creada la crea

	Pergunte(cPerg,.F.)

	Processa({ |lEnd| MOVD3CONF("Impresion del Purchase Order")},"Imprimindo , aguarde...")
	Processa({|lEnd| fImpPres(@lEnd,wnRel,cString,nReg)},titulo)

Return

Static Function MOVD3CONF(Titulo)///configuracion impresora
	Local cFilename := 'POIMPML'
	Local i 	 := 1
	Local x 	 := 0

	cCaminho  := GetTempPath()
	cArquivo  := "POIMPML" + dToS(date()) + "_" + StrTran(time(), ':', '-')
...
Static Function fImpPres(lEnd,WnRel,cString,nReg)
	...
	*/
	...
	cQuery:= " SELECT C7_NUM,C7_FORNECE,A2_NOME,C7_MOEDA,C7_EMISSAO,C7_CONTATO,C7_COND,E4_DESCRI,C7_UOFERTA,C7_NUMSC,LTRIM(RTRIM(C7_INCOTER)) + ' - ' + LTRIM(RTRIM(DB6_DESCR)) C7_UCONENT, "///cabecera
	...
	cQuery+= " LEFT JOIN " + RetSQLname('DB6') + " DB6 ON DB6_COD = C7_INCOTER AND SY1.D_E_L_E_T_ = '' "
	...
	TCQuery cQuery New Alias "VW_C7PRD"

	fImpCab(.t.,VW_C7PRD->C7_NUM,VW_C7PRD->C7_FORNECE,VW_C7PRD->A2_NOME,VW_C7PRD->C7_MOEDA,VW_C7PRD->C7_EMISSAO,VW_C7PRD->C7_CONTATO,VW_C7PRD->C7_COND,VW_C7PRD->E4_DESCRI,VW_C7PRD->C7_UOFERTA,VW_C7PRD->C7_NUMSC,VW_C7PRD->C7_UCONENT,VW_C7PRD->C7_COMPRA,VW_C7PRD->Y1_NOME)
	...
	if ! VW_C7PRD->(EoF())
		...
		While ! VW_C7PRD->(EoF())
			...
			//items
			oPrint:SayAlign(nLinAtu, COL_ITEMM, VW_C7PRD->C7_ITEM,     oFontDet, 030, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_COD, VW_C7PRD->C7_PRODUTO,     oFontDet, 270, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_FABCAN,VW_C7PRD->B1_UCODFAB,     oFontDet, 100, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_UMED, VW_C7PRD->C7_UM ,     oFontDet, 0170, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_QANTIDA,ALLTRIM(TRANSFORM(VW_C7PRD->C7_QUANT,"@E 999,999,999,999.99")) ,     oFontDet, 100, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_PRECUNIT,ALLTRIM(TRANSFORM(VW_C7PRD->C7_PRECO,"@E 999,999,999,999.99999")) ,     oFontDet, 100, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_PRECTOTAL,ALLTRIM(TRANSFORM(VW_C7PRD->C7_TOTAL,"@E 999,999,999,999.99")) ,     oFontDet, 100, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_FENTREGA, DTOC(STOD(VW_C7PRD->C7_UPLZENT)),     oFontDet, 100, nTamLin, CLR_BLACK, PAD_LEFT, 0)

			aDescri := TexToArray(ALLTRIM(VW_C7PRD->C7_UESPECI),40)

			for i := 1 to len(aDescri)//por lo menos dos

				oPrint:SayAlign(nLinAtu, COL_DESCESPE,aDescri[i] ,     oFontDet, 0200, nTamLin, CLR_BLACK, PAD_LEFT, 0)

				nLinAtu += nTamLin
			next i

			aDescri := TexToArray(ALLTRIM(VW_C7PRD->C7_UESPEC2),40)
	...
	oPrint:SayAlign(nLinAtu, COL_ITEMM + 70, alltrim(Posicione("SC7",1,xFilial("SC7")+SC7->C7_NUM,"C7_UOBSPC")) ,     oFontDetN, 0200, nTamLin, CLR_BLACK, PAD_LEFT, 0)
...
Static Function fImpCab(lImp,cNum,cFornec,cFornome,nMoeda,dEmiss,cContac,cCond,cConddes,cOfert,cNumscc,cUconent,cYcod,cYnome)
	Local cTexto   := ""
	Local nLinCab  := 030
	Local nLinInCab := 015
	Local nDetCab := 090
	...
	if lImp
		...
		nLinCab += (nTamLin * 1.8)
		oPrint:SayAlign(nLinCab, COL_ITEMM, "CONTACTO:", oFontDetN, 0100, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		oPrint:SayAlign(nLinCab, COL_ITEMM+50, cContac, oFontDet8, 0400, nTamLin, CLR_BLACK, PAD_LEFT, 0)

		oPrint:SayAlign(nLinCab, COL_QANTIDA, "OFERTA PROVEEDOR: ",     oFontDetN, 0100, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		oPrint:SayAlign(nLinCab, COL_QANTIDA+80, cOfert, oFontDet8, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)

		nLinCab += (nTamLin * 1.8)
		oPrint:SayAlign(nLinCab, COL_ITEMM, "CONDICION DE PAGO: ",     oFontDetN, 0100, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		oPrint:SayAlign(nLinCab, COL_ITEMM+80, cCond + " - " + cConddes, oFontDet8, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)

		oPrint:SayAlign(nLinCab, COL_QANTIDA, "INCOTERM: ",     oFontDetN, 0100, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		oPrint:SayAlign(nLinCab, COL_QANTIDA+50, cUconent, oFontDet8, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)

		nLinCab += (nTamLin * 1.8)
		oPrint:SayAlign(nLinCab, COL_ITEMM, "SOLICITUD: ",     oFontDetN, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		oPrint:SayAlign(nLinCab, COL_ITEMM+50, cNumscc, oFontDet8, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)

		oPrint:SayAlign(nLinCab, COL_QANTIDA, "COMPRADOR: ",     oFontDetN, 0100, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		oPrint:SayAlign(nLinCab, COL_QANTIDA+50, cYcod + " - " + cYnome , oFontDet8, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
	...
	oPrint:SayAlign(nLinCab, COL_FABCAN, "COD. FABRICANTE",     oFontDetN, 0270, nTamLin, CLR_BLACK, PAD_LEFT, 0)
	oPrint:SayAlign(nLinCab, COL_DESCESPE, "DESCRIPCION",     oFontDetN, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
	...
	oPrint:SayAlign(nLinCab, COL_FENTREGA, "FECHA DE ENTREGA",     oFontDetN, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
