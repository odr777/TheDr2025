...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
/*/
User Function PEDCLIEF()
    Local oReport
    Local cPerg := "PEDCLIEF"
...
Static Function ReportDef()
    ...
    oReport := TReport():New("PEDCLIEF","Cuentas cliente pedido efectivo",cPerg,{|oReport| ReportPrint(oReport)},;
    ...
    oSection2 := TRSection():New(oReport,"Cuentas cliente pedido efectivo",,)
...
Static Function ReportPrint(oReport)
	...
	*/
	...
	cQuery2+= " AND ISNULL(SEL.EL_TIPODOC, 'EF') IN('EF') "
   ...
   cQuery2+= " AND ISNULL(SA1.D_E_L_E_T_, ' ') = ' ' " 
    ...
    While TMP2->(!Eof())
		...
		if cAct != cDoc .or. lFirtra  //si es otro documento salta de pagina
			...
			if (TMP2->F2_MOEDA == 1)
				...
				nValCobUs = xMoeda(TMP2->VALOR_PAGADO,1,2,stod(TMP2->Fecha_emi),2)  //TMP2->VALOR_PAGADO
				nValTotUs = xMoeda(TMP2->TOTSAL,1,2,stod(TMP2->Fecha_emi),2)
				nValImpBs = TMP2->Base_imp
				nValCobBs = TMP2->VALOR_PAGADO ///TMP2->VALOR_PAGADO
				...
				nValCobBs = xMoeda(TMP2->VALOR_PAGADO,2,1,stod(TMP2->Fecha_emi),2)//TMP2->VALOR_PAGADO
				nValTotBs = xMoeda(TMP2->TOTSAL,2,1,stod(TMP2->Fecha_emi),2)
				nValImpUs = TMP2->Base_imp
				nValCobUs = TMP2->VALOR_PAGADO  ////TMP2->VALOR_PAGADO
		...
		//PEDIDO+F2_MOEDA+SUCURSAL+CLIENTE+CLIENTE_NOM+VENDEDOR_COD+VENDEDOR+FECHA_EMI+NRO_DOC+F2_SERIE+CANTIDAD+PRECIO+BASE_IMP+MONTO_COBRADO+TOTSAL+Fecha_vcto
	...
	aDados[VALRECB] := nTtoRecb //ALLTRIM(TRANSFORM((nTtoRecb)	,"@E 999,999,999.99"))	
