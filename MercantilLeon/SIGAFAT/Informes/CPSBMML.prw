...
ฑฑบFuno    ณ CPSBMML  บ Autor ณ Erick Etcheverry 	   บ Data ณ  28/08/2019 บฑฑ
...
Static Function ReportDef()
	...
	oReport := TReport():New("CPSBMML","Cuentas por pagar por grupo de producto",cPerg,{|oReport| ReportPrint(oReport)},;
	"Cuentas por pagar por grupo de producto",,,,,,,0.5)
	...
	oSection2 := TRSection():New(oReport,"CUENTAS POR PAGAR",,)
...
Static Function ReportPrint(oReport)
	...
	cQuery2:= " SELECT SUBSTRING(D2_EMISSAO, 1, 4)+'-'+SUBSTRING(D2_EMISSAO, 5, 2) ANIO_MES, BM_DESC Grupo, B1_DESC Producto,"
	cQuery2+= " D2_PEDIDO Pedido,F2_MOEDA,CASE WHEN F2_MOEDA = 1 THEN CASE WHEN D2_BASIMP1 = 0 THEN D2_TOTAL ELSE D2_BASIMP1 END "
	cQuery2+= " ELSE NULL END Monto_Bs,CASE WHEN F2_MOEDA = 2 THEN CASE WHEN D2_BASIMP1 = 0 THEN D2_TOTAL ELSE D2_BASIMP1 END "
	...
	cQuery2+= " WHERE M2_DATA = D2_EMISSAO) END TC,CASE WHEN F2_MOEDA = 1 THEN CASE WHEN D2_BASIMP1 = 0 THEN D2_TOTAL ELSE D2_BASIMP1 END "
	cQuery2+= " ELSE CASE WHEN D2_BASIMP1 = 0 THEN D2_TOTAL ELSE D2_BASIMP1 END*CASE WHEN F2_TXMOEDA <> 0 THEN F2_TXMOEDA ELSE (SELECT MAX(M2_MOEDA2) "
	cQuery2+= " FROM "+ RetSQLname('SM2') + " SM2 "
	cQuery2+= " WHERE M2_DATA = D2_EMISSAO) END END Total_Bs,A1_FILIAL Sucursal, A1_COD Cliente, A1_LOJA Tienda, A1_NOME Cliente_nom, A1_OBSERV Obs_Cliente,  "
	cQuery2+= " ISNULL(A3_COD, 'S/V') Vendedor_cod, ISNULL(A3_NOME, 'SIN VENDEDOR') Vendedor,D2_FILIAL Sucursal_trans, D2_DTDIGIT Fecha_dig, "
	cQuery2+= " D2_EMISSAO Fecha_emi, D2_DOC Nro_Doc, D2_SERIE, D2_COD Producto_cod, D2_ITEM,D2_CLIENTE Cliente_Cod,D2_QUANT Cantidad, "
	cQuery2+= " D2_PRCVEN Precio, D2_BASIMP1 Base_imp,(CASE WHEN F2_MOEDA = 1 THEN CASE WHEN D2_BASIMP1 = 0 THEN D2_TOTAL ELSE D2_BASIMP1 END "
	cQuery2+= " ELSE CASE WHEN D2_BASIMP1 = 0 THEN D2_TOTAL ELSE D2_BASIMP1 END*CASE WHEN F2_TXMOEDA <> 0 THEN F2_TXMOEDA ELSE (SELECT MAX(M2_MOEDA2) "
	cQuery2+= " FROM "+ RetSQLname('SM2') + " SM2 "
	cQuery2+= " WHERE M2_DATA = D2_EMISSAO) END END)/ISNULL(VALOR,1)*ISNULL(VALOR-TOTSAL, 0) MONTO_COBRADO, "
	cQuery2+= " (CASE WHEN F2_MOEDA = 1 THEN CASE WHEN D2_BASIMP1 = 0 THEN D2_TOTAL ELSE D2_BASIMP1 END "
	cQuery2+= " ELSE CASE WHEN D2_BASIMP1 = 0 THEN D2_TOTAL ELSE D2_BASIMP1 END*CASE WHEN F2_TXMOEDA <> 0 THEN F2_TXMOEDA ELSE(SELECT MAX(M2_MOEDA2) "
	cQuery2+= " FROM "+ RetSQLname('SM2') + " SM2 "
	cQuery2+= " WHERE M2_DATA = D2_EMISSAO) END END)/ISNULL(VALOR,1)*ISNULL(TOTSAL, 0) TOTSAL,E1_VENCTO Fecha_vcto "
	...
	cQuery2+= " LEFT JOIN (SELECT E1_FILIAL, E1_NUM, E1_SERIE, E1_CLIENTE, E1_LOJA,E1_VENCTO, SUM(E1_VALOR) VALOR, SUM(E1_SALDO) TOTSAL"
	...
	cQuery2+= " AND D2_EMISSAO BETWEEN '" + DTOS(MV_PAR07) + "' AND '" + DTOS(MV_PAR08) + "' ORDER BY D2_PEDIDO"	
	...
	While TMP2->(!Eof())
		...
		if (TMP2->F2_MOEDA == 1)
			...
			nValCobUs = xMoeda(TMP2->MONTO_COBRADO,1,2,stod(TMP2->Fecha_emi),2)
			...
			nValCobBs = xMoeda(TMP2->MONTO_COBRADO,2,1,stod(TMP2->Fecha_emi),2)
