...
Static Function ReportDef()
	...
	TRCell():New(oSection2,"F2_UNITCLI","SF2","NIT FACTURA",,17)
	TRCell():New(oSection2,"F2_UNOMCLI","SF2","NOMBRE FACTURA",,17)
...
Static Function ReportPrint(oReport)
	...
	oSection2:Cell("F2_UNITCLI"):SetBlock( { || aDados[UNITCLI] })
	oSection2:Cell("F2_UNOMCLI"):SetBlock( { || aDados[UNOMCLI] })
	...
	cQuery2:= " SELECT F2_UNITCLI,F2_UNOMCLI,SUBSTRING(D2_EMISSAO, 1, 4)+'-'+SUBSTRING(D2_EMISSAO, 5, 2) ANIO_MES,"
	cQuery2+= " D2_PEDIDO Pedido,F2_MOEDA,CASE WHEN F2_MOEDA = 1 THEN CASE WHEN sum(D2_BASIMP1) = 0 THEN sum(D2_TOTAL) ELSE sum(D2_BASIMP1) END "
	...
	cQuery2+= " WHERE M2_DATA = D2_EMISSAO) END TC,CASE WHEN F2_MOEDA = 1 THEN CASE WHEN sum(D2_BASIMP1) = 0 THEN sum(D2_TOTAL) ELSE sum(D2_BASIMP1) END "
	...
	cQuery2+= " D2_EMISSAO Fecha_emi, D2_DOC Nro_Doc, D2_SERIE,D2_CLIENTE Cliente_Cod,SUM(D2_QUANT) Cantidad, "
	...
	cQuery2+= " ELSE CASE WHEN sum(D2_BASIMP1) = 0 THEN sum(D2_TOTAL) ELSE sum(D2_BASIMP1) END*CASE WHEN F2_TXMOEDA <> 0 THEN F2_TXMOEDA ELSE (SELECT MAX(M2_MOEDA2) "
	cQuery2+= " FROM "+ RetSQLname('SM2') + " SM2 "
	cQuery2+= " WHERE M2_DATA = D2_EMISSAO) END END)/ISNULL(SUM(VALOR),1)*ISNULL(SUM(VALOR)-SUM(TOTSAL), 0) MONTO_COBRADO, "
	cQuery2+= " (CASE WHEN F2_MOEDA = 1 THEN CASE WHEN sum(D2_BASIMP1) = 0 THEN sum(D2_TOTAL) ELSE sum(D2_BASIMP1) END "
	cQuery2+= " ELSE CASE WHEN sum(D2_BASIMP1) = 0 THEN sum(D2_TOTAL) ELSE sum(D2_BASIMP1) END*CASE WHEN F2_TXMOEDA <> 0 THEN F2_TXMOEDA ELSE(SELECT MAX(M2_MOEDA2) "
	cQuery2+= " FROM "+ RetSQLname('SM2') + " SM2 "
	cQuery2+= " WHERE M2_DATA = D2_EMISSAO) END END)/ISNULL(SUM(VALOR),1)*ISNULL(SUM(TOTSAL), 0) TOTSAL,E1_VENCTO Fecha_vcto "
	...
	cQuery2+= " GROUP BY F2_UNITCLI,F2_UNOMCLI,D2_EMISSAO,D2_PEDIDO,F2_MOEDA,A1_FILIAL,A1_COD,A1_LOJA,A1_NOME,A1_OBSERV,A3_COD,A3_NOME,D2_FILIAL,D2_DTDIGIT,D2_DOC,D2_SERIE,D2_CLIENTE,E1_VENCTO,F2_TXMOEDA "
	
	cQuery2+= " ORDER BY D2_PEDIDO"
    ...
    While TMP2->(!Eof())
		...
		aDados[UNITCLI] :=    TMP2->F2_UNITCLI
		aDados[UNOMCLI] :=    TMP2->F2_UNOMCLI
