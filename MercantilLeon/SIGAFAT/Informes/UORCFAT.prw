...
ฑฑบPrograma  ณ  RPPRE01  บAutor  ณJorge Saavedra       บ                  บฑฑ
ฑฑ           ณ  UORCFAT  บAutor  ณAmby Arteaga Rivero  บ Date 25/11/2015  บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑDescripcion Impresion de Proforma de Venta                              บฑฑ
ฑฑบ          ณNahim 10/01/2018 lote en validacion                         บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ Mercantil LEON SRL                                         บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
...
//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
	...
	cQuery := "SELECT CJ_FILIAL,CJ_EMISSAO,CJ_USRREG,CJ_NUM,A1_COD,A1_NOME,A1_END,A1_TEL,A1_CGC,CASE WHEN CJ_STATUS='A' THEN 'APROBADO' ELSE 'PENDIENTE' END CJ_STATUS,"
// cQuery += " CASE WHEN CJ_MOEDA=1 then 'BOLIVIANOS' ELSE 'DOLARES' END CJ_MOEDA,CK_PEDCLI,CK_PRODUTO,CK_DESCRI,B1_DESC,CK_UM,CK_QTDVEN,CJ_FRETE,CJ_DESPESA,CJ_SEGURO,CK_ULOTE," Nahim T - 10/01/2018 LOTE (solo usado en VALIDACION / no fue pasado a produccion)
	...
	cQuery += " CK_PRCVEN,CK_VALOR,CK_OBS,CJ_VALIDA,CK_ENTREG,CK_VALDESC,CJ_DESC1,CK_UESPECI,CK_ITEM,CK_ITECLI,CJ_UVEND,A1_LOJA,A1_UNOMFAC CJ_UNOMFAT,E4_DESCRI,CJ_UTPOENT,CJ_ULUGENT,CJ_MOEDA CJMOEDA,CJ_UOBSERV  "
	...
	cQuery += " AND SCJ.D_E_L_E_T_<>'*' AND SE4.D_E_L_E_T_<>'*' "
...
Static Function fImpItem(nLin)
	...
	EndIf
	oPrn:Say( Tpix(nLin+01.5), Tpix(04.3), Left(CK_UESPECI,45), oFont10C)
...
//   	If !Empty(CK_ULOTE) Nahim 10/01/2018 lote en validacion
//       nLin := nLin+00.3
...
//diasfin:=DateDiffDay(CTOD(CJ_VALIDA),CTOD(CJ_EMISSAO))  //substr(CJ_VALIDA,8,2)
	...
	If SM0->(Eof())
		SM0->( MsSeek( cEmpAnt + tCJ_FILIAL , .T. ))
	Endif
	...
	oPrn:Say( Tpix(nLin+01.5), Tpix(01.0), "Sucursal: " + SM0->M0_FILIAL, oFont10CN) // CJ_EMISSAO
	oPrn:Say( Tpix(nLin+01.5), Tpix(14.0), "Numero: " + tCJ_NUM								 			, oFont10CN)
	oPrn:Say( Tpix(nLin+02.0), Tpix(01.0), "Cliente: "+ alltrim(tA1_COD) + alltrim(tA1_LOJA) +' - '+ ALLTRIM(tA1_NOME), oFont10CN)
	oPrn:Say( Tpix(nLin+02.0), Tpix(14.0), "Fecha: " + ffechalarga(tCJ_EMISSAO), oFont10CN) // CJ_EMISSAO
	oPrn:Say( Tpix(nLin+02.5), Tpix(01.0), "Direccion: " + tA1_END, oFont10CN)
	...
	oPrn:Say( Tpix(nLin+03.5), Tpix(01.0), "Proforma Valida hasta el : " + ffechalarga(dtos( stod(CJ_VALIDA) )) 			, oFont10CN)  //NT
	...
	oPrn:Say( Tpix(nLin+04.0), Tpix(14.0), "Vendedor: " + LTRIM(GetAdvFval('SA3',"A3_NOME",xFilial('SA3') + tCJ_UVEND ,1,"" )) 			, oFont10CN)
...
Static Function fImpNota(nLin)
	...
	oPrn:Say( Tpix(nLin+4.0), Tpix(4.0), "ESTIMADO CLIENTE:", oFont10C)
	oPrn:Say( Tpix(nLin+4.5), Tpix(4.0), "TODA COMPRA MAYOR A BS. 50,000.00 SE DEBE CANCELAR CON CHEQUE O "  , oFont10C)
	oPrn:Say( Tpix(nLin+5.0), Tpix(4.0), "DEPOSITO EN CUENTA BANCARIA Y EL DEPOSITANTE DEBE SER EL MISMO DE LA FACTURA." , oFont10C)

	oPrn:Say( Tpix(nLin+5.5), Tpix(4.0), AllTrim(GetMv("MV_UBCODES")), oFont10C)
	oPrn:Say( Tpix(nLin+6.0), Tpix(4.0), AllTrim(GetMv("MV_UBCOBOL")), oFont10C)
	oPrn:Say( Tpix(nLin+6.0), Tpix(10.0), AllTrim(GetMv("MV_UBCODOL")), oFont10C)

	oPrn:Say( Tpix(nLin+6.5), Tpix(4.0), AllTrim(GetMv("MV_UBCODE2")), oFont10C)
	oPrn:Say( Tpix(nLin+7.0), Tpix(4.0), AllTrim(GetMv("MV_UBCOBO2")), oFont10C)
	oPrn:Say( Tpix(nLin+7.0), Tpix(10.0), AllTrim(GetMv("MV_UBCODO2")), oFont10C)

	oPrn:Say( Tpix(nLin+7.5), Tpix(4.0), AllTrim(GetMv("MV_UBCODE3")), oFont10C)
	oPrn:Say( Tpix(nLin+8.0), Tpix(4.0), AllTrim(GetMv("MV_UBCOBO3")), oFont10C)
	oPrn:Say( Tpix(nLin+8.0), Tpix(10.0), AllTrim(GetMv("MV_UBCODO3")), oFont10C)

	oPrn:Say( Tpix(nLin+8.5), Tpix(4.0), AllTrim(GetMv("MV_UBCODE4")), oFont10C)
	oPrn:Say( Tpix(nLin+9.0), Tpix(4.0), AllTrim(GetMv("MV_UBCOBO4")), oFont10C)
	oPrn:Say( Tpix(nLin+9.0), Tpix(10.0), AllTrim(GetMv("MV_UBCODO4")), oFont10C)

	oPrn:Say( Tpix(nLin+9.2), Tpix(01.0), "__________________________________________________________________________________________________________________________"	, oFont12C)
	oPrn:Say( Tpix(nLin+9.7), Tpix(01.0), "MERCANTIL LEON SRL NO SE HACE RESPONSABLE POR LOS DAัOS CAUSADOS EN EL TRANSPORTE" , oFont10C)
	oPrn:Say( Tpix(nLin+10.2), Tpix(01.0), "STOCK SUJETO A VENTAS, CONFIRMAR LA EXISTENCIA PREVIO AL ENVอO DE LA ORDEN DE COMPRA O PAGO" , oFont10C)
