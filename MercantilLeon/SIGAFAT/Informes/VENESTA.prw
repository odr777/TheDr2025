...
User Function venesta()
	Local oReport
	PRIVATE cPerg   := "VENESTA"   // elija el Nombre de la pregunta
	criasx1(cPerg)
...
Static Function ReportDef()
	...
	pergunte(cperg, .F.)
	oReport	 := TReport():New("vendep",NombreProg,cPerg,{|oReport| PrintReport(oReport)},"Reporte de ventas por depto")
...
Static Function PrintReport(oReport)
	...
	case  WHEN B1_TIPCONV = 'M' THEN  ISNULL(B1_CONV * SUM(D2_QUANT),0) 
	ELSE ISNULL(SUM(D2_QUANT)/ NULLIF(B1_CONV,0),0) END D2_QTSEGUM,
	...
	GROUP BY
	C6_PRODUTO, B1_DESC, B1_UESPEC2,B1_ESPECIF, B1_GRUPO, C6_UM, C6_TES, B1_CONV,B1_TIPCONV,A1_EST 
	ORDER BY A1_EST,C6_PRODUTO
	EndSql
	/*
	LEFT JOIN
	(
	...
	*/
	...
	While QRYSA1->(!Eof())
		...
		if QRYSA1->A1_EST != cEst
			cLinha := Replicate("_",400)
			nRow := oReport:Row()
			oReport:PrintText(cLinha, nRow, 20)
			oReport:SkipLine()
			aDados[2] := "Total " + Posicione("SX5",1,xFilial("SX5")+ "12" + alltrim(cEst) ,"X5_DESCRI")
			aDados[8] := nSubDest
			aDados[9] := nSub
