...
±±ºPrograma  ³  LIPRE2ML ºAutor ³ERICK ETCHEVERRY º   º Date 08/12/2019   º±±
...
Static Function MOVD3CONF(Titulo)///configuracion impresora
	Local cFilename := 'LIPRE2ML'
	Local i 	 := 1
	Local x 	 := 0

	cCaminho  := GetTempPath()
	cArquivo  := "LIPRE2ML" + dToS(date()) + "_" + StrTran(time(), ':', '-')
...
Static Function fImpPres(lEnd,WnRel,cString,nReg)
	...
	TCQuery cQuery New Alias "VW_PRECPRD"
	...
	if ! VW_PRECPRD->(EoF())
		...
		While ! VW_PRECPRD->(EoF())
			...
			oPrint:SayAlign(nLinAtu, COL_COD, VW_PRECPRD->B1_COD,     oFontDet, 270, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_FABCAN,VW_PRECPRD->B1_UCODFAB,     oFontDet, 100, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_DESCESPE, VW_PRECPRD->ZMA_DESCRI ,     oFontDet, 0170, nTamLin, CLR_BLACK, PAD_LEFT, 0)

			oPrint:SayAlign(nLinAtu, COL_QANTIDA,ALLTRIM(TRANSFORM(VW_PRECPRD->DA1_PRCVEN-(VW_PRECPRD->DA1_PRCVEN *(mv_par07/100)),"@E 999,999,999,999.99")) ,     oFontDet, 100, nTamLin, CLR_BLACK, PAD_LEFT, 0)

			oPrint:SayAlign(nLinAtu, COL_PRECUNIT, VW_PRECPRD->B1_UM ,     oFontDet, 100, nTamLin, CLR_BLACK, PAD_LEFT, 0)

			if !empty(VW_PRECPRD->B1_BITMAP)
				cFile := RepositArq(VW_PRECPRD->B1_BITMAP)//trae imagen repositorio
			...
			aDescri := TexToArray(ALLTRIM(VW_PRECPRD->B1_ESPECIF),40)
			...
			aDescri := TexToArray(ALLTRIM(VW_PRECPRD->B1_UESPEC2),40)
...
Static Function fImpCab(lImp)
	...
	oPrint:SayAlign(nLinCab, COL_QANTIDA, "PRECIO Bs" ,     oFontDetN, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
...
Static Function fImpRod(lImp)//RODAPE PIE PAGINA
	...
	cTextoDir := "Página " + cValToChar(nPagAtu) + " " + cFecha + "-PRC"+cValToChar(mv_par07)
