...
* @since: 02/12/2020
* @description: Consulta estándar de Productos para almacen
*
*/
User Function Conproal(_lSXB)
	...
	*/
	...
	If lBrowse
		...
		oListBox := TWBrowse():New( 160,(aTamVen[3] / 2) + 10,((aTamVen[3] / 2) - 10),100,,{"Cod.Alm.","Almacén","Disponible"},{25,65,42,42,42,42,42,42,42,42},oDlg,,,,,,,,,,,,.F.,,.T.,,.F.,,,)
	...
	IF TCCANOPEN("SB2"+SM0->M0_CODIGO+"0") // .and. TCCANOPEN("SC6"+SM0->M0_CODIGO+"0") .and. TCCANOPEN("SF2"+SM0->M0_CODIGO+"0")
		...
		While ( !Eof() )
			...
			If lBrowse
				...
				TransForm(cDescAlm,PesqPict("NNR","NNR_DESCRI")), IIF((cCursor)->B2_QATU > 0,"Si","No");
				})
			Else
				aAdd(aViewB2,{;//_cNomeEmp,;
				(cCursor)->B2_LOCAL,;
				cDescAlm,IIF((cCursor)->B2_QATU > 0,"Si","No");
				})
			EndIf
