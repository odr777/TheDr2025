...
User Function ConsProd(_lSXB)
	...
	Private nPosTes		:= 0
	Private nPosPru     := 0
	Private _aColsCop	:={}   ///aCols para copiar al browse
	...
	*/
	...
	If lBrowse
		...
		@ 055,280 SAY "MONEDA 01=Bs. " Object oSayProd
		...
		If FunName() == 'MATA410'//Pedido de Venta
			...
			If !Empty(_cProdSel) .and. lVal   ///se selecciono algun producto?

				//alert(_cProdSel)

				For _nCont:=1 to Len(_aColsCop)  //Recorre array a copiar

					DbSelectArea('SB1')
					DbSetOrder(1)
					If DbSeek(xFilial("SB1")+_aColsCop[_nCont,nPoscod],.f.)
						IF _lnovalinha  ///adiciona nueva linea al acols
				...
				_aColsCop:=aCols  ///pasamos lo que tiene aCols a acolscop
				aCols:={}  //vacia el acols para copiar nuevamente lo modificado en la busqueda por cantidad

				_nItem:=0
				N:=0
				///solo se copiaran los items que tengan cantidad mayor a 0 en la pantalla
				For _nCont := 1 to  Len(_aColsCop) 
					If _aColsCop[_nCont,nPQtdVen] > 0.00

						DbSelectArea('SB1')
						DbSetOrder(1)
						If DbSeek(xFilial("SB1")+_aColsCop[_nCont,nPoscod],.f.)  ///que producto es

							N++
							AADD(aCols,_aColsCop[_nCont])  //copiamos toda la fila
							...
							cxDepTo := _aColsCop[_nCont,nPosDep]
							nxPrunit:= _aColsCop[_nCont,nPosPru]

							//				alert("Item: " + cvaltochar(StrZero(N,Len(SC6->C6_ITEM))))
							//HASTA ACA ESTA IGUAL EL DEPOSITO

							A093Prod(.f.)
							A410Produto(_aColsCop[_nCont,nPoscod],.t.)
							A410MultT()
							...
							aCols[LEN(aCols),nPosDep] := cxDepTo
							
							if  M->C5_MOEDA == 1 .and. lMoeda .AND. (SB1->B1_XIMPORT == "S" .OR. EMPTY(SB1->B1_XIMPORT))
								aCols[LEN(aCols),nPosPru] := nxPrunit
							endif

							if funname() $ "MATA410"
								if M->C5_MOEDA == 1 .and. lMoeda .AND. (SB1->B1_XIMPORT == "S" .OR. EMPTY(SB1->B1_XIMPORT))
									aCols[LEN(aCols),nPPrcVen]	:= round(iif(nPreco == NIL,0,nPreco), 2) //precio venta
									aCols[LEN(aCols),nPValor]	:= round(iif(nxValTot == NIL,0,nxValTot),2) //valor total
									aCols[LEN(aCols),nPosPru]	:= round(iif(nxPrunit == NIL,0,nxPrunit),2) //valor lidys

									nValPDesc := acols[LEN(aCols)][nPrcDesc]  /// porcentaje
									aCols[LEN(aCols),nValDesc] := 0
									nxValDes := 0

									nxPreco := u_FtDescItem(u_FtDescCab(aCols[LEN(aCols),nPosPru],{M->C5_DESC1,M->C5_DESC2,M->C5_DESC3,M->C5_DESC4}),@aCols[LEN(aCols),nPPrcVen],acols[LEN(aCols)][nPQtdVen],@acols[LEN(aCols)][nPValor],@nValPDesc,@nxValDes,@nxValDes,1,,NIL)

									acols[LEN(aCols)][nPPrcVen] := round(nxPreco,2)   //precio venta
									acols[LEN(aCols)][nPValor] := round(acols[LEN(aCols)][nPQtdVen] * acols[LEN(aCols)][nPPrcVen],2)
									acols[LEN(aCols)][nPrcDesc] := round(nValPDesc,2)
									acols[LEN(aCols)][nValDesc] := round(nxValDes,2)
	...
	If DbSeek(xFilial("SB1")+cProduto,.f.)
		...
		While !Eof()
			...
			If 	TCCANOPEN("DA1"+SM0->M0_CODIGO+"0")
				...
				/////Adicion para moneda auxiliar
				nTC = POSICIONE("SM2",1,DATE(),"M2_MOEDA2")///TRAER TASA DE CAMBIO 2
				nTC2 = POSICIONE("SM2",1,DATE(),"M2_MOEDA5")///TRAER TASA DE CAMBIO 2

				IF SB1->B1_XIMPORT == "S" .OR. EMPTY(SB1->B1_XIMPORT) ////preguntamos si es importacion para activar la tasa auxiliar
					nTC = nTC2
				endif
				...
				If nTC > 0
					...
					IIF(funname() $ "MATA410",cQuery += "AND DA1_CODTAB = '" + M->C5_TABELA+ "' ", " ")
		...
		Else
			...
			If DbSeek(xFilial("SB1")+_cProdSel,.f.)
				...
				If aItens[oBrowse:nAt][4] > 0.00
					...
					if !empty(_aColsCop) ////SI SELECCIONE VARIOS OSEA YA LA SEGUNDA VEZ

						// validacion de nuevos SI ES EL MISMO PRODUCTO Y QUE LINEA ES
						if _cProdSel == _aColsCop[LEN(_aColsCop),nPoscod] .and. aItens[oBrowse:nAt][1] == _aColsCop[LEN(_aColsCop),nPosDep]
							...
							if len(aViewDA1) > 0
								if funname() $ "MATA410"
									IF M->C5_MOEDA == 1 .and. lMoeda .AND. (SB1->B1_XIMPORT == "S" .OR. EMPTY(SB1->B1_XIMPORT))
										_aColsCop[LEN(_aColsCop),nPPrcVen] := round(nDa1Prc, 2)
										_aColsCop[LEN(_aColsCop),nPValor] := round(aItens[oBrowse:nAt][4]*_aColsCop[LEN(_aColsCop),nPPrcVen], 2)
										_aColsCop[LEN(_aColsCop),nPosPru] := round(nDa1Prc, 2)
									endif
								endif
							endif
							...
							///cargamos el valor aceptado de la lista de precio
							if len(aViewDA1) > 0
								if funname() $ "MATA410"
									IF M->C5_MOEDA == 1 .and. lMoeda .AND. (SB1->B1_XIMPORT == "S" .OR. EMPTY(SB1->B1_XIMPORT))
										_aColsCop[LEN(_aColsCop),nPPrcVen] := round(nDa1Prc, 2)
					...
					else////PRIMERA BUSQUEDA ENTRA ACA
						aadd(_aColsCop,Array(nUsado+1))
						_aColsCop[LEN(_aColsCop),nPoscod]  := _cProdSel					//Codigo del Producto
						_aColsCop[LEN(_aColsCop),nPQtdVen]  := aItens[oBrowse:nAt][4]	//Cantidad
						_aColsCop[LEN(_aColsCop),nPosDep]  := aItens[oBrowse:nAt][1]	//Almacen
				...
				else  ///entro aca por que no encontro cantidades disponibles esto devuelto por nXCant getstockneg
					if(!empty(_aColsCop))///si es diferente de vacio y  validamos que sea mismo producto
						if _cProdSel == _aColsCop[LEN(_aColsCop),nPoscod] .and. aItens[oBrowse:nAt][1] == _aColsCop[LEN(_aColsCop),nPosDep]
							_aColsCop[LEN(_aColsCop),nPoscod]  := _cProdSel					//Codigo del Producto
							_aColsCop[LEN(_aColsCop),nPQtdVen]  := aItens[oBrowse:nAt][4]	//Cantidad
							_aColsCop[LEN(_aColsCop),nPosDep]  := aItens[oBrowse:nAt][1]	//Almacen
