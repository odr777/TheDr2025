...
User Function impFacen
	...
	pergunte("MTXRED",.T.)
	// alert("NUEVO")
	@ 000,000 TO 120,380 DIALOG oDlg TITLE OemToAnsi("Importacion de Facturas de entrada")
	@ 003,003 TO 058,188
...
Static Function Importar
	...
	* <ExpC3> - Diretorio inicial se necessário
	...
	*/
	...
	If File( cFile )
		...
		cLinea	:= FT_FREADLN()
		// alert("primerArray " + cLinea)
		aCpos	:= Str2Array(cLinea,';',',','|')
		// alert("TerminaPrimerArray")
		...
		If !lError
			For nX := 1 To Len(aTotCabs)
				...
				nPosGlosa := aScan(aTotCabs[nX],{|x| Upper(AllTrim(x[1])) == "F1_XOBS"})
				...
				Endif
				cxGLosa	:= If(nPosGlosa > 0	,aTotCabs[nX,nPosGlosa][2]	, ""	)
				...
				*/
				...
				//³ Caso la Remision tuviera ItemContable /Relaciona con el Proyecto/Tarea (F10) ³
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function Str2Array(cString,cSep ,cSep2 , cSep3)
	Local 	aReturn := { }		,;
	cAux    := cString	,;
	nPos    := 0
	Local Csepaux := ';'// estándar

	if At( cSep, cAux ) < 14 .and. At( cSep, cAux ) > 0
		Csepaux := ';'
		// alert("entro ;")
	elseif At( cSep2, cAux ) < 14  .and. At( cSep2, cAux ) > 0
		Csepaux := ','
		// alert("entro ,")
	elseif At( cSep3, cAux ) < 14  .and. At( cSep3, cAux ) > 0
		Csepaux := '|'
		// alert("entro |")
	end if

	While At( Csepaux, cAux ) > 0
		// alert(Csepaux)
		nPos  := At( Csepaux, cAux )
