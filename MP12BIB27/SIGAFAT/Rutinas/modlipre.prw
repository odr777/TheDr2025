...
User Function modlipre()
	Private oDlg

	@ 000,000 TO 120,380 DIALOG oDlg TITLE OemToAnsi("Importaci๓n de Lista de precios")
	@ 003,003 TO 058,188
	@ 010,018 Say "Este programa tiene como objetivo modificar una o mas "
...
//				.AND. cxProd  == aTotCabs[nX,nPosProd][2]*/
					...
					nMoeda := aScan(aTotCabs[nX],{|x| Upper(AllTrim(x[1])) == "DA1_MOEDA" })


					cxProdit		:= If(nCodProdto 	> 0	,aTotCabs[nX,nCodProdto][2]	, ""	)
					nPreco		:= If(nPrecioVta 	> 0	,aTotCabs[nX,nPrecioVta][2]	, 1	)
					nMoedaItem		:= If(nPrecioVta 	> 0	,aTotCabs[nX,nMoeda][2]	,2	)
					
					// DA1_FILIAL+DA1_CODTAB+DA1_CODPRO+DA1_INDLOT+DA1_ITEM
					cItem := GetAdvFVal("DA1", "DA1_ITEM", xFilial("DA1") + cListaPrc + cxProdit, 1, "")
					if Empty(cItem)
						aadd(aItens,{"LINPOS",     "DA1_ITEM",cItem })
					ENDIF
            		// aAdd(aItens,{"DA1_ITEM" , StrZero(nX,4,0), NIL})
					aAdd(aItens,{"DA1_CODPRO" , cxProdit, NIL})
					aAdd(aItens,{"DA1_MOEDA" , nMoedaItem, NIL})
				...
				MSExecAuto({|X,Y,Z| OMSA010(X,Y,Z)}, aCabecalho, aLinha, 4)
