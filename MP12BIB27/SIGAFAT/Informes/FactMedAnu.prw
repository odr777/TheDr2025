    // *********************************
    //  No está compilada en BIB2310
	//	Se uniformizó campo F2_UNOME (antes F2_UNOMFAT)
	// 	ODR 12.03.2024
    // *********************************
...
User Function FactMedAnu(cxFil,cxNro1,cxNro2,cxSerie,nLinInicial)
	Pergunte("FACANUL",.T.)

	FatLocal('Factura Anulada',.F.,mv_par01,mv_par02,mv_par02,mv_par03,1)
Return nil
...
Static function FatLocal(cTitulo,bImprimir,cFil,cNroFat1,cNroFat2,cSerie,nLinInicial)
	...
	DEFINE FONT oFont12 NAME "Arial" SIZE 0,11 OF oPrn
	DEFINE FONT oFont12N NAME "Arial" SIZE 0,11 Bold  OF oPrn
...
static function FatMat(cFil1,cDoc1,cDoc2,cSerie,cTitulo,bImprimir,nCopia,nLinInicial)
	...
	Local cSql	:= "SELECT F2_FILIAL,F2_SERIE,F2_DOC,F2_NUMAUT,F2_EMISSAO,F2_UNOME,F2_UNITCLI,F2_COND,F2_VALMERC,ROUND(F2_DESCONT,2) F2_DESCONT,F2_BASIMP1,F2_CODCTR,FP_DTAVAL,F2_USRREG,D2_PEDIDO,A1_MUN, A1_BAIRRO,A1_END"
	...
	If .t.//!Empty(FP_NUMAUT)	//Imprimir Serie que tengan Nro de Autorización en la Dosificación
		While !(NextArea)->(Eof())
			...
			If ( bSw == .T.)
				...
				AADD(aDupla,(NextArea)->F2_UNOME)         //6
				...
				FatImp(cTitulo,bImprimir,aDupla,aDetalle,nLinInicial,"ANULADA",cDireccion,cTelefono)    	//Imprimir Factura
...
Static Function CabFact(nLinInicial,aMaestro,cTipo,cDireccion,cTelefono)
	...
	oPrn:Say( nLinInicial + 185, 1620,"NOTA No: "  ,oFont12N )
	endif
	...
	oPrn:Say( nLinInicial + 200, 1110,cTipo,oFont105N )
	...
	oPrn:Box( nLinInicial + 640 , 80 ,  1030 , 2370 )
	//Titulos reporte
	...
	//Lineas verticales
	oPrn:Box( nLinInicial + 640 , 0310 ,  1030 , 0310 )
	oPrn:Box( nLinInicial + 640 , 1540 ,  1030 , 1540 )
	oPrn:Box( nLinInicial + 640 , 2010 ,  1030 , 2010 )


	cFLogoMarkas := GetSrvProfString("Startpath","") + "LogoMarkas.bmp"
	oPrn:SayBitmap(090,80, cFLogoMarkas,200,200)
	oPrn:Say(280, 80, "MARKAS SRL." ,oFont11N )
...
Static Function PieFact(nLinInicial,aMaestro,nCont)
	...
	oPrn:Box( nLinInicial + 2630 , 810 ,  1240 , 1170 )
	oPrn:Box( nLinInicial + 2700 , 810 ,  1160 , 1170 )

	oPrn:Box( nLinInicial + 2630 , 1210 ,  1240 , 1570 )
	oPrn:Box( nLinInicial + 2700 , 1210 ,  1160 , 1570 )

	oPrn:Box( nLinInicial + 2630 , 1610 ,  1240 , 1970 )
	oPrn:Box( nLinInicial + 2700 , 1610 ,  1160 , 1970 )

	oPrn:Box( nLinInicial + 2630 , 2010 ,  1240 , 2370 )
