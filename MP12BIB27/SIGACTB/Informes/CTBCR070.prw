...
Static Function ReportPrint( oReport )
	...
	Local cQuery		:= "CT2"
	Local n				:= 0
	Local cLote			:= IIF(funname() == "CTBA102","",M->CLOTE)
	Local cSubLote		:= IIF(funname() == "CTBA102","",M->cSubLote)
	Local cDoc      	:= IIF(funname() == "CTBA102","",M->CDOC)	
	Local cLoteRes      := ""
	Local cSubRes		:= ""
	Local cDocRes    	:= ""
	Local nTotalDeb	 	:= 0
	...
	Else
		#IFDEF TOP
			If TcSrvType() <> "AS/400"
				...
				cQuery += "WHERE CT2_FILIAL  = '" + xFilial("CT2") + "' AND "
				cQuery +=       "CT2_DATA   >= '" + IIF(funname() == "CTBA102",DTOS(mv_par01),DTOS(M->DDATALANC))  + "' AND "
				cQuery += 	    "CT2_DATA   <= '" + IIF(funname() == "CTBA102",DTOS(mv_par02),DTOS(M->DDATALANC))  + "' AND "
				cQuery += 	    "CT2_LOTE   >= '" + IIF(funname() == "CTBA102",mv_par03,M->CLOTE)  + "' AND "
				cQuery += 	    "CT2_LOTE   <= '" + IIF(funname() == "CTBA102",mv_par04,M->CLOTE)  + "' AND "
				cQuery += 	    "CT2_SBLOTE >= '" + IIF(funname() == "CTBA102",mv_par15,M->cSubLote) + "' AND "
				cQuery += 		"CT2_SBLOTE <= '" + IIF(funname() == "CTBA102",mv_par16,M->cSubLote) + "' AND "
				cQuery +=   	"CT2_DOC    >= '" + IIF(funname() == "CTBA102",mv_par05,M->CDOC) + "' AND "
				cQuery +=       "D_E_L_E_T_ LIKE '' AND "
				cQuery +=	   	"CT2_MOEDLC = '02'                      AND "
				cQuery += 		"CT2_DOC    <= '" + IIF(funname() == "CTBA102",mv_par06,M->CDOC) + "' ) CT22 On "				
				...
				cQuery += 		"CT2_SBLOTE <= '" + IIF(funname() == "CTBA102",mv_par16,M->cSubLote) + "' AND "
				cQuery +=   	"CT2_DOC    >= '" + IIF(funname() == "CTBA102",mv_par05,M->CDOC) + "' AND "
				cQuery +=   	"CT2_DOC    <= '" + IIF(funname() == "CTBA102",mv_par06,M->CDOC) + "' AND "
				cQuery +=       "D_E_L_E_T_ LIKE '' AND "
				...
				cQuery +=   	"CT2_DOC    >= '" + IIF(funname() == "CTBA102",mv_par05,M->CDOC) + "' AND "
				cQuery +=   	"CT2_DOC    <= '" + IIF(funname() == "CTBA102",mv_par06,M->CDOC) + "' AND "
				cQuery += 		"CT2_MOEDLC = '"  + IIF(funname() == "CTBA102",mv_par07,'01') + "' AND "
...
//			cQuery += "ORDER BY 1,CT2_FILIAL, CT2_DATA, CT2_LOTE, CT2_SBLOTE, CT2_DOC, CT2_LINHA" Nahim 2019/08/14
		...
		While !Eof() .And. (cAliasCT2)->CT2_FILIAL == xFilial("CT2") .And. (cAliasCT2)->CT2_DATA <= IIF(funname() == "CTBA102",mv_par02,M->DDATALANC) .And. !oReport:Cancel()
			lCabecera := .T.
...
Static Function impCuadro()
	//CAJAS
	oReport:Box( 620,80,2450,2370)	//Recuadro
	oReport:Box( 620,80,695,2370)	// Recuadro Titulos
	oReport:Box( 2330,80,2450,2370)	// Recuadro Totales
	oReport:Box( 2550,80,3000,2370)	// Recuadro Firmas

	oReport:SAY(650, 120, "Código", oFont10TN	,1000,CLR_BLACK ,CLR_BLACK ) //Titulos
	oReport:SAY(650, 330, "Cuenta", oFont10TN	,1000,CLR_BLACK ,CLR_BLACK ) //Titulos
	oReport:SAY(650, 1030, "Glosa", oFont10TN	,1000,CLR_BLACK ,CLR_BLACK ) //Titulos
	oReport:SAY(650, 1780, "Debe", oFont10TN	,1000,CLR_BLACK ,CLR_BLACK ) //Titulos
	oReport:SAY(650, 2080, "Haber", oFont10TN	,1000,CLR_BLACK ,CLR_BLACK ) //Titulos
	//oReport:SAY(580, 2120, "Cheque", oFont10TN	,1000,CLR_BLACK ,CLR_BLACK ) //Titulos

	oReport:Box( 620,300,2330,300)		//Linea Vertical
	...
	oReport:Line(620,1750,2450,1750) // Nahim
	oReport:Line(620,2050,2450,2050) // Nahim
