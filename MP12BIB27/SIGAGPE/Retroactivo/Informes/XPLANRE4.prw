...
User Function XPLANRE4()
	Local oReport
	Private cPerg   := "XPLANRE4"   // elija el Nombre de la pregunta
	Private aTabS011:= {}
...
Static Function ReportDef()
	...
	oReport	 := TReport():New("XPLANRE4",NombreProg,cPerg,{|oReport| PrintReport(oReport)},"Planilla Retroactivo Mensual")
	...
	TRCell():New(oSection,"HBDICIEM"	,"SRA","HABER BASICO DICIEMBRE")
	TRCell():New(oSection,"NUEVOHB"		,"SRA","HABER BASICO CON INCREMENTO")

	TRCell():New(oSection,"ENERET"		,"SRC","Ret. Enero")
	TRCell():New(oSection,"ENEBA"		,"SRC","Bono Antigüedad Enero")
	TRCell():New(oSection,"ENEHE"		,"SRC","Horas Extras Enero")
	TRCell():New(oSection,"ENETOT"		,"SRC","Total Enero")

	TRCell():New(oSection,"FEBRET"		,"SRC","Ret. Febrero")
	TRCell():New(oSection,"FEBBA"		,"SRC","Bono Antigüedad Febrero")
	TRCell():New(oSection,"FEBHE"		,"SRC","Horas Extras Febrero")
	TRCell():New(oSection,"FEBTOT"		,"SRC","Total Febrero")

	TRCell():New(oSection,"MARRET"		,"SRC","Ret. Marzo")
	TRCell():New(oSection,"MARBA"		,"SRC","Bono Antigüedad Marzo")
	TRCell():New(oSection,"MARHE"		,"SRC","Horas Extras Marzo")
	TRCell():New(oSection,"MARTOT"		,"SRC","Total Marzo")

	TRCell():New(oSection,"ABRRET"		,"SRC","Ret. Abril")
	TRCell():New(oSection,"ABRBA"		,"SRC","Bono Antigüedad Abril")
	TRCell():New(oSection,"ABRHE"		,"SRC","Horas Extras Abril")
	TRCell():New(oSection,"ABRTOT"		,"SRC","Total Abril")

	TRCell():New(oSection,"TOTALRET"	,"SRC","TOTAL RETROACTIVO (Bs)")
	TRCell():New(oSection,"AFP"			,"SRC","AFP 12.71%", PesqPict( 'SRC', 'RC_VALOR' ))
	TRCell():New(oSection,"AFPSOL"		,"SRC","AFP Solidario", PesqPict( 'SRC', 'RC_VALOR' ))
...
Static Function PrintReport(oReport)
	...
	Local nSumaRBA	:= 0//Suma los bonos de antiguedad de enero, febrero, marzo y abril por empleado
	...
	Local nRE1HB	:= 0//Suma haber básico
	Local nRE1BA	:= 0//Suma los bonos de antiguedad 
	Local nRE1EX	:= 0//Suma los dominicales
	Local nRE1Tot	:= 0//Suma Total

	Local nRE2HB	:= 0//Suma haber básico
	Local nRE2BA	:= 0//Suma los bonos de antiguedad 
	Local nRE2EX	:= 0//Suma los dominicales
	Local nRE2Tot	:= 0//Suma Total

	Local nRE3HB	:= 0//Suma haber básico
	Local nRE3BA	:= 0//Suma los bonos de antiguedad 
	Local nRE3EX	:= 0//Suma los dominicales
	Local nRE3Tot	:= 0//Suma Total

	Local nRE4HB	:= 0//Suma haber básico
	Local nRE4BA	:= 0//Suma los bonos de antiguedad 
	Local nRE4EX	:= 0//Suma los dominicales
	Local nRE4Tot	:= 0//Suma Total
	...
	ElseIf(nOrdem == 2)
		...
		cOrdLan	  := "RA_PRISOBR, RA_SECSOBR, RA_PRINOME, RA_SECNOME, RA_FILIAL, RA_MAT"
	EndIf
	...
	#IFDEF TOP
		...
		While (cAliasQry)->(! EOF())
			...
			cCi			:= TRIM((cAliasQry)->RA_RG) + " " + TRIM((cAliasQry)->RA_ALFANUM)
			...
			if( (cAliasQry)->RV_CODFOL == HBCODFOL )				
				nSumaRHB+= (cAliasQry)->RC_VALOR
				IF 		(cAliasQry)->RC_ROTEIR == "RE1" 
					nRE1HB += (cAliasQry)->RC_VALOR
					nRE1Tot += (cAliasQry)->RC_VALOR
				ELSEIF	(cAliasQry)->RC_ROTEIR == "RE2" 
					nRE2HB += (cAliasQry)->RC_VALOR
					nRE2Tot += (cAliasQry)->RC_VALOR
				ELSEIF	(cAliasQry)->RC_ROTEIR == "RE3" 
					nRE3HB += (cAliasQry)->RC_VALOR
					nRE3Tot += (cAliasQry)->RC_VALOR
				ELSEIF	(cAliasQry)->RC_ROTEIR == "RE4" 
					nRE4HB += (cAliasQry)->RC_VALOR
					nRE4Tot += (cAliasQry)->RC_VALOR
				ENDIF
			EndIf
			...
			IF((cAliasQry)->RV_CODFOL == CODDOMIN)//Dominical				
				...
				IF 		(cAliasQry)->RC_ROTEIR == "RE1" 
					nRE1EX += (cAliasQry)->RC_VALOR
					nRE1Tot += (cAliasQry)->RC_VALOR
				ELSEIF	(cAliasQry)->RC_ROTEIR == "RE2" 
					nRE2EX += (cAliasQry)->RC_VALOR
					nRE2Tot += (cAliasQry)->RC_VALOR
				ELSEIF	(cAliasQry)->RC_ROTEIR == "RE3" 
					nRE3EX += (cAliasQry)->RC_VALOR
					nRE3Tot += (cAliasQry)->RC_VALOR
				ELSEIF	(cAliasQry)->RC_ROTEIR == "RE4" 
					nRE4EX += (cAliasQry)->RC_VALOR
					nRE4Tot += (cAliasQry)->RC_VALOR
				ENDIF
			EndIf

			IF((cAliasQry)->RV_CODFOL == BACODFOL)//Bono de antiguedad				
				nSumaRBA+= (cAliasQry)->RC_VALOR
				IF 		(cAliasQry)->RC_ROTEIR == "RE1" 
					nRE1BA += (cAliasQry)->RC_VALOR
					nRE1Tot += (cAliasQry)->RC_VALOR
				ELSEIF	(cAliasQry)->RC_ROTEIR == "RE2" 
					nRE2BA += (cAliasQry)->RC_VALOR
					nRE2Tot += (cAliasQry)->RC_VALOR
				ELSEIF	(cAliasQry)->RC_ROTEIR == "RE3" 
					nRE3BA += (cAliasQry)->RC_VALOR
					nRE3Tot += (cAliasQry)->RC_VALOR
				ELSEIF	(cAliasQry)->RC_ROTEIR == "RE4" 
					nRE4BA += (cAliasQry)->RC_VALOR
					nRE4Tot += (cAliasQry)->RC_VALOR
				ENDIF
			EndIf
			...
			If(cValida <> (cAliasQry)->RA_FILIAL + (cAliasQry)->RA_MAT)
				//nTotalRET	:= nHBEne + nHBFeb + nHBMar + nHBAbr + nSumaDOM + nSumaRBA + nSumaHE + nSumaHN
				nTotalRET	:= nSumaRHB + nSumaDOM + nSumaRBA + nSumaHE + nSumaHN
				nTotalDESC	:= nAFP + nRCIVA // + nOtrDesc

				cFilialPer	:= U_xGetFilialPeriodoRet(cMat,cPerDic)//obtiene la sucursal del empleado en el periodo enviado.
				...
				nBADic		:= GetAdvFVal("SRD", "RD_VALOR", cFilialPer + cMat + cPerDic + cPerDic + cPDBA ,3, 0)
				nPorcBA		:= GetAdvFVal("SRD", "RD_HORAS", cFilialPer + cMat + cPerDic + cPerDic + cPDBA ,3, 0)
				nNuevoBA	:= nBADic + nSumaRBA
				nNuevoHB	:= nHbDic + if(EMPTY(cDemissa),0,nSumaRHB)
				...
				oSection:Cell("ENERET"):SetValue( nRE1HB )
				oSection:Cell("ENEBA"):SetValue( nRE1BA )
				oSection:Cell("ENEHE"):SetValue( nRE1EX )
				oSection:Cell("ENETOT"):SetValue( nRE1Tot )

				oSection:Cell("FEBRET"):SetValue( nRE2HB )
				oSection:Cell("FEBBA"):SetValue( nRE2BA )
				oSection:Cell("FEBHE"):SetValue( nRE2EX )
				oSection:Cell("FEBTOT"):SetValue( nRE2Tot )

				oSection:Cell("MARRET"):SetValue( nRE3HB )
				oSection:Cell("MARBA"):SetValue( nRE3BA )
				oSection:Cell("MARHE"):SetValue( nRE3EX )
				oSection:Cell("MARTOT"):SetValue( nRE3Tot )

				oSection:Cell("ABRRET"):SetValue( nRE4HB )
				oSection:Cell("ABRBA"):SetValue( nRE4BA )
				oSection:Cell("ABRHE"):SetValue( nRE4EX )
				oSection:Cell("ABRTOT"):SetValue( nRE4Tot )
