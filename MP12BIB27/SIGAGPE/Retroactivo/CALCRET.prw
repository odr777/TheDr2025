...
user function CALCRET(cRA_FILIAL, cRA_MAT)
	...
	Local cPDBasHB	:= "660"//Concepto base - Haber bแsico antes del retroactivo
	Local cPDBasBA	:= "661"//Concepto base - Bono de antiguedad antes del retroactivo
	Local cFilialPer:= U_xGetFilialPeriodoRet(cRA_MAT,cPerRET)
	Local nX		:= 1
	...
	WHILE !EOF() .AND. SRD->RD_MAT == cRA_MAT .AND. SRD->RD_PERIODO == cPerRET;
		...
		for nX := 1 to len(aListaPD)
			if(SRD->RD_PD == aListaPD[nX])
				...
				AADD (aTipo1, SRD->RD_TIPO1)
				AADD (aTipo2, SRD->RD_TIPO2)
			endif
		next nX
	...
	while nListPER <= len(aListaPer)
		if val(aListaPer[nListPER]) >= val(cPerRET) .AND. val(aListaPer[nListPER]) < val(cPerActual)
			...
			ElseIf(aPD[nListPER] == cPDBonoAnt)//Bono de Antiguedad
				nValAnterior := aVal[nListPER]
				nVal	:= calculaBA(aVal[nListPER], nMinimoActual, aHoras[nListPER], nDiasTrab, aListaPer[nListPER], cRA_FILIAL, cRA_MAT)
				nValInfo:= nSMN

				// Guardar Bono antiguedad actual c๓mo concepto base
				ReClock("SRC",.T.)
				SRC-> RC_MAT	:= cRA_MAT
				SRC-> RC_FILIAL	:= cRA_FILIAL
				SRC-> RC_PD		:= cPDBasBA
				SRC-> RC_TIPO1	:= aTipo1[nListPER]
				SRC-> RC_HORAS	:= aHoras[nListPER]
				SRC-> RC_VALINFO:= nValInfo
				SRC-> RC_VALOR	:= nValAnterior
				SRC-> RC_DTREF	:= dDataRef
				SRC-> RC_DATA	:= dDataRef
				SRC-> RC_SEMANA	:= aSEM[nListPER]
				SRC-> RC_CC		:= aCC[nListPER]
				SRC-> RC_TIPO2	:= aTipo2[nListPER]
				SRC-> RC_SEQ	:= aSeq[nListPER]
...
*/
User function xGetFilialPeriodoRet(cMat,cPer)
	Local aArea	:= getArea()
	Local StrSql:= ""
	Local cRet	:= ""
...
* @author: Denar Terrazas Parada
* @since: 04/06/2021
* @updated: 30/05/2022 - Denar Terrazas
...
*				cPer		-> Periodo de cแlculo pasado(ENE,FEB,MAR o ABR)
*				cXSuc		-> Sucursal del empleado(SRA->RA_FILIAL)
*				cXMat		-> Matrํcula del empleado(SRA->RA_MAT)
*/
static function calculaBA(nBAAnterior, nMinimo, nPorcentaje, nDiasTrab, cPer, cXSuc, cXMat)
	Local aArea		:= getArea()
	Local nRet		:= 0
	Local mNemoBA	:= "M_LPRORRATBA"//Mnemonico->PRORRATEA BONO DE ANTIGUEDAD (RETROACTIVO)

	nRet:= (nMinimo * 3) * (nPorcentaje / 100)
	...
	if(lProrratea)//Si prorratea bono de antiguedad con los dํas trabajados
		cRA_PROCES	:= GetAdvFVal("SRA", "RA_PROCES", cXSuc + cXMat, 1, "")//RA_FILIAL+RA_MAT+RA_NOME
		if(!Empty(cRA_PROCES))
			cXPerAux	:= SUBSTR(DTOS( MONTHSUB(STOD(cPer+"01"), 1) ), 1, 6)
			nXBonoMesAnt:= GetAdvFVal("SRD", "RD_VALOR", cXSuc + cXMat + cXPerAux + cXPerAux + FGETCODFOL("0671") ,3,0)
			nXDiasMes	:= IIF( GetAdvFVal("RCJ", "RCJ_CALEND", XFILIAL("RCJ") + cRA_PROCES, 1, "") = "1", 30, DAY(LASTDATE(STOD(cPer+"01"))) )
			IF ( nXBonoMesAnt <= 0 )
				dRA_ADMISSA	:= GetAdvFVal("SRA", "RA_ADMISSA", cXSuc + cXMat, 1, CTOD("  /  /    "))//RA_FILIAL+RA_MAT+RA_NOME
				If(!Empty(dRA_ADMISSA))
					nDiasTrab:= nXDiasMes - DAY(dRA_ADMISSA) + 1
				EndIf
			EndIF
		EndIf
