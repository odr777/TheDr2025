...
User Function GP120VLD()
	...
	aNetos:= getNetos(cFilRCH, cProces, cPeriod, cNumPag, cRot, cPDValid)
...
*/
Static Function getNetos(cFilRCH, cProces, cPeriod, cNumPag, cRot, cPDValid)
	Local aArea			:= getArea()
	...
	BeginSql Alias OrdenConsul
		...
		FROM %table:SRC%
		WHERE SUBSTRING(RC_FILIAL,1,LEN(%exp:cFilRCH%)) = %exp:cFilRCH%
		AND RC_PROCES = %exp:cProces%
