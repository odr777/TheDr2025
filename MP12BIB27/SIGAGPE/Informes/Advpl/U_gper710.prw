...
User Function GPER710()
	...
	Private nVCol27     := 0
	Private nVCol28     := 0
	Private lIsRET		:= .F.//Utilizado para saber si se trata de un procedimiento de Retroactivo(RE1, RE2, RE3, RE4)
	...
	lIsRET:= (!Empty(cXRoteir) .AND. cXRoteir $ 'RE1|RE2|RE3|RE4')
...
Static Function ReportDef(oReport)
	...
	if(nTipo == 1)//Prevision
		...
		TRCell():New(oSection1, "RA_NATURAL",	cString, "(14) EXTENSIÓN"	,, 25  ,,,"CENTER", , , , , .T. , , , .T.)  //"EXT"
		...
		TRCell():New(oSection1, "RA_ESTADO",	cString, "(F) DEPARTAMENTO"			,, 25 ,,,"CENTER", , , , , .T. , , , .T.)  //"DEPARTAMENTO"
		TRCell():New(oSection1, "NOVEDAD",		cString, "(17) NOVEDAD I/R/L/S"				,, 50  ,,,"CENTER", , , , , .T. , , , .T.)  //"NOVEDAD"
		TRCell():New(oSection1, "FECHNOVEDAD",	cString, "(18) FECHA NOVEDAD dd/mm/aaaa"		,, 50  ,,,"CENTER", , , , , .T. , , , .T.)  //"FECH-NOVEDAD"
		TRCell():New(oSection1, "DIAS-COT",		cString, "(19) DIAS COTIZADOS"					,, 50  ,,,"CENTER", , , , , .T. , , , .T.)  //"DIAS-COT"
		TRCell():New(oSection1, "RA_TPSEGUR",	cString, "(20) TIPO DE ASEGURADO (M/C/E)"		,, 50  ,,,"CENTER", , , , , .T. , , , .T.)  //"TP Segurado"
		TRCell():New(oSection1, "nVCol21",		cString, "(21) TOTAL GANADO DEPENDIENTE < 65 AÑOS O ASEGURADO CON PENSION DEL SIP < 65 AÑOS QUE DECIDE APORTAR AL SIP"			,, 50  ,,,"CENTER", , , , , .T. , , , .T.)
		TRCell():New(oSection1, "nVCol22",		cString, "(22) TOTAL GANADO DEPENDIENTE > 65 AÑOS O ASEGURADO CON PENSION DEL SIP > 65 AÑOS QUE DECIDE APORTAR AL SIP"			,, 50  ,,,"CENTER", , , , , .T. , , , .T.)
		TRCell():New(oSection1, "nVCol23",		cString, "(23) TOTAL GANADO ASEGURADO CON PENSION DEL SIP < 65 AÑOS QUE DECIDE NO APORTAR AL SIP"			,, 50  ,,,"CENTER", , , , , .T. , , , .T.)
		TRCell():New(oSection1, "nVCol24",		cString, "(24) TOTAL GANADO ASEGURADO CON PENSION AL SIP > 65 AÑOS QUE DECIDE NO APORTAR AL SIP"			,, 50  ,,,"CENTER", , , , , .T. , , , .T.)
		TRCell():New(oSection1, "nVCol25",		cString, "(25) COTIZACION ADICIONAL"				,, 50  ,,,"CENTER", , , , , .T. , , , .T.)
		TRCell():New(oSection1, "nVCol26",		cString, "(26) TOTAL GANADO FONDO DE VIVIENDA"		,, 50  ,,,"CENTER", , , , , .T. , , , .T.)
		TRCell():New(oSection1, "nVCol27",		cString, "(27) TOTAL GANADO FONDO SOLIDARIO"		,, 50  ,,,"CENTER", , , , , .T. , , , .T.)
		TRCell():New(oSection1, "nVCol28",		cString, "(28) TOTAL GANADO FONDO SOLIDARIO MINERO"	,, 50  ,,,"CENTER", , , , , .T. , , , .T.)
	endIf
...
Static Function GR710Imp(oReport)
	...
	For nAux:=1 to Len(aPerFechado)
		...
		While (cQrySRD)->(!Eof()) .And.  (cQrySRA)->(RA_FILIAL+RA_MAT)+aPerFechado[nAux][7]== (cQrySRD)->(RD_FILIAL+RD_MAT+RD_PROCES)
				...
				lMaior := nIdade >= 65
			If(lIsRET)
					cTipoCod:= GetAdvFVal("SRV", "RV_TIPOCOD", xFilial("SRV") + (cQrySRD)->RD_PD ,1,"")//RV_FILIAL+RV_COD
					//lVerba := (cQrySRD)->RD_PD == aCodFol[730,1]
					lVerba := Trim(cTipoCod) == '1'//Si es concepto de Remuneración
			Else
					lVerba := (cQrySRD)->RD_PD == aCodFol[731,1]//Estandar
			EndIf

			If lVerba
					//If !lJubilac .AND. "2" $ (cQrySRA)->RA_AFPOPC
					If(lIsRET)
			...
			if((cQrySRD)->RD_PD == aCodFol[1112,1])
					nVCol25:= (cQrySRD)->RD_VALOR
			endIf
			if((cQrySRD)->RD_PD == aCodFol[0737,1])
					nVCol26:= (cQrySRD)->RD_VALOR
			endIf
			if((cQrySRD)->RD_PD == aCodFol[1227,1])
					nVCol27:= (cQrySRD)->RD_VALOR
			endIf
				/*nVCol25:= IIF((cQrySRD)->RD_PD == aCodFol[1112,1], (cQrySRD)->RD_VALOR, 0)
				nVCol26:= IIF((cQrySRD)->RD_PD == aCodFol[0737,1], (cQrySRD)->RD_VALOR, 0)
				nVCol27:= IIF((cQrySRD)->RD_PD == aCodFol[1227,1], (cQrySRD)->RD_VALOR, 0)*/
	...
	If lReg
		...
		if(nTipo == 2)
				...
				oSection1:Cell("nVCol21"):SetValue(0)  //Total Ganado (21) RD_VALOR
				oSection1:Cell("nVCol22"):SetValue(0)
				oSection1:Cell("nVCol23"):SetValue(0)
				oSection1:Cell("nVCol24"):SetValue(0)
				oSection1:Cell("nVCol25"):SetValue(nVCol25)
				oSection1:Cell("nVCol26"):SetValue(nVCol26)
				oSection1:Cell("nVCol27"):SetValue(nVCol27)
				oSection1:Cell("nVCol28"):SetValue(nVCol28)
			...
			elseIf((cQrySRA)->RA_AFPOPC == TIPOCOT2)
					oSection1:Cell("nVCol22"):SetValue(nVCol21)
			elseIf((cQrySRA)->RA_AFPOPC == TIPOCOTC)
					oSection1:Cell("nVCol23"):SetValue(nVCol21)
			elseIf((cQrySRA)->RA_AFPOPC == TIPOCOTD)
					oSection1:Cell("nVCol24"):SetValue(nVCol21)
			endIf
		else
