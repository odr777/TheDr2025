...
Static Function PrintReport(oReport)
	...
	BeginSql alias "QRYSR6"
		...
		(COALESCE(T.GANADOS,0)+COALESCE(T.DOUDECIMAS,0)-COALESCE(T.UTILIZADOS,0)-COALESCE(T.PROGRAMADOS,0)) SALDO
		FROM
		...
		/*(SELECT SUM(SR8.R8_DURACAO) FROM %Table:SR8% SR8 WHERE SR8.%notDel% AND SRA.RA_FILIAL = SR8.R8_FILIAL AND SRA.RA_MAT = SR8.R8_MAT AND SR8.R8_STATUS = '' AND SR8.R8_PD = %Exp:cPDVAC%
		AND SR8.R8_PER = (SELECT TOP 1 RCH_PER FROM RCH010 RCH WHERE RCH_FILIAL = %Exp:xFilial('RCH')% AND RCH_PROCES = SRA.RA_PROCES AND RCH_ROTEIR = 'FOL' AND RCH_DTFECH = '' AND RCH.%notDel% ORDER BY RCH_PER)
		) PROGRAMADOS*/
