...
User Function IMPRESBOL()
	...
	Local lUltimo		:= .F. //Creada para determinar si el empleado fue despedido en el ultimo día del mes
	...
	nAumento:= getAumento()//El aumento indica si será Carta u Oficio
	...
	lUltimo		:= (SRA->RA_DEMISSA == LASTDAY(SRA->RA_DEMISSA))
	...
	oPrint:say ( INT(0520 * nAumento), 1410, AllTrim(UPPER(SM0->M0_ENDENT)), oFont07 )
	...
	oPrint:say  ( INT(0729 * nAumento), 0064, STR0010 + cCarnet, oFont09 )    //"CI: "
	...
	oPrint:say  ( INT(0835 * nAumento), 0520, STR0016, oFont09 )    //"ANOS"
	...
	oPrint:say  ( INT(0835 * nAumento), 0725, STR0017, oFont09 )    //"MESES"
	...
	oPrint:say  ( INT(0835 * nAumento), 0920, STR0018, oFont09 )    //"DIAS"
	...
	Else
		RetHoraVal( "0754", @cHoras, @nValor, @cDesc, @cMesMed )
		lMes1:= IIF(Empty(nValor), .F., .T.)
		If(lMes1)//Si existe el promedio indemnizable del mes 1
			...
			If !Empty(cMesMed)
				...
				oPrint:say ( INT(1012 * nAumento), 0630, Upper(FDESC_MES(Val(SubsTr(cMesMed,5,2)))), oFont08)
			EndIf
			if( SRA->RA_ADMISSA <= STOD(cMesMed + "01") .OR. (SUBSTR(DTOS(SRA->RA_ADMISSA), 1, 6) == cMesMed .AND. Day(SRA->RA_ADMISSA) == Day(SRA->RA_DEMISSA)) )//Valida si el funcionario trabajó tres meses o más
				nValor:= getSalMes(SRA->RA_FILIAL, SRA->RA_MAT, cMesMed, .F.)
		...
		RetHoraVal( "0755", @cHoras, @nValor, @cDesc, @cMesMed )
		lMes2:= IIF(Empty(nValor), .F., .T.)
		If(lMes2)//Si existe el promedio indemnizable del mes 2
			if(Empty(AllTrim(cMesMed)))
				If(lUltimo)
		...
		RetHoraVal( "0756", @cHoras, @nValor, @cDesc, @cMesMed )
		lMes3:= IIF(Empty(nValor), .F., .T.)
		If(lMes3)//Si existe el promedio indemnizable del mes 3
			if(Empty(AllTrim(cMesMed)))
				If(lUltimo)
					cMesMed:= SUBSTR(DTOS(dDateRetiro), 1, 6)
				Else
					cMesMed:= SUBSTR(DTOS(MonthSub(dDateRetiro, 1)), 1, 6)
				EndIf
			endIf
			If !Empty(cMesMed)
				...
				oPrint:say ( INT(1012 * nAumento), 1600, Upper(FDESC_MES(Val(SubsTr(cMesMed,5,2)))), oFont08)
			EndIf
			nValor:= getSalMes(SRA->RA_FILIAL, SRA->RA_MAT, cMesMed, lUltimo)
			oPrint:say ( INT(1065 * nAumento), 1600, Transform(nValor, "@E 99,999,999.99"), oFont08)
			nTot3Remun := nValor
			nTotRemuner+= nValor
		EndIf
	...
	aPDVal:= getPDs(SRA->RA_FILIAL, SRA->RA_MAT, cMes1, cMes2, cMes3, lUltimo)

	cMes1Mes2:= cMes1
	If(!lMes1)
		cMes1Mes2:= cMes2
	EndIf
	if(SUBSTR(cMes1Mes2, 5, 2) $ '02|03|04')//Si el mes 1 está en alguno de los periodos de retroactivo(FEB,MAR,ABR)
	...
	for i:= 1 to Len(aPDVal)
		...
		If(lMes1)
			If( SRA->RA_ADMISSA <= STOD(cMes1 + "01")  .OR. (SUBSTR(DTOS(SRA->RA_ADMISSA), 1, 6) == cMes1 .AND. Day(SRA->RA_ADMISSA) == Day(SRA->RA_DEMISSA)) )//Valida si el funcionario trabajó tres meses o más
				oPrint:say ( INT(nLine * nAumento), 0650, Transform(aPDVal[i][NMES1], "@E 99,999,999.99"), oFont08 )
				nTot1Remun+= aPDVal[i][NMES1]
				nPromIndem+= aPDVal[i][NMES1]
			EndIF
		EndIf
		...
		If(lMes2)
			...
			nPromIndem+= aPDVal[i][NMES2]
		EndIf

		If(lMes3)
			oPrint:say ( INT(nLine * nAumento), 1600, Transform(aPDVal[i][NMES3], "@E 99,999,999.99"), oFont08 )
			nTot3Remun+= aPDVal[i][NMES3]
			nPromIndem+= aPDVal[i][NMES3]
		EndIf

		oPrint:say ( INT(nLine * nAumento), 2000, Transform(nPromIndem, "@E 99,999,999.99"), oFont08 )	//TOTAL
		oPrint:line( INT((nLine + 45) * nAumento), 0062, INT((nLine + 45) * nAumento), nColMax-21 ) 		   	//Linha Horizontal
		nLine+= 53
	next i

	if(Len(aPDVal) < 7) //Imprime 7 casillas vacías en la sección de "OTROS CONCEPTOS PERCIBIDOS EN EL MES"
		nXLineas := 7 - Len(aPDVal)
		for i:= 1 to nXLineas
			oPrint:say ( INT(nLine * nAumento), 0510, STR0023, oFont09 )    //"Bs"
			oPrint:say ( INT(nLine * nAumento), 0960, STR0023, oFont09 )    //"Bs"
			oPrint:say ( INT(nLine * nAumento), 1410, STR0023, oFont09 )    //"Bs"
...
oPrint:say ( INT(nLine * nAumento), 0064, STR0035, oFont09 ) 	//"OTROS"
	...
	/*
	Código estandar. Antes de realizar el proceso de separación del finiquito(FOL|FIN)
	La variable aPdd viene con datos
	*/
	/*If !(len(aPdd) < 1)
	If len(aPdd)<= 6
	nAte:= len(aPdd)
...
Endif
	...
	oPrint:Box ( INT(0020 * nAumento), 0035, INT(nLine * nAumento), nColMax )
	oPrint:Box ( INT(0027 * nAumento), 0045, INT((nLine-8) * nAumento), nColMax-7 )
...
Endif
	...
	oPrint:say ( 0500, 0700, cCarnet, oFont09 )
...
if( (VALTYPE(dImpresion) <> "D") .OR. EMPTY(dImpresion) )
		dImpresion:= DATE()	//Fecha del S.O.
EndIf

	oPrint:say ( 0750, 0100, STR0053+" "+aInfo[5], oFont09 )    //"LUGAR Y FECHA"
	oPrint:say ( 0750, 0780, ",", oFont09 )
	oPrint:say ( 0750, 0800, Transform(Day(dImpresion),"99"), oFont09)
	...
	oPrint:say ( 1050, 1650, "REPRESENTANTE LEGAL"/*STR0055*/, oFont09 )    //"APODERADOS LEGALES"
...
Static Function RetHoraVal(cIdCalc, cHoras, nValor, cDesc, cMesMed)
	...
	For nAux:=1 To nQtdLan
		If aBol[nAux][1] == cIdCalc
			If(U_existSX3("SRR", "RR_XFOLFIN"))//Si existe el campo RR_XFOLFIN
				cXFolFin:= getRRXFolFin(cIdCalc)
			Else
				cXFolFin:= CROTFIN
			EndIf
			If(cXFolFin == CROTFIN)
...
Static Function getPDs(cSuc, cMat, cMes1, cMes2, cMes3, lUltimo)
	Local aArea			:= getArea()
	Local aRet			:= {}
	...
	BeginSql Alias OrdenConsul
		...
		SELECT RV_COD, RV_DESC, SUM(MES1) AS MES1, SUM(MES2) AS MES2, SUM(MES3) AS MES3, SUM(MES_ACTUAL) AS MES_ACTUAL
		FROM
		(
		SELECT DISTINCT RV_COD, RV_DESC,
		COALESCE((SELECT SUM(RD_VALOR) FROM %table:SRD% WHERE RD_PD = SRD.RD_PD AND RD_FILIAL = %exp:cSuc% AND RD_MAT = %exp:cMat% AND RD_PERIODO = %exp:cMes1% AND RD_ROTEIR = 'FOL' AND %notdel%), 0) AS MES1,
		COALESCE((SELECT SUM(RD_VALOR) FROM %table:SRD% WHERE RD_PD = SRD.RD_PD AND RD_FILIAL = %exp:cSuc% AND RD_MAT = %exp:cMat% AND RD_PERIODO = %exp:cMes2% AND RD_ROTEIR = 'FOL' AND %notdel%), 0) AS MES2,
		COALESCE((SELECT SUM(RD_VALOR) FROM %table:SRD% WHERE RD_PD = SRD.RD_PD AND RD_FILIAL = %exp:cSuc% AND RD_MAT = %exp:cMat% AND RD_PERIODO = %exp:cMes3% AND RD_ROTEIR = 'FOL' AND %notdel%), 0) AS MES3,
		0 AS MES_ACTUAL
		FROM %table:SRD% SRD
		JOIN %table:SRV% SRV ON SRD.RD_PD = SRV.RV_COD
		WHERE SRV.RV_TIPOCOD = '1'
		AND SRD.RD_ROTEIR = 'FOL'
		AND SRV.RV_CODFOL NOT IN ('0031')
		AND SRD.RD_FILIAL = %exp:cSuc%
		...
		0 AS MES2,
		0 AS MES3,
		COALESCE(RR_VALOR, 0) AS MES_ACTUAL
		FROM %table:SRR% SRR
		JOIN %table:SRV% SRV ON SRR.RR_PD = SRV.RV_COD
		WHERE SRV.RV_TIPOCOD = '1'
		AND SRV.RV_CODFOL NOT IN ('0006', '0007', '1276', '0740', '0739', '0738', '0741', '0031', '0086', '0087', '0110', '0757', '0758', '0759' ,'0762', '0768', '0763', '0084')
		AND SRR.RR_FILIAL = %exp:cSuc%
		AND SRR.RR_MAT = %exp:cMat%
		AND SRR.RR_PERIODO = %exp:cMes3%
		AND SRR.%notdel%
		AND SRV.%notdel%
		) A
	...
	If (OrdenConsul)->(!Eof())
		while ( (OrdenConsul)->(!Eof()))
			...
			If(lUltimo)//Si fue despedido el ultimo dia del mes
				AADD(aDados, (OrdenConsul)->MES_ACTUAL)
			else
		...
		while ( (OrdenConsul)->(!Eof()))
			...
			If(lMes1)
				...
				Else
					...
					AADD(aDados, 0)
				EndIf
			Else//Utilizado para los casos que sólo se paga 2 meses(obreros)
				If(SUBSTR(cMes2, 5, 2) == "04")//Si el mes 2 mes es abril
					AADD(aDados, 0)
					AADD(aDados, (OrdenConsul)->ABR)
					AADD(aDados, 0)
				Else
...
Static Function getSalMes(cSuc, cMat, cPer, lUltimo)
	Local aArea			:= getArea()
	Local OrdenConsul	:= GetNextAlias()
	Local nRet			:= 0
	Default lUltimo		:= .F.
...
* @author: Denar Terrazas Parada
* @since: 30/01/2020
* @description: Funcion que devuelve el valor informado en el campo RR_XFOLFIN, que indica si el pago de finiquito
fue separado o no.
*/
Static Function getRRXFolFin(cCodFol)
	Local aArea	:= getArea()
	Local cRet	:= ""
	Local OrdenConsul	:= GetNextAlias()
	...
	BeginSql Alias OrdenConsul
		...
		SELECT RR_XFOLFIN
		FROM %table:SRR% SRR
		JOIN %table:SRV% SRV ON SRR.RR_PD = SRV.RV_COD AND SRV.RV_CODFOL = %exp:cCodFol%
		WHERE RR_FILIAL = %exp:SRA->RA_FILIAL%
		AND RR_MAT = %exp:SRA->RA_MAT%
		AND SRR.%notdel%
		AND SRV.%notdel%
...
* @author: Denar Terrazas Parada
* @since: 30/01/2020
* @description: Funcion que devuelve las deducciones que tiene el empleado.
* @Return:Array con tres valores:
Posicion 1 -> Codigo del concepto (RV_COD)
Posicion 2 -> Descripcion del concepto (RV_DESC)
Posicion 3 -> Valor de descuento del concepto (RR_VALOR)
*/
Static Function getDescuentos()
	...
	If(U_existSX3("SRR", "RR_XFOLFIN"))//Si existe el campo RR_XFOLFIN
		cWhere:= "% RR_XFOLFIN = '" + cRot + "' %"
	EndIf
	// consulta a la base de datos
	BeginSql Alias OrdenConsul

		SELECT SRV.RV_COD, SRV.RV_DESC, SRR.RR_VALOR
		FROM %table:SRR% SRR
		JOIN %table:SRV% SRV ON SRR.RR_PD = SRV.RV_COD
		WHERE RR_FILIAL = %exp:SRA->RA_FILIAL%
		AND RR_MAT = %exp:SRA->RA_MAT%
	...
	If (OrdenConsul)->(!Eof())
		while ( (OrdenConsul)->(!Eof()))
			aDados:={}
			AADD(aDados, (OrdenConsul)->RV_COD)
			AADD(aDados, (OrdenConsul)->RV_DESC)
			AADD(aDados, (OrdenConsul)->RR_VALOR)
			AADD(aRet, aDados)
			(OrdenConsul)->(dbSkip())
		End
	endIf
...
* @author: Denar Terrazas Parada
* @since: 30/01/2020
* @description: Funcion que devuelve las ingresos que tiene el empleado sin el haber básico.
*/
Static Function getIngresos()
	Local aArea			:= getArea()
	Local nRet			:= 0
	Local OrdenConsul	:= GetNextAlias()
	Local cRot			:= CROTFIN
	...
	BeginSql Alias OrdenConsul
		...
		SELECT SUM(SRR.RR_VALOR) RR_VALOR
		FROM %table:SRR% SRR
		JOIN %table:SRV% SRV ON SRR.RR_PD = SRV.RV_COD
		WHERE RR_FILIAL = %exp:SRA->RA_FILIAL%
		AND RR_MAT = %exp:SRA->RA_MAT%
		AND SRV.RV_TIPOCOD = '1'
		AND SRV.RV_CODFOL NOT IN ('0031','0048','0086','0087','0110','0757','0758','0759','0762','0763','0766','0767', '0768')
		AND %Exp:cWhere%
		AND SRR.%notdel%
		AND SRV.%notdel%
...
* @author: Denar Terrazas Parada
* @since: 29/01/2021
* @description: Función que retorna el aumento que determina si el tamaño de impresión será carta u oficio
*
*/
static function getAumento()
	Local nOpcCarta	:= 1
	Local nOpcOficio:= 2
	Local cTitulo	:= "Tamaño de impresión"
	Local nRet		:= 1
	Local nOpcAviso
	nOpcAviso:= Aviso(cTitulo,'Por favor indique el tamaño de impresión que desee.';
		,{'Carta', 'Oficio'})

	If(nOpcAviso == nOpcCarta)
		nRet:= 1.1
	ElseIf(nOpcAviso == nOpcOficio)
		nRet:= 1.5
	EndIf
