...
static function buscaTV1()
    ...
    cDataServ := "% TV1_DTSERV LIKE '"+ dtos(dUltdia) +"%' %"
	...
	BeginSql alias cTemp
        ...
        SELECT SUM(TV2.TV2_XTOTCO) TOTALHORAS,TV1_FILIAL,T9_CODIMOB,TV1.TV1_CODBEM,ST9.T9_NOME FROM TV1010 TV1 
        JOIN 
        TV2010 TV2 ON 
		...
		left outer JOIN FNA010 FNA ON FNA.FNA_CBASE = SUBSTRING(T9_CODIMOB, 1, 10) AND %EXP:cDataApunt%
		AND FNA.FNA_ESTORN LIKE '2' AND FNA.FNA_OCORR LIKE 'P2' 
		WHERE 
        %EXP:cDataServ%
        AND SUBSTRING(T9_CODIMOB, 1, 10) IN(SELECT N3_CBASE FROM SN3010 s WHERE D_E_L_E_T_ = ' ' AND N3_TIPO = '10')
        and TV1_INDERR = 2
	...
	While !(cTemp)->(eof())
		...
		*/
        ...
        allTrim((cTemp)->T9_CODIMOB),(cTemp)->TOTALHORAS,AllTrim((cTemp)->T9_NOME), .F.})
