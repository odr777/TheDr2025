...
#Include "RWMAKE.ch"
#Include "PROTHEUS.CH"
#INCLUDE "FWADAPTEREAI.CH" // Integraç?o via Mensagem Única
#include "Topconn.ch"

//---------------------------------------------------------------------
/*/{Protheus.doc} MNTA851
Implementaç?o da rotina OAS1A003 (OAS) para o padr?o SIGAMNT
...
/*/
//---------------------------------------------------------------------
user Function XMNTA851()

    Local lRet        := .T.
    Local aNGBEGINPRM := NGBEGINPRM() // Guarda conteúdo e declara variáveis padr?es

    Private aRotina := MenuDef()
    Private cCadastro  := "Cadastro de Parte Diária" // "Cadastro de Parte Diária"
    Private cPrograma  := "MNTA851"

    Private aZEMPS     := {} // Variável obrigatória para as Perguntas De/Ate Empresa/Filial

    // Tabelas a serem abertas, já que o processo de correç?o de erros de importaç?o de registros, trabalha como multi filial/empresa
    ...
    //---------------------------------------
    // Retorna conteúdo de variáveis padr?es
    //---------------------------------------
...
@since 10/05/2010
@version P11
@return true or false, conforme validaç?o
/*/
//---------------------------------------------------------------------
    ...
    If Empty(aHelpVld)
        ...
        If Inclui
            ...
            If dbSeek( xFilial( "TV1" ) + cEmpAnt + M->TV1_CODBEM + DTOS( M->TV1_DTSERV ) + M->TV1_TURNO + M->TV1_HRINI + M->TV1_HRFIM )
                ...
                aHelpVld := {"99", "Já existe registro com esta informaç?o", "Troque a chave principal deste registro."} // "Já existe registro com esta informaç?o" ## Troque a chave principal deste registro.
            ...
            ElseIf dbSeek( xFilial( "TV1" ) + cEmpAnt + M->TV1_CODBEM + DTOS( M->TV1_DTSERV ) + M->TV1_TURNO )
                ...
                While !EoF() .And. TV1->TV1_FILIAL == xFilial( "TV1" ) .And.;
                    ...
                    STR0004,; // "Favor alterar o horário desta Parte Diária, ou cancelar a operaç?o."
                    STR0002 } // "Atenç?o"
                    Exit
            ...
            If Empty(aHelpVld) .And. lHlpVisual
                ...
                NGTRETCON(M->TV1_CODBEM,M->TV1_DTSERV,M->TV1_CONFIM,M->TV1_HRFIM,1,,,"C")
            Endif
        ...
        ElseIf Altera
            ...
            If Empty(aHelpVld) .And. TV1->TV1_INDERR == "1" // Registro inconsistente
                //verifica se ja existe o registro de contador
                //Registra contador inicial da PD
                NGTRETCON(M->TV1_CODBEM,M->TV1_DTSERV,M->TV1_CONINI,M->TV1_HRINI,1,,,"C")
                //Registra contador Final da PD
                NGTRETCON(M->TV1_CODBEM,M->TV1_DTSERV,M->TV1_CONFIM,M->TV1_HRFIM,1,,,"C")
...
/*/
//---------------------------------------------------------------------
static Function MNT851LIOK(lRetLgc)
    ...
    If Empty(aHelpVld)
        ...
        If nQtd > 1 .And. !Empty( cCODATI )
            aHelpVld := {"99",	"Já existe registro com esta informaç?o"} // "Já existe registro com esta informaç?o" ## Troque a chave principal deste registro.
        Endif
            ...
            While !EoF() .And. TV1->TV1_FILIAL == xFilial( "TV1" ) .And. ;
                ...
                If lHlpVisual
                    If !MsgYesNo( STR0007 + CRLF + CRLF +; //"Esta Atividade está com o horário em conflito com outro cadastro de Parte Diária."
                        STR0008,STR0002 ) //"Deseja confirmar mesmo assim?"###"Atenç?o"
                        aHelpVld := {""}
                    EndIf
                Else
...
@return aRotina - Estrutura
	...
	[n,3] Reservado
	[n,4] Tipo de Transaç?o a ser efetuada:
		1 - Pesquisa e Posiciona em um Banco de Dados
		2 - Simplesmente Mostra os Campos
		3 - Inclui registros no Bancos de Dados
		4 - Altera o registro corrente
		5 - Remove o registro corrente do Banco de Dados
		6 - Alteraç?o sem inclus?o de registros
		7 - Cópia
		8 - Imprimir
	[n,5] Nivel de acesso
...
Static Function MenuDef()
    ...
    { STR0010  , "U_xMNTA85K" , 0, 2 },;       	// "Visualizar"
    { STR0011  , "U_xMNTA85K" , 0, 3 },;       	// "Incluir"
    { STR0012  , "U_xMNTA85K" , 0, 4 },;       	// "Alterar"
    { STR0013  , "U_xMNTA85K"  , 0, 5, 3 } }    	// "Excluir"
...
@param Integer nOpcx: indica número da operaç?o
...
user Function XMNTA85k( cAlias, nRecno, nOpcx )
    ...
    If nOpcx == 4 .And. TV1->TV1_INDERR == "1" // Caso alteraç?o e registro com erro
    ...
    // Condiç?o enquanto para recuperar dados da TV2
    ...
    If IsInCallStack( "MNTA852" )
        ...
        { "Deve estar cadastrada no sistema para utilizaç?o." },2 ) } // "Deve estar cadastrada no sistema para utilizaç?o."
        // Filial
...
static Function MNT851GRV( nOpcx )
    ...
    Private aDados := {} //Array contendo os apontamentos que ser?o excluídas
    Private aRet   := {} //Contém os retornos do envio dos XML para o RM

    //Bloco de código com a chamada do envio da mensagem de integraç?o
    bBlock   := {|| FWIntegDef("MNTA851", EAI_MESSAGE_BUSINESS, TRANS_SEND, Nil)}
    ...
    //Se for alteraç?o, avalia se houve deleç?o de atividades da GetDados
    ...
    If !Empty(aDados)
        //Envia informaç?es referente ao apontamento deletado
        //Caso falhe o envio, aborta a operaç?o de exclus?o
        If Type("oMainWnd") == "O"
            MsgRun("Aguarde integraç?o com backoffice...", "Appointment", bBlock)
        Else
            Eval(bBlock)
        EndIf
    EndIf
    ...
    If lContinua
        If ( nOpcx == 3 ) .Or. ( nOpcx == 4 )
            ...
            For nT := 1 To Len( oGet:aCols )
                ...
                // Inicializa as variáveis de memória para campos que n?o aparecem na get dados
                For nI := 1 To Len( aGetNao )
                    xx := "M->" + aGetNao[nI][1]
                    yy := aGetNao[nI][2]
                    If !Empty( yy )
                ...
                dbSetOrder(1)
                // T9_FILIAL+T9_CODBEM
                IF dbSeek(xFilial("ST9")+TV1->TV1_CODBEM)
                    ...
                    IF dbSeek(xFilial("SN3")+ST9->T9_CODIMOB) // Encuentra SN3
                        if !empty(SN3->N3_CLVLCON)  // no tiene que estár vacio
                            cProducto := getProByCl(SN3->N3_CLVLCON)// obtengo el código del producto
                            if !empty(cProducto) // obtengo el producto de costo de mano de obra
                                ...
                                AADD( aRegTv2,M->TV2_UTOTHD)  // 2
                                AADD( aRegTv2,M->TV2_DTSERV)  // 3
                                AADD( aRegTv2,M->TV2_CODFRE)  // 4
                    ...
                    alert("NO RELACION EN LA ST9 (Bienes) - Mant. " +TV1->TV1_CODBEM)
                endif
                // Nahim Finaliza
            ...
            // Nahim  Verificando si debería hacer el apunte de parte diario
            if !empty(aArrayPTd)
                cDoc := "PT" + Alltrim(TV1->TV1_CODBEM) + DTOC(TV1->TV1_DTSERV) // Nro de docuemtno
                // Realiza el costo de mano de obra
                u_cosParDi(cDoc,"512",aArrayPTd)

                // Realizando apunte activo fijo
                For nI := 1 To Len(aArrayPTd)
                    getATF(aArrayPTd[nI][5],aArrayPTd[nI][6])
                    if ! VW_CPX->(EoF())
    ...
    Else
        ...
        MsgInfo(STR0065)	//"Falha no envio dos appointments excluídos sentido Protheus - RM! Abortando processo de deleç?o."
        Return .F.
...
Static Function MNTA851PLA()
    ...
    If Empty(aHelpVld) .And. ST9->T9_TEMCONT != "S"
        aHelpVld := {"99",STR0014} // "Bem n?o é controlado por contador."
    EndIf

    If Empty(aHelpVld) .And. ST9->T9_PARTEDI != "1"
        aHelpVld := {"99",STR0015 } // "Bem n?o é controlado por parte diária."
    Endif
...
@since 10/05/2010
@version P11
@return Boolean lRETVH: conforme validaç?o do operador
/*/
//---------------------------------------------------------------------
Static Function MNTA851OPE()
    ...
    If cNGMNTRH = "S"
        ...
        IF !Empty( dDEMISSAO ) .And. dDEMISSAO < M->TV1_DTSERV
            HELP( " ",1,STR0006,,STR0017 + CHR( 13 ) + STR0018,3,1 ) //"ATENÇ?O"###"Data Invalida"###"Lançamento após demiss?o."
            lRETVH := .F.
        EndIf
        ...
        If SR8->R8_TIPO = "F"
            If M->TV1_DTSERV > SR8->R8_DATAINI .And. M->TV1_DTSERV < SR8->R8_DATAFIM
                HELP( " ",1,STR0006,,STR0019,3,1 ) //"ATENÇ?O"###"Operador em período de férias!"
                lRETVH := .F.
            EndIf
        EndIf
...
@since 11/05/2010
@version P11
@return true ou false conforme validaç?o
/*/
//---------------------------------------------------------------------
static Function MNTA851CON( nVAR )
    ...
    xRetValid := NGCHKHISTO( M->TV1_CODBEM, M->TV1_DTSERV, nPosCont, cHrCont, 1, , lHlpVisual )
    ...
    xRetValid := NGVALIVARD( M->TV1_CODBEM, nPosCont, M->TV1_DTSERV, cHrCont, 1, lHlpVisual )
...
Programa de exclus?o de Atividade Diária.
...
/*/
//---------------------------------------------------------------------
static Function MNTA851EX(cAlias, nRecno, nOpc)
    ...
    If lHlpVisual .And. !MsgYesNo( STR0020, STR0006 ) // "Deseja realmente Excluir esse registro?"###"ATENÇ?O"
        Return .F.
    EndIf
...
/*/
//---------------------------------------------------------------------
static Function MNTA851HR( nVar, cVAR )
...
Static Function MNTA851BEM()
    ...
    If ST9->T9_TEMCONT != "S"
        aHelpVld := {"99", STR0031} // "Bem n?o é controlado por contador."
    EndIf

    If Empty(aHelpVld) .And. ST9->T9_PARTEDI != "1"
        aHelpVld := {"99", STR0032} // "Bem n?o é controlado por parte diária."
    EndIf
...
Static Function MNT851CALE()
    ...
    If !Empty(M->TV1_TURNO)
        ...
        EndIf
    Else
        aHelpVld := {"99", STR0034} // "O Turno deve ser preenchido para a validaç?o da Data."
    EndIf
...
Static Function Periodo()
    ...
    If lShowMsg
        If lHlpVisual
            If !MsgYesNo(STR0036 + CRLF + "Confirma inclus?o?", STR0006) //"Lançamento está fora do turno previsto." ## "Confirma inclus?o?" ## "ATENÇ?O"
                aHelpVld := {""}
            Endif
        Else
...
@since 19/11/2010
@version P11
@return Boolean lRet: conforme validaç?o
/*/
//---------------------------------------------------------------------
...
@param Boolean lVerAtiv: Opcional, indica se é para verificar as atividades
						 default é .F. : n?o verifica as atividades.
...
@since 24/05/2011
@version P11
@return Boolean true or false: conforme validaç?o
/*/
//---------------------------------------------------------------------
    ...
    Else //Se o Bem nao possui Transferencias, apenas valida se o Bem esta Inativo ou Transferido
        ...
        If dbSeek( xFilial( "ST9" ) + cCodBem )
            ...
            If ST9->T9_SITBEM == "T"
                ...
                If lHlpVisual
                    If !MsgYesNo(cMsgHlp + "'" + Chr(13) + Chr(10) + ;
                            STR0047, STR0002) //"Confirmar?"###"Atenç?o"
                        aHelpVld := {""}
                    EndIf
                Else
                    aHelpVld := {"05", cMsgHlp }
            ...
            ElseIf ST9->T9_SITBEM == "I"
                ...
                If lHlpVisual
                    If !MsgYesNo(cMsgHlp + "'" + Chr(13) + Chr(10) + ;
                            STR0049, STR0002) //"Confirmar?"###"Atenç?o"
                        aHelpVld := {""}
                    EndIf
                Else
                    aHelpVld := {"04", cMsgHlp }
...
/*/
//---------------------------------------------------------------------
static Function MNT851CONT( _nLinhaAtu,_cConAti,_cConIni,_cConFim )
...
Retorna transfer?ncias do bem até a data informada no parametro
dData.
...
Static Function MNTA851TRF( cCodBem,dData )
    ...
    Local aTabelas := { { "TQ2" } } //  Histórico de transfer?ncias do Bem
    Local aTransfs := {}
    Local aOrigem  := {}
    Local aDestino := {}
    Local aTrs     := {}
    ...
    // Para futura implementaç?o que d? suporte a transfer?ncias de bens entre empresas
	/*DbGoTop()
            ...
            If dbSeek( aEmpFil[nX][2][nY] + cCodBem )
                While !EoF() .And. TQ2->TQ2_FILIAL == PadR( aEmpFil[nX][2][nY],Len( TQ2->TQ2_FILIAL ) ) .And. TQ2->TQ2_CODBEM == PadR( cCodBem,Len( TQ2->TQ2_CODBEM ) )
                    ...
                    If TQ2->TQ2_DATATR <= dData
						...
						// cEmprOrig := TQ2->TQ2_EMPORI // Para implementaç?o de transferencia de bens entre empresas
						cEmprOrig := cEmpAnt
						cFiliOrig := TQ2->TQ2_FILORI
						// cEmprDest := TQ2->TQ2_EMPDES // Para implementaç?o de transferencia de bens entre empresas
						cEmprDest := cEmpAnt
...
/*/
//---------------------------------------------------------------------
static Function MNT851VLCP(cCampo, lShowHlp)
...
Static Function MNT851VLBM()
    ...
    If Empty(aHlpVld)
        aHelpVld := MNTA851BEM(M->TV1_CODBEM)
        SetHelpRet( aHelpVld )
    Endif
...
Static Function MNT851VLPL()
    ...
    aHelpVld := MNTA851PLA(M->TV1_PLACA)
    SetHelpRet( aHelpVld )
...
Static Function ShowHlpVld(aHelpVld)
    ...
    If nHlp > 0 .And. !Empty(aHelpVld[1])
        ShowHelpDlg(	  If( nHlp >= 4, aHelpVld[4], STR0002 )    	,; // "Atenç?o"
...
/*/
//---------------------------------------------------------------------
static Function MNT851VLGD(cCampo,lRetLgc)
...
Static Function fDefEmpFil()
    ...
    If lErro
        MsgAlert("A Empresa/Filial é inválida. Favor corrigir estas informaç?es primeiro.")
    Else
        Return .T.
    EndIf
    ...
    oGetEmpres:bHelp := {|| ShowHelpCpo("Empresa",;
        ...
        {"Deve estar cadastrada no sistema para utilizaç?o."},2)}
    ...
    If lDefOK
        lErro := ( !MNT851VLEF(,.F.) .Or. !MNT851VLEF(2,.F.) )
        If lErro
            MsgStop("A Empresa/Filial é inválida!","Atenç?o")
...
static Function MNT851VLEF(nVld,lShowMsg)
    ...
    If nVld == 1 // Valida Empresa
        ...
        If !lAcesso .Or. Len(aVerRet) == 0
            If lShowMsg
                MsgStop("A Empresa é inválida.","Atenç?o")
            EndIf
...
Essa funç?o é a implementaç?o para o padr?o, da funç?o OASRETEF que
...
Funç?o para buscar as empresas e filiais as quais um determinado
usuario possui acesso.

Essa funç?o é a implementaç?o para o padr?o, da funç?o OASACESS que
se encontra no fonte OAS1UTIL (OAS).
André Felipe Joriatti
...
Static Function fAccesEFUSR( cVerCodUsr,lShowMsg )
    ...
    If PswSeek(cVerCodUsr,.T.)
        ...
        //Verifica se a restriç?o será pelo grupo de usuários
        If Len(PswRet()[1][10]) > 0 .And. PswRet()[2][11]
            ...
            Next nGroup
        Else
            //Utiliza as restriç?es direto do usuário
    ...
    If lShowMsg .And. Len(aUsrAccess) == 0
        //ApMsgAlert("Usuário n?o encontrado no cadastro do sistema.")//"Usuário n?o encontrado no cadastro do sistema."
        Return aUsrAccess
    EndIf
...
Static Function fErros851()
    ...
    Define MsDialog oDlgInc From 020,000 To 350,400 COLOR CLR_BLACK,CLR_WHITE STYLE;
        nOr( DS_SYSMODAL,WS_MAXIMIZEBOX,WS_POPUP ) Of oMainWnd Pixel // "Erros de Importaç?o"
    ...
    @ 019,035 Say "Erros de importaç?o do cabeçalho da parte diária" COLOR CLR_HBLUE Of oGrpLbl Size 200,050 Pixel // "Erros de importaç?o do cabeçalho da parte diária:"
    ...
    oConfirm:SetCss( StaticCall( TNGPG,CSSButton,.T. ) )

    Define sButton oCancel From 160,170 Type 2 Enable Of oTPanel Action ( oDlgInc:End() ) Pixel
    oCancel:SetCss( StaticCall( TNGPG,CSSButton ) )
