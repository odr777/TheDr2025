...
30/06/2020
Nahim Terrazas modified to integrate with Stock and fixed asset

/*/
//---------------------------------------------------------------------
user Function XMNTA852()
    ...
    //Alterar
    oBtnAltera := TButton():New(001, 001, OemToAnsi("Alterar"), oPnlLEFT, {|| SetAltera(), nRecnoTV1 := TV1->( Recno() ), nOpca := U_XMNTA85k( "TV1", nRecnoTV1,4 ),If( nOpca == 1,( (c852TRB)->( DbGoTop()), fUpdateTRB(nRecnoTV1, .F.) ), Nil ),RegToMemory( "TV1",.F. ),fAtualizar() },;
        40, 11, , , .F., .T., .F., , .F., , , .F.)
    oBtnAltera:Align := CONTROL_ALIGN_TOP

    //Excluir
    oBtnExclui := TButton():New(001, 001, OemToAnsi("Excluir"), oPnlLEFT, {|| SetExclui(), nRecnoTV1 := TV1->( Recno() ), nOpca := U_XMNTA85k( "TV1", nRecnoTV1,5 ),If( nOpca == 1,( (c852TRB)->( DbGoTop()), fUpdateTRB(nRecnoTV1, .T.) ), Nil ),RegToMemory( "TV1",.F. ),fAtualizar() },;
        40, 11, , , .F., .T., .F., , .F., , , .F.)
...
/*/
//---------------------------------------------------------------------
static Function Mnt852SALI(lSetFilter,lAtuBrw)
