				...
				IF SF3->F3_TIPOMOV == "V"
					DO CASE
						CASE cStatus $ "V|C"   //NAHIM ajuste 24/04/2019
							...
							IF !EMPTY(TRIM(SF2->F2_UNOME))
								cRazSoc	:= SF2->F2_UNOME
							ENDIF
...
Static Function PesqDocOri(dDtNFOri, cNFOri, cAutNFOri, nValNFOri, cFilIni, lTabSF2,cNodia)
	...
	If lTabSF2
		cQuery := "SELECT F2_DOC, F2_EMISSAO, F2_NUMAUT, F2_VALFAT, F2_MOEDA, F2_TXMOEDA, F2_UNOME, F2_NODIA FROM "
	Else
...
//			nValNFOri := Round(NoRound(xMoeda((cArqTmp)->F2_VALFAT,(cArqTmp)->F2_MOEDA,1,(cArqTmp)->F2_EMISSAO,,(cArqTmp)->F2_TXMOEDA )),2)
			...
			If !Empty((cArqTmp)->F2_UNOME)			
				cRazSoc	  := (cArqTmp)->F2_UNOME
			EndIf
