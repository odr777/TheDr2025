	...
	IF !EMPTY(ALLTRIM(MV_PAR01))
		...
		DBUseArea(.T., "TOPCONN", TCGenQry(,,Query(cCpo,aAux[2])), cAreaQuery, .T., .T.)
			...
			while !(cAreaQuery)->(Eof())
				...
				oSection1:Cell("A_CGC"):SetBlock({||		(cAreaQuery)->A_CGC })
				...
				oSection1:Cell("X_NIT"):SetBlock({||	(cAreaQuery)->X_NIT })
				oSection1:Cell("X_NOMBRE"):SetBlock({||	(cAreaQuery)->X_NOMBRE })
				oSection1:Cell("X_OBS"):SetBlock({||	(cAreaQuery)->X_OBS })
		...
		oSection1:Cell("X_NIT"):SetBlock({||	" " })
		oSection1:Cell("X_NOMBRE"):SetBlock({||	" " })
		oSection1:Cell("X_OBS"):SetBlock({||	" " })
		oSection1:PrintLine()
		oSection1:Finish()
...
DEFAULT cPerg:='FISR801' 
	...
	TRCell():New(oSection1,"A_CGC",cAlias,STR0103)	//STR0103 "NIT"
	TRCell():New(oSection1,"A_NOME",cAlias,STR0016)	//STR0016 "Nombre Prov/Cli"
	...
	TRCell():New(oSection1,"X_NIT",cAlias,STR0101,X3Picture("X_NIT")) //"NIT+ ")	
	TRCell():New(oSection1,"X_NOMBRE",cAlias,STR0102,X3Picture("X_NOMBRE")) //"Nombre+ ")	
	TRCell():New(oSection1,"X_OBS",cAlias,STR0100,,100) //"Observación")	
Return(oReport)
...
Local cTabla:=IIF(MV_PAR04==1,"2","1")	// 2 COMPRAS	1 VENTAS
DEFAULT cCpo:=""
DEFAULT cTES:=""
	IF !Empty(ALLTRIM(mv_par01))
		...
		IF !Empty(cCpo)
			...
			cQuerySQL +=" F3_SERIE,"
			cQuerySQL +=" A"+cTabla+"_CGC A_CGC,"
			cQuerySQL +=" A"+cTabla+"_NOME A_NOME,"
			cQuerySQL +=" F3_ESPECIE,"
			IF cTabla == '2'
				cQuerySQL +=" F1_XOBS X_OBS, F1_UNIT X_NIT, F1_UNOMBRE X_NOMBRE"
			ELSEIF cTabla == '1'
				cQuerySQL +=" F2_UOBSERV X_OBS, F2_UNITCLI X_NIT, LI X_NOMBRE"
			ENDIF
			...
			IF cTabla == '2'
				cQuerySQL +=" LEFT JOIN "+RetsqlName("SF1")+" SF1 ON SF3.F3_CLIEFOR=SF1.F1_FORNECE AND SF3.F3_NFISCAL = SF1.F1_DOC AND SF3.F3_SERIE = SF1.F1_SERIE"				
			ELSEIF cTabla == '1'
				cQuerySQL +=" LEFT JOIN "+RetsqlName("SF2")+" SF2 ON SF3.F3_CLIEFOR=SF2.F2_CLIENTE AND SF3.F3_NFISCAL = SF2.F2_DOC AND SF3.F3_SERIE = SF2.F2_SERIE"
			ENDIF			
			...
			cQuerySQL +=" F3_ALQIMP"+cCpo+",F3_NFISCAL,F3_SERIE,A"+cTabla+"_CGC,A"+cTabla+"_NOME,F3_ESPECIE"
			IF cTabla == '2'
				cQuerySQL +=" , F1_XOBS, F1_UNIT, F1_UNOMBRE"				
			ELSEIF cTabla == '1'
				cQuerySQL +=" , F2_UOBSERV, F2_UNITCLI, F2_UNOME"
			ENDIF	
