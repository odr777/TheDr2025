...
Static Function ApurITR(aFiliais,aApuAnt)
	...
	cQry := " SELECT"
	cQry += " SUM(SF3.F3_VALIMP6) AS VALIMP"
...
Static Function ApurIVA(aFiliais,aApuAnt)
	...
	cQryC += " 	SELECT"
	cQryC += " 	CASE"
	cQryC += " 		WHEN F1_MOEDA = 1 THEN F1_DESCONT"
	cQryC += " 		WHEN F1_MOEDA = 2 THEN"
	cQryC += " 		("
	cQryC += " 			CASE"
	cQryC += " 				WHEN F1_TXMOEDA > 1 THEN (F1_DESCONT * F1_TXMOEDA)"
	cQryC += " 				ELSE (F1_DESCONT * (SELECT TOP 1 M2_MOEDA2 FROM "+RetSqlName("SM2")+" WHERE D_E_L_E_T_ <> '*' AND M2_DATA = SF3.F3_EMISSAO))"
	cQryC += " 			END"
	cQryC += " 		)"
	cQryC += " 	END AS F1_DESCONT"
	cQryC += " 	FROM "+RetSqlName("SF1")
	cQryC += " 	WHERE F1_DOC = SF3.F3_NFISCAL"
	cQryC += " 	AND F1_SERIE = SF3.F3_SERIE"
	cQryC += " 	AND F1_FORNECE = SF3.F3_CLIEFOR"
	cQryC += " 	AND F1_LOJA = SF3.F3_LOJA"
	...
	// cQryC += " AND SF3.F3_TES NOT IN ('010', '020', '011', '021')"
	...
	// ALTERAR aDados[21]
	If ExistBlock("SALF200")				
		cPeriodo := AllTrim(StrZero(MV_PAR02,4))+AllTrim(StrZero(MV_PAR01,2))
		aDados[21] := ExecBlock("SALF200",.F.,.F.,{cPeriodo,aFiliais})		
	else
...
Static Function ApurIT(aFiliais,aApuAnt)
	...
	// ALTERAR aDados[3]
	If ExistBlock("SALF400")
		cPeriodo := AllTrim(StrZero(MV_PAR02,4))+AllTrim(StrZero(MV_PAR01,2))
		aDados[3] := ExecBlock("SALF400",.F.,.F.,{cPeriodo,aFiliais})		
	endif
...
Static Function ApurIUE(aFiliais,aApuAnt)
	...
	Local cPosIUE	:= "0"	//Posición campos IUE retenciones
	Local cPosITR	:= "0"	//Posición campos IUE retenciones

	cPosIUE	:= cValToChar(GETADVFVAL("SFB","FB_CPOLVRO",xfilial("SFB")+"IU"	,1,"0"))
	cPosITR	:= cValToChar(GETADVFVAL("SFB","FB_CPOLVRO",xfilial("SFB")+"ITR",1,"0"))
	...
	cQry += " SF3.F3_TES, "
	cQry += " CASE "
	cQry += " WHEN SF3.F3_TES = '" + cTes010 + "' THEN SUM(SF3.F3_BASIMP" + cPosIUE + ")"
	cQry += " WHEN SF3.F3_TES = '" + cTes020 + "' THEN SUM(SF3.F3_BASIMP" + cPosIUE + ") + SUM(SF3.F3_VALIMP" + cPosITR + ") + SUM(SF3.F3_VALIMP" + cPosIUE + ")"
	cQry += " WHEN SF3.F3_TES = '" + cTes011 + "' THEN SUM(SF3.F3_BASIMP" + cPosIUE + ")"
	cQry += " WHEN SF3.F3_TES = '" + cTes021 + "' THEN SUM(SF3.F3_BASIMP" + cPosIUE + ") + SUM(SF3.F3_VALIMP" + cPosITR + ") + SUM(SF3.F3_VALIMP" + cPosIUE + ")"
	cQry += " ELSE 0"
	cQry += " END AS VALIMP"
	cQry += " FROM "+RetSqlName("SF3")+" SF3"
	cQry += " WHERE SF3.D_E_L_E_T_ = ' '"
	...
	aDados[3] := aDados[1] * (50/100)				// 1001
	aDados[4] := aDados[2] * (20/100)				// 1002

	aDados[5] := (aDados[3] + aDados[4]) * (25/100)	// 909
	nDados3 := aDados[3] * (25/100)
	nDados4 := aDados[4] * (25/100)

	aDados[6] := MV_PAR06							// 622
	aDados[7] := MV_PAR07							// 640

	aDados[8] := aDados[6] + aDados[7] - aDados[5]	// 747
	If(aDados[8] <= 0)
		aDados[8] := 0
	EndIf

	aDados[9] := aDados[5] - aDados[6] - aDados[7]	// 996
	If(aDados[9] <= 0)
		aDados[9] := 0
	EndIf

	aDados[10] := MV_PAR08							// 677

	aDados[11] := aDados[9] - aDados[10]			// 576
	If(aDados[11] <= 0)			
		aDados[11] := 0
	EndIf
