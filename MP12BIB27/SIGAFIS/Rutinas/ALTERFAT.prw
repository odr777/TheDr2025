...
User Function ALTERFAT
	...
	cCnit	:= if(!EMPTY(SF1->F1_UNIT), SF1->F1_UNIT, POSICIONE("SA2", 1, xFilial("SA2") + SF1->F1_FORNECE + F1_LOJA,"A2_CGC"))
	cCnomRaz:= if(!EMPTY(SF1->F1_UNOMBRE), SF1->F1_UNOMBRE, POSICIONE("SA2", 1, xFilial("SA2") + SF1->F1_FORNECE + F1_LOJA,"A2_NOME"))
	lModifica := .f.
	//alert(SF1->F1_FORNECE)
	//"MV_UMODLC": Proveedores autorizados para que se pueda cambiar nit y razon social en factura de entrada
