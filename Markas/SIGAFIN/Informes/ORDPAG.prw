...
Local Cabec1 := Cabec2 := ""
Local cFornece, cLoja, cOrdPago, dDtBaixa, nBaixa, nTotal,_cUsuario,_dFecha,_cHora
...
While !Eof()
	...
	While !Eof() .and. EK_ORDPAGO == cOrdPago
		...
		ElseIf EK_TIPODOC == "CP"
			If EK_TIPO = "CA"
				nValorCA := IIf(EK_MOEDA=="1",xMoeda(EK_VALOR,Val(EK_MOEDA),2,,2,1,EK_TXMOE02),EK_VALOR)
				cMoneda  := IIf(EK_MOEDA=="1","2","2")
				Aadd(aChqPr  ,{EK_TIPO,EK_NUM,nValorCA,cMoneda,EK_BANCO,EK_AGENCIA,EK_CONTA,EK_VENCTO})
			Else
		...
		ElseIf EK_TIPODOC == "DE"
			...
			Endif
		EndIf          
/*		_cUser := Embaralha(EK_USERLGI,1)
                 
	_cUsuarioA := If (!Empty(_cUser),Subs(_cUser,1,15),"")
_cDataa := IIF(!Empty(_cUser),CTOD("01/01/96") + Load2in4(Substr(_cUser,16)),"")
ALERT(_cUser)           
ALERT(	_cUsuarioA )
             ALERT(_cDataa)
                  alert(Load2in4(_cUser))*/
		DbSkip()
	EndDo
	...
	If nTotal > 0 
	    lCabec  := .T.
		Cabec85A(cOrdpago,@nLin,Cabec1,Cabec2,dDtBaixa)
		If lCancelado
		   nLin ++		
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function Cabec85A(cOrdpago,nLin,Cabec1,Cabec2,dDtBaixa)
local _cArea:=GetArea()

nLin := Cabec(Titulo,Cabec1,Cabec2,NomeProg,Tamanho,nTipo) + 1

@ nLin,000 PSAY SM0->M0_NOMECOM
@ nLin,057 PSAY OemToAnsi(STR0007) + cOrdPago  //"ORDEM DE PGTO NR "
nLin ++
...
DbSelectArea('SE5')
SE5->(DbSetOrder(8))
SE5->(DbSeek(xFilial('SE5')+cOrdPago))
*/ 
