...
User Function UCIFIN01(_hNumero)        // incluido por el asistente de conversión del AP5 IDE en 30/11/00
	...
	SetPrvt("NPRIMPIE,CNOMBCOM,CUSUA,CCLASCOM,COBSPCOM,CTIPOREM,CNOCOCOM")
	...
	±±³Descripci¢n ³ Formato de Salida para Remitos de KLUBER L.A. S.A.       ³±±
	...
	dbSelectarea( "SE5" )                //Posiciono SE5
	dbSetOrder(10)
	dbSeek( xFilial("SE5")+cParDesde) //(E5_DOCUMEN)
	lSalida := .F.             //Salida no prevista
	lUltimo := .F.             //Salida por finalizaci¢n

	querem := SE5->E5_DOCUMEN

	While querem <= cParHasta .and. !EOF() .and. (SE5->E5_DATA >= cDeFecha .or. SE5->E5_DATA <= cAFecha)
		cTm := SE5->E5_HISTOR
		...
		nPagsCom := 0
		nValrCom := 0
		dFechCom := SE5->E5_DATA
		cUsua    := CUSERNAME

		OpenSM0() //Abrir tabla SM0 (Empresa/Filial)
		dbSelectArea("SM0") //Abro la SM0
		SM0->(dbSetOrder(1))
		SM0->(dbSeek("01"+SE5->E5_FILIAL,.T.)) //Posiciona  empresa-grupo empresa
		...
		_CTotal := 0
		dbSelectArea( "SE5" )
		While querem == SE5->E5_DOCUMEN
			//		SA6->(dbSelectArea( "SA6" ))
			//		SA6->(dbSetOrder(1))
			//        SA6->(dbSeek( xFilial("SA6")+SE5->E5_BANCO+SE5->E5_AGENCIA+SE5->E5_CONTA, .F. ))
			//lIsCaja := SUPERGETMV("MV_CARTEIR", .f., "")

			cNome	:= Posicione("SA6", 1, xFilial("SA6")+SE5->E5_BANCO+SE5->E5_AGENCIA+SE5->E5_CONTA, "SA6->A6_NOME")
			nMoeda	:= Posicione("SA6", 1, xFilial("SA6")+SE5->E5_BANCO+SE5->E5_AGENCIA+SE5->E5_CONTA, "SA6->A6_MOEDA")
			//alert("MV_MOEDA" + cValtoChar(nMoeda))
			cMoeda	:= UPPER(GETMV('MV_MOEDA' + cValtoChar(nMoeda)))	//Descripcion de la moneda

			AADD( acCodItm, alltrim(LEFT(cNome,40)) + " - " + alltrim(SE5->E5_CONTA ))
			AADD( acDetItm, SE5->E5_BENEF  )
			//AADD( acUniItm, SE5->E5_MOEDA + ' - ' +Tabela('06',SE5->E5_MOEDA) )
			AADD( acUniItm, cMoeda )
			//		AADD( acUniIt, SB1->B1_SEGUM )
			//ALERT(SE5->E5_NATUREZ)
			AADD( anCntItm, SE5->E5_NATUREZ)
			//		AADD( anCntIt, SD3->D3_QTSEGUM)
			AADD( acUniIt, SE5->E5_VALOR)
			AADD( anCntIt, SE5->E5_CCUSTO)
			AADD( acTipItm, SE5->E5_ITEMD )
			AADD( acObserv, SE5->E5_NUMCHEQ)

			_CTotal := _CTotal + SE5->E5_VALOR

			SE5->( dbSkip(1) )
			/*		dbSelectarea( "SX5" )
			dbSetOrder(1)
			dbSeek( xFilial("SX5")+"03"+SB1->B1_GRUPO, .F. )
			aSector = SX5->X5_DESCRI
			dbSelectarea( "SD3" )
			SD3->( dbSkip(1) )
			IF (querem == SD3->D3_DOC.and. ALLTRIM(SD3->D3_CF) == "RE4" ).OR. (querem == SD3->D3_DOC .and. ALLTRIM(SD3->D3_CF)== "DE4")
			AADD( acTipIt, SD3->D3_LOCAL )
			SD3->( dbSkip(1) )
			ELSE
			AADD( acTipIt, space(0))
			ENDIF
			*/
		EndDo
...
Static Function Impencab()
	...
	****************************/
	...
	oPrn:Say( nLine, 2020, DToC(dFechCom ), oFont30, 100 )
	nLine += 150

	//oPrn:Say( nLine +  00, 0100, " ", oFont30, 100 )
	oPrn:Say( nLine, 1300,  cNombCom , oFont30, 100 )
	oPrn:Say( nLine + 030, 0070, cNomeSuc + " - " + SE5->E5_FILIAL, oFont31, 100 )
	oPrn:Say( nLine, 1850, "T/C    :", oFont8, 100 )
	oPrn:Say( nLine, 2020, cvaltochar(POSICIONE("SM2", 1, SE5->E5_DATA, "M2_MOEDA2")), oFont30, 100 )   // xmoeda
