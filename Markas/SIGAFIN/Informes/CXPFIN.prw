...
±±ºDescripcion ³ Recibo  para Cuentas por Pagar  				  		  º±±
...
User Function CXPFIN()
	Local _wnrel        	:= "CXPFIN"  // Nombre del archivo usado para impresión en disco segun el nombre del funcion
	...
	Local _titCob       	:= "Cuenta por pagar"
	Local _Cabec1        	:= ""
	Local _Cabec2        	:= ""
	Local _cString		  	:= ""
	Local aArea    := GetArea()
	Private _tamanho      	:= "P"     //P=Vertical(80) G=horizontal(220)	--> M=
	Private _nomeprog     	:= "CXPFIN" // Coloque aquí el nombre del programa para impresión en el encabezado
	Private _nTipo        	:= 18
	...
	_wnrel := SetPrint("SE2",_nomeProg,_cPergCD,@_titCob,_cDesc1,_cDesc2,_cDesc3,.F.,,.T.,_Tamanho,,.F.)
	If nLastKey == 27
		Return
	Endif
...
static function f4_ROfC3(cTitulo,bImprimir)
	...
	_csql := "SELECT "  + Chr(13)
	_csql := _csql + " E2_FORNECE,COALESCE(ISNULL(NULLIF(E2_NOMFOR, ''), null),A2_NOME)E2_NOMFOR,E2_FILIAL,E2_MOEDA,E2_NUM,E2_LOJA, " + Chr(13)
	_csql := _csql + " A2_TEL,A2_END,E2_EMISSAO,E2_HIST,E2_SALDO,E2_VLCRUZ,E2_TXMOEDA,E2_VALOR,E2_VENCTO " + Chr(13)
	_csql := _csql + " FROM " + RetSqlName("SE2") + " SE2 "
	_csql := _csql + " LEFT JOIN " + RetSqlName("SA2") + " SA2 ON " + CHR(13)
	_csql := _csql + " A2_COD = E2_FORNECE AND SA2.D_E_L_E_T_ = ' ' " + CHR(13)
	_csql := _csql + " WHERE E2_FILIAL = '" + _xfilial + "' AND E2_NATUREZ = '" + SE2->E2_NATUREZ +"' " + CHR(13)
	_csql := _csql + " AND E2_FORNECE = '" + SE2->E2_FORNECE + "' AND E2_LOJA = '" + SE2->E2_LOJA + "' " + CHR(13)
	_csql := _csql + " AND E2_PREFIXO = '" + SE2->E2_PREFIXO + "' AND E2_NUM = '" + SE2->E2_NUM + "' " + CHR(13)
	_csql := _csql + " AND E2_PARCELA = '" + SE2->E2_PARCELA + "' AND E2_TIPO = '" + SE2->E2_TIPO + "' " + CHR(13)
	_csql := _csql + " AND SE2.D_E_L_E_T_ =' ' AND E2_SALDO > 0" + CHR(13)
	...
	if (cUArea)->(!eof())
		...
		dbSelectArea(cUArea)
		_nEspCab := 0
		//ALERT(VALTYPE((cUArea)->E2_MOEDA))
		//_nCambio := (cUArea)->E2_TXMOEDA
		if (cUArea)->E2_MOEDA == 1
			_nCambio := 1
			cSimb = "Bs"
		else
			cSimb = "$us"
		...
		_dFecha := (cUArea)->E2_EMISSAO
		_cCobDiv := (cUArea)->E2_NUM
		_nInterLin = _nInterLin - 40
		oPrn:Say(_nLin, _nCol4, "Proveedor: " + Alltrim((cUArea)->E2_FORNECE)+" - " + Alltrim((cUArea)->E2_NOMFOR),oFont7)
		_nLin := _nLin + _nInterLin
		oPrn:Say(_nLin, _nCol4, "Dirección: " + Alltrim((cUArea)->A2_END),oFont7)
		oPrn:Say(_nLin, _nCol1-80, "Telefono: " + Alltrim((cUArea)->A2_TEL),oFont7)
		...
		oPrn:Say(_nLin , _nCol3 , " Monto " ,oFont1)
		oPrn:Say(_nLin , _nCol3 + 660, " Moneda " ,oFont1)
		oPrn:Say(_nLin , _nCol3 + 1060,  " Vencimiento ",oFont1)

		//********************
		_cCliente := "("+(cUArea)->E2_FORNECE+"-"+(cUArea)->E2_LOJA+")"+Alltrim((cUArea)->E2_NOMFOR)

		_nLin := _nLin + 50

		oPrn:Say(_nLin , _nCol3 , alltrim(Transform((cUArea)->E2_VALOR, "@E 999,999,999.99")) ,oFont2)
		oPrn:Say(_nLin , _nCol3 + 680 , cSimb ,oFont2)
		oPrn:Say(_nLin , _nCol3 + 1080, ALLTRIM(dtoC(stod((cUArea)->E2_VENCTO))) ,oFont2)
		/*oPrn:Say(_nLin , _nCol3 +  680 , Alltrim((cUArea)->E2_CONTA) ,oFont2)*/
		...
		If Len(alltrim(E2_HIST)) > 38
			oPrn:Say(_nLin , _nCol3 + 180 , SubStr((cUArea)->E2_HIST,1,38) ,oFont8)
			oPrn:Say(_nLin + 27 , _nCol3 + 180 , SubStr((cUArea)->E2_HIST,39,34),oFont8)
		ELSE
			oPrn:Say(_nLin , _nCol3 + 180 , AllTrim((cUArea)->E2_HIST) ,oFont2)
		EndIf
