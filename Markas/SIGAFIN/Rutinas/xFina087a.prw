...
user Function xFina087a(cNumFatAut,cDadosAdt,aRecAdt,oPagVenNT)// Nahim Adicionando el objeto JSON para
	...
	Private cIdTransc := cvaltochar(Randomize(1,34000)) // Nahim adicionando el ID por transacción
		...
		if OPagBajasNT:PAGOS[nW]:TIPOPAGO == "CH"// caso sea cheque.
			...
			if !empty(OPagBajasNT:PAGOS[nW]:NUMCHEQUE) // Nahim 28/05/2020
				...
				if nW > 1 // caso sea que sea mayor a uno
					ACOLS[nW][nPosNNum]	:=  cRecibo + "-1"// chéque
				else
					ACOLS[nW][nPosNNum]	:=  cRecibo // chéque
				endif
			endif
...
//			ACOLS[nW][AScan(aCposSEL,"EL_PREFIXO")]	:= OPagBajasNT:PAGOS[nW]:TIPOPAGO // "CH"
	...
	//PANEL DE BAJAS SE1 TITULOS ERICK
	If !lIncAdt
		...
		/////DEBERIA MONTAR EL GRID DE LA SE1
...
//	Nahim			nT1+=14
//	Nahim			@ nT1,004 SAY OemToAnsi( "- " + STR0173 ) SIZE 53, 07 OF oPPagto PIXEL  //"Retenções"
...
//	Nahim		nT1+=14
//	Nahim		@ nT1,004 SAY OemToAnsi( STR0201  ) 	SIZE 53, 07 OF oPPagto PIXEL //"+ Valores Acessórios"
...
//	Nahim	OPANEL[nPainel]:NCLRPANE:= RGB(233,233,233) //13165800
	...
	For nK := 1  to Len(aLinSE1)
		...
		// nahim realizo una comparación de las bajas
		...
		for  nP := 1 to len(aBajasNT)
			...
			if ASCAN(aDadosBaja,{|x| x[1]+x[2]+x[3]== cNumTitu+cNumParcela+cNumPrefijo } ) <> 0 // Verificando que no se repita para no dar error
				// conout("ENCONTRO DEBERÍA PASAR PASA")
				nP++
				LOOP
			ELSE
				// conout("NO ENCUENTRA")
			...
			if aLinSE1[nK][3] == cNumTitu .and. aLinSE1[nK][4] == cNumParcela .and. aLinSE1[nK][Ascan(aCposSE1,"E1_PREFIXO")] == cNumPrefijo   // numero titulo
				...
				if nNumMoneda == 1 // caso que la moneda del título sea en M1
					nMontPagar  :=  nMonM1 +  aLinMoed[2][2] * aBajasNT[nP]:MONTOM2 // suma la moneda uno con Mon2 convertida

				else // caso que la moneda del título sea en M2
					nMontPagar  :=  nMonM2 +  aBajasNT[nP]:MONTOM1 / aLinMoed[2][2]// suma la moneda2 con la Mone1 convertida
				endif

				if nMontPagar > aLinSE1[nK][Ascan(aCposSE1,"E1_SALDO")] // Nahim validación de monto a bajar mayor al saldo del título
					nMontPagar := aLinSE1[nK][Ascan(aCposSE1,"E1_SALDO")]  // el monto toma el valor del saldo

					if nNumMoneda == 2 // si es dólares
						lPartido := .T. // indica si el monto está distribuido y tiene que ser cortado
						...
						if lPartido  // monto distribuido (SE NECESITAN AMBOS PARA)
							nMonM1 := (aLinSE1[nK][Ascan(aCposSE1,"E1_SALDO")] * aLinMoed[2][2]) - (nMonM2 * aLinMoed[2][2])  // disminuyo lo que estaba pagando menos el saldo en dólares
						endif
					ELSE // bs. TODO VALIDAR BS.
						...
						if lPartido  // monto distribuido (SE NECESITAN AMBOS PARA)
							nMonM1 := (aLinSE1[nK][Ascan(aCposSE1,"E1_SALDO")] ) - (nMonM2 * aLinMoed[2][2])  // disminuyo lo que estaba pagando menos el saldo en dólares
						endif
					endif
				endif
				...
				aLinSE1[nK][nPosMulta] := aBajasNT[nP]:NMULTA   ///erick
				aLinSE1[nK][nPosDesc] := aBajasNT[nP]:NDESCUENTO   ///erick


				aLinMoed[1][4] := aLinMoed[1][4] -  nMonM1
				aLinMoed[2][4] := aLinMoed[2][4] -  nMonM2

				if aLinMoed[1][4] < 0 // Caso se esté pagando de más los bs
					aLinMoed[2][4]  := aLinMoed[2][4] - ( ABS(aLinMoed[1][4]) / aLinMoed[2][2] ) // lo negativo resto
				endif
				if aLinMoed[2][4] < 0 // Caso se esté pagando de más los Sus
					aLinMoed[1][4] :=  aLinMoed[1][4] - ( ABS(aLinMoed[2][4]) * aLinMoed[2][2] ) // lo negativo resto
				endif

				FAPagtoAtu()///ACTUALIZA EL VAALOR EN MULTA O DESCUENTO

				Fa087aBaixaOK(cNumFatAut,nP)
...
//	Begin Transaction // Nahim quitanod el begin transaction
	...
	If cPaisLoc == "MEX" .and. lCfdi33 .And. nQtdF055 == 0
		If nImpAuto <> 0
			...
			EndIf
		ElseIf MSGYESNO( STR0210, STR0209 ) //"¿Desea realizar el Complemento de Recepción de Pago?" - "Complemento de Recepción de Pagos"
			FISA815(cRecibo,cSerie)
		EndIf
	EndIf
	...
	ElseIf nTipo==2
		...
		If lGrvMov
			...
			IF !EMPTY(SEL->EL_BANCO).And. MovBcoBx(cMotBx, .T.) .and. !lAdiantamento
				...
				dbSelectArea("SA6")
				SA6->(DbSetOrder(1))
				conout(SA6->(dbSeek(cFilial+cBanco+cAgencia+cConta)))
				If !dbRLock() // verifica si puede bloquear los saldos y la SA6
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³FINA087A  ºAutor  ³Microsiga           º Data ³  03/31/11   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ AP                                                         º±±
		...
		If nOpcX == 2
			If (aCtbTmp != Nil) .And. (ValType(aCtbTmp) == "A")
				...
				For nX := 1 To Len(aCtbTmp)
					...
					//nombre de variable temporal dinámica declarada en func Fina087
		...
		If __lF087EAI
			...
			If !EMPTY(oXX4:= If(FINDFUNCTION('FINCLSXX4'),oFINCLSXX4():New(cRotina,cMsg),NIL))
				...
				If (ValType(lRetExec) == "L" .and. lRetExec .and. !(oXX4:GetFldValue('_METODO')=='1'))
					If lMSG
						HELP(" ",1,"FA087INTEG",,"NAHIMSTR0224",2,0) //"El adapter de Baja de cunetas porCobrar (FINA087A) está configurado como NO SINCRONIZADO en el configurador. Configurar como SINCRONIZADO."
					EndIf
					lRet := .F.
				EndIf
				oXX4:CleanUp()
	...
	If !Empty(cSer) .Or. !Empty(cRec)
		...
		If SEL->(dbSeek(xFilial("SEL") + cSer + cRec))
			If SEL->EL_CANCEL .And. !Empty(SEL->EL_UUID) .And. !Empty(SEL->EL_FECTIMB)
				lRet := .T.
			Else
				MsgAlert("STR0227NAHIM", "STR0230NAHIM") //"Solo se permite sustituir complementoS para recepción de pagos que ya fueron anulados y contienen un Timbre Fiscal." "Aviso"
				Return(.F.)
			EndIf
		Else
			MsgAlert("STR0228" + IIf(!Empty(cSer),Alltrim(cSer) + "-","") + IIf(!Empty(cRec),Alltrim(cRec),"") + "STR0229NAHIM", "STR0230") //"El complemento para recepción de pagos [" "] a sustituir no existe." "Aviso"
			Return(.F.)
		EndIf
	EndIf
Return

Static Function Fa087aBaixaOK(cNumFatAut,nxPos)
	Local nPosTpSE1	:=	0
	Local nI := 0
	Local nValorBx   := 0
	DEFAULT cNumFatAut := ""
	...
	aLinSE1[nxPos][nPosDesc]	:= nDescont
	aLinSE1[nxPos][nPosJuros]	:= nJuros
	aLinSE1[nxPos][nPosMulta]	:= nMulta
	aLinSE1[nxPos][nPosBx]	:=  nValorBx
