   	...
   	While !(cAliasSF3)->(Eof())
		...
		If nLivro == 1	//Compras  		
			cPoliza:=Replicate(" ",13)
			 If SF1->(DbSeek((cAliasSF3)->(F3_FILIAL+F3_NFISCAL+F3_SERIE+F3_CLIEFOR+F3_LOJA)))
		        cRazSoc := alltrim(SF1->F1_UNOMBRE)
  		        cNIT	   := SF1->F1_UNIT
  		        cPoliza:=  alltrim(SF1->F1_UPOLIZA)
		    ...
		    if empty((cAliasSF3)->F3_DTCANC)    
				...
				If nPos > 0
					If nLivro == 1
						...
						aDados[nPos,09] += Iif(lVALIMP,FieldGet(FieldPos("F3_VALIMP"+mv_par08)),0) 				//08 Total ICE. (B)
