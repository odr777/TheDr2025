    ...
    If lProcLiv
        ...
        If Empty(cChave) .Or. cChave <> SF3->F3_FILIAL + SF3->F3_NFISCAL + SF3->F3_SERIE + SF3->F3_CLIEFOR + SF3->F3_LOJA
            ...
            ElseIf SF3->F3_TIPOMOV == "V"   //Vendas 
                ...
                EndIf
                cNIT := SF2->F2_UNITCLIU
                cRazSoc	:= SF2->F2_UNOMCLI
            Endif
    		
            If SF3->(FieldPos("F3_NIT")) > 0 .AND. !Empty(SF3->F3_NIT)
                cNIT := SF3->F3_NIT
            Else
                If SF3->F3_TIPOMOV == "C"  //Compras(C) 
            ...
            If SF3->(FieldPos("F3_RAZSOC")) > 0 .AND. !Empty(SF3->F3_RAZSOC)
            ...
            ElseIf cLivro == "V"
                //nDescont := SF2->F2_DESCONT//Round(xMoeda(SF2->F2_DESCONT,SF2->F2_MOEDA,1,SF2->F2_EMISSAO,4,SF2->F2_TXMOEDA ),2)
                nDescont := SF3->F3_VALOBSE
                //nDescont := ROUND(SF2->F2_DESCONT, 2)
            EndIf
        ...
        LCV->VALCONT += SF3->F3_VALCONT+nDescont
