...
cQuery := "SELECT CJ_FILIAL,CJ_EMISSAO,CJ_NUM,A1_COD,A1_NOME,A1_END,A1_TEL,A1_CGC,CASE WHEN CJ_STATUS='A' THEN 'APROBADO' ELSE 'PENDIENTE' END CJ_STATUS,"
...
//Alert("Item: " + CK_ITEM + " Item Cliente: " + CK_ITECLI)
	nImpIteCli := Len(AllTrim(TRB->CK_ITECLI))
    nLindet := 06.2             				
	nPag := 1
	cNroProforma:=""
	nMoneda:=0
	...
	While !EOF()    
		nMoneda:=CJMOEDA
		dfecha:= TRB->CJ_EMISSAO
		If nPag == 1 .and. TRB->CJ_NUM <> cNroProforma .and.cNroProforma==""
			cNroProforma :=TRB->CJ_NUM      

			tCJ_NUM	:= TRB->CJ_NUM
			tA1_COD := TRB->A1_COD
			tA1_LOJA := TRB->A1_LOJA
			tA1_NOME := TRB->A1_NOME
			tA1_END	:= TRB->A1_END
			tA1_TEL	:= TRB->A1_TEL
			tCJ_EMISSAO :=TRB->CJ_EMISSAO
			tCJ_UVEND := TRB->CJ_UVEND
			tA1_CGC	:= TRB->A1_CGC
			tCJ_MOEDA := TRB->CJ_MOEDA
			tE4_DESCRI := TRB->E4_DESCRI
			tCJ_DESC1 := IF(TRB->CJ_DESC1<>0,TRB->CJ_DESC1,TRB->CJ_PDESC)
...
//			tCJ_USRREG := CJ_USRREG
			...
			oPrn:StartPage() 
		ElseIf TRB->CJ_NUM <> cNroProforma  
			cNroProforma :=TRB->CJ_NUM
...
//			tCJ_USRREG := cUserName
	    ...
	    nTotal:=nTotal+TRB->CK_VALOR 
	    nDesc:=nDesc+TRB->CK_VALDESC   
	    nRecargo:=(TRB->CJ_FRETE+TRB->CJ_DESPESA+TRB->CJ_SEGURO)
...
Static Function fImpItem(nLin)
	oPrn:Say( Tpix(nLin+01.5), Tpix(01.0), TRB->CK_ITEM, oFont10C)
	If nImpIteCli > 0                                                  
		oPrn:Say( Tpix(nLin+01.5), Tpix(01.5), TRB->CK_ITECLI, oFont10C)
		oPrn:Say( Tpix(nLin+01.5), Tpix(02.1), TRB->CK_PRODUTO, oFont10C)
	Else
		oPrn:Say( Tpix(nLin+01.5), Tpix(01.8), TRB->CK_PRODUTO, oFont10C)
	EndIf
   	oPrn:Say( Tpix(nLin+01.5), Tpix(04.5), TRB->CK_DESCRI, oFont10C)
	oPrn:Say( Tpix(nLin+01.5), Tpix(12.5), TRB->CK_UM, oFont10C)
	oPrn:Say( Tpix(nLin+01.5), Tpix(12.7),Transform(TRB->CK_QTDVEN,"@A 99,999,999.99"), oFont10C)
	oPrn:Say( Tpix(nLin+01.5), Tpix(15.1),Transform(TRB->CK_PRCVEN,"@A 99,999,999.99"), oFont10C)  
	oPrn:Say( Tpix(nLin+01.5), Tpix(17.5), Transform(TRB->CK_VALOR,"@A 999,999,999.99"), oFont10C)
...
//	EndIf
    ...
    If Len(AllTrim(TRB->CK_OBS)) > 0
       nLin := nLin+00.3
	   oPrn:Say( Tpix(nLin+01.5), Tpix(04.3), Left(TRB->CK_OBS,45), oFont10C)																	
    EndIf
   	If Len(AllTrim(TRB->CK_OBS)) > 45
       nLin := nLin+00.3
	   oPrn:Say( Tpix(nLin+01.5), Tpix(04.3), Right(TRB->CK_OBS,35), oFont10C)																	
   	EndIf
...
//diasfin:=DateDiffDay(CTOD(CJ_VALIDA),CTOD(CJ_EMISSAO))  //substr(CJ_VALIDA,8,2)
	...
	If SM0->(Eof())                                
		SM0->( MsSeek( cEmpAnt + TRB->CJ_FILIAL , .T. ))
	Endif
