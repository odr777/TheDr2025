...
static function FatMat(cDoc1,cDoc2,cSerie,cTitulo,bImprimir,nCopia,nLinInicial) //Datos Maestro de factura; ,nLinInicial
	...
	// cSql		:= cSql+" FROM " + RetSqlName("SF2 ") +" SF2 JOIN " + RetSqlName("SA1") +" SA1 ON (A1_COD=F2_CLIENTE AND A1_LOJA=F2_LOJA AND A1_FILIAL=F2_FILIAL AND SF2.D_E_L_E_T_ =' ' AND SA1.D_E_L_E_T_ =' ') "
...
Static Function CabFact(nLinInicial,aMaestro,cTipo,cDireccion,cTelefono,oBrush,cFile)
	...
	oPrn:Say( nLinInicial + 570, 1800,aMaestro[25],oFont12 ) //
	...
	//	oPrn:Say( nLinInicial + 715, 100,aMaestro[16]+ "  " + AllTrim(aMaestro[17]) ,oFont10 ) // NTP 18/03/2019
	oPrn:Say( nLinInicial + 715, 100,"Teléfono : " + AllTrim(SM0->M0_TEL) ,oFont10 ) // NTP 18/03/2019

	oPrn:Say( nLinInicial + 715, 100, AllTrim(aMaestro[17]) ,oFont10 )
...
Static Function PieFact(nLinInicial,aMaestro)
	...
	oPrn:Say(nLinInicial + 2670, 0700, "Usr: " + cUserName + "  " + time(),oFont12 )  //Usuario

	aObs := TexToArray(aMaestro[28],70)
	for i := 1 to len(aObs)
		if i <= 1
			oPrn:Say(nLinInicial + 2770, 0100, "Obs:   " + aObs[i],oFont12 )  //observacion
		else
			oPrn:Say(nLinInicial + 2770, 0100, aObs[i],oFont12 )  //observacion
		endif
		nLinInicial +=60
	next i
	nLinInicial := 0
