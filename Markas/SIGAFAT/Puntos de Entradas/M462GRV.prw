...
Local _nPosSer:=Ascan(aHeader,{|x| Alltrim(x[2])=="D2_NUMSERI"})
Local _nPosLoc:=Ascan(aHeader,{|x| Alltrim(x[2])=="D2_LOCALIZ"})
...
For _nCont:=1 to Len(aCols)
    If SC9->(DbSeek(xFilial('SC9')+aCols[_nCont,_nPosPed]+aCols[_nCont,_nPosIte]+aCols[_nCont,_nPosSeq]+aCols[_nCont,_nPosPro])) 
	    aCols[_nCont,_nPosSer]:= SC9->C9_NUMSERI     
	    aCols[_nCont,_nPosLoc]:= SC9->C9_ULOCALI
