...
static function f4_ROfC3(cTitulo,bImprimir)
	...
	_csql := "SELECT "  + Chr(13)
	_csql := _csql + " CTO_SIMB, CTO_DESC, M2_MOEDA2,  EL_MOEDA, " + Chr(13)
	...
	_csql := _csql + " INNER JOIN " + RetSqlName("CTO") + " CTO ON CAST(CTO_MOEDA AS TinyInt) = EL_MOEDA AND CTO_FILIAL = EL_FILIAL AND CTO.D_E_L_E_T_ = ' '" + Chr(13)
	...
	_csql := _csql + " (EL_BANCO + EL_AGENCIA + RTRIM(EL_CONTA)+ EL_FILIAL =  A6_COD + A6_AGENCIA + RTRIM(A6_NUMCON) + A6_FILIAL) AND SA6.D_E_L_E_T_ = ' '" + CHR(13)
	...
	_cCond := " AND SEL.R_E_C_D_E_L_ = 0  AND EL_CANCEL='F'"  + Chr(13)
	...
	_csql2 := _csql2 + " EL_RECIBO, EL_TIPODOC, EL_NUMERO, EL_SERIE ,EL_PARCELA, EL_TIPO, EL_TXMOEDA, EL_CLIENTE, EL_LOJORIG, " + Chr(13)
	...
	_csql2 := _csql2 + " INNER JOIN " + RetSqlName("CTO") + " CTO ON CAST(CTO_MOEDA AS TinyInt) = EL_MOEDA AND CTO_FILIAL = EL_FILIAL AND CTO.D_E_L_E_T_ = ' '" + Chr(13)
	...
	_csql2 := _csql2 + " WHERE  SE1.R_E_C_D_E_L_ = 0 AND SA1.D_E_L_E_T_ = ' ' " + CHR(13)
	_csql2 := _csql2 + " AND EL_RECIBO=E1_RECIBO "  + Chr(13)
...
MemoWrite("CFinI204.sql",_csql2)
	...
	While !Eof()
		...
		oPrn:Say(nLin1, _nCol1 - 80, "RECIBO No.:" + _cCobDiv, oFont4) //EL_RECIBO sería del siguiente...

		oPrn:Say(nLin3, _nCol1-80, "FECHA:" + (dtoC(stod(_dFecha))) ,oFont4)
		oPrn:Say(nLin4, _nCol1 -80, "TC." + AllTrim(CValToChar(_nCambio)) ,oFont4)
		oPrn:Say(nLin4, _nCol1 + 50, "S:" + AllTrim(_cSerie) ,oFont4)
		//oPrn:Say(nLin5, _nCol1, "- - - - - - - - -",oFont4)



		_nLin := _nLin + _nInterLin
		oPrn:Say(_nLin, _nCol3, "Titulo", oFont1)
		oPrn:Say(_nLin, _nCol3 + 135, "Cta Parcela", oFont1)
		oPrn:Say(_nLin, _nCol3 + 345, "Prefijo", oFont1)
		oPrn:Say(_nLin, _nCol3 + 545, "Tipo", oFont1)
		oPrn:Say(_nLin, _nCol3 + 750, "Valor", oFont1)
		oPrn:Say(_nLin, _nCol3 + 970, "Glosa", oFont1)
		_nLin := _nLin + _nInterLin
		...
		Do While (EL_RECIBO == _cCobDiv2) .And. !Eof() .And. _nFinHoja>_nLin
			...
			oPrn:Say(_nLin, _nCol3, AllTrim(EL_NUMERO) ,oFont2)
			oPrn:Say(_nLin, _nCol3 + 145, AllTrim(EL_PARCELA) ,oFont2)
			oPrn:Say(_nLin, _nCol3 + 345, AllTrim(EL_PREFIXO) ,oFont2)
			oPrn:Say(_nLin, _nCol3 + 545, AllTrim(EL_TIPO) ,oFont2)
			oPrn:Say(_nLin , _nCol3 + 675 , Transform(EL_VALOR, "@E 999,999,999.99") ,oFont2)
			//oPrn:Say(_nLin, _nCol3 + 745, AllTrim(EL_VALOR) ,oFont2) 
			oPrn:Say(_nLin, _nCol3 + 945, AllTrim(E1_HIST) ,oFont2)								
			...
			if ALLTRIM(E1_ORIGEM)$"MATA467N|MATA460|FINA040"
				...
				oPrn:Say(_nLin , _nCol3 + 1300 , Transform(EL_VALOR, "@E 999,999,999.99") ,oFont2)
				oPrn:Say(_nLin , _nCol3 + 1550 , Capital(CTO_SIMB) ,oFont2)
				_nLin := _nLin + 50 //2*nInterLin
			ELSE
		...
		End Do
			...
			+ Transform(_aSumas[1][nDolares],"@E 9,999,999,999.99") ,oFont4 )
			if _nEspCab != 0
