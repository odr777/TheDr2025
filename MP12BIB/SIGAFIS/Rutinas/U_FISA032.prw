...
static function getValorActualizado(nValor)
	...
	dData1:= LastDate( MonthSub(dData2,1) )
	...
	BeginSql Alias aliasQuery
		...
		FROM SM2010 SM2
		WHERE M2_DATA IN (%exp:(cData1)%, %exp:(cData2)%)
		AND SM2.%notdel%
		ORDER BY 1
	...
	If(nUFV1 == 0)
		alert("El valor de la UFV en fecha: " + DTOC(dData1) + " no se ha registrado o es igual a 0")
	EndIf

	If(nUFV2 == 0)
		alert("El valor de la UFV en fecha: " + DTOC(dData2) + " no se ha registrado o es igual a 0")
	EndIf
