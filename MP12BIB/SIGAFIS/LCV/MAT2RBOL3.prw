...
//	Local oSection2:= Nil
	...
	If MV_PAR03 == 1 // Compras
	    ...
	    TRCell():New(oSection1,"TIPO" ,"","TIPO","@!",01)                //"Especie"
	...
	//    TRCell():New(oSection1,"POLIZA","","STR0007","@!",16)						//"No de DUE"
	    ...
	    TRCell():New(oSection1,"VALCONT","@R 999,999,999.99",cSTR0057,"",24)	//"VALOR TOTAL DA COMPRA"
	    TRCell():New(oSection1,"EXENTAS","",cSTR0058,"",24)	//"VALOR NAO SUJEITO A CREDITO FISCAL"
	    TRCell():New(oSection1,"SUBTOT","@R 999,999,999.99",STR0059,"",24)   //"SUBTOTAL"
	    //TRCell():New(oSection1,"DESCONT","",STR0060,"",14)   //"Descontos Bonificaç?es e Abatimentos Obtidos"  
	    TRCell():New(oSection1,"DESCONT","",cDesCom,"",24)   //ODR 02/08/2019
	    TRCell():New(oSection1,"BASEIMP","@R 999,999,999.99",cSTR0014,"",24)	//"Valor Base para Crédito Fiscal" 
	    TRCell():New(oSection1,"VALIMP","",cSTR0013,"",24)	   //"Credito Fiscal"    
	...
	ElseIf MV_PAR03 == 2  // Vendas
	    ...
	    TRCell():New(oSection1,"VALCONT","",cSTR0010,"",20)   //"VALOR TOTAL DE VENDA"(IMPORTE TOTAL DE VENTA)
	    TRCell():New(oSection1,"EXENTAS","",cSTR0011,"",20)   //"VALOR ICE/ IEHD/ TAXAS"(IMPORTE ICE/ IEHD/ TASAS)
	    TRCell():New(oSection1,"EXPORT","",cSTR0061,"",20)    //"EXPORTACOES E OPERACOES ISENTAS"(EXPORTACIONES Y OPERACIONES EXENTAS)
	    TRCell():New(oSection1,"TAXAZERO","",cSTR0012,"",20)   //"VENDAS GRAVADAS A TAXA ZERO"(VENTAS GRAVADAS A TASA CERO)
	    TRCell():New(oSection1,"SUBTOT","",STR0059,"",20)    //"SUBTOTAL"
	    ...
	    TRCell():New(oSection1,"DESCONT","",cDesVen,"",20)   //ODR 02/08/2019
	    TRCell():New(oSection1,"BASEIMP","",cSTR0017,"",20)   //"VALOR BASE PARA DEBITO FISCAL"(IMPORTE BASE PARA DEBITO FISCAL)
	    TRCell():New(oSection1,"VALIMP","",cSTR0018,"",20)    //"DEBITO FISCAL"(DEBITO FISCAL)
	...
	Else //Compras Deb/Cre e ou Ventas Deb/Cre
	    ...
	    MsgRun(STR0016,STR0001,{|| U_DavinciV(Iif(MV_PAR03==3,"CDC","VDC"))}) //"Preparando as informacoes para o livro de vendas"###"Livros fiscais"
//	    MsgRun(STR0016,STR0001,{|| U_DavinciV(Iif(MV_PAR03==3,"CDC","VDC"))}) //"Preparando as informacoes para o livro de vendas"###"Livros fiscais"
	    /*  
	    Campos do detalhe: {nome do campos, tamanho para impressao, mascara, tipo, cabecalho} */
...
Static Function ReportPrint(oReport)
	...
	//    MsgRun(STR0016,STR0001,{|| DaVinci(Iif(MV_PAR03==3,"CDC","VDC"))}) //"Preparando as informacoes para o livro de vendas"###"Livros fiscais"
	    ...
	    Aadd(aCposImp,{"VALCONT" ,14,"@R 999,999,999.99","N",STR0042+" "+Iif(MV_PAR03==3,STR0050,STR0051)})      // "Valor total da Devoluç?o ou Rescis?o"##"Recebida"##"Efetuada"    
	    Aadd(aCposImp,{"VALIMP" ,14,"@R 999,999,999.99","N",Iif(MV_PAR03==3,STR0043,STR0054)})       // "Crédito Fiscal"##"Debito Fiscal"
	    ...
	    Aadd(aCposImp,{"VALNFORI",14,"@R 999,999,999.99","N",STR0048})      // "Valor Total Fatura Original"
	...
	//		oSection1:Cell(NOMBRE DE TU ATRIBUTO DEL OBJETO OS):SetValue(aDados[3])
			...
			// Omar 23/10/2019
			 DO CASE
			    ...
			    CASE MV_PAR03 == 4
			    	oSection1:Cell("TIPO"):SetValue(4)			    
			ENDCASE 
			...
			// oSection1:Cell("TIPO"):SetValue(9)
