...
╠╠╨Programa  ЁConsListaP ╨Autor  ЁNain Terrazas      ╨ Data Ё  18.10.00   ╨╠╠
╠╠лммммммммммьммммммммммймммммммоммммммммммммммммммммйммммммоммммммммммммм╧╠╠
╠╠╨Desc.     ЁFuncion responsable por F3 de la SC6 que busca con lista de ╨╠╠
╠╠╨          Ё precio, actualizado para Buscar 							  ╨╠╠
...
//	alert("Nahim")
	...
	If lBrowse
		...
		//Ё Inicia montagem do browse (Busca Produtos).                             Ё
		//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
		oListProd := TWBrowse():New(  070,05,545,065,,{"Codigo","Descr.","Descr. Especifica","Cod.Fabrica","Grupo","Marca"},{40,150,300,50,20,80},oDlg,,,,,{||IF(LEN(_aBuscaProd)>0,MostrarDatos(_aBuscaProd[oListProd:nAT,1],.T.),.t.)},,,,,,,.F.,,.T.,,.F.,,,)
		...
		//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		//Ё Inicia montagem do browse (Produtos Similares).                         Ё
		//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
		...
		//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		//Ё Inicia montagem do browse (Saldos em Estoque).                          Ё
		//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
		oListBox := TWBrowse():New( 140,05,545,060,,{"Empresa/Sucursal","Alm.","Disponible","Saldo Actual","Pedido Ventas","Reservada","Prevista Entrada","Reservada S.A.","Reservada"},{65,15,42,42,42,42,42,42,42,42,42},oDlg,,,,,,,,,,,,.F.,,.T.,,.F.,,,)
		oListBox:SetArray(aViewB2)
		oListBox:bLine := { || If(Len(aViewB2)>0,aViewB2[oListBox:nAT],.t.)}

		//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		//Ё Inicia montagem do browse (Lista do Preco).                          Ё
		//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
...
Static Function BuscaProd(nTipo)
	...
	If _nContPre == 0
		Aviso("Importante","Necesario llenar uno de los campos para la bЗsqueda",{"OK"})
		Return
	Endif
	...
	For _nCont:=1 to Len(_aGets)
		...
		If !Empty(_aGets[_nCont,2])
			...
			While _lContinua
				...
				If _nPos == 0
					//NT Adicionado para buscar en Nombre y DescripciСn
					If _nCont == 2
		...
		End
	Next _nCont
	if nTipo == 2 // si es 2 busca SСlo con Stock
	...
	IF BUSCAPROD->(!EOF()) .AND. BUSCAPROD->(!BOF())
		...
		WHILE BUSCAPROD->(!EOF())
			if _nCont==1
				...
				_aBuscaProd[1,6]:=alltrim(BUSCAPROD->B1_UMARCA)
				_nCont++
			Else
				...
				AADD(_aBuscaProd,{BUSCAPROD->B1_COD,alltrim(BUSCAPROD->B1_DESC),alltrim(BUSCAPROD->B1_ESPECIF),alltrim(BUSCAPROD->B1_UCODFAB),alltrim(BUSCAPROD->B1_GRUPO),alltrim(BUSCAPROD->B1_UMARCA)})
			End
			BUSCAPROD->(DbSkip())
		END
	End
...
Static Function MostraEst(cProduto,_lSim,cArmazem,lBrowse,aArmazem)
	...
	//Ё Trata parametro de pesquisa por Armazem. Caso o parametro nao seja     Ё
	//Ё passado, listara todos os armazens encontrados.                        Ё
	...
	//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
	//Ё Trata parametro de visualizacao do Browse. Caso o parametro nao seja   Ё
	//Ё passado, apresenta o Browse de consulta.                               Ё
	//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
	If ValType(lBrowse) == 'U'
		lBrowse := .t.
	EndIf
	//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
	//Ё Posiciona o cadastro de produtos                                       Ё
	//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
	DbSelectArea('SB1')
	DbSetOrder(1)
	If DbSeek(xFilial("SB1")+cProduto,.f.)
		...
		//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		//Ё Abre arquivo de empresas para obter empresas para consulta do estoque  Ё
		//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
		DbSelectArea("SM0")
		DbSetOrder(1)
		DbGoTop()
		...
		//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		//Ё Monta cQuery para realizar select no SB2 de todas as Empresas e Filiais Ё
		//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
		_cOrdQuery:=1
		_cOrdQueryAux:=50
		_cOrdQueryAct:=0
		...
		While !Eof()
			//CondiГЦo adicionada enquando se configura os arquivos da filial 02
			/*
			...
			*/
			...
			If 	TCCANOPEN("SB2"+SM0->M0_CODIGO+"0") // .and. TCCANOPEN("SC6"+SM0->M0_CODIGO+"0") .and.  TCCANOPEN("SF2"+SM0->M0_CODIGO+"0")
				...
				//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
				//Ё Faz tratamento para pesquisa dos Armazens.                              Ё
				//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
				If Len(aArmazem) > 0
			...
			//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
			//Ё Acrescenta clausula "Union" para juntar todas as select's.              Ё
			//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
		...
		//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		//Ё Valida Query a ser executada.                                           Ё
		//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
		...
		//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		//Ё Cria Alias temporario com o resultado da Query.                         Ё
		//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
		dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),cCursor,.T.,.T.)

		//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		//Ё Ajusta os campos que nao sao Caracter de acordo com a estrutura do SB2  Ё
		//Ё uma vez que a TcGenQuery retorna todos os campos como Caracter.         Ё
		//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
		...
		//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		//Ё Inicia montagem do array (aViewB2) para visualizacao no Browse e        Ё
		//Ё posterior retorno da funcao.                                            Ё
		//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
		_cNomeEmp  := ""
		_cNomeEmpA := ""
		...
		While ( !Eof() )
			...
			//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
			//Ё Trata variaveis a serem passadas para o Array para nao poluir o Browse. Ё
			//Ё Caso lBrowse seja .f., deixa o Array sem os devidos tratamentos.        Ё
			//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
		...
		//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		//Ё Fecha arquivo temporario da TcGenQuery                                  Ё
		//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
		DbSelectArea(cCursor)
		DbCloseArea()
		DbSelectArea("SB2")
	...
	if !_lSim
		//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		//Ё Carregando browse de Produtos Similares                                 Ё
		//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
		MontaSim(cProduto)
	End
...
╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠
╠╠иммммммммммяммммммммммкмммммммяммммммммммммммммммммкммммммяммммммммммммм╩╠╠
╠╠╨Programa  ЁMontaSim  ╨Autor  ЁMicrosiga           ╨ Data Ё  12/06/04   ╨╠╠
╠╠лммммммммммьммммммммммймммммммоммммммммммммммммммммйммммммоммммммммммммм╧╠╠
╠╠╨Desc.     Ё                                                            ╨╠╠
╠╠╨          Ё                                                            ╨╠╠
╠╠лммммммммммьмммммммммммммммммммммммммммммммммммммммммммммммммммммммммммм╧╠╠
...
Static Function MontaSim(_cProduto)
	...
	ASIZE(_aProdSim,0)
	//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
	//Ё Inicia montagem da Matriz com Dados de Produtos Similares               Ё
	//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
	DbSelectArea("SZ3")
	...
	If DbSeek(xFilial("SZ3")+_cProduto,.f.)
		...
		//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		//Ё Adiciona todos os produtos no array, relacionados a familia ...        Ё
		//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
	...
	If !Empty(_cProduto)
		...
		If SB1->(DbSeek(xFilial("SB1")+_cProduto))
			...
			Else
				...
				oBitPro:lStretch:=.T. //Ajustar al tamaЯo del Dialog
				oBitPro:Refresh()
			Endif
...
//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
//Ё FunciСn que muestra los datos del Sigamat - Tabla SM0                   Ё
//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
...
Static Function MostraLista(cProduto,_lSim,cArmazem,lBrowse,aArmazem)
	...
	If DbSeek(xFilial("SB1")+cProduto,.f.)
		...
		//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		//Ё Inicia montagem do array (aViewDA1) para visualizacao no Browse e        Ё
		//Ё posterior retorno da funcao.                                            Ё
		//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
		_cNomeEmp  := ""
		_cNomeEmpA := ""
