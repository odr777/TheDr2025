...
user function CALCRET(cRA_FILIAL, cRA_MAT)
	...
	Local cFilialPer:= getFilialPeriodoRet(cRA_MAT,cPerRET)
	Local nX		:= 1
	...
	while nListPER <= len(aListaPer)
		if val(aListaPer[nListPER]) >= val(cPerRET) .AND. val(aListaPer[nListPER]) < val(cPerActual)
			...
			ElseIf(aPD[nListPER] == cPDBonoAnt)//Bono de Antiguedad
				nVal	:= calculaBA(aVal[nListPER], nMinimoActual, aHoras[nListPER], nDiasTrab)
				nValInfo:= nSMN
...
*/
static function getFilialPeriodoRet(cMat,cPer)
	Local aArea	:= getArea()
	Local StrSql:= ""
	Local cRet	:= ""
...
*/
static function calculaBA(nBAAnterior, nMinimo, nPorcentaje, nDiasTrab)
	Local aArea		:= getArea()
	Local nRet		:= 0
