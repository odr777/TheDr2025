...
User Function FISA820(cCliIni,cFilIni,cCliFim,cFilFim,dDataIni,dDataFim,nTipo,lGeraTxt,cPasta,lGeraRel,nMonMin)
...
Static Function FISA820A(cCliIni,cFilIni,cCliFim,cFilFim,dDataIni,dDataFim,nTipo,lGeraTxt,cPasta,aResult,lRet,nMonMin,cLinArq)
	...
	Local cLinhas 	:= ''
	Local cMesAno	:= STRZERO(Month(dDataBase), 2, 0)+AllTrim(Str(Year(dDataBase)))
	Local aCliAtu	:= {}
	Local aCliAnt	:= {}
	Local lSa1Comp  := .F.
	Local cLinAcm3  := ''
	...
	// FILTRAR SOLO NF ODR 26/11/2020
...
Static Function FNGERARQ(cPessoa,cArquivo,cNIT,cCliente,cNomCli,cMesAno,cPasta,aResult,dDataFim)
	...
	If cPessoa == '7'
		...
		nHdlSimp 	:= FCREATE(cPasta + "\SIMPLIFICADO_" + cMesAno + "_" + AllTrim(SM0->M0_CGC) + ".TXT",,,.F.)
		If nHdlSimp > 0
			FWrite(nHdlSimp, cArquivo)
			aAdd(aResult,{cPasta + "\SIMPLIFICADO_" + cMesAno + "_" + AllTrim(SM0->M0_CGC) + ".TXT",STR0006})
	...
	ElseIf cPessoa == '3'
		...
		nHdlPN 	:= FCREATE(cPasta + "\PERSONASNATURALES_" + cMesAno + "_" + AllTrim(SM0->M0_CGC) + ".TXT",,,.F.)
		If nHdlPN > 0
			FWrite(nHdlPN, cArquivo)
			aAdd(aResult,{cPasta + "\PERSONASNATURALES_" + cMesAno + "_" + AllTrim(SM0->M0_CGC) + ".TXT",STR0006})
