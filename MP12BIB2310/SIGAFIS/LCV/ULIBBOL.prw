...
±±³Personalización columnas nuevas.            							   ³±±
...
User Function LIBBOL(cLivro, cFilIni, cFilFim,cTipDoc,cTipoExc)
	...
	If lPeriodoOK .or. BOL3Periodo()
		If cPaisLoc == "BOL" .And. LocBol() 
			...
			EndIf
			If lProc
				U_ProcLivro(cLivro,cFilIni,cFilFim,cFiltro)
				U_GeraXLS(cLivro,cTipoExc)
			EndIf
		EndIf
	EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
//User Static Function GeraTemp(cLivro,cFiltro)
User Function GeraTemp(cLivro,cFiltro)
	...
	aAdd(aStru, {"USUARIO"	, "C", 020, 0})
	aAdd(aStru, {"DTDIGIT"	, "D", 010, 0})
	aAdd(aStru, {"SERIE"	, "C", 003, 0})
	aAdd(aStru, {"NODIA"	, "C", 010, 0})
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
User Function ProcLivro(cLivro,cFilIni,cFilFim,cFiltro)
		...
		If lProcLiv
			...
			If Empty(cChave) .Or. cChave <> SF3->F3_FILIAL + SF3->F3_NFISCAL + SF3->F3_SERIE + SF3->F3_CLIEFOR + SF3->F3_LOJA
				...
				If cLivro == "C"
					...
					// Agregar Columnas ODR
					LCV->NODIA := SF1->F1_NODIA
					LCV->USUARIO	:= FWLeUserlg("F1_USERLGI", 1)
				...
				ElseIf cLivro == "V"
					...
					LCV->USUARIO	:= FWLeUserlg("F2_USERLGI", 1)
			...
			// Adicionar Columnas ODR
			LCV->EMISSAO	:= dDemissao
			LCV->FILIAL		:= SF3->F3_FILIAL				
			LCV->DTDIGIT	:= SF1->F1_DTDIGIT	
...
User Function GeraXLS(cLivro,cTipoExc)
	Local aArea        := GetArea()
	Local oFWMsExcel
	...
	If cLivro =="V"
		...
		// ADICIONAR COLUMNAS
		oFWMsExcel:AddColumn(cNome,"Productos","SUCURSAL",1)
		oFWMsExcel:AddColumn(cNome,"Productos","USUARIO",1)
		oFWMsExcel:AddColumn(cNome,"Productos","COD.CTB.",1)
		oFWMsExcel:AddColumn(cNome,"Productos","SERIE",1)
	Else
		oFWMsExcel:AddColumn(cNome,"Productos","Nº",1)
		oFWMsExcel:AddColumn(cNome,"Productos","ESPECIFICACION",1)
		...
		oFWMsExcel:AddColumn(cNome,"Productos","USUARIO",1)
		oFWMsExcel:AddColumn(cNome,"Productos","COD.CTB.",1)
		oFWMsExcel:AddColumn(cNome,"Productos","DIGITACIÓN",1)
	EndIf      
