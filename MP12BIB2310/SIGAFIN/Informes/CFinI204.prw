...
//	_csql2 := _csql2 + " E1_PREFIXO + E1_NUM + E1_PARCELA + E1_TIPO = EL_PREFIXO + EL_NUMERO + EL_PARCELA + EL_TIPO AND EL_RECIBO=E1_RECIBO " + CHR(13)nahim quitando recibo 03/05/2019
	...
	_csql2 := _csql2 + " E1_PREFIXO = EL_PREFIXO AND E1_PARCELA = EL_PARCELA AND EL_TIPO = E1_TIPO AND EL_NUMERO = E1_NUM " + CHR(13)  
...
//				oPrn:Say(_nLin , _nCol3 + 1600,  " Cambio(s) ",oFont1)
			...
			Do While EL_RECIBO == _cCobDiv .And. !Eof()
				if alltrim(EL_TIPODOC) != 'TB'// Nahim Adicionado para las compensaciones 03/05/19
					_nLin := _nLin + 50
//					oPrn:Say(_nLin , _nCol3 -  80, Capital(Alltrim(A6_NOME)) ,oFont2)// nahim 11/04/2019
//					oPrn:Say(_nLin , _nCol3 +  300 , Alltrim(A6_NUMCON) ,oFont2) //nahim 11/04/2019
					if alltrim(EL_TIPODOC) == "CH" // si es cheque
//						oPrn:Say(_nLin , _nCol3 ,  Tabela( "05", EL_TIPODOC, .F.) + " "+ Capital(Alltrim(EL_BANCO)) + " (" + AllTrim(EL_NUMERO) +")" ,oFont2)
...
//					oPrn:Say(_nLin , _nCol3 + 1550 , Transform(EL_UCAMBIO, "@E 999,999,999.99") ,oFont2)
					...
					if !Empty(EL_UOBSTAR)
						_nLin := _nLin + 50
						oPrn:Say(_nLin, _nCol3, " Observaciones : " ,oFont1)
						oPrn:Say(_nLin, _nCol3 + 500, AllTrim(EL_UOBSTAR) ,oFont2)
					end if
...
//			end if
			if ALLTRIM(E1_ORIGEM)$"MATA467N|MATA460|FINA040"
				...
				oPrn:Say(_nLin , _nCol3 + 1090 , Transform(EL_VALOR, "@E 999,999,999.99") ,oFont2)
				oPrn:Say(_nLin , _nCol3 + 1320 , Capital(CTO_SIMB) ,oFont2)
				_nLin := _nLin + 50 //2*nInterLin
			ELSE
