...
//cRecibo := GETSXENUM('SEL','EL_RECIBO')  
	...
	@ C(195),C(275) Button "&Confirmar" Size C(037),C(012) PIXEL OF _oDlg ACTION IIf(UFA087vld('cRecibo',"   ",cRecibo,cbanco,cnumero,cNatureza,cvalorcob,cagenban),_oDlg:End(),(nOpcA := 1,cRecibo  	:= 	GETSXENUM('SEL','EL_RECIBO')))
...
static Function UFA087vld(cVar   ,cSerie,cRecibo,cBanco,cTitulo,cModalidad,cvalorcob,cagenban)
	Local lRet 	:= .T.
	Local _aArea := GetArea()
