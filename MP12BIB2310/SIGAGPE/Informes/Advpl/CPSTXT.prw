...
User Function CPSTXT

//	cDir	:= "c:\users\usuario\downloads\excel\20191220091502"
//	
//	If !ExistDir(cDir)
//		AutoGrLog( "Creando directorio.......: " + Dtoc(MsDate()) )
//		MakeDir(cDir)
//	Endif
//	
////	FT_FUSE()
...
//	
//	MostraErro(cDir, "informe.txt") // Sempre que o micro comeca a apitar esta ocorrendo um erro desta forma
	...
	Private Titulo	:= STR0001 // "Geração do arquivo TXT - Aportes Caja Salud de la Banca Privada"
	Private cFilialDe
	Private cFilialAte
	Private cMesAno
	Private cMes
	...
	AADD(aButtons, { 1,.T.,{|o| nOpca := 1, FechaBatch() }})// ,IF(gpTudoOK(cPerg),FechaBatch(),nOpca:=0) }} )
...
Static Function GPER625()
	/*
	ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	³ Define Variaveis Locais (Basicas)                            ³
	ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ*/
	Local cDesc1 		:= "Aportes Caja de Salud de la Caja Petrolera"
	Local cDesc2 		:= "Se imprimira de acuerdo con los parametros solicitados por el usuario."
	Local cDesc3 		:= ""
	...
	ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ*/
	Private Titulo 		:= "Aportes Caja de Salud de la Caja Petrolera"
	Private nTamanho		:= "M"
	Private nOrdem
	Private cFilialDe   	:= ""
	Private cFilialAte  	:= ""
	...
	oPrint 	:= TMSPrinter():New( "Aportes Caja de Salud de la Caja Petrolera" )
...
Static Function ImpAsCajaS(lTXT)
	...
	cQuery += " RA_PRINOME, RA_SECNOME, RA_PRISOBR, RA_SECSOBR, RA_BCDEPSA, RA_CATFUNC, RA_SEGUROS "
	...
	While (cQrySRA)->( !Eof() )
		...
		*/
		...
		cQuery := "SELECT SUM(RC_VALOR) RC_VALOR "
		cQuery += " FROM " + RetSqlName(cAlias) + " SRC " 
		cQuery += " JOIN " + RetSqlName("SRV") + " SRV ON SRC.RC_PD = SRV.RV_COD AND SRV.RV_TIPOCOD = 1 AND SRV.RV_INDEN = 1 "
		cQuery += " WHERE SRC.D_E_L_E_T_ = '' " 
		cQuery += " AND SRC.RC_MAT = '" + (cQrySRA)->RA_MAT + "' "
		cQuery += " AND SRC.RC_PERIODO IN " + cPeriodos
		...
		If !(len(aPerAberto) < 1)
			...
			//montagem da query
			cQuery := "SELECT"
			cQuery += " RC_FILIAL, RC_MAT, RC_PROCES, RC_ROTEIR, RC_PERIODO, RC_SEMANA, RC_VALOR, RC_PD, RC_HORAS"
			cQuery += " FROM " + RetSqlName(cAlias)
			cQuery += " WHERE"
			...
			While (cQrySRC)->(!Eof()) .And. (cQrySRA)->(RA_FILIAL+RA_MAT) == (cQrySRC)->(RC_FILIAL+RC_MAT)
				Do Case
					Case (cQrySRC)->RC_PD == aCodFol[0031,1]
					nDiasTrab := (cQrySRC)->RC_HORAS
		...
		If	!(len(aPerFechado) < 1)
			...
			//montagem da query
			cQuery := "SELECT"
			cQuery += " RD_FILIAL, RD_MAT, RD_PROCES, RD_ROTEIR, RD_PERIODO, RD_SEMANA, RD_VALOR, RD_PD, RD_HORAS"
			cQuery += " FROM " + RetSqlName(cAlias)
			cQuery += " WHERE"
			...
			While (cQrySRD)->(!Eof()) .And.  (cQrySRA)->(RA_FILIAL+RA_MAT) == (cQrySRD)->(RD_FILIAL+RD_MAT)
				Do Case
					Case (cQrySRD)->RD_PD == aCodFol[0031,1]
					nDiasTrab := (cQrySRD)->RD_HORAS
...
Static Function G624Grava()
	...
	RecLock(cAliasTMP,.T.)
	(cAliasTMP)->TMP_CI		:= (cQrySRA)->RA_SEGUROS
	(cAliasTMP)->TMP_DT		:= nDiasTrab
	(cAliasTMP)->TMP_APORTE	:= nSalary
	/*
	HARDCODE: CHANGE THE C FOR FULL JOB/PART JOB
	*/
	(cAliasTMP)->TMP_MOD	:= "C" // (cQrySRA)->RA_CATFUNC
...
Static Function G624CriTMP()
	...
	aAdd( aCampos, { 'TMP_DT'	, "N", 10	, 2 } )
	aAdd( aCampos, { 'TMP_APORTE'	, "N", 12	, 2 } )
	aAdd( aCampos, { 'TMP_MOD'		, "C", 1	, 0 } )
...
Static Function G624GerTXT()
	...
	While !Eof()
		...
		//1|123456789|30|2307|C|TARQUI SOLARES JULIO CESAR
		cLinDet	:=	Ltrim(Str(i))		+cSep+;    
		AllTrim(TMP_CI)	 		+cSep+;
		cValToChar(TMP_DT)		+cSep+;    
		LTrim(Str(TMP_APORTE,12,2))+cSep+;
		AllTrim(TMP_MOD)			+cSep+;
		AllTrim(TMP_NOME) + cSep
		
		If lFirst
			FWrite(nHandle,cLinDet)
		Else
			FWrite(nHandle,CRLF+cLinDet)		
		Endif
