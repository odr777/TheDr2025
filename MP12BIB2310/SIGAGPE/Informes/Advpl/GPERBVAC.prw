...
static function ejecQuery(cId,cTitulo,bImprimir)
	...
	cSql	+= " R8_DTBLEG, R8_DATA, R8_NUMID, R8_CODMEMO, RJ_DESC, CTT_DESC01, R8_PD, R8_TIPOAFA, RV_CODFOL, R8_PER "
...
Static Function printVacaciones(nLinB, aDados)
	...
	oPrint:Say(nLinB + 220, 840 ,"BOLETA DE"  ,oFont13N)
	oPrint:Say(nLinB + 340, 840 ,aDados[15]  ,oFont13N)
	...
	oPrint:Say(nLinB + 220, 1750 ,"Nro. Boleta: " ,oFont11N)
	oPrint:Say(nLinB + 220, 2000 , aDados[10] ,oFont11)
	oPrint:Say(nLinB + 320, 1750 ,"Fecha Boleta: " ,oFont11N)
	oPrint:Say(nLinB + 320, 2035 ,DTOC(STOD(aDados[9])),oFont11)
	...
	oPrint:Say(nLinB + 630, 840 ,"C. Costo : ",oFont09N)
	oPrint:Say(nLinB + 630, 1000 ,Alltrim(aDados[14]) ,oFont09)
	...
	oPrint:Say(nLinB + 750, 100 ,"Vacaciones Con Saldo" ,oFont11N)
	...
	oPrint:Say(nLinB + 750, 850 ,"Desde el :" ,oFont09N)
	oPrint:Say(nLinB + 750, 1020 ,DTOC(STOD(aDados[5])),oFont09)

	oPrint:Say(nLinB + 800, 850 ,"Hasta el :" ,oFont09N)
	oPrint:Say(nLinB + 800, 1020 ,DTOC(STOD(aDados[6])),oFont09)

	// oPrint:Say(nLinB + 950, 840 ,"Fecha Autorizaci๓n : " ,oFont09N)
	// oPrint:Say(nLinB + 950, 1170 ,DTOC(STOD(aDados[9])),oFont09)

	oPrint:Say(nLinB + 850, 850 ,"Solicitado :" ,oFont09N)
	oPrint:Say(nLinB + 850, 1020 , TRIM(cValtoChar(aDados[7])) + " dํas." ,oFont09)

	oPrint:Say(nLinB + 750, 1700 ,"Vacaciones programadas a la fecha:" ,oFont09N)
	oPrint:Say(nLinB + 750, 2250 , cValtoChar(diasUtilizados(aDados[16], aDados[1], aDados[10])) + " dํas." ,oFont09)

	oPrint:Say(nLinB + 800, 1700 ,"Saldo proyectado al cierre del mes: " ,oFont09N)
	oPrint:Say(nLinB + 800, 2250 , cValtoChar(saldoVac(aDados[16], aDados[1], CTOD(Substr(aDados[6],7,2) + "/" + Substr(aDados[6],5,2) + "/" + Substr(aDados[6],1,4)))) + " dํas." ,oFont09)
	//oPrint:Say(nLinB + 800, 2250 , cValtoChar(saldoVac(aDados[16], aDados[1], LASTDAY(CTOD("01/" + Substr(aDados[17],5,2) + "/" + Substr(aDados[17],1,4))))) ,oFont09)

	oPrint:Box(nLinB + 1050 , 800 ,nLinB + 1050, 2500)	// horizontal line

	oPrint:Say(nLinB + 1100, 850 ,"Observaci๓n: " ,oFont09N)
	cObs:= MSMM(aDados[11])	//Obtiene la descripci๓n del campo memo
	if(Empty(cObs))
		cObs:= MSMM(aDados[11],,,,,,,"RDY")
	EndIf
...
cMat -> Matricula del empleado
*/
static function saldoVac(cSuc, cMat, dRef)
	Local aArea:= getArea()
	Local nRet:= 0
	Local OrdenConsul	:= GetNextAlias()
	// consulta a la base de datos
	// 						RF_DFERVAT GANADOS				RF_DFERAAT DOUDECIMAS		RF_DFERANT UTILIZADOS
	BeginSql Alias OrdenConsul
		SELECT (COALESCE(SUM(RF_DFERVAT),0) + COALESCE(SUM(RF_DFERAAT),0) - COALESCE(SUM(RF_DFERANT),0) -
		(SELECT COALESCE(SUM(R8_DURACAO),0) R8_DURACAO FROM %table:SR8% SR8
		WHERE SR8.R8_FILIAL = %exp:cSuc% AND SR8.R8_MAT = %exp:cMat% AND SR8.R8_STATUS = ' ' AND SR8.R8_DATAFIM <= %exp:dRef% AND SR8.R8_PD = %exp:cPdVac% AND SR8.%notdel%)
		) SALDO
		FROM %table:SRF% SRF
