...
aDetalle:= R680ImpR4() ///registros a cargar
...
Static Function R680ImpR4()
	...
	Local cPerBusq	:= ""	//Periodos de b˙squeda
	Local aRet		:= {}
	...
	EndIf
	///el primer union es para prima, el segundo tambien pero cuando la prima este en otro periodo
	...
	While (cAliasQry)->(!Eof())
		//valores basicos para enviar a las listas de contratados y despedidos
		...
		///hace diferentes calculos si fue despedido
		(cAliasQry)->(DbSkip())
		If(cCondicao <> (cAliasQry)->RA_FILIAL + (cAliasQry)->RA_MAT)

			If(lDespedido)
				aDesp:= valoresDespedidos(cMat, STOD(cAdmissa), STOD(cDemissa),cPerFin,cCodFunc)
				If(len(aDesp) > 0)
					AADD(aRet, aDesp[1])
				EndIf
			Else ///array de personas que continuan trabajando
				
				aDesp:= valoresContratado(cMat, STOD(cAdmissa), STOD(cDemissa),cPerFin,cCodFunc)
				If(len(aDesp) > 0)
					AADD(aRet, aDesp[1])
				EndIf
			EndIf
...
* @author: Denar Terrazas Parada
* @since: 09/09/2020
* @description: FunciÛn que devuelve los periodos de los 3 meses que fueron tomados para el c·lculo de las Primas
*/
...
* @author: Denar Terrazas Parada
* @since: 03/12/2020
* @description: FunciÛn para obtener los valores de los empleados que fueron despedidos antes del periodo de la prima
*/
static function valoresDespedidos(cMat, dAdmissa, dDemissa,cxPerfin,cxCodFun)
	Local aArea			:= getArea()
	Local aRet			:= {}
	Local OrdenConsul	:= GetNextAlias()
	...
	Local cPeriodos		:= ""
	Local cPerFin		:= ""
	Local nFactor 		:= 0 ///total real de meses trabajados
	Local cCondicao := ""
	Local cPrCodFol	:= PRCODFOL
	Local cSFCodFol	:= SFCODFOL
	Local aVerba	:= Array(8,2)
	...
	dtPerFin := stod(cxPerfin+"31")

	cxPerIni := cvaltochar(YEAR(dtPerFin))+"0101"   ///fin de gestion anio lo colocamos al inicio de gestion anio
	////verificamos la gestion
	if stod(cxPerIni) > dAdmissa   //verificamos si entro antes que la gestion o en la gestion
		dAdmissa := stod(cxPerIni)
	endif

	if dDemissa > dtPerFin  ///se retiro en la gestion o despues si es despues tomamos el fin de gestion nomas
		nFactor = u_getMesCom(dAdmissa, dtPerFin)
		dDemissa = dtPerFin
	else
		nFactor = u_getMesCom(dAdmissa, dDemissa)  ///si lo despidieron dentro de la gestion
	endif

	if nFactor > 3
		nFactor = 3
	endif
	///acaba factor por gestion

	nNroMeses := DATEDIFFMONTH( dAdmissa , dDemissa ) + 1  ///meses para buscar en los periodos
	...
	IF ( nNroMeses < 4 ) ///tiene menos de 3 meses

		CMANOBRA:= GETADVFVAL("SRJ", "RJ_MAOBRA", XFILIAL("SRJ") + cxCodFun, 1, "")
		...
		EndIF*/
	EndIF
	///SI ES UN MES PERO ES OBRERO

	cPeriodos:= getPeriodosPrima(cPerFin)  ///TRAE LOS PERIODOS ULTIMOS A LA SALIDA
	...
	While (OrdenConsul)->(!Eof())
		...
		Case (OrdenConsul)->RV_INFSAL == OPCOTRB	//Otros bonos
			...
			nTotalGa	+= (OrdenConsul)->RC_VALOR
		Case (OrdenConsul)->RV_INFSAL == OPCREMUN .and. (OrdenConsul)->RC_HORAS == 30	//Haber b·sico
		...
		If(cCondicao <> (OrdenConsul)->RA_FILIAL + (OrdenConsul)->RA_MAT)
			...
			If(aVerba[8,1] > 0)//SÛlo se imprime si el empleado tiene el concepto de Primas
				aCols := ARRAY(NDATOS)
				...
				aCols[NMESESTRAB ]:= ( aVerba[8,2] / nFactor)
...
* @description: FunciÛn para obtener los valores de los empleados que fueron despedidos antes del periodo de la prima
*/
static function valoresContratado(cMat, dAdmissa, dDemissa,cxPerfin,cxCodFun)
	Local aArea			:= getArea()
	Local aRet			:= {}
	Local OrdenConsul	:= GetNextAlias()
	Local nNroMeses		:= 0 //Meses trabajados
