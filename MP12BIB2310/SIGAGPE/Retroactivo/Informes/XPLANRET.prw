...
#DEFINE HBCODFIN	"0048"//Haber básico Finiquito
...
±±³Fun‡…o    ³ XPLANRET ³ Autor ³ Denar Terrazas							³±±
...
User Function XPLANRET()
	Local oReport
	Private cPerg   := "XPLANRET"   // elija el Nombre de la pregunta
	Private aTabS011:= {}
...
Static Function ReportDef()
	...
	oReport	 := TReport():New("XPLANRET",NombreProg,cPerg,{|oReport| PrintReport(oReport)},"Planilla Retroactivo")
...
Static Function PrintReport(oReport)
	...
	Local cCodFols	:= "'"+HBCODFOL+"','"+HBCODFIN+"','"+BACODFOL+"','"+AFPVEJEZ+"','"+AFPRIESGO+"','"+AFPCOMIS05+"','"+AFPSOLID05+"','"+CODRCIVA+"'"
	Local cPDLiqRet	:= PDLIQRET//Liquido pagable retroactivo
	Local cPDHabBas	:= FGETCODFOL(HBCODFOL)
	Local cPDHabFin	:= FGETCODFOL(HBCODFIN)
	...
	#IFDEF TOP
		...
		While (cAliasQry)->(! EOF())
			...
			if( ((cAliasQry)->RV_CODFOL == HBCODFOL .OR. (cAliasQry)->RV_CODFOL == HBCODFIN) .AND. (cAliasQry)->RC_ROTEIR = CROTENE )//Haber básico ENERO
				nHBEne	:= (cAliasQry)->RC_VALOR
			EndIf
			if( ((cAliasQry)->RV_CODFOL == HBCODFOL .OR. (cAliasQry)->RV_CODFOL == HBCODFIN) .AND. (cAliasQry)->RC_ROTEIR = CROTFEB )//Haber básico FEBRERO
				nHBFeb	:= (cAliasQry)->RC_VALOR
			EndIf
			if( ((cAliasQry)->RV_CODFOL == HBCODFOL .OR. (cAliasQry)->RV_CODFOL == HBCODFIN) .AND. (cAliasQry)->RC_ROTEIR = CROTMAR )//Haber básico MARZO
				nHBMar	:= (cAliasQry)->RC_VALOR
			EndIf
			if( ((cAliasQry)->RV_CODFOL == HBCODFOL .OR. (cAliasQry)->RV_CODFOL == HBCODFIN) .AND. (cAliasQry)->RC_ROTEIR = CROTABR )//Haber básico ABRIL
			...
			IF((cAliasQry)->RV_INFSAL == OPCHEYN)//Horas Extras
			...
			If(cValida <> (cAliasQry)->RA_FILIAL + (cAliasQry)->RA_MAT)
				nTotalRET	:= nHBEne + nHBFeb + nHBMar + nHBAbr + nSumaBA + nSumaHE
				nTotalDESC	:= nAFP + nRCIVA + nOtrDesc

				cFilialPer	:= U_getFilialPeriodoRet(cMat,cPerDic)//obtiene la sucursal del empleado en el periodo enviado.
