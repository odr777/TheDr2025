...
±±³Fun‡…o    ³ XPLANRE3 ³ Autor ³ Denar Terrazas							³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ Planilla de Retroactivo									   	³±±
±±³          ³ Se agregaron las columnas de Dominical y Recargo Nocturno	³±±
±±³          ³ Filtra solo un proceso de retroactivo						³±±
±±³          ³ Planilla para la caja de salud		31/08/2023				³±±
...
User Function XPLANRE3()
	Local oReport
	Private cPerg   := "XPLANRE3"   // elija el Nombre de la pregunta
	Private aTabS011:= {}
...
Static Function ReportDef()
	...
	oReport	 := TReport():New("XPLANRE3",NombreProg,cPerg,{|oReport| PrintReport(oReport)},"Planilla Retroactivo Mensual")
	...
	TRCell():New(oSection,"RA_SEGUROS"	,"SRA","MATRÍCULA CAJA")	
	...
	TRCell():New(oSection,"BADICIEM"	,"SRC","Bono Antigüedad Diciembre")
	TRCell():New(oSection,"PORCBA"		,"SRC","Porcentaje Bono Antigüedad")
	TRCell():New(oSection,"NUEVOBA"		,"SRC","Bono Antigüedad con Incremento")
	TRCell():New(oSection,"REINTHB"		,"SRC","Reintegro Haber Básico")
	TRCell():New(oSection,"REINTBA"		,"SRC","Reintegro Antigüedad")
...
Static Function PrintReport(oReport)
	...
	#IFDEF TOP
		...
		cQuery+= " SELECT SRA.RA_FILIAL, SRA.RA_MAT, SRA.RA_RG, SRA.RA_ALFANUM, RA_UFCI, SRA.RA_SEGUROS, SRA.RA_CC, SRA.RA_PRISOBR, SRA.RA_SECSOBR, SRA.RA_PRINOME, SRA.RA_SECNOME, SRA.RA_NACIONA, SRA.RA_NASC, SRA.RA_SEXO, SRA.RA_CODFUNC, SRA.RA_ADMISSA, SRA.RA_DEMISSA,"
		...
		cQuery+= " AND SRC.RC_ROTEIR = '" + cProcedi + "'"
		cQuery+= cFiltro
		...
		cQuery+= " AND SRD.RD_ROTEIR = '" + cProcedi + "'"
		cQuery+= cFiltro
		cQuery+= " AND SRA.D_E_L_E_T_ <> '*'"
		...
		While (cAliasQry)->(! EOF())
			...
			cCi			:= TRIM((cAliasQry)->RA_RG) + "-" + TRIM((cAliasQry)->RA_ALFANUM) + " " + TRIM((cAliasQry)->RA_UFCI)
			// cCiComp		:= TRIM((cAliasQry)->RA_ALFANUM)
			// cLugarExp	:= TRIM((cAliasQry)->RA_UFCI)
			cMatCaja	:= (cAliasQry)->RA_SEGUROS
			...
			//nNuevoHB:= (cAliasQry)->RA_SALARIO + if(EMPTY(cDemissa),0,nSumaRHB)
			...
			If(cValida <> (cAliasQry)->RA_FILIAL + (cAliasQry)->RA_MAT)
				...
				oSection:Cell("RA_SEGUROS"):SetValue( cMatCaja )
				...
				oSection:Cell("BADICIEM"):SetValue( nBADic )
				oSection:Cell("PORCBA"):SetValue( nPorcBA )
				oSection:Cell("NUEVOBA"):SetValue( nNuevoBA )

				oSection:Cell("REINTHB"):SetValue( nSumaRHB )
				oSection:Cell("REINTBA"):SetValue( nSumaRBA )				
				oSection:Cell("HORASEXT"):SetValue( nSumaHE )
				oSection:Cell("DOMINICAL"):SetValue( nSumaDOM + nSumaHN )
...
Static Function CriaSX1(cPerg)  // Crear Preguntas
	...
	*/
	...
	xPutSX1(cPerg,"13","¿Procedimiento?"	, "¿Procedimiento?"	,"¿Procedimiento?"	,"MV_CHD","C",03,0,0,"G","","SRY","","","MV_PAR13",""       ,""            ,""        ,""     ,""   ,"")
