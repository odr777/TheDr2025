...
±±³Fun‡…o	 ³ CtbcR070	³ Autor ³ Nahim Terrazas		³ Data ³ 14.09.21 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ Asiento contable / Comprobante contable  TdeP estándar     ³±±
...
/*/
	...
	±±³Omar Delgadillo	³12/01/25	³Ajustes en formato de glosa³ 			  ³±±
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function UCTBR070R4(nOrigen)
	...
	IF ValType( oReport ) == "O"
		if funname() == "CTBA102" .or. funname() == "MATA101N"
			oReport :PrintDialog()
		else
			// oReport :Print()
			oReport:print(.F.)
			// FOPEN(oReport:cfile,64)
			// shellExecute( "Open", oReport:cfile+".pdf", " /k dir", "C:\", 1 )
		end
	ENDIF
Return
/*/
...
Static Function ReportDef()
	...
	If cPaisLoc == "BOL"
		...
		oReport:cFile := cReport
  		// oReport:setDevice(6) // pdf
  		oReport:nDevice := IMP_PDF // pdf
		// oReport:lviewpdf := .T.
		// oReport:LCLRBACK := .T.
		// oReport:LCLRFORE :=.T.
		// oReport:LCUSTOMPAR:=.F.
		// oReport:LDISABLEORIENTATION :=.F.
		// oReport:LDYNAMIC := .F.
		// oReport:LECMFLDGROUP:= .F.
		// oReport:LUSERFILTER := .F.
	Else
		cReport := cReport + cValToChar(Randomize( 100, 999 ))
	...
	TRCell():New( oSection1, "CFISCAL"   , ,"F/NF"/*Titulo*/,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*CodeBlock*/)
	TRCell():New( oSection1, "CCORREL"   , ,"CORRELATIVO"/*Titulo*/,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*CodeBlock*/)
	...
	Else
		...
		TRCell():New( oSection11, "CCONTAD"		, ,STR0049+" "+STR0010,/*Picture*/,aTamConta[1]/*Tamanho*/,/*lPixel*/,/*CodeBlock*/)
		TRCell():New( oSection11, "CCONTAC"		, ,STR0049+" "+STR0011,/*Picture*/,aTamConta[1]/*Tamanho*/,/*lPixel*/,/*CodeBlock*/)
		TRCell():New( oSection11, "CNCONTA"		, ,"NOMBRE DE CUENTA",/*Picture*/,-10/*Tamanho*/,/*lPixel*/,/*CodeBlock*/)	//Nombre de cuenta
		TRCell():New( oSection11, "CCUSTO" 		, ,cSayCC/*Titulo*/,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*CodeBlock*/)
		TRCell():New( oSection11, "CITEM" 		, ,cSayItem/*Titulo*/,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*CodeBlock*/)
		TRCell():New( oSection11, "CCLASSE"		, ,cSayCV/*Titulo*/,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*CodeBlock*/)
		TRCell():New( oSection11, "CDEBITO" 	, ,STR0050+" "+STR0010,cPictVal/*Picture*/,18/*Tamanho*/,/*lPixel*/,/*CodeBlock*/,"RIGHT",,"RIGHT")
...
Static Function ReportPrint( oReport )
	...
	Local cQuery		:= "CT2"
	Local n				:= 0
	Local cLote			:= IIF(funname() == "CTBA102".or. funname() == "MATA101N","",M->CLOTE)
	Local cSubLote		:= IIF(funname() == "CTBA102".or. funname() == "MATA101N","",M->cSubLote)
	Local cDoc      	:= IIF(funname() == "CTBA102".or. funname() == "MATA101N","",M->CDOC)	
	Local cLoteRes      := ""
	Local cSubRes		:= ""
	Local cDocRes    	:= ""
	Local nTotalDeb	 	:= 0
...
Static Function RPrintBol( oReport )
	...
	Local cPdobr	 	:= "3"
	Local nStr		 	:= 0
	local cIndiceSf1 := "" // Nahim vacio 23/06/2021
	Local i
	Private cSequencial    := ""
	Private cTipoCambio   := ""
	Private cTipoSus := ""
	...
	Else
		#IFDEF TOP
			If TcSrvType() <> "AS/400"
				...
				// cQuery := "SELECT CASE	WHEN CT2_DC = 4 THEN 4 ELSE 0 END ORDEN," // Nahim  2019-09-26
				cQuery := "SELECT " // Nahim  2019-09-26
				cQuery += "   	CASE"
				cQuery += "		WHEN CT2_DC = 4 "
				cQuery += "			THEN (SELECT MIN(CT2_DC) FROM CT2010 WHERE CT2_FILIAL + CT2_DATA + CT2_LOTE+CT2_SBLOTE +CT2_DOC +CT2_SEQLAN = CT21.CT2_FILIAL + CT21.CT2_DATA + CT21.CT2_LOTE+CT21.CT2_SBLOTE +CT21.CT2_DOC +CT21.CT2_SEQLAN AND CT2_DC <> 4 ) "
				cQuery += "		ELSE CT2_DC "
				cQuery += "	END "
				cQuery += "	ORDEN, "
				...
				cQuery += "CT2_NODIA, "
				cQuery += "HISTORIAL, "
				cQuery += "SUM(LEN(HISTORIAL) + LEN(isnull(CTH_DESC01,'')) + LEN(isnull(CTT_DESC01,'')) +LEN(isnull(CTD_DESC01,'')) +  LEN(isnull(CV0_DESC,''))) OVER()  TOTHOJAS,"
				cQuery += "CT21.*,CT22.*,isnull(CTH_DESC01,'') CTH_DESC01,isnull(CTT_DESC01,'') CTT_DESC01 ,isnull(CTD_DESC01,'') CTD_DESC01,isnull(CV0_DESC,'') CV0_DESC "
				cQuery += " FROM " + RetSqlName("CT2") + " CT21 "
				
				cQuery += "LEFT JOIN "
				cQuery += "("
				cQuery += " SELECT CT2_VALOR VAL,CT2_FILIAL FILI, CT2_DATA DATA, CT2_LOTE LOTE, CT2_SBLOTE SBLOTE , CT2_DOC DOCUM, CT2_MOEDLC MONEDA, CT2_DC DC,CT2_TPSALD TPSAL,CT2_LINHA LINEA"
				cQuery += " from  "  + RetSqlName("CT2")  +" "
				cQuery += "WHERE CT2_FILIAL  = '" + xFilial("CT2") + "' AND "
				cQuery +=       "CT2_DATA   >= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",DTOS(mv_par01),DTOS(M->DDATALANC))  + "' AND "
				cQuery += 	    "CT2_DATA   <= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",DTOS(mv_par02),DTOS(M->DDATALANC))  + "' AND "
				cQuery += 	    "CT2_LOTE   >= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",mv_par03,M->CLOTE)  + "' AND "
				cQuery += 	    "CT2_LOTE   <= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",mv_par04,M->CLOTE)  + "' AND "
				cQuery += 	    "CT2_SBLOTE >= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",mv_par15,M->cSubLote) + "' AND "
				cQuery += 		"CT2_SBLOTE <= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",mv_par16,M->cSubLote) + "' AND "
				cQuery +=   	"CT2_DOC    >= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",mv_par05,M->CDOC) + "' AND "
				cQuery +=       "D_E_L_E_T_ LIKE '' AND "
				cQuery +=	   	"CT2_MOEDLC = '02'                      AND "
				cQuery += 		"CT2_DOC    <= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",mv_par06,M->CDOC) + "' ) CT22 On "				
				...
				cQuery += "   CT21.CT2_LINHA     =  CT22.LINEA     	"




				// Nahim 29/04/2021 adicionando historial completo
				cQuery += "LEFT JOIN "
				cQuery += "("
				cQuery += " SELECT CT2_SEQLAN LANZ,CT2_DATA DATA,CT2_LOTE LOTE,CT2_SBLOTE SBLOTE,CT2_DOC DOCUM,STRING_AGG( TRIM(CT2_HIST),null) HISTORIAL"
				cQuery += " from  "  + RetSqlName("CT2")  +" "
				cQuery += "WHERE CT2_FILIAL  = '" + xFilial("CT2") + "' AND "
				cQuery +=       "CT2_DATA   >= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",DTOS(mv_par01),DTOS(M->DDATALANC))  + "' AND "
				cQuery += 	    "CT2_DATA   <= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",DTOS(mv_par02),DTOS(M->DDATALANC))  + "' AND "
				...
				cQuery += 		"CT2_SBLOTE <= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",mv_par16,M->cSubLote) + "' AND "
				cQuery +=   	"CT2_DOC    >= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",mv_par05,M->CDOC) + "' AND "
				cQuery +=   	"CT2_DOC    <= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",mv_par06,M->CDOC) + "' AND "
				cQuery +=       "D_E_L_E_T_ LIKE '' AND "
				cQuery +=	   	"CT2_MOEDLC = '01'   "
				cQuery += 		"GROUP BY CT2_DATA,CT2_LOTE,CT2_SBLOTE,CT2_DOC,CT2_SEQLAN,CT2_NODIA ) CT23 On "
				cQuery += "   CT21.CT2_DATA   =  CT23.DATA   and   	"
				cQuery += "   CT21.CT2_LOTE   =  CT23.LOTE   and   	"
				cQuery += "   CT21.CT2_SBLOTE =  CT23.SBLOTE and   	"
				cQuery += "   CT21.CT2_DOC    =  CT23.DOCUM    and 	"
				cQuery += "   CT21.CT2_SEQLAN    =  CT23.LANZ     	"
				...
				// Nahim 22/03/2021 Adicionando impresión item cuenta
				cQuery += "  LEFT JOIN "  + RetSqlName("CTD")  +" CTD ON     	"
				cQuery += "  (CTD.CTD_ITEM = CT21.CT2_ITEMC OR CTD.CTD_ITEM = CT21.CT2_ITEMD )  	"
				...
				cQuery +=   	"CT2_DOC    >= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",mv_par05,M->CDOC) + "' AND "
				cQuery +=   	"CT2_DOC    <= '" + IIF(funname() == "CTBA102".or. funname() == "MATA101N",mv_par06,M->CDOC) + "' AND "
				cQuery += 		"CT2_MOEDLC = '"  + IIF(funname() == "CTBA102".or. funname() == "MATA101N",mv_par07,'01') + "' AND "
...
//			cQuery += "ORDER BY 1,CT2_FILIAL, CT2_DATA, CT2_LOTE, CT2_SBLOTE, CT2_DOC, CT2_LINHA" Nahim 2019/08/14
			// cQuery += "ORDER BY 1 DESC,3 DESC, CT2_VALOR desc,CT2_FILIAL,CT2_DATA,CT2_LOTE,CT2_SBLOTE,CT2_DOC" // Nahim 2019/09/26
			// cQuery += "ORDER BY 1 DESC,3 DESC, CT2_VALOR desc,CT2_FILIAL,CT2_DATA,CT2_LOTE,CT2_SBLOTE,CT2_DOC" // Nahim 27/04/2021
				cQuery += 		"    ORDER BY        "
				cQuery += 		"    1,              "
				cQuery += 		"    CT2_VALOR desc, "
				cQuery += 		"    4,              "
				cQuery += 		"    5,              "
				cQuery += 		"    CT2_FILIAL,     "
				cQuery += 		"    CT2_DATA,       "
		...
		//³ Define bloco de codigo com as variaveis para impressao das celulas de totais  ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		clancPadr :=(cAliasCT2)->CT2_LP // Nahim 16/03/2021
		nContInd = 1// Nahim 18/05/2020 sólo para que tome el valor del segundo item
		While !Eof() .And. (cAliasCT2)->CT2_FILIAL == xFilial("CT2") .And. (cAliasCT2)->CT2_DATA <= IIF(funname() == "CTBA102".or. funname() == "MATA101N",mv_par02,M->DDATALANC) .And. !oReport:Cancel()
			lCabecera := .T.
			...
			cAglutinado := (cAliasCT2)->CT2_AGLUT // Nahim Verifiica si está o no aglutinado
			cTextEmp	:= alltrim(SM0->M0_NOMECOM) + " - " + alltrim(SM0->M0_FILIAL)
			cTextFecha  := "FECHA: " + AllTrim(Str(nDia)) + " de " + cMes + " del "+AllTrim(Str(nAnio))
			oReport:IncMeter()
			If oReport:Cancel()
				Exit
			EndIf
			...
			While (cAliasCT2)->(!Eof()) .And. (cAliasCT2)->CT2_FILIAL == xFilial("CT2") 	.And.;
				...
				If (lCabecera)
					...
					*/
				EndIf
				if empty(clancPadr)	 // Nahim buscando
					// alert("busqueda")
					clancPadr :=(cAliasCT2)->CT2_LP // Nahim 18/07/2021
				endif
				if !empty((cAliasCT2)->CT2_KEY)
					cIndiceSf1 :=(cAliasCT2)->CT2_KEY // Nahim 18/05/2020
					//					ALERT(cIndiceSf1)
				endif
				nContInd++
...
//				cGlosa := (cAliasCT2)->CT2_HIST NTP 20190805
				cGlosa :=  (cAliasCT2)->HISTORIAL
				// verifica concepto completo. //
				// aArQuery := (cAliasCT2)->(GetArea())
				// cSeqLanz := (cAlTiasCT2)->CT2_SEQLAN
				// nCanSeqLi := 0
				// while (cAliasCT2)->CT2_SEQLAN == cSeqLanz .and. (cAliasCT2)->(!Eof())
				...
				// if 
				// dbSkip(-nCanSeqLi)
				// while (cAliasCT2)->(DbSkip())
				
				cGlosa +=  iif(!empty((cAliasCT2)->CTT_DESC01)," - CC: " + (cAliasCT2)->CTT_DESC01 ,"")
				cGlosa +=  iif(!empty((cAliasCT2)->CTD_DESC01)," - IC: " + (cAliasCT2)->CTD_DESC01 ,"")
				cGlosa +=  iif(!empty((cAliasCT2)->CTH_DESC01)," - CV: " + (cAliasCT2)->CTH_DESC01 ,"")
				cGlosa +=  iif(!empty((cAliasCT2)->CV0_DESC)," - " + (cAliasCT2)->CV0_DESC ,"")

				/*if alltrim((cAliasCT2)->CT2_ROTINA) $ "FINA085" ///SI ES UNA OP
					cEkGlos := Posicione("SEK",1,xFilial("SEK")+SUBSTR((cAliasCT2)->CT2_HIST,11,6),"EK_UGLOSA")  ///
			...
			If (cAliasCT2)->CT2_DC = "1"
					...
					cADetalle 	:= Posicione("CT1",1,xFilial("CT1")+(cAliasCT2)->CT2_DEBITO,'CT1_DESC01')
					cATexto 	:= "Glosa: " + cGlosa
				...
				nPrimerLinea := .T.				
				oReport:PrintText(cADetalle,oReport:Row(),	330,CLR_BLACK)
				oReport:SkipLine(1)
				oReport:PrintText(substr(cATexto, 1,nAnchoDet),oReport:Row(),	430,CLR_BLACK)
				if nPrimerLinea
				...
				oReport:SkipLine(1)
				if len(cATexto) > nAnchoDet
					oReport:PrintText(substr(cATexto, nAnchoDet+1, len(cATexto)-nAnchoDet),oReport:Row(),	430,CLR_BLACK)
					oReport:SkipLine(1)
				endif
				// odr lo de arriba nuevo
			...
			ElseIf (cAliasCT2)->CT2_DC = "2"
				...
				// ArrayTexto :=  CabToArray(cGlosa,35) // toma el array
				cADetalle 	:= Posicione("CT1",1,xFilial("CT1")+(cAliasCT2)->CT2_CREDITO,'CT1_DESC01')
				cATexto 	:= "Glosa: " + cGlosa
				...
				if nPrimerLinea
					...
					nTotLotCrd+= (cAliasCT2)->CT2_VALOR
					//(cAliasCT2)->( DbSkip() )
			ElseIf (cAliasCT2)->CT2_DC = "3"  ////PARTIDA DOBLE AJUSTE ERICK
				For n := 1 TO 2
						dbSelectArea( cAliasCT2 )
					If mv_par08 == 2
							dbSelectArea("CT1")
...
//				RpPrintHist( cAliasCT2 , oSection11 )
				...
				//(cAliasCT2)->( DbSkip() )
				(cAliasCT2)->( DbSkip() )
			If (nLinea >= 2200 .AND. (cAliasCT2)->( !EOF() )) // Nahim 28/06/2021 sólamente si es diferente de vacio
					//impresion del encabezado
					oReport:EndPage()
					oReport:StartPage()
					//					alert("Pasa 2: " + cvaltochar(oReport:Row()))
					lCabecera := .T.
					fCabec() // Nahim imprime cuadro y cabecera
					impCuadro() // Nahim imprime cuadro y cabecera
			EndIf
		Enddo
			oSection11:Finish()
		If lTotal
				...
				nVM3 := xMoeda(nTotalDeb,1,2,dFecha,5)
				nVM4 := xMoeda(nTotalCrd,1,2,dFecha,5)
				cTotalStrM3	:= Extenso(ROUND(nVM3,2),lQuantid,nMoeda,,cIdioma,lCent,lFrac)
				...
				// ODR 01/06/2022 Quitar y confirmar ver si es necesario
				/*
				if clancPadr $ "650|655" .and. !empty(cIndiceSf1) .and. cAglutinado <> '2'
					aAreSF1 := SF1->(getArea())
					cIndSF1 :=  SUBSTR(cIndiceSf1,1,33) //
					dbSelectArea("SF1")
					dbSetOrder(1)
					If dbSeek(cIndSF1)
						if ALLTRIM(SF1->F1_ESPECIE) $ "NF|NDP|NCC" //.and. SF1->F1_VALIMP1 > 0
							cTesSD1 := GetAdvFVal("SD1","D1_TES",SF1->F1_FILIAL+SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE+SF1->F1_LOJA,1,"")
							// F4_FILIAL+F4_CODIGO
//							cGenLibF := GetAdvFVal("SF4","F4_GERALF",XFILIAL("SF4") + cTesSD1,1,"2") // Nahim comentando porque igual puede imprimir las que no estén en la SF3
							cReten := GetAdvFVal("SF4"  ,"F4_XRETEN",XFILIAL("SF4") + cTesSD1,1,"") // NO
							// if cGenLibF == '1' // sólo si es factura y genera libros fiscales.
							if !cReten $ '2|3' // Nahim 10/08/2021
								if empty(SF1->F1_UNOMBRE)
									cNomFor :=  GetAdvFVal("SA2","A2_NOME"	,XFilial("SA2")+IF(EMPTY(SF1->F1_COMPANH),SF1->F1_FORNECE+ SF1->F1_LOJA,SF1->F1_COMPANH + SF1->F1_LOJCOMP) ,1,"")
									cNitProv :=  GetAdvFVal("SA2","A2_CGC"	,XFilial("SA2")+IF(EMPTY(SF1->F1_COMPANH),SF1->F1_FORNECE+ SF1->F1_LOJA,SF1->F1_COMPANH + SF1->F1_LOJCOMP) ,1,"")
								else
									cNomFor :=  SF1->F1_UNOMBRE
									cNitProv :=  SF1->F1_UNIT
								endif
									cInfo := ALLTRIM(cNitProv) + " - " + ALLTRIM(cNomFor) + " | Nro. Factura: "+ alltrim(SF1->F1_DOC) +" , "+ DTOC(SF1->F1_EMISSAO) + " - " + CVALTOCHAR(SF1->F1_VALBRUT)
									
								if !empty(SF1->F1_NUMAUT)
									cInfo +=  " | Nro. Aut.: " + alltrim(SF1->F1_NUMAUT)
								endif
								if !empty(SF1->F1_CODCTR)
									cInfo +=  " | Cod.Contr: " + alltrim(SF1->F1_CODCTR)
								endif
								oReport:SAY(cFila, 150, cInfo , oFont10T ,1000,CLR_BLACK ,CLR_BLACK )//;	oReport:SkipLine(02)
							endif
						endif
					endif
					if funname() == "CTBA102"
...
Static function fCabec()
	...
	oReport:SAY(oReport:Row(), 1000, SubTitulo, oFont12TN	,1000,CLR_BLACK ,CLR_BLACK )
	oReport:SAY(oReport:Row(), 1850, cDocumento, oFont12TN	,1000,CLR_BLACK ,CLR_BLACK )		;	oReport:SkipLine(02)
	oReport:SAY(oReport:Row(), 1850, "T.C.: " + cValToChar(cTipoSUS), oFont12TN	,1000,CLR_BLACK ,CLR_BLACK )		;	oReport:SkipLine(02)
	oReport:SAY(oReport:Row(), 1850, "Pagina: " + cValToChar(nPagina) + " de " + cvaltochar(iif(round((cAliasCT2)->TOTHOJAS/860,0) ==0,1,round((cAliasCT2)->TOTHOJAS/860,0))), oFont12TN	,1000,CLR_BLACK ,CLR_BLACK )	
	// oReport:SkipLine(3,5)
	oReport:SkipLine(3,4)
		// oReport:SAY(oReport:Row(), 210, cTextFecha, oFont13TN	,1000,CLR_BLACK ,CLR_BLACK )
	oReport:SAY(oReport:Row(), 80, cTextEmp, oFont13TN	,1000,CLR_BLACK ,CLR_BLACK )
	oReport:SkipLine(2) 
	oReport:SAY(oReport:Row(), 80, cTextFecha, oFont13TN	,1000,CLR_BLACK ,CLR_BLACK )
	...
	Endif*/
	if !empty((cAliasCT2)->HISTORIAL) //.and. (cAliasCT2)->CT2_DC == "4"
		oReport:SkipLine(2)
		// oReport:SAY(oReport:Row(), 210, "CONCEPTO: " , oFont13TN	,1000,CLR_BLACK ,CLR_BLACK )
		// oReport:SAY(oReport:Row(), 500, SUBSTRING((cAliasCT2)->HISTORIAL,1,100), oFont13TN	,1000,CLR_BLACK ,CLR_BLACK )
		cCabMostrar := "CONCEPTO: " + (cAliasCT2)->HISTORIAL
		ArrayTexto :=  CabToArray(cCabMostrar,123) // toma el array
		for nHist:=1 to len(ArrayTexto)
			if nHist == 1
				oReport:SAY(oReport:Row(), 80, ArrayTexto[NHIST] , oFont13TN	,1000,CLR_BLACK ,CLR_BLACK )
			else
				oReport:SkipLine(1.1)
				oReport:SAY(oReport:Row(), 300, alltrim(ArrayTexto[NHIST]) , oFont13TN	,1000,CLR_BLACK ,CLR_BLACK )
			endif
		next nHist
		// oReport:SAY(oReport:Row(), 80, "CONCEPTO: " , oFont13TN	,1000,CLR_BLACK ,CLR_BLACK )
		// oReport:SAY(oReport:Row(), 370, SUBSTRING((cAliasCT2)->HISTORIAL,1,100), oFont13TN	,1000,CLR_BLACK ,CLR_BLACK )
		
		// IF(LEn(ALLTRIM((cAliasCT2)->HISTORIAL)) > 100 )// Nahim adicionando validación de historial
		// 	oReport:SkipLine(1.1)
		// 	oReport:SAY(oReport:Row(), 370, SUBSTRING((cAliasCT2)->HISTORIAL,101,150) , oFont13TN	,1000,CLR_BLACK ,CLR_BLACK )
		// ENDIF
		// if alltrim((cAliasCT2)->CT2_ROTINA) $ "FINA085" ///SI ES UNA OP
		// 		cEkGlos := Posicione("SEK",1,xFilial("SEK")+SUBSTR((cAliasCT2)->CT2_HIST,11,6),"EK_UGLOSA")  ///
		// 		oReport:SkipLine(1)
		// 		oReport:SAY(oReport:Row(), 500, cEkGlos , oFont13TN	,1000,CLR_BLACK ,CLR_BLACK )
		// endif
...
Static Function impCuadro()
	nBajar := 120
	//CAJAS
	oReport:Box( nBajar+620,80,2450,2370)	//Recuadro
	oReport:Box( nBajar+620,80,nBajar+695,2370)	// Recuadro Titulos
	oReport:Box( 2330,80,2450,2370)	// Recuadro Totales
	oReport:Box( 2550,80,3000,2370)	// Recuadro Firmas

	oReport:SAY(nBajar+650, 120, "Código", oFont10TN	,1000,CLR_BLACK ,CLR_BLACK ) //Titulos
	oReport:SAY(nBajar+650, 330, "Cuenta", oFont10TN	,1000,CLR_BLACK ,CLR_BLACK ) //Titulos
	
	// odr quito título de glosa
	//oReport:SAY(650, 1030, "Glosa", oFont10TN	,1000,CLR_BLACK ,CLR_BLACK ) //Titulos

	oReport:SAY(nBajar+650, 1780, "Debe", oFont10TN	,1000,CLR_BLACK ,CLR_BLACK ) //Titulos
	oReport:SAY(nBajar+650, 2080, "Haber", oFont10TN	,1000,CLR_BLACK ,CLR_BLACK ) //Titulos
	//oReport:SAY(580, 2120, "Cheque", oFont10TN	,1000,CLR_BLACK ,CLR_BLACK ) //Titulos

	oReport:Box( nBajar+620,300,2330,300)		//Linea Vertical

	// odr quito linea de glosa
	// oReport:Box( 620,1000,2330,1000)		//Linea Vertical
	
	//		oReport:Box( 550,1100,2560,1100)	//Linea Vertical
	//		oReport:Box( 550,1750,2450,1450)	//Linea Vertical
	// oReport:Box( 620,2050,2450,1750)	//Linea Vertical
		
	oReport:Line(nBajar+620,1750,2450,1750) // Nahim	
	oReport:Line(nBajar+620,2050,2450,2050) // Nahim
	...
	oReport:SAY(3020, 2200,FWLeUserlg("CT2_USERGA"), oFont13TN	,1000,CLR_BLACK ,CLR_BLACK )
