...
Local cDescri := OemToAnsi("Genera un informe de asientos resumidos")
	...
	TRCell():New( oSection2,"CDEBE"		, "   ", "     Debe  "			, '@E 99,999,999,999,999.99'	, TamSX3("CT2_VALOR")[1]+13,/*lPixel*/,{|| (cAliasCT2)->SALDO })	// 4
	TRCell():New( oSection2,"CHABER"	, "   ", "     Haber "			, '@E 99,999,999,999,999.99'	, TamSX3("CT2_VALOR")[1]+13,/*lPixel*/,{|| (cAliasCT2)->SALDO })	// 5
	TRCell():New( oSection2,"HISTORIA"	, "   ", "Historial"			, PesqPict("CT2","CT2_HIST")	, TamSX3("CT2_HIST")[1]	-5	,/*lPixel*/,{|| (cAliasCT2)->HISTORIAL })// 6
	TRCell():New( oSection2,"SALDODIS"		, "   ", "	"				, PesqPict("CT2","CT2_VALOR")	, TamSX3("CT2_VALOR")[1]	,/*lPixel*/,{|| (cAliasCT2)->SALDO })	// 7
	TRCell():New( oSection2,"XSPAN"		, "   ", "	        "				, "@!"	, 8	,/*lPixel*/,{||  })	// 7
...
Private	aPos      := {}
	...
	aadd(xlotes,{ '008810',5} )
	aadd(xlotes,{ '008820',6} )
	aadd(xlotes,{ '008850',7} )
	aadd(xlotes,{ '008840',12} )
	aadd(xlotes,{ '008860',14} )
	aadd(xlotes,{ '999999',98} )
	aadd(xlotes,{ '000001',99} )
cSend:=AllTrim( MV_PAR09 )+";"

DO While (nAt := AT(";",cSend)) > 0
   cPart := Subs(cSend,1,nAT-1)
		If !Empty(cPart)
				aadd(XDATA,{cPart } )               
		Endif
		cSend := Subs(cSend,nAT+1)
EndDO
	...
	cNull    := "%''%"
			...
			FOR NX:=1 TO LEN(xlotes)
				cIn += "  WHEN LOTECT2 = '"+ xlotes[NX][1]+"' THEN "+alltrim(str(xlotes[NX][2])) +" "
       ...
       //fecha
	   		cdt := "% CASE WHEN LOTECT2 not in ( "
			FOR NX:=1 TO LEN(XDATA)
		...
		BeginSql Alias cAliasCT2
			...
			ORDER BY FILIALCT2, FECHA, ORDENAMIENTO , LOTECT2, SUBLOTE, DOCUMENT, SALDO, CUENTA
		EndSql
...
//		oSection2:Cell("SALDODIS"):Show()
	...
	oSection2:Init()
	oReport:ThinLine()
	oSection2:SetHeaderSection(.F.)	// No muestro la cabecera XSPAN
	//oSection2:SetColSpace(5) //dejo espacio
	oSection2:Cell("CDEBE"):Show()
	oSection2:Cell("XSPAN"):Show()
	oSection2:Cell("CHABER"):Show()
