...
Else   ////REMITO DE ENTRADA
	...
	While !eof() .and. SF1->F1_DOC <= mv_par02 .and. SF1->F1_SERIE == mv_par03 .and. lContinua
		...
		aCF      :={}                         // Classificacao quanto natureza da Operacao
		aLoliz := {}
		aEspecif := {} // Nahim Especifico 19/09/2019
		aFabric	:={} 
...
//			AADD(aLoliz       ,SD1->D1_LOCALIZ)             // Codigo Fiscal
			AADD(aEspecif       ,POSICIONE("SB1",1,XFILIAL("SB1")+SD1->D1_COD,"B1_ESPECIF"))             // DESCRIPCION ESPECIFICA
...
//  	@ nLin, 016 PSAY "CODIGO            DESCRIPCION                      UNID.        CANTIDAD        VAL UNIT.        VALOR TOTAL     LOTE         VALIDEZ       TES     PED.COMPRA  UBICACION         FABRICANTE"
  	    @ nLin, 016 PSAY "CODIGO            DESCRIPCION                      UNID.        CANTIDAD        VAL UNIT.        VALOR TOTAL     TES          PED.COMPRA    FABRICANTE          DESC ESPECIFICA"
  	//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  end
  nLin := nLin+1
@ nLin, 00 PSAY REPLICATE('-',213) 
...
//		@ nLin, 128  PSAY alltrim(aPre_Lot[_I]) // lote     NAHIM 
//		@ nLin, 140  PSAY aPre_Fel[_I]       NAHIM
//		@ nLin, 155  PSAY cvaltochar(alltrim(aTES[_I]))
		@ nLin, 128  PSAY cvaltochar(alltrim(aTES[_I])) // TES POR LOTE
		If mv_par04 == 1
//			@ nLin, 165  PSAY aPedido [_I]
			@ nLin, 140  PSAY aPedido [_I] // CAMBIANDO VALIDEZ POR PEDIDO
			//@ nLin, 165  PSAY POSICIONE('SC7',3,XFILIAL('SC7')+ SF1->F1_FORNECE + SF1->F1_LOJA+ aPedido [_I],'C7_NUMSC')
		end
//		@ nLin, 155  PSAY cvaltochar(alltrim(aTES[_I])) // fabricante
		@ nLin, 155  PSAY "aFabric[_I]"//fabricante nahim fabricante
		@ nLin, 177  PSAY aEspecif[_I] //ubicacion NAHIM
//		J:=J+1 
    ELSE
       _i:=ntamdet
	END	
