...
USER FUNCTION AFLCR001() //VALOR ACTUAL DE LINEA, VLRLIN LIMITE DE LA LINEA
	Local aCpos		    := {}
	Local aCampos		:= {}
	Local nI			:= 0
	Local cAlias		:= "SN1"
	...
	Private VLRLIN := SA6->A6_UVLRLIN     //VALOR DE LA LINEA antes SA6->A6_LIMCRED
	Private aRotina     := {}
	Private aRecLC 	:= {}
	Private cCadastro   := "Registro de Activos para linea de crédito:"+LINEA
	Private aCores := {}
	Private oDlgAF

	//SetKey( K_ALT_F4, 		{ || MsgAlert( "Tecla 'Alt + F4' foi pressionada", "UniversoADVPL" ) } )

	if  SA6->A6_BLOCKED == '2' //if SA6->A6_ULINEA == '1' .and. SA6->A6_BLOCKED == '2'

		/*aCores := { {"empty(N1_ULINEA1).and.empty(N1_ULINEA2)","BR_VERDE"} ,;
		{"((!empty(N1_ULINEA1).and.empty(N1_ULINEA2)).or.(!empty(N1_ULINEA1).and.!empty(N1_ULINEA2)) ","BR_LARANJA"} ,;
		...
		*/
		...
		AADD(aRotina,{"Añadir Activos Seleccionados","u_AddLnCr" ,0,1})
		...
		cExprFilTop := " SUBSTRING(N1_GRUPO,1,1) IN " + cGrupos //+ " AND (TRIM(N1_ULINEA1)='' OR TRIM(N1_ULINEA2)='')"

		//" AND TRIM(N1_DTCLASS) <> '' AND (TRIM(N1_ULINEA1)='' OR TRIM(N1_ULINEA2)='')"
		//cExprFilTop := " ((N1_VLRLIN1+N1_VLRLIN2) < (round(N1_VLIHIPO,2))) OR (N1_VLRCONT < (round(N1_VLHIPO,2)))  AND (N1_VLRLIN1+N1_VLRLIN2+N1_VLRCONT <N1_VLHIPO) AND N1_GRUPO IN "+cGrupos //" AND TRIM(N1_DTCLASS) <> '' AND (TRIM(N1_ULINEA1)='' OR TRIM(N1_ULINEA2)='')"
		...
		alert("Solo se puede asociar activos fijos a lineas de crédito o bancos vigentes")
	endif
...
user FUNCTION AddLnCr() //añade los activos a la linea de credito
	...
	While SN1->(!EOF())
		...
		DO CASE
			CASE cGrp = '1'
			nValAtf := round(SN1->N1_VLCOMER * round(SA6->A6_UPRHIP/100,2),2)  //Inmuebles
			CASE cGrp = '2'
			nValAtf := round(SN1->N1_VLCOMER * round(SA6->A6_UPRHIP/100,2),2)  //Inmuebles
			CASE cGrp = '4'
			nValAtf := round(SN1->N1_VLCOMER * round(SA6->A6_UPRMYV/100,2),2)  //Maquinaria y equipo
			CASE cGrp = '6'
			nValAtf := round(SN1->N1_VLCOMER * round(SA6->A6_UPOROP/100,2),2)  //vehiculos
			CASE cGrp = '9' .and. substr(SN1->N1_GRUPO,4,4) = '7'  //PIGNORADO
			nValAtf := round(SN1->N1_VLCOMER * round(SA6->A6_USPPIGN/100,2),2)
			CASE cGrp = '9' .and. substr(SN1->N1_GRUPO,4,4) = '8'  //DPFs
			nValAtf := round(SN1->N1_VLCOMER * round(SA6->A6_UHIPDPF/100,2),2)
			CASE cGrp = '9' .and. substr(SN1->N1_GRUPO,4,4) = '9'  //Y asfaltos
			nValAtf := round(SN1->N1_VLCOMER * round(SA6->A6_UASFALT/100,2),2)
			OTHERWISE
			nValAtf := 0
		ENDCASE
	...
	If Len(aRecLC) > 0 .AND. MsgYesNo( "Confirma asociar a la linea de crédito, los Activos selecionados? ")
		cContrato := PADR(' ',12)
		For nX := 1 to Len(aRecLC)
			...
			Define MsDialog oDlgAF Title "Ingrese Valor del activo: "+SN1->N1_CBASE+" para la Linea: " + LINEA From 0,0 To 280, 500 Of oMainWnd Pixel OF oDlgAF PIXEL
			...
			if empty(SN1->N1_ULINEA1).and.(nRadio == 1.or.nRadio == 2)
				SN1->N1_ULINEA1 := LINEA
				SN1->N1_VLRLIN1 := nValAtf  // SN1->N1_UVLHIPO == valorcomercial* porcentaje(bco x tipo bien)
				SN1->N1_UDOCCON := cContrato
				SN1->N1_UVLHIPO := nValAtf
				AADD(aAFtoLC,{SN1->N1_CBASE,"LINEA1",cValtoChar(nValAtf),dtos(DDATABASE),SN1->N1_DESCRIC,SN1->N1_GRUPO})
			elseif empty(SN1->N1_ULINEA2).and.(nRadio == 1.or.nRadio == 2)
				SN1->N1_ULINEA2 := LINEA
				SN1->N1_VLRLIN2 := nValAtf  // SN1->N1_UVLHIPO == valorcomercial* porcentaje(bco x tipo bien)
				SN1->N1_UDOCCON := cContrato
				SN1->N1_UVLHIPO := nValAtf
				AADD(aAFtoLC,{SN1->N1_CBASE,"LINEA2",cValtoChar(nValAtf),dtos(DDATABASE),SN1->N1_DESCRIC,SN1->N1_GRUPO})
			else
				AADD(aErr,{SN1->N1_CBASE,SN1->N1_ULINEA1+":"+SN1->N1_ULINEA2,cValtoChar(SN1->N1_VLRCONT),SN1->N1_DESCRIC,SN1->N1_GRUPO})
			endif
			MsUnLock()
		Next nX
		LimpaMarca()
		...
		IF Len(aErr) > 0
			alert("Items no procesados/mas de dos lineas")
...
static Function TRadio(nRadio, oSayTSal,oValAtf,nValAtf)
	...
	DO CASE
		CASE cGrp = '1'
		nValAtf := round(SN1->N1_VLCOMER * round(SA6->A6_UPRHIP/100,2),2)  //Inmuebles
		nPorcentaje := round(SA6->A6_UPRHIP/100,2)
		CASE cGrp = '2'
		nValAtf := round(SN1->N1_VLCOMER * round(SA6->A6_UPRHIP/100,2),2)  //Inmuebles
		nPorcentaje:=round(SA6->A6_UPRHIP/100,2)
		CASE cGrp = '4'
		nValAtf := round(SN1->N1_VLCOMER * round(SA6->A6_UPRMYV/100,2),2)  //Maquinaria y equipo
		nPorcentaje := round(SA6->A6_UPRMYV/100,2)
		CASE cGrp = '6'
		nValAtf := round(SN1->N1_VLCOMER * round(SA6->A6_UPOROP/100,2),2)  //vehiculos
		nPorcentaje := round(SA6->A6_UPOROP/100,2)
		CASE cGrp = '9' .and. substr(SN1->N1_GRUPO,4,4) = '7'  //PIGNORADO
		nValAtf := round(SN1->N1_VLCOMER * round(SA6->A6_USPPIGN/100,2),2)
		CASE cGrp = '9' .and. substr(SN1->N1_GRUPO,4,4) = '8'  //DPFs
		nValAtf := round(SN1->N1_VLCOMER * round(SA6->A6_UHIPDPF/100,2),2)
		nPorcentaje := round(SA6->A6_UHIPDPF/100,2)
		CASE cGrp = '9' .and. substr(SN1->N1_GRUPO,4,4) = '9'  //ASFALTOS
		nValAtf := round(SN1->N1_VLCOMER * round(SA6->A6_UASFALT/100,2),2)
		nPorcentaje := round(SA6->A6_UASFALT/100,2)
		OTHERWISE
		nValAtf := 0
		nPorcentaje := 1
	ENDCASE
...
static function AddGar(nValact,cGrupo)
	...
	DO CASE
		...
		CASE cGrupo = '1'
		Reclock("SA6",.F.)
		A6_USINMU +=nValact  //Inmuebles
		MsUnlock()
		CASE cGrupo = '2'
		Reclock("SA6",.F.)
		A6_USIMMU +=nValact  //Inmuebles
		MsUnlock()
		CASE cGrupo = '4'
		Reclock("SA6",.F.)
		A6_USMYV +=nValact  //Maquinaria y equipo
		MsUnlock()
		CASE cGrupo = '6'
		Reclock("SA6",.F.)
		A6_USVEI +=nValact  //vehiculos
		MsUnlock()
		CASE cGrupo = '9' .and. cGaspdf = '7' //PIGNORADO
		Reclock("SA6",.F.)
		A6_USPIGNO +=nValact
