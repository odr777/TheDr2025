...
//		nContador++	//edson. cuenta los item de la factura 		
		...
		if !EMPTY(aDetalle[nI][8])
			cDetdesc+= " - "+ alltrim(SubStr(aDetalle[nI][8], 1, At( " ", aDetalle[nI][8] ) ))//Corta hasta encontrar el primer espacio " "
		ENDIF
		if !EMPTY(aDetalle[nI][7])
			cDetdesc+= " - "+alltrim(aDetalle[nI][7])
		ENDIF
		oPrn:Say(_nInterLin,430,memoline(cDetdesc,_tamdesc,1,,.T.),oFont08)  //Descripcion
...
//		alert(xMoeda(aDetalle[nI][6],2,1,,2,,))
		...
		*/
		...
		_nlindet:=mlcount(allTrim(cDetdesc),_tamdesc)
		...
		If _nlindet > 1
			For nIb := 2 To  _nlindet
				...
				oPrn:Say(_nInterLin,430,memoline(cDetdesc,_tamdesc,nIb,,.T.),oFont08)  //Descripcion
				_nInterLin:=_nInterLin+50
			Next nIb
		EndIf
...
Static Function CabFact(nLin,aMaestro,cTipo,cDireccion,cTelefono)
	...
	oPrn:Say( nLinInicial + 270,  290,"CASA MATRIZ"  ,oFont09 )
	// oPrn:Say( nLinInicial + 310,  210,"Avenida 4to. anillo Nro. S/N"  ,oFont09 )
	// oPrn:Say( nLinInicial + 350,  130,"Zona/Barrio: Cotas Norte UV. 066 Mza. 019"  ,oFont09 )
	oPrn:Say( nLinInicial + 310,  130,"OTRO CERCANO A LA DOBLE VIA SANTA CRUZ - WARNES NRO. S/N"  ,oFont09 )
	oPrn:Say( nLinInicial + 350,  130,"Zona/Barrio: CLARA CHUCHIO"  ,oFont09 )
	oPrn:Say( nLinInicial + 390,  130,"Telefono: 67707897 * Telf: 3258881"  ,oFont09 )
	oPrn:Say( nLinInicial + 430,  130,"Warnes, Santa Cruz - Bolivia"  ,oFont09 )
...
//	oPrn:Say( nLinInicial + 465, 1810,"INGENIERIA CIVIL"  ,oFont10 )
	...
	oPrn:Say( nLinInicial + 730, 380,aMaestro[6] ,oFont11 ) // Nombre del Cliente
...
Static Function PieFact(nLinInicial,aMaestro,nCont,aDesc)
	...
	local nIb := 0
	Local nLin := 00.0
	local cExt := Extenso(aMaestro[11],.F.,1)
	cDescripcion := "Son: " + cExt +" BOLIVIANOS"
	// if len(Extenso > 50 ) // totlam
	// 	cDescripcion := "Son: " + cExt +" BOLIVIANOS"
	// 	oPrn:Say( nLinInicial + 2510, 100, cDescripcion ,oFont11 ) //Total  Escrito
	// 	oPrn:Say( nLinInicial + 2560, 100, Extenso(aMaestro[11],.F.,1)+" BOLIVIANOS" ,oFont11 ) //Total  Escrito
	// else
	// 	oPrn:Say( nLinInicial + 2560, 100, Extenso(aMaestro[11],.F.,1)+" BOLIVIANOS" ,oFont11 ) //Total  Escrito
	// endif
	_nlindet:=mlcount(cDescripcion,80)
	nImpr := nLinInicial + 2510
	// If _nlindet > 1
	For nIb := 1 To  _nlindet
		//alert(memoline(aDetalle[nI][2],_tamdesc,nIb))
		// oPrn:Say(_nInterLin,430,memoline(aDetalle[nI][2],_tamdesc,nIb,,.T.),oFont08)  //Descripcion
		oPrn:Say( nImpr, 100, memoline(cDescripcion,80,nIb,,.T.) ,oFont11 ) //Total  Escrito
		nImpr:=nImpr+50
	Next nIb
	// EndIf
...
//	alert(aMaestro[10])
	nTotGral := Round(aMaestro[9]+aMaestro[10],2) 
//	nTotGral := Round(xMoeda(aMaestro[9]+aMaestro[10],aMaestro[28],1),2) // Nahim Cambio 21/05/2020
//	alert(nTotGral)	
...
//	oPrn:Say(nLinInicial + 2400, 2300,FmtoValor(aDesc[1],14,2) ,oFont12N,,,,1 ) 
...
//	oPrn:Say(nLinInicial + 2650, 1610, "Total Bs :",oFont12N )
	...
	oPrn:SayBitmap(2700,2000,cFile,320,320)
...
static function FatDet (cDoc,cSerie) // Datos detalle de factura
	...
	cSql				:= cSql+"D2_LOTECTL,D2_DTVALID,SUM(D2_QUANT) D2_QUANT,(ROUND(SUM(D2_TOTAL),2) / SUM(D2_QUANT))  D2_PRUNIT,(ROUND(SUM(D2_TOTAL),2) / SUM(D2_QUANT)) * SUM(D2_QUANT) D2_TOTAL, MIN(D2_PEDIDO) D2_PEDIDO , D2_DOC ,D2_FILIAL,C6_UPLACA,C6_UCLVL, C6_UNOMCLV "
	...
	cSql				:= cSql+"GROUP BY B1_COD,B1_DESC, D2_DOC ,D2_FILIAL,C6_UPLACA,C6_UCLVL, C6_UNOMCLV,D2_LOTECTL,D2_DTVALID"
