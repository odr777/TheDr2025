...
User Function ImpRemito()
	...
	Private Cvendedor       := ""
	private cxFornec		:= ""
	private cAlmacen		:= "" // Nahim Almacen
...
Static Function RptDetailLoc()
	...
	Else
		...
		DbSetOrder(1)
		dbSeek(xFilial()+mv_par01+mv_par03+cxFornec,.t.)
...
//+-----------------------------------------------------------+
	...
	Else
		...
		While !eof() .and. SF1->F1_DOC <= mv_par02 .and. SF1->F1_SERIE == mv_par03 .and. SF1->F1_FORNECE == cxFornec .and. lContinua
			...
			aPre_Lot :={}                         // Lote
			aPre_Fel :={}                         // Fecha de Vencimiento del lote
			...
			While sd1->(!eof()) .and. SF1->F1_FILIAL+DTOS(SF1->F1_EMISSAO)+SF1->(F1_DOC+F1_SERIE+F1_FORNECE+F1_LOJA) ==;
				...
				AADD(aCC       ,GETADVFVAL("CTT","CTT_DESC01",XFILIAL("CTT")+SD1->D1_CC,1,""))          // CC
				AADD(aClvl     ,GETADVFVAL("CTH","CTH_DESC01",XFILIAL("CTH")+SD1->D1_CLVL,1,""))          // Clase de Valor
...
/*
If mv_par04 == 1
	@ 08, 120 PSAY "X"     //Marca da NF Saida
Else
	@ 08, 108 PSAY "X"     //Marca da NF Entrada
Endif
*/
...
//@ 10, 100 PSAY "Vendedor: "
//@ 10, 110 PSAY cVendedor
//@ 11, 006 PSAY cCod_Cli
	@ 08, 170 PSAY "Fecha: "
	@ 08, 178 PSAY dEmissao              // Data da Emissao do Documento
//@ 12, 006 PSAY "DIR. ENTREGA: " + cRec_Cli
//@ 13, 006 PSAY "SERIE: " 
//@ 13, 014 PSAY trim(cSerie)
//@ 13, 060 PSAY "BARRIO "
//@ 13, 120 PSAY "TELEFONO "
/*
If !EMPTY(cCGC_Cli)                   // Se o C.G.C. do Cli/Forn nao for Vazio // DIRECCION DEL PROVEEDOR
	@ 14, 006 PSAY ">"+cCGC_Cli Picture "@R 99.999.999/9999-99"
Else
	@ 14, 006 PSAY " "                // Caso seja vazio
Endif              
*/
//@ 13, 006 PSAY cEnd_Cli                                 // Endereco
//@ 14, 060 PSAY cBairro                                  // Bairro
//@ 14, 006 PSAY "TIPO REMITO: " 
/*IF(FUNNAME()=='MATA102N') 
	@ 09, 006 PSAY "Almacén: " + POSICIONE('ZAM',1,XFILIAL('ZAM')+SF1->F1_UALMACE,'ZAM_DESCRI')
	@ 09, 170 PSAY "Número de Ingreso: " + SF1->F1_UCORREL
ENDIF*/
//@ 14, 120 PSAY cTel_Cli                               // Telefone/FAX
//@ 15, 123 PSAY cCep_Cli Picture "@R 99999-999"           // CEP
//@ 15, 143 PSAY " "                                      // Reservado  p/Data Saida/Entrada
...
// @ 17, 00 PSAY REPLICATE('-',213)
@ 09, 0016 PSAY "Deposito : " + cAlmacen +" - " + GETADVFVAL("NNR","NNR_DESCRI",XFILIAL("SF1")+cAlmacen,1,"") // +cCod_Cli + " - " + cNome_Cli
@ 09, 170 PSAY "Número de Ingreso: " + SF1->F1_UCORREL
@ 10, 0016 PSAY "Glosa : " +  SF1->F1_XOBS  // +cCod_Cli + " - " + cNome_Cli
  ...
  */
	...
	IF(FUNNAME()<>'MATA462N')
		...
		Else
			...
			@ nLin, 0100  PSAY "......................."
			nLin:= nLin+1
			@ nLin, 016   PSAY "     Entregado por     "
			@ nLin, 0100  PSAY "        Recibido       "
			nLin:= nLin+1
			@ nLin, 016   PSAY "Nombre Completo y firma"
			@ nLin, 0100   PSAY "Nombre Completo y firma"
		End
	ENDIF
/*
if nMoneda==1
	@ 38, 120  PSAY "BOLIVIANOS"
else
	@ 38, 120  PSAY "DOLARES"
Endif
  */
...
Static Function IMPDETLOC()
	...
	Local J :=1
	Local L :=nLin
	Local nXAumento:=70/2  //Si se necesita más espacio para la descripción del producto, basta con aumentar el valor de esta variable
	Local nXaumen2:=-6
	Local nXaumen3:=-4
	Local nXaumen4:=-15
	Local nXaumen5:=-5
//alert('impdetloc')
	...
	nLin := nLin-3
	@ nLin, 00 PSAY REPLICATE('-',223)
	nLin := nLin+1
	If mv_par04 == 2
		//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		@ nLin, 016 PSAY "CODIGO            DESCRIPCION" + SPACE(nXAumento) + "UNID.  CANTIDAD    VAL UNIT.        VALOR TOTAL     LOTE         VALIDEZ  TES  "
		//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	else
		//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		@ nLin, 016 PSAY "CODIGO            DESCRIPCION" + SPACE(nXAumento) + "UNID.  CANTIDAD    VAL UNIT.        VALOR TOTAL     LOTE         VALIDEZ  TES PED.CP       CC        CV"
		//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	end
	nLin := nLin+1
	@ nLin, 00 PSAY REPLICATE('-',223)
	...
	For _I:=1 to nTamDET
		...
		IF _I <= Len(aCod_Pro)
			...
			cTexto := TexToArray(aDescricao[_I],(nXAumento + 7+1))
			
			//aviso("ALERTA"," len(cTexto): " + CValToChar(len(cTexto)) ,{'ok'},,,,,.t.)
			
			// alert(aTes[_I])
			@ nLin, 016  PSAY alltrim(Substr(aCod_Pro[_I],1,15))	Picture "@!"
			//@ nLin, 034  PSAY alltrim(Substr(aDescricao[_I],1,(nXAumento + 7)))
			@ nLin, 034  PSAY alltrim(cTexto[1])
			//@ nLin, 078  PSAY RTRIM (aCod_Trib[I])
			@ nLin, 066 + (nXAumento - 20)  PSAY aUnid_Pro[_I]
			@ nLin, 076 + (nXAumento+nXaumen2 - 20) PSAY aQtd_Pro[_I]               Picture"@E 999,999.99"
			@ nLin, 091 + (nXAumento+nXaumen2+nXaumen3 - 20) PSAY aPre_Uni[_I]               Picture"@E 99,999,999.99"
			@ nLin, 109 + (nXAumento+nXaumen2+nXaumen3 - 20) PSAY aVal_Merc[_I]              Picture"@E 99,999,999.99"
			@ nLin, 127 + (nXAumento+nXaumen2+nXaumen3 - 20) PSAY alltrim(Substr(aPre_Lot[_I],1,10))
			@ nLin, 139 + (nXAumento+nXaumen2+nXaumen3 - 20) PSAY alltrim(aPre_Fel[_I])
			@ nLin, 142 + (nXAumento+nXaumen2+nXaumen3+nXaumen4 - 20) PSAY cvaltochar(alltrim(aTES[_I]))
			If mv_par04 == 1
				@ nLin, 152 + (nXAumento+nXaumen2+nXaumen3+nXaumen4+nXaumen5 - 20) PSAY aPedido [_I]
				//@ nLin, 165  PSAY POSICIONE('SC7',3,XFILIAL('SC7')+ SF1->F1_FORNECE + SF1->F1_LOJA+ aPedido [_I],'C7_NUMSC')
			end
			@ nLin, 162 + (nXAumento+nXaumen2+nXaumen3+nXaumen4+nXaumen5 - 20) PSAY alltrim(Substr(aCC[_I],1,15))
			@ nLin, 178 + (nXAumento+nXaumen2+nXaumen3+nXaumen4+nXaumen5 - 20) PSAY alltrim(Substr(aClvl[_I],1,15))

			//J:=J+1
			
			nLin :=nLin+1
			//@ nLin, 034  PSAY alltrim(Substr(aDescricao[_I],(nXAumento + 7+1),(nXAumento + 7-1)))
			IF len(cTexto) > 1
				@ nLin, 034  PSAY alltrim(cTexto[2])
			EndIf

			@ nLin, 162 + 22 + (nXAumento+nXaumen2+nXaumen3+nXaumen4+nXaumen5 - 20) PSAY alltrim(Substr(aCC[_I],16,10))
			@ nLin, 178 + 22 + (nXAumento+nXaumen2+nXaumen3+nXaumen4+nXaumen5 - 20) PSAY alltrim(Substr(aClvl[_I],16,10))
...
// TextToArray separa las líneas sin cortar las palabras
