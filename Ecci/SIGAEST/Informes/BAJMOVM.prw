...
Static Function MOVD3CONF(Titulo)
	Local cFilename := 'BAJMOVM'
	Local i 	 := 1
	Local x 	 := 0

	cCaminho  := GetTempPath()
	cArquivo  := "zBAJMOVM_" + dToS(date()) + "_" + StrTran(time(), ':', '-')
...
Static Function fImpPres(lEnd,WnRel,cString,nReg)
	...
	cQuery += "  AND D3_NUMSA BETWEEN '" + SCP->CP_NUM + "' AND '" + SCP->CP_NUM + "' "
	...
	fImpCab(.t.,cCust + "-" +VW_D3->CTT_DESC01,VW_D3->CTH_DESC01,VW_D3->D3_DOC,VW_D3->D3_TMDES,VW_D3->D3_EMISSAO, VW_D3->D3_NUMSA, Alltrim(VW_D3->D3_LOCAL) + "-" + alltrim(VW_D3->NNR_DESCRI),VW_D3->D3_OBSERVA,VW_D3->CUSUR,VW_D3->D3_UPROV,VW_D3->D3_TM)
