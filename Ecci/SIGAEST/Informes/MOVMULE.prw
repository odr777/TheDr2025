...
±±Descripcion Impresion de movimientos multiples ECCI	                  º±±
±±º          ³                                                            º±±
...
Static Function fImpPres(lEnd,WnRel,cString,nReg)
	...
	cQuery := "	SELECT D3_CC,D3_NUMSERI,D3_CUSTO1,D3_CF,D3_ESTORNO,D3_DOC,D3_TM+'-'+F5_TEXTO D3_TMDES,D3_NUMSA CUSUR, D3_COD,D3_LOTECTL,D3_LOCALIZ,D3_NUMLOTE,D3_TM,D3_FILIAL,D3_CLVL,D3_NUMSA,D3_ITEMCTA, "
	...
	cQuery += "  LEFT JOIN " + RetSqlName("NNR") + " NNR ON NNR_CODIGO = D3_LOCAL AND NNR.D_E_L_E_T_ = ' '"
	...
	While ! VW_D3->(EoF())
		if  VW_D3->D3_ESTORNO == 'S' .AND. 'DE' $ ALLTRIM(VW_D3->D3_CF)
			VW_D3->(DbSkip())
			LOOP
		endif
		nItem++
		...
		oPrint:Box( nLinAtu, COL_CLVL+8, nLinAtu + 10, COL_CLVL+8, "-2")
		...
		oPrint:SayAlign(nLinAtu, COL_CANTIDAD, ALLTRIM(TRANSFORM(VW_D3->D3_QUANT,"@E 9,999,999.99")) ,  oFontDet, 0038, nTamLin, CLR_BLACK, PAD_RIGHT, 0)
		oPrint:SayAlign(nLinAtu, COL_CLVL + 10, SUBSTR(Posicione("CTH",1,xFilial("CTH")+VW_D3->D3_CLVL,"CTH_DESC01"),1,15)  ,  oFontDet, 0038, nTamLin, CLR_BLACK, PAD_LEFT, 0)
...
Static Function fImpCab(lImp,cCc,cItct,cNum,cSolit,dEmis,cOp,cLocal,cObserv,cSolUsu,cProvx,cxTM)
	...
	if lImp
		...
		if VW_D3->D3_ESTORNO == 'S'
			oPrint:SayAlign(nLinCab, COL_PROY+ 10, "DOCUMENTO REVERTIDO", oFontTitt, 0300, nTamLin* 6, CLR_BLACK, PAD_LEFT, 0)
		endif 
		nLinCab += (nTamLin * 2.5)
		oPrint:Line(nLinCab, nColIni, nLinCab,nColFin)
	else
	...
	oPrint:FillRect({nLinCab+0.45, COL_ITEM -5 , nLinCab + 24, COL_CLVL+8}, oBrush)

	oPrint:FillRect({nLinCab+0.45, COL_CLVL+8 , nLinCab + 24, COL_OPERA-3}, oBrush2)
	...
	oPrint:Box( nLinCab, COL_CLVL+8, nLinCab + 12.5, COL_CLVL+8, "-2")
	...
	oPrint:SayAlign(nLinCab, COL_CLVL + 10, "CLVL",     oFontDetN, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
