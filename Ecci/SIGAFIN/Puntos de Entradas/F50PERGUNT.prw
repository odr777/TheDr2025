	...
	If AllTrim(Funname()) $ "FISA032"
		...
		Do Case
			...
			Case nXForms == 2		//Historial IT
				If (nPosHist := AScan(aAutocab,{|x|AllTrim(x[1])== "E2_HIST"})) > 0 .And. !Empty(aAutocab[nPosHist,2])
					aAutocab[nPosHist,2] := "IMPUESTOS IT, Periodo " + substr(nXPeriodo, 5, 2) + "/" +substr(nXPeriodo, 1, 4) 
				EndIf
				
			Case nXForms == 3		//Historial ITR
				If (nPosHist := AScan(aAutocab,{|x|AllTrim(x[1])== "E2_HIST"})) > 0 .And. !Empty(aAutocab[nPosHist,2])
					aAutocab[nPosHist,2] := "IMPUESTOS ITR, Periodo " + substr(nXPeriodo, 5, 2) + "/" +substr(nXPeriodo, 1, 4) 
				EndIf

			Case nXForms == 4		//Historial IUE Retenciones
				If (nPosHist := AScan(aAutocab,{|x|AllTrim(x[1])== "E2_HIST"})) > 0 .And. !Empty(aAutocab[nPosHist,2])
					aAutocab[nPosHist,2] := "IMPUESTOS IUE Retenciones, Periodo " + substr(nXPeriodo, 5, 2) + "/" +substr(nXPeriodo, 1, 4) 
				EndIf
			OtherWise
				MsgAlert(XSTR001) //"Formulario Incorrecto."
		EndCase
