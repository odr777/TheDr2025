...
User Function LOCXPE16()
	...
	IF alltrim(FunName()) $ "MATA101N"
		...
		//If upper(M->F1_SERIE)$'CCH|FAC'
		For nC := 1 To LEN(aCols)
			//alert(aCols[nC][cPosCC])
			If(M->F1_SERIE <> 'IHD')
				If (aCols[nC][cPosRate])=='2' // valida si NO es prorrateado (TOP es Prorrateado)
					If Empty(aCols[nC][cPosCC])
						Alert('Debe LLenar el Centro de Costo ')
						Return .F.
					ENDIF
					If Empty(aCols[nC][cPosIC]) .AND. Empty(aCols[nC][cPosCLV])
						Alert('Debe LLenar el Item Contable o Clase de Valor ')
						Return .F.
					EndIf
				ENDIF
			EndIf
