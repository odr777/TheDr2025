...
±±³Fun‡…o    ³ TRPlaCt2 ³ Autor ³ Denar Terrazas							³±±
...
Static Function ReportDef()
	...
	Local cNomeArq		:= "Planilla-2-Costos-" + cPeriodo
		...
		LEFT JOIN
			...
			FROM
				...
				SELECT
					...
					TOPPRJ.CODCCUSTO,
					TAREAS.CODTRF,
					SUBSTRING(MIEMBRO.CODMEMBRO, 9, LEN(MIEMBRO.CODMEMBRO)) AS CHAPA,
					INSUMOS.DATAHORAINICIO,
					CONVERT(VARCHAR(8), INSUMOS.DATAHORAINICIO, 112) FECHA,
					CONVERT(VARCHAR(6), INSUMOS.DATAHORAINICIO, 112) PERIODO,
					
					SUM(INSUMOS.HORASAPONTADAS) QUANTIDADE,
					(
						SELECT
							SUM(HORASAPONTADAS) 
						FROM
						...
						WHERE
							...
							AND IDEQUIPEMEMBRO IN (SELECT IDEQUIPEMEMBRO FROM MEQUIPEPRJMEMBRO WHERE SUBSTRING(CODMEMBRO, 9, LEN(CODMEMBRO)) = SUBSTRING(MIEMBRO.CODMEMBRO, 9, LEN(MIEMBRO.CODMEMBRO)))
							AND CONVERT(VARCHAR(6), DATAHORAINICIO, 112) = %exp:cPeriodo%
					)
					AS TOTAL_HORAS,
					
					(
						SUM(INSUMOS.HORASAPONTADAS) / ( 
						SELECT
							SUM(HORASAPONTADAS) 
						FROM
							MAPONTMAODEOBRA 
					...
					(
						...
						FROM
							...
							LEFT JOIN
								...
								AND SN3.%notdel%
						WHERE
							TV1.TV1_DTSERV = CONVERT(VARCHAR(8), INSUMOS.DATAHORAINICIO, 112) 
							AND TV1.TV1_OPERAD COLLATE Latin1_General_CS_AI = SUBSTRING(MIEMBRO.CODMEMBRO, 9, LEN(MIEMBRO.CODMEMBRO))
							AND TV1.%notdel%
					)
					AS CLASE_VALOR 
					
					FROM
					MAPONTMAODEOBRA INSUMOS 
					JOIN
						MTAREFA TAREAS 
					...
					JOIN
						MEQUIPEPRJMEMBRO MIEMBRO
						ON MIEMBRO.IDEQUIPEMEMBRO = INSUMOS.IDEQUIPEMEMBRO
						AND MIEMBRO.IDPRJ = INSUMOS.IDPRJ
					WHERE
					STATUS NOT IN
					(
						1
					)
					AND CONVERT(VARCHAR(6), INSUMOS.DATAHORAINICIO, 112) = %exp:cPeriodo%
					GROUP BY
					TOPPRJ.CODPRJ,
					TOPPRJ.CODCCUSTO,
					TAREAS.CODTRF,
					INSUMOS.IDEQUIPEMEMBRO,
					SUBSTRING(MIEMBRO.CODMEMBRO, 9, LEN(MIEMBRO.CODMEMBRO)),
					INSUMOS.DATAHORAINICIO 
					)
					AS APUNTES
			GROUP BY
...
Static Function CabecPak(oReport) // opcional, si se requiere una cabecera especifica
	...
	aCabec := {     "__LOGOEMP__" ,cChar + "        " + cChar + RptFolha+" "+cvaltochar(page);
