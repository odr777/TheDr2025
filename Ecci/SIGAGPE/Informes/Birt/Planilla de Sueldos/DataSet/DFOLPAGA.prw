...
#DEFINE NLIQUIDOP      24
#DEFINE NFIRMA         25
#DEFINE NMATRICULA     26
#DEFINE NCCOSTO        27	//Codigo Centro de Costo
#DEFINE NCCDESC        28	//Descripcion Centro de Costo
#DEFINE NAGRUPADO      29	//Agrupado por: 1:=Sucursal, 2:=Centro Costo
#DEFINE NSUCURSAL      30
#DEFINE NSUCNOMBR      31
#DEFINE NEMPCOD        32
#DEFINE NEMPDESC       33
...
Private nPerEvent	:= self:execParamValue("MV_PAR12")
Private cNomEmpr	:= self:execParamValue("MV_PAR13")
Private cNumDocId	:= self:execParamValue("MV_PAR14")
Private nOrdem		:= self:execParamValue("MV_PAR15")
Private nAgrupa		:= IIF(EMPTY(self:execParamValue("MV_PAR16")), 1, self:execParamValue("MV_PAR16"))//Agrupado por: 1:=Empresa, 2:=Sucursal, 3:=Centro Costo
Private aRoteiros	:= {}
...
³ mv_par10        //  Situacao                                 ³
³ mv_par11        //  Categoria                                ³
³ mv_par12        //  Personal - Eventual/Permanente           ³
...
Static Function R680ImpR4()
	...
	Do While (cAliasFun)->(!Eof())
		...
		If aVerba[1,1] > 0
			...
			aCols[NCI        ]:= ( /*PadL(*/ AllTrim((cAliasFun)->RA_CIC)/*, 12)*/ + AllTrim((cAliasFun)->RA_UFCI))
			...
			aCols[NEMPDESC   ]:= ( FWEmpName(SUBSTR((cAliasFun)->RA_FILIAL,1,2)))
