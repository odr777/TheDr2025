...
user function PONCALD()
	...
	Local dDataFin	//Fecha final del periodo informado en los parámetros
	Private cProceso:= MV_PAR01
	Private cPeriodo:= MV_PAR02
	Private cNumPag := MV_PAR03
	Private cMatDe	:= MV_PAR10
	Private cMatAte	:= MV_PAR11

	dDataFin:= GetAdvFVal("RFQ","RFQ_DTFIM", xFilial("RFQ") + cProceso + cPeriodo + cNumPag, 1, CTOD("  /  /    "))//RFQ_FILIAL+RFQ_PROCES+RFQ_PERIOD+RFQ_NUMPAG+DTOS(RFQ_DTINI)+DTOS(RFQ_DTFIM)+RFQ_MODULO
	...
	If(SRA->RA_MAT >= cMatDe .AND. SRA->RA_MAT <= cMatAte)
		...
		If(!Empty(cATRCod))
			If(SPB->(dbSeek(SRA->RA_FILIAL + SRA->RA_MAT + cATRCod + DTOS(dDataFin))))
				RECLOCK("SPB", .F.)
				If(SPB->PB_HORAS >= (30/60) .AND. SPB->PB_HORAS <= (89/60))
					SPB->PB_HORAS:= 0.5//Medio día de descuento
				ElseIf(SPB->PB_HORAS >= (90/60) .AND. SPB->PB_HORAS <= (119/60))
					SPB->PB_HORAS:= 1//1 día de descuento
				ElseIf(SPB->PB_HORAS >= (120/60) .AND. SPB->PB_HORAS <= (239/60))
					SPB->PB_HORAS:= 2//2 día de descuento
				ElseIf(SPB->PB_HORAS >= (240/60))
