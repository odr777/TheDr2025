...
user function getItemContable(cBien,cDataServ)
    ...
    cQueryaf9	:= ""
    // consulta a la base de datos
    BeginSql Alias consItemC //Verifica Item contable desde TOP
            SELECT
    ...
    //cQueryaf9:=GetLastQuery()
    // aviso("",cQueryaf9[2],{'ok'},,,,,.t.)

    DbSelectArea(consItemC)
    cSD3MaxDoc:= ''
    If (consItemC)->(Eof())
        DbcloseArea(consItemC)
        BeginSql Alias consItemC //Verifica Item contable desde TV2
                    SELECT
                    AF9_ITEMCC,AF9_TAREFA,AF9_DESCRI,AF9_PROJET
                    FROM
                    AF9010 AF9 
                    WHERE
                    TRIM(AF9_PROJET)+TRIM(AF9_TAREFA) IN
                                (
                                            SELECT TRIM(TV2_UPROJE)+TRIM(TV2_UTAREF)
                                            FROM
                                            %Table:TV2% TV2 
                                            WHERE
                                            D_E_L_E_T_ = ' ' 
    ...
    If (consItemC)->(Eof())
        DbcloseArea(consItemC)
        BeginSql Alias consItemC //Verifica Item contable desde TV2 con LIKE para EDTs (Cabecera de Tareas)
                    SELECT
                    AF9_ITEMCC,AF9_TAREFA,AF9_DESCRI,AF9_PROJET
                    FROM
                    AF9010 AF9 
                    WHERE
                    TRIM(AF9_PROJET)+TRIM(AF9_TAREFA) LIKE
                                (
                                            SELECT MAX(TRIM(TV2_UPROJE)+TRIM(TV2_UTAREF))+'%'
                                            FROM
                                            %Table:TV2% TV2 
                                            WHERE
                                            D_E_L_E_T_ = ' ' 
                    ...
                    AND AF9.D_E_L_E_T_ = ' ' AND AF9_ITEMCC <> ' ' ORDER BY AF9_TAREFA
         EndSql
        //cQueryaf9:=GetLastQuery()
        //aviso("",cQueryaf9[2],{'ok'},,,,,.t.)
    Endif
    ...
    If (consItemC)->(!Eof()) // Encuentro
        if !empty((consItemC)->AF9_ITEMCC)
            ...
            cAviso := "Bien: " + cBien + " | fecha: " + cDataServ + CRLF
            cAviso += "La tarea:" + (consItemC)->AF9_TAREFA +" - " + (consItemC)->AF9_DESCRI +" del Proyecto: " + (consItemC)->AF9_PROJET 
            cAviso += " no está relacionada con ningun item contable,"+ CRLF + " favor digitarlo en 'Tareas del proyecto'"
            Aviso( "varpartdi 1", cAviso , {"Ok"} )
        endif
    Else
        cAviso := "Bien: " + cBien + " | fecha: " + cDataServ + CRLF
        cAviso += "La tarea:" + (consItemC)->AF9_TAREFA +" - " + (consItemC)->AF9_DESCRI +" del Proyecto: " + (consItemC)->AF9_PROJET 
        cAviso += " no está relacionada con ningun item contable,"+ CRLF + " favor digitarlo en 'Tareas del proyecto'"
        Aviso( "varpardi 2", cAviso , {"Ok"} )
