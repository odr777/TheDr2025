...
User Function genmpdin()
	...
	aadd(aHeader,{"Total contador."    ,"TV2_XTOTCO"     ,"@!"  , 2,0,,"€€€€€€€€€€€€€€ ","N","SC5","R"})
	aadd(aHeader,{"Codigo Frente."    ,"TV2_CODFRE"     ,"@!"  , 11,0,,"€€€€€€€€€€€€€€ ","N","SC5","R"})
	aadd(aHeader,{"Secuencia"    ,"TV2_SEQREL"     ,"@!"  , 3,0,,"€€€€€€€€€€€€€€ ","N","SC5","R"})
	aadd(aHeader,{"Actividad."    ,"TV2_CODATI"     ,"@!"  , 3,0,,"€€€€€€€€€€€€€€ ","N","SC5","R"})
	aadd(aHeader,{"Hora inicial","TV2_HRINI"     ,"@!"  , 5,0,,"€€€€€€€€€€€€€€ ","N","SC5","R"})
	aadd(aHeader,{"Hora final","TV2_HRFIM"     ,"@!"  , 5,0,,"€€€€€€€€€€€€€€ ","N","SC5","R"})
...
static function Llamacompensa()
	...
	MsgRun ("Generando Apuntes Costos SD3", "Por favor espere", {|| compensa() } )
	close(oDlgTela)
...
static function compensa()
	...
	For nX := 1 to Len(oGetDados:aCols)
		...
		if(oGetDados:aCols[nX,1] == 'LBOK') // realiza una compensación
			cTV2_XTOTCO :=  oGetDados:aCols[nX,8]
			cTV2_CODFRE :=  oGetDados:aCols[nX,9]
			cTV2_SEQREL :=  oGetDados:aCols[nX,10]
			cTV2_CODATI :=  oGetDados:aCols[nX,11]
			cTV2_HRINI  :=  oGetDados:aCols[nX,12]
			cTV2_HRFIM  :=  oGetDados:aCols[nX,13]
            ...
            // If dbSeek( cKey )
			aArrayPTd :={}
			// While cTV2_of() ) .And. cKey == cTV2_FILIAL + cTV2_EMPRES + cTV2_CODBEM + DToS( cTV2_DTSERV )
				//Nahim posicione en la TV0 para veriificar si corresponde el apunte
				// TV0_FILIAL+TV0_CODATI
				// cMesage := ''
				// dbSelectArea('TV0')
				// TV0->(dbSetOrder(1))
				// TV0->(dbSeek(xFilial("TV0") + cTV2_CODATI)) // posicionandomé en la actividad
				...
				// if TV0->TV0_TIPHOR $ '1|4' // Tipo de hora trabajada y planificación deberían apuntarse 29/12/2020
					...
					IF dbSeek(xFilial("ST9")+cBemTv1)
						...
						IF dbSeek(xFilial("SN3")+ST9->T9_CODIMOB) // Encuentra SN3
							if ST9->T9_CODIMOB = SN3->N3_CBASE+SN3->N3_ITEM
								if !empty(SN3->N3_CLVLCON)  // no tiene que estár vacio
									...
									if !empty(cProducto) // obtengo el producto de costo de mano de obra
										...
										cMax := U_isInSD3(cBemTv1,dDtTv1,cTV2_SEQREL)
										IF cTV2_XTOTCO <> 0 .AND. cMax <> "XX"// Nahim 22/06/2021
											//alert("XTOTCO <> 0 .AND. cMax <> XX" + cBemTv1+DTOC(cTV2_DTSERV)+cTV2_SEQREL)
											cItemCont := U_getItemContable(cBemTv1,dDtTv1) // Busca código del item cuenta NT 20/07/2020
												aRegTv2 := {}
												AADD( aRegTv2,cProducto)      // 1
												AADD( aRegTv2,cTV2_XTOTCO)  // 2
												AADD( aRegTv2,dDtTv1)  // 3
												AADD( aRegTv2,cTV2_CODFRE)  // 4
												AADD( aRegTv2,SN3->N3_CLVLCON)  // 5
												AADD( aRegTv2,SN3->N3_FILIAL)  // 6
												AADD( aRegTv2,cItemCont)  // 7 item cuenta
												AADD( aRegTv2,cTV2_SEQREL)  //8 secuencia
				...
				// endif
				cMesage += 'Bien ' +  cBemTv1 + ' - Actividad: ' + cTV2_CODATI + ' / ' + cTV2_HRINI + ' - ' + cTV2_HRFIM + CRLF
				// cTV2_Skip() )
			// End
        ...
        EndIf
	Next nX
	aviso("Se registraron los siguientes partes diarios en Costos SD3. ",cCompensaciones,{'ok'},,,,,.t.)
	//	ALERT(cPrimComp)
	//	ALERT(cUltimaCom)
	//	alert("compensando..")
return
...
static function buscaTV1()
	...
	BeginSql alias cTemp
		...
		TV2_HRFIM,
   		TV2.TV2_SEQREL,
        CONCAT('PT'+trim(TV1.TV1_CODBEM)+ SUBSTRING(TV1_DTSERV,3,8),'-',TV2.TV2_SEQREL),
        TV2_XTOTCO,
        TV0.TV0_TIPHOR, TV0.TV0_CODATI ,TV2.TV2_CODATI, D3_UOBSERV
        ,CONCAT('PT'+trim(TV1.TV1_CODBEM)+ SUBSTRING(TV1_DTSERV,3,8),'-',TV2.TV2_SEQREL), TV1.*
		FROM TV1010 TV1 
		LEFT JOIN TV2010 TV2 ON 
		TV1.TV1_FILIAL + TV1.TV1_EMPRES + TV1.TV1_CODBEM + TV1.TV1_DTSERV =  TV2_FILIAL + TV2_EMPRES + TV2_CODBEM  + TV2_DTSERV 
		AND TV2.D_E_L_E_T_ LIKE ''
		left outer JOIN SD3010 SD3 ON 
		CONCAT('PT'+trim(TV1.TV1_CODBEM)+ SUBSTRING(TV1_DTSERV,3,8),'-',TV2.TV2_SEQREL)
		=  trim(SUBSTRING(D3_UOBSERV,0,CASE WHEN len(D3_UOBSERV) > 0 THEN len(D3_UOBSERV)+1 ELSE 0 END))
		AND D3_EMISSAO BETWEEN %exp:MV_PAR03% AND %exp:MV_PAR04%
		AND D3_TIPO = 'MO'
		AND TV2.D_E_L_E_T_ LIKE ''
		LEFT JOIN TV0010 TV0 ON 
	 	TV2.TV2_CODATI = TV0.TV0_CODATI AND TV0.D_E_L_E_T_ LIKE ''
		WHERE
        TV1_DTSERV BETWEEN %exp:MV_PAR01% AND %exp:MV_PAR02%
        and TV1_INDERR = 2
        AND TV0.TV0_TIPHOR IN ('1','4')
		and TV1.D_E_L_E_T_ = ''
		AND SD3.D3_UOBSERV is NULL
		AND TV2_XTOTCO > 0
		AND TV1_CODBEM IN(SELECT T9_CODBEM FROM ST9010 s, SN3010 s2 WHERE s.D_E_L_E_T_  = ' ' AND s2.D_E_L_E_T_ = ' '
			AND T9_CODIMOB = N3_CBASE+N3_ITEM AND N3_TPDEPR = '5')
		AND TV1_CODBEM BETWEEN  %exp:MV_PAR03% AND %exp:MV_PAR04%
		ORDER BY TV1_DTSERV,TV1.TV1_CODBEM
	...
	While !(cTemp)->(eof())
		...
		(cTemp)->TV1_CONFIM,(cTemp)->TV1_NOMEOP,;
		(cTemp)->TV2_XTOTCO,;
		(cTemp)->TV2_CODFRE,;
		(cTemp)->TV2_SEQREL,;
		(cTemp)->TV2_CODATI,;
