...
/*
POSICIONANDO EN LA SN3
ArrTeste := {}
AADD(Arrteste, GetSXENum("FNA", "FNA_IDMOV")) //IDMOV               ok
AADD(Arrteste, '001') //ITMOV                                       ok
AADD(Arrteste, dEmissa)   //                                        TV1_DTSERV
AADD(Arrteste, 'P2') ///TIPO APUNTE                                 ok
AADD(Arrteste, dEmissa) //PERIODO INICIAL                           TV1_DTSERV
AADD(Arrteste, dEmissa) //PERIODO FINAL                             TV1_DTSERV
AADD(Arrteste, aItCols[i][8][2]) // cantidad de horas D1_UHORAEQ    ok
AADD(Arrteste, VW_CPX->N3_CBASE)// N3_CBASE                         ok
AADD(Arrteste, VW_CPX->N3_ITEM)// N3_ITEM                           ok
AADD(Arrteste, VW_CPX->N3_TIPO)// N3_TIPO                           ok
AADD(Arrteste, VW_CPX->N3_SEQ) // N3_SEQ                            ok
*/
    ...
    For nX := 1 To Len(aTxDepr)
        ...
        If nX == 1
            ...
            Case aMemFNA[4] $ "P2|P3|P4|"		//Apont. de produco e estorno
                ...
                If SN3->(MsSeek(FNA->FNA_FILIAL+FNA->FNA_CBASE+FNA->FNA_ITEM+FNA->FNA_TIPO+"0"+FNA->FNA_SEQ))
                    //Gera as movimentaηυes na tabela SN4 e SN5
            ...
            Case aMemFNA[4] == "P8"		//Estorno de revisao de estim. de produco
                ...
                If FNA->(MsSeek( aRecFNA[1]+aRecFNA[2]+aRecFNA[3]+aRecFNA[4]+aRecFNA[5]+aRecFNA[6]+aRecFNA[7] ))
                    ...
                    //Verifica o ϊltimo apontamento de revisao de estimativa de produco
                    nUltApto := Len(aAptos)
                        ...
                        If SN3->(MsSeek(FNA->FNA_FILIAL+FNA->FNA_CBASE+FNA->FNA_ITEM+FNA->FNA_TIPO+"0"+FNA->FNA_SEQ))
                            ...
                            If SN4->(MsSeek( FNA->(FNA_FILIAL+FNA_CBASE+FNA_ITEM+FNA_TIPO+DTOS(FNA_DATA))+nOcorr+FNA->FNA_SEQ ))
                                While SN4->(!EoF())	;
                                    ...
                                    //Apaga os registros das movimentaηυes
                                    RecLock("SN4",.F.)
                                    dbDelete()
                                    MsUnlock()
                                    SN4->(dbSkip())
