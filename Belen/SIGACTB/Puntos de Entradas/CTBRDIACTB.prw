...
DO CASE
	...
	//|Cobros Diveros						|
	//+-------------------------------------+
	CASE  FUNNAME() $ "FINA087A" //cLancPad $ "575"
        cCodDia := cDiario
		// lCmp := .T.
		// If FUNNAME() $ "FINA087A"
		...
		// 	cCodDia := TP_INGRESO
		// Endif
	CASE cLancPad $ "573" /////REPOSICION CAJA CHICA
		if ALLTRIM(FUNNAME()) $ "FINA550" .and. cTipoDeb == '2'  //son los EF Y TF
			
			cCodDia := TP_EGRESO
			
		endif
	CASE cLancPad $ "530" /////REPOSICION CAJA CHICA
		if  SE5->E5_BANCO <> '  ' .AND. AllTrim(SE5->E5_TIPO) $ "PA|NCP" // caso tenga banco no pea
			cCodDia := TP_INGRESO
		else
			cCodDia := TP_EGRESO
		endif
