...
User Function xFinA371(lDireto)
	...
	If cPaisLoc $ "ANG|COL|MEX|PER" .And. SFR->(FieldPos("FR_LA")) == 0
	...
	If nOpcA == 1
		Processa({|lEnd| FA371Processa()})  // Chamada da funcao de Contabilizacao Off-Line
	Endif
...
*/
Static Function FA371Processa(lBat)
		...
		If aSM0[nInc][1] == cEmpAnt .AND. aSM0[nInc][2] >= cFilDe .AND. aSM0[nInc][2] <= cFilAte
			...
			If mv_par06	==	1 .or. mv_par06	==	3
				...
				Else
					...
					cChaveSEL := "EL_FILIAL+DTOS(EL_DTDIGIT)+EL_SERIE+EL_RECIBO+EL_PREFIXO+EL_NUMERO+EL_PARCELA+EL_TIPO+EL_CLIENTE+EL_LOJA"
				EndIf
				...
				#ENDIF
				/*
				Eduar Andia 09/03/2020 Para no separar asientos.

				*/
				dbSetOrder(15)	//dbSetOrder(nIndSEL+1)
				dbGoTop()
			...
			If (!SEK->(EOF().And.BOF()).And. Alltrim(STR(mv_par06))$ "23") .or. (!SEL->(EOF().And.BOF()).And. Alltrim(STR(mv_par06))	$ "13") .or. ( cPaisLoc $ "ANG|COL|MEX|PER" .And. !SFR->(EOF().And.BOF()) )
...
Static Function OrdPagProc(lBat)
	...
	Local aFormasPgto:= {}
	Local nC := 0
	local aITf := {} //obtengo los itfs que se deben contabilizar
	local aAreaitf  // Area del ITF para restaurar NTP 09/03/2020
		...
		For nLaco := 1 to nPeriodo
				...
				Do while !SEK->(EOF()).And.( SEK->EK_DTDIGIT >= mv_par04 .And. SEK->EK_DTDIGIT <= mv_par05 )
					If !SEK->EK_CANCEL
						...
						If lLancPad70
							...
							// 	NTP BUSCA ITF 07/03/2020
							aAreaitf  := GetArea('SE5')
							// 	E5_FILIAL+E5_TIPODOC+E5_PREFIXO+E5_NUMERO+E5_PARCELA+E5_TIPO+DTOS(E5_DATA)+E5_CLIFOR+E5_LOJA+E5_SEQ
							//	E5_FILIAL+E5_PROCTRA
							SE5->(DbSetOrder(2))
							lItf := .F.
							If SE5->(DbSeek(xFilial("SE5")+"VL"+SEK->EK_PREFIXO + SEK->EK_NUM + SEK->EK_PARCELA + SEK->EK_TIPO))// ENCUENTRA la transferencia
								cProcTrac := SE5->E5_PROCTRA
								// pesquisa si tiene ITF
								// ALERT(cProcTrac)
								SE5->(DbSetOrder(15))

								While xFilial("SE5") == SE5->E5_FILIAL .And. cProcTrac == SE5->E5_PROCTRA  .and. !empty(cProcTrac)
									If SE5->E5_TIPODOC == 'IT' // ENCONTRO
										// Contabilizar ITF
										nEncuentra := .f.// nEncuentraITF
										for nItf := 1 to len(aItf)

											if aItf[nItf] == SE5->(Recno())
												nEncuentra := .T. // ENCONTRÓ NO DEBERÍA CONTABILIZAR
											endif
											// encuentra ITF Recno
										next nItf
										if !nEncuentra // caso no encuentre debería agregarlo para contabilizarlo
											aadd(aItf,SE5->(Recno()))
										endif
									EndIf
									SE5->(DbSkip())
							...
							EndIf
							restArea(aAreaitf)
							// NTP 07/03/2020 Finaliza contabilización de ITF
					...
					(cOrdPago+cFornece+cLoja#SEK->EK_ORDPAGO+Iif(lCpoSEK,SEK->EK_FORNEPG+SEK->EK_LOJAPG,SEK->EK_FORNECE+SEK->EK_LOJA)  .Or.  Eof())// Por documento
						// Contabiliza ITFs Nahim 09/03/2020
						aAreaitf  := GetArea('SE5')
						dbSelectArea("SE5")
						for nItf := 1 to len(aItf)
							DBGOTO(aItf[nItf])
							FinProcITF( SE5->(Recno()),8,SE5->E5_VALOR,.F.,{nHdlPrv,'570',"FINA085A","FINA085A",cLote})
						next nItf
						aItf = {} // array borrando valores. (09/03/2020)
						restArea(aAreaitf)
						// Contabiliza ITFs Nahim 09/03/2020
				...
				Do while !SEL->(EOF()).And.SEL->EL_DTDIGIT >= mv_par04 .And. SEL->EL_DTDIGIT <= mv_par05
					If !SEL->EL_CANCEL
						...
						If lLancPad75
							...
							//¦ Posiciona Archivo Proveedores si necesario.      ¦
							//+--------------------------------------------------+
							If cRecibo+cSerREc+cCliente+cLoja#SEL->EL_RECIBO+SEL->EL_SERIE+Iif(lCpoSEL,SEL->EL_CLIORIG+SEL->EL_LOJORIG,SEL->EL_CLIENTE+SEL->EL_LOJA)
								SA1->(DbSetOrder(1))
							...
							/*aTabRecOri*/,;
							/*aDadosProva*/ )
							//							alert("ejecutando detprova COBRO " + CVALTOCHAR(nTotDoc) )
							// 	NTP BUSCA ITF 07/03/2020
							aAreaitf  := GetArea('SE5')
							// 	E5_FILIAL+E5_TIPODOC+E5_PREFIXO+E5_NUMERO+E5_PARCELA+E5_TIPO+DTOS(E5_DATA)+E5_CLIFOR+E5_LOJA+E5_SEQ
							//	E5_FILIAL+E5_PROCTRA
							SE5->(DbSetOrder(2))
							lItf := .F.
							If SE5->(DbSeek(xFilial("SE5")+"VL"+SEL->EL_PREFIXO+SEL->EL_NUMERO+SEL->EL_PARCELA+SEL->EL_TIPO))// ENCUENTRA la transferencia
								lEnCorrSE5 := .f.
								cTrSE5 := xFilial("SE5")+"VL"+SEL->EL_PREFIXO+SEL->EL_NUMERO+SEL->EL_PARCELA+SEL->EL_TIPO // Nahim verifica que sea la correcta porque tiene que ser situaca diferente de C 18/03/2020
								While cTrSE5 == xFilial("SE5")+"VL"+SEL->EL_PREFIXO+SEL->EL_NUMERO+SEL->EL_PARCELA+SEL->EL_TIPO .and. !lEnCorrSE5
									If SE5->E5_SITUACA <> 'C'
										lEnCorrSE5:=.T. // ENCUENTRA TRANSACCIÓN CORRECTA que esté sin reversión 
									Else
										SE5->(DbSkip())
									EndIf
								EndDo
					...
					SEL->(DbSkip())
					If (mv_par14 == 2 .And. nTotalLanc > 0)  .And.;
					(cRecibo+cSerREc+cCliente+cLoja#SEL->EL_RECIBO+SEL->EL_SERIE+Iif(lCpoSEL,SEL->EL_CLIORIG+SEL->EL_LOJORIG,SEL->EL_CLIENTE+SEL->EL_LOJA)  .Or.  Eof())// Por documento
						// Contabiliza ITFs Nahim 09/03/2020
						aAreaitf  := GetArea('SE5')
						dbSelectArea("SE5")
						for nItf := 1 to len(aItf)
							DBGOTO(aItf[nItf])
							FinProcITF( SE5->(Recno()),8,SE5->E5_VALOR,.F.,{nHdlPrv,'575',"FINA087A","FINA087A",cLote})
							//							nTotDoc += DetProva( nHdlPrv,;
							//							'56A',;
							//							"COFFA01",;
							//							cLote,;
							...
							//							/*aTabRecOri*/,;
							//							/*aDadosProva*/ )
							//							alert("ejecutando detprova ITF" + CVALTOCHAR(nTotDoc) )
							//							nTotalLanc += nTotDoc
							//							nTotProc   += nTotDoc
						next nItf
						aItf = {} // array borrando valores. (09/03/2020)
			...
			If lCabecalho .And. nTotalLanc > 0 .And. mv_par14 == 1 // Por periodo
				...
				If lLanctOk .AND. !lUsaFlag
					...
					If cPaisLoc $ "ANG|COL|MEX|PER" .And. Len(aRegSFR) > 0
						nRecAnt := SFR->(Recno())
