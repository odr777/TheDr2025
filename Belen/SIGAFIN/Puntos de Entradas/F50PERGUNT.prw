...
±±º          ³ (Rotina FINA050)                                           º±±
±±º		³ Omar Delgadillo		³ Data	³ 27/04/2021					  º±±
±±º		³ Adicionar parámetos para ITR, IUE, IT.  						  º±±
±±º		³ Configurar Histórico del título.		  						  º±±
		...
		Do Case
			Case nXForms == 1		
				...
				If (nPosHist := AScan(aAutocab,{|x|AllTrim(x[1])== "E2_HIST"})) > 0 .And. !Empty(aAutocab[nPosHist,2])
					aAutocab[nPosHist,2] := "IMPUESTO IVA, Periodo " + substr(nXPeriodo, 5, 2) + "/" +substr(nXPeriodo, 1, 4) 
				EndIf
			...
			Case nXForms == 2		
				...
				//Historial IT
				If (nPosHist := AScan(aAutocab,{|x|AllTrim(x[1])== "E2_HIST"})) > 0 .And. !Empty(aAutocab[nPosHist,2])
					aAutocab[nPosHist,2] := "IMPUESTO IT, Periodo " + substr(nXPeriodo, 5, 2) + "/" +substr(nXPeriodo, 1, 4) 
				EndIf
			...
			Case nXForms == 3		
				...
				//Historial ITR
				If (nPosHist := AScan(aAutocab,{|x|AllTrim(x[1])== "E2_HIST"})) > 0 .And. !Empty(aAutocab[nPosHist,2])
					aAutocab[nPosHist,2] := "IMPUESTO IT Retenciones, Periodo " + substr(nXPeriodo, 5, 2) + "/" +substr(nXPeriodo, 1, 4) 
				EndIf

			Case nXForms == 4		
				//Fornecedor+Loja IUE Retenciones
				If !Empty(cProvIUR)
					SA2->(DbSetOrder(1))
					...
					Endif
				Endif
				//Historial IUE Retenciones
				If (nPosHist := AScan(aAutocab,{|x|AllTrim(x[1])== "E2_HIST"})) > 0 .And. !Empty(aAutocab[nPosHist,2])
					aAutocab[nPosHist,2] := "IMPUESTO IUE Retenciones, Periodo " + substr(nXPeriodo, 5, 2) + "/" +substr(nXPeriodo, 1, 4) 
				EndIf
			OtherWise
