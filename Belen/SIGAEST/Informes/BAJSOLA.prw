...
Static Function fImpPres(lEnd,WnRel,cString,nReg)
	...
	cQuery:= " SELECT CP_NUM,CP_ITEM,CP_PRODUTO,CP_DATPRF,CP_UM,B1_DESC CP_DESCRI,CP_QUANT,CP_OBS,CP_SOLICIT,CP_CONTA,CP_ITEMCTA,CP_CC,CP_CLVL,CP_STATSA,CP_STATUS,CP_LOCAL "
	...
	fImpCab(.t.,VW_F1PRD->CP_CC,VW_F1PRD->CP_NUM,VW_F1PRD->CP_SOLICIT,VW_F1PRD->CP_DATPRF,VW_F1PRD->CP_STATSA,VW_F1PRD->CP_STATUS,VW_F1PRD->CP_LOCAL)

	//Tributo
	cObserva := VW_F1PRD->CP_OBS
	
	cAlmac :=  VW_F1PRD->CP_LOCAL
	...
	if ! VW_F1PRD->(EoF())
		...
		While ! VW_F1PRD->(EoF())
			...
			oPrint:SayAlign(nLinAtu, COL_DESCR, VW_F1PRD->CP_PRODUTO,     oFontDet, 270, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_BRUTO,VW_F1PRD->CP_UM,     oFontDet, 0050, nTamLin, CLR_BLACK, PAD_LEFT, 0)
			oPrint:SayAlign(nLinAtu, COL_IVA, ALLTRIM(TRANSFORM(VW_F1PRD->CP_QUANT,"@E 999,999,999,999.99")) ,     oFontDet, 0170, nTamLin, CLR_BLACK, PAD_LEFT, 0)

			aDescri := TexToArray(ALLTRIM(VW_F1PRD->CP_DESCRI),50)
	...
	oPrint:SayAlign(nLinAtu, COL_BRUTO,"Total:",     oFontDetN, 0050, nTamLin, CLR_BLACK, PAD_LEFT, 0)
	oPrint:SayAlign(nLinAtu, COL_IVA, ALLTRIM(TRANSFORM(nTot,"@E 999,999,999,999.99")) ,     oFontDet, 0170, nTamLin, CLR_BLACK, PAD_LEFT, 0)
	nLinAtu += (nTamLin * 2)
	
	oPrint:SayAlign(nLinAtu, COL_GRUPO, "OBSERVACION: " ,     oFontDetN, 0100, nTamLin, CLR_BLACK, PAD_LEFT, 0)
	oPrint:SayAlign(nLinAtu, COL_DESCR+15, cObserva ,     oFontDet, 0170, nTamLin, CLR_BLACK, PAD_LEFT, 0)
...
Static Function fImpCab(lImp,cCc,cNum,cSolit,dEmis,cEst,cEstu,cLoca)
	Local cTexto   := ""
	Local nLinCab  := 030
	Local nLinInCab := 015
	Local nDetCab := 090
	...
	if lImp
		...
		oPrint:SayAlign(nLinCab, 460, "NRO: " + cNum, oFontDetN, 400, 30, , , 0)
		cTexto := "PEDIDO"
		oPrint:SayAlign(nLinCab, nColMeio - 180, cTexto, oFontTitt, 400, 30, , PAD_CENTER, 0)
		nLinCab += (nTamLin * 3)
		oPrint:SayAlign(nLinCab, nColMeio - 165, "Almacen: " + cLoca + " - " +POSICIONE("NNR",1,xfilial("NNR")+cLoca,"NNR_DESCRI"), oFontTit, 400, 30, , PAD_CENTER, 0)		
		...
		nLinCab += nTamLin
		oPrint:SayAlign(nLinCab, COL_GRUPO, "SOLICITADO POR: ",     oFontDetN, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		oPrint:SayAlign(nLinCab, COL_GRUPO+70, cSolit, oFontDet8, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)

		oPrint:SayAlign(nLinCab, COL_BRUTO, "PRIORIDAD: ",     oFontDetN, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		oPrint:SayAlign(nLinCab, COL_BRUTO+70, "", oFontDet8, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		//oPrint:SayAlign(nLinCab, COL_GRUPO+0170, cDoc + space(10) + cUcob + space(7) + cOrg,     oFontDetN, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		//recuadro
		
		nLinCab += (nTamLin * 1.8)
		oPrint:SayAlign(nLinCab, COL_GRUPO, "CENTRO COSTO:", oFontDetN, 0100, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		oPrint:SayAlign(nLinCab, COL_GRUPO+70, Posicione("CTT",1,xFilial("CTT")+cCc,"CTT_DESC01"), oFontDetN, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)

		oPrint:SayAlign(nLinCab, COL_BRUTO, "ESTADO: ",     oFontDetN, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)

		IF cEst == "L" .AND. EMPTY(cEstu)
			oPrint:SayAlign(nLinCab, COL_BRUTO+70,"Atendida" , oFontDet8, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		elseif cEst == "L" .AND. cEstu == "E"
			oPrint:SayAlign(nLinCab, COL_BRUTO+70,"Pendiente" , oFontDet8, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		elseif cEst == "B"
			oPrint:SayAlign(nLinCab, COL_BRUTO+70,"En Aprobacion" , oFontDet8, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		else
			oPrint:SayAlign(nLinCab, COL_BRUTO+70,"Rechazada" , oFontDet8, 0300, nTamLin, CLR_BLACK, PAD_LEFT, 0)
		ENDIF
	...
	nLinCab += (nTamLin * 0.5)
	oPrint:SayAlign(nLinCab, COL_COD, "Item",     oFontDetN, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
	oPrint:SayAlign(nLinCab, COL_DESCR, "Codigo",     oFontDetN, 0270, nTamLin, CLR_BLACK, PAD_LEFT, 0)
	oPrint:SayAlign(nLinCab, COL_QFAT, "DESCRIPCION",     oFontDetN, 0270, nTamLin, CLR_BLACK, PAD_LEFT, 0)
	oPrint:SayAlign(nLinCab, COL_BRUTO, "U/M",     oFontDetN, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
	oPrint:SayAlign(nLinCab, COL_IVA, "CANTIDAD",     oFontDetN, 0070, nTamLin, CLR_BLACK, PAD_LEFT, 0)
...
Static Function fImpRod(lImp)//RODAPE PIE PAGINA
	...
	if lImp
		...
		oPrint:SayAlign(nLinRod, COL_QFAT-30, '____________________________', oFontRod, 400, 05, CLR_GRAY, PAD_LEFT,  0)
		oPrint:SayAlign(nLinRod, COL_PRECO-10, '____________________________', oFontRod, 400, 05, CLR_GRAY, PAD_LEFT,  0)
		oPrint:SayAlign(nLinRod, COL_IVA+10, '____________________________', oFontRod, 400, 05, CLR_GRAY, PAD_LEFT,  0)

		nLinRod += 7
		oPrint:SayAlign(nLinRod, nColIni+27,"RESPONSABLE"  , oFontRod, 100, 05, CLR_GRAY, , 0)
		oPrint:SayAlign(nLinRod, COL_QFAT-4,"SOLICITADO POR" , oFontRod, 100, 05, CLR_GRAY, , 0)
		oPrint:SayAlign(nLinRod, COL_PRECO+17,"JEFE DE PLANTA" , oFontRod, 100, 05, CLR_GRAY, , 0)
		oPrint:SayAlign(nLinRod, COL_IVA+35,"AUTORIZADO POR" , oFontRod, 100, 05, CLR_GRAY, , 0)
	else
		nLinRod += 7
	endif
	nLinRod += 20
