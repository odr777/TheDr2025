...
//    M->H6_QTDPROD := u_zLeBalanca("TOLEDO")
...
User Function zLeBalanca(cMarca)
    ...
    If ! Empty(cMarca)
        ...
        If cMarca == "LIDER"
            ...
            If !lRet
                MsgStop("<b>Falla</b> al conectar con la puerta serial. Detalles:"+;
                        "<br><b>Puerta:</b> "        +cPorta+;
                        "<br><b>Velocidad:</b> "    +cVelocid+;
                        "<br><b>Paridade:</b> "        +cParidade+;
                        "<br><b>Bits:</b> "            +cBits+;
                        "<br><b>Stop Bits:</b> "    +cStopBits,"Atención")
...
//				  AT( cPesq, cText ) // Resultado: 7
                nPosIni := At("<",cInfNah)+2
//                nnSPos := AT( "<", cInfNah ) // Resultado: 7
                //Obtendo apenas o peso da balança
                
                cKilos := SubStr(cInfNah,nPosIni,5) // kilos
                cGramos := SubStr(cInfNah,nPosIni+5,2) // obtengo peso
                cPesoLido = cKilos + "." + cGramos
//                Aviso("Info",cvaltochar(cInfNah),{"si"},,,,,.T.)
//                Aviso("Info",cvaltochar(cBuffer),{"si"},,,,,.T.)
                
//                Aviso("Info",cvaltochar(cPesoLido),{"si"},,,,,.T.)
            EndIf
