...
Local _cQuery 
Local _cCodigo :=""
Local _cPrefijo := SUBSTR(AllTrim(M->B1_TIPO) + AllTrim(M->B1_GRUPO),1,4)

If !Empty(M->B1_TIPO) .AND. !Empty(M->B1_GRUPO) .AND. INCLUI
		...
		_cQuery += " And LEFT(B1_GRUPO,2) = '" + SUBSTR(M->B1_GRUPO,1,2) + "' " 
		_cQuery += " And LEFT(B1_COD,4) = '" +_cPrefijo +"' "
