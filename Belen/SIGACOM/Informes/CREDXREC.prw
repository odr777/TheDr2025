...
Static Function ReportDef()
	...
	TRCell():New(oSection2,"CF_REC"				,"SF1","CF_REC", "@E 99,999,999,999.99", 17)
	TRCell():New(oSection2,"CRED_FISC"			,"SF1","CRED_FISC", "@E 99,999,999,999.99", 17)
	TRCell():New(oSection2,"SALDO"				,"SF1","SALDO", "@E 99,999,999,999.99", 17)
	TRCell():New(oSection2,"TES"				,"SD1","TES")

	oBreak = TRBreak():New(oSection2,oSection:Cell("F1_FORNECE"),"Sub Total por Proveedor")
	TRFunction():New(oSection2:Cell("CF_REC") ,NIL,"SUM",oBreak,/*cTitle*/,"@E 99,999,999,999.99"/*cPicture*/,/*uFormula*/,.F. /*lEndSection*/,.F./*lEndReport*/,.F./*lEndPage*/)
	TRFunction():New(oSection2:Cell("CRED_FISC") ,NIL,"SUM",oBreak,/*cTitle*/,"@E 99,999,999,999.99"/*cPicture*/,/*uFormula*/,.F. /*lEndSection*/,.F./*lEndReport*/,.F./*lEndPage*/)
	TRFunction():New(oSection2:Cell("SALDO") ,NIL,"SUM",oBreak,/*cTitle*/,"@E 99,999,999,999.99"/*cPicture*/,/*uFormula*/,.F. /*lEndSection*/,.F./*lEndReport*/,.F./*lEndPage*/)
...
Static Function PrintReport(oReport)
	...
	cParam17:= ALLTRIM(MV_PAR17)

	lParamOK:= validParamTES()
	If(!lParamOK)
		return
	EndIf
	...
	#IFDEF TOP
		...
		cQuery+= " SUM(CASE WHEN F4_GERALF='1' THEN D1_VALIMP1 ELSE 0 END) CRED_FISC, SD1.D1_TES TES"
		...
		cQuery+= " AND F1_FILIAL = D1_FILIAL AND F1_DOC = D1_DOC AND F1_SERIE = D1_SERIE AND SF1.F1_FORNECE = SD1.D1_FORNECE AND SF1.F1_LOJA = SD1.D1_LOJA "
		...
		cQuery+= " AND SD1.D1_TES IN(" + cParam17 + ")"
//09-03-2023 wico2k "contemplar valores en una sola moneda (bolivianos)"
		cQuery+= " AND F1_MOEDA = '1'		                                                                                                              "
		cQuery+= " GROUP BY F1_FORNECE, F1_LOJA, F1_HAWB, F1_DOC, F1_SERIE, SF1.F1_EMISSAO, SA2.A2_NOME, SD1.D1_TES                                                    "
		cQuery+= " UNION		                                                                                                              "
		cQuery+= " SELECT F1_FORNECE, F1_LOJA, F1_HAWB, F1_DOC, F1_SERIE, SF1.F1_EMISSAO, SA2.A2_NOME, "
		cQuery+= " SUM(CASE WHEN F4_GERALF='1' THEN 0 ELSE D1_VALIMP1*F1_TXMOEDA END) CF_REC, "
		cQuery+= " SUM(CASE WHEN F4_GERALF='1' THEN D1_VALIMP1*F1_TXMOEDA ELSE 0 END) CRED_FISC, SD1.D1_TES TES"
		cQuery+= " FROM " + RetSqlName("SF1") + " SF1, " + RetSqlName("SD1") + " SD1, " + RetSqlName("SF4") + " SF4, " + RetSqlName("SFC") + " SFC, " + RetSqlName("SA2") + " SA2 "
		cQuery+= " WHERE SF1.D_E_L_E_T_ = ' ' AND SD1.D_E_L_E_T_ = ' ' AND SF4.D_E_L_E_T_ = ' ' AND SFC.D_E_L_E_T_ = ' ' AND SA2.D_E_L_E_T_ = ' '          "
		cQuery+= " AND F1_FILIAL = D1_FILIAL AND F1_DOC = D1_DOC AND F1_SERIE = D1_SERIE AND SF1.F1_FORNECE = SD1.D1_FORNECE AND SF1.F1_LOJA = SD1.D1_LOJA "
		cQuery+= " AND SD1.D1_TES = SF4.F4_CODIGO                                                                                                          "
		...
		If __CUSERID $ '000000/000001' .Or. cUserName = 'jbravo'
			aviso("",cQuery,{'ok'},,,,,.t.)
		EndIf
		...
		While (cAliasQry)->(! EOF())
			...
			nSALDO:= (cAliasQry)->CF_REC - (cAliasQry)->CRED_FISC
			oSection2:Cell("SALDO"):SetValue( nSALDO )
			oSection2:Cell("TES"):SetValue( (cAliasQry)->TES )
...
* @since: 04/11/2021
* @description: Función que verifica si el parámetro MV_PAR17 de las TES esté correctamente ingresado
* @return: Valor Lógico(.T. / .F.)
*/
Static Function validParamTES()

	If(EMPTY(cParam17))
		alert("Debe indicar al menos una TES en los parámetros.")
		return .F.
	EndIf

	If( (";" $ cParam17) .OR. ("." $ cParam17) .OR. ("/" $ cParam17) .OR. ("'" $ cParam17) .OR. (",," $ cParam17))
		alert("Las TES deben estar separadas comas(,). Por favor revisar el parámetro.")
		return .F.
	EndIf

	If( RIGHT(cParam17, 1) == "," )
		cParam17:= LEFT(cParam17, (LEN(cParam17)-1))
	EndIf
...
Static Function CriaSX1(cPerg)  // Crear Preguntas
	...
	*/
	...
	xPutSX1(cPerg,"11","¿De Importación?" 	, "¿De Importación?"	,"¿De Importación?"	,"MV_CHB","C",20,0,0,"G","","","","","MV_PAR11",""       ,""            ,""        ,""     ,""   ,"")
	xPutSX1(cPerg,"12","¿A Importación?" 	, "¿A Importación?"		,"¿A Importación?"	,"MV_CHC","C",20,0,0,"G","NaoVazio()","","","","MV_PAR12",""       ,""            ,""        ,""     ,""   ,"")
	...
	xPutSX1(cPerg,"17","¿TES?" 				, "¿TES?"				,"¿TES?"			,"MV_CHH","C",60,0,0,"G","NaoVazio()","","","","MV_PAR17",""       ,""            ,""        ,""     ,""   ,"")//Las TES deben estar separadas por comas. Ejemplo: 001,002,003
