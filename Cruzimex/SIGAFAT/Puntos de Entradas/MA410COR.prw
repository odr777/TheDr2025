...
_aCores := {{"U_PVCorBrow() == 'ABERTO'",'ENABLE' },;		//Pedido em Aberto
			...
			{ "U_PVCorBrow() == 'VTAFUTU'",'BR_PINK' },;	//Pedido Abierto para venta Futura
		  	 {"U_PVCorBrow() == 'CONRA'" ,"BR_VERDE_ESCURO"},;   //Pedido de venta con RA -- Nahim Terrazas 21/03/2019
		  	 {"U_PVCorBrow() == 'CONCH'" ,"BR_AZUL_CLARO"},;   //Pedido de venta con Cheque -- Nahim Terrazas 21/03/2019
...
Do Case
       ...
       Case U_PVBloq() <> ""
       		Return cResp
       Case AllTrim(SC5->C5_NOTA)!='REMITO' .AND. !Empty(SC5->C5_LIBEROK).And.Empty(SC5->C5_NOTA).And. Empty(SC5->C5_BLQ).and.SC5->C5_LIBEROK<>'P' .and. (Empty(SC5->C5_USTATUS) .or. SC5->C5_USTATUS=='00')//Pedido Liberado
       		Return 'LIBERA'
