...
User Function IMPQUIBOL()
	...
	oPrint:say ( 0517, 1210, STR0005 + /*aInfo[4]*/ AllTrim(UPPER(SM0->M0_ENDCOB)), oFont09 )		//"DOMICILIO: "
	...
	*/
	...
	oPrint:say ( 0750, 0800, Transform(Day(dDtPagoQui),"99"), oFont09)
	oPrint:say ( 0750, 0850, ",", oFont09 )
	oPrint:say ( 0750, 1000, STR0030, oFont09 )    //"DE"
	oPrint:say ( 0750, 1070, MesExtenso(Month(dDtPagoQui)), oFont09)
	oPrint:say ( 0750, 1500, STR0030, oFont09 )    //"DE"
	oPrint:say ( 0750, 1570, Transform(Year(dDtPagoQui),"9999"), oFont09)
...
Static Function getPDs(cMat, cMes1, cMes2, cMes3)
	...
	BeginSql Alias OrdenConsul
		...
		COALESCE((SELECT RD_VALOR FROM SRD010 WHERE RD_PD = SRD.RD_PD AND RD_MAT = %exp:cMat% AND RD_PERIODO = %exp:cMes1% AND %notdel%), 0) AS MES1,
		COALESCE((SELECT RD_VALOR FROM SRD010 WHERE RD_PD = SRD.RD_PD AND RD_MAT = %exp:cMat% AND RD_PERIODO = %exp:cMes2% AND %notdel%), 0) AS MES2,
		COALESCE((SELECT RD_VALOR FROM SRD010 WHERE RD_PD = SRD.RD_PD AND RD_MAT = %exp:cMat% AND RD_PERIODO = %exp:cMes3% AND %notdel%), 0) AS MES3
		FROM SRD010 SRD
