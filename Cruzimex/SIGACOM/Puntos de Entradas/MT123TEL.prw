...
If nOpcx == 2 .OR. nOpcx == 4	//Visualizar o Modificar
	...
	cA123Impo := "001"   ///importador
	cDescImpo := Posicione("DB7",1,xFilial("DB7")+cA123Impo,"DB7_NOME")
	cA123Compr := "001" ////comprador
	cDescCompr := Posicione("SY1",1,xFilial("SY1")+cA123Compr,"Y1_NOME")
	nMoedaPed:= 2
	cDescMoed := "DOLAR"
	nTxMoeda:= RecMoeda(dDataBase,nMoedaPed)
	///pie
	cA123Emb := "003"
	cDescEmb := Posicione("DB8",1,xFilial("DB8")+cA123Emb,"DB8_NOME")
	cA123Via := "001"
