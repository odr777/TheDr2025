...
User Function obDesCont(nDescuento)
	...
	BeginSql alias cQuerDes
		...
		and	SD2.D_E_L_E_T_ = ''
		WHERE
		C5_NOTA = %exp:SD2->D2_DOC%  AND
		C5_SERIE = %exp:SD2->D2_SERIE%
	...
	if nDescuento == 1 // descuento nro 1
		nRespNt :=  (cQuerDes)->DESCUENTO4
	elseif nDescuento == 2 // descuento 2
		nRespNt :=  (cQuerDes)->DESCUENTO1
	elseif nDescuento == 3 // descuento 3
	...
	if  SD2->D2_ITEM == '01' // caso sea el primer item va a calcular la diferencia
		...
		if (cQuerDes)->DESCUENTO4 > 0.00 // debo aumentar al 1
		...
		elseif (cQuerDes)->DESCUENTO1 > 0.00 // debo aumentar al segundo
			if nDescuento == 2
				nRespNt -= ((cQuerTem2)->SUMADESC - SF2->F2_DESCONT)  // sumando la diferencia
