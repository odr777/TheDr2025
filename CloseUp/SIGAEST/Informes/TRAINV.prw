...
If _cTipoOp=='E'                                                                                        
	cQuery := "select F1_DOC,F1_EMISSAO,F1_FILORIG,B1_DESC,D1_CUSTO,(D1_QUANT*D1_CUSTO) as total, "//c6_num,
	cQuery += "D1_ITEM,D1_COD,D1_QUANT,D1_VUNIT,D1_TOTAL,D1_PEDIDO,D1_ITEMORI,D1_NFORI ,F1_MOEDA,NNR.NNR_DESCRI as almdes,D1_VUNIT,D1_TOTAL,F1_SERIE,D1_LOCAL,D1_LOTECTL,D1_LOCALIZ,D1_NUMSERI,D1_NFORI,D1_SERIORI,F1_USRREG,F1_FILIAL  "//B2_QATU,B2_CM1  "
...
Else
	cQuery := "select F2_FILIAL,F2_DOC,F2_EMISSAO,F2_FILDEST,B1_DESC,D2_CUSTO1,(D2_QUANT*D2_CUSTO1) as total,A2_NOME, max(A2_NOME) as cli1,min(A2_NOME) as cli2, "//c6_num,
...
TCQUERY cQuery NEW ALIAS "OrdenesCom"
 ...
 While OrdenesCom->(!EOF())
	...
	If _cTipoOp=='E'
		 ...
		 @ li,27/*50*/ PSAY SUBSTR(OrdenesCom->B1_DESC,1,40) 
		 	   
		 @ li,88/*118*/ PSAY OrdenesCom->D1_QUANT	Picture	 "@E 999,999.99"   
		 ...
		 //@ li,113 PSAY OrdenesCom->D1_total Picture "@E 99,999,999.99"	
		
		 	
			//If len(cNumPart)> 15		
			//@ li,20 PSay SubStr(cNumPart,1,15)
			@ li,60/*85*/ PSAY  OrdenesCom->D1_LOTECTL	   
   ...
   /*
	     ...
	     */
		 ...
		 @ li,27 PSay SubStr(OrdenesCom->B1_DESC,1,40/*len(OrdenesCom->B1_DESC)*/)
		 @ li,93/*118*/ PSAY OrdenesCom->D2_QUANT  
	     //@ li,98 PSAY XMOEDA(OrdenesCom->D1_VUNIT,OrdenesCom->F1_MOEDA,MV_PAR07,DDATABASE) Picture "@E 99,999,999.99"   
		 //@ li,113 PSAY XMOEDA(OrdenesCom->D1_total,OrdenesCom->F1_MOEDA,MV_PAR07,DDATABASE) Picture "@E 99,999,999.99"																									
		...
		//If Len(cNumPart)> 15 .Or. Len(OrdenesCom->B1_DESC)> 20 .OR. Len(OrdenesCom->almdes)> 25
			...
			//If len(cNumPart)> 15		
			//@ li,20 PSay cNumPart
		@ li,60/*85*/ PSAY  OrdenesCom->D2_LOTECTL //erick	   
		@ li,73/*85*/ PSAY  OrdenesCom->D2_LOCALIZ
			//Endif
...
*/   
 ...
 @ 012,25 PSAY "Descripci๓n"  
 @ 012,60 PSAY "Lote"	   
 @ 012,73 PSAY "Ubicaci๓n"
 @ 012,93 PSAY "Cantidad"													
 //@ 012,103 PSAY "Costo"												
 //@ 012,118 PSAY "Total"  
