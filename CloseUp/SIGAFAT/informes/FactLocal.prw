...
±±ºPrograma ³ GFATI301 ³Denar Terrazas/Erick Etcheverry	 º Data ³ 22/03/17º±±
...
static function FatMat(cDoc1,cDoc2,cSerie,cTitulo,bImprimir,nCopia,nLinInicial)
	...
	cSql		:= cSql+",F2_CLIENTE,F2_LOJA,F2_ESPECIE,F2_VEND1,D2_REMITO,D2_SERIREM,D2_CLIENTE,D2_COD,D2_ITEM,D2_FILIAL,D2_LOJA,D2_ITEMREM,FP_NUMAUT,FP_DTAVAL "
	cSql		:= cSql+" FROM " + RetSqlName("SF2 ") +" SF2 JOIN " + RetSqlName("SA1") +" SA1 ON (A1_COD=F2_CLIENTE AND SA1.D_E_L_E_T_ =' ' AND A1_LOJA=F2_LOJA  ) "
	...
	If (NextArea)->FP_DTAVAL >= (NextArea)->F2_EMISSAO//!Empty(FP_NUMAUT)	//Imprimir Serie que tengan Nro de Autorización en la Dosificación
		While !(NextArea)->(Eof())
			bSw		:= .T.
			//		If (F2_UNROIMP>0)
			//			If (VldImpFat()==.F.)
			...
			If ( bSw == .T.)
				...
				AADD(aDupla,(NextArea)->F2_VALFAT)          //9
				AADD(aDupla,(NextArea)->F2_DESCONT)         //10
				AADD(aDupla,(NextArea)->F2_VALFAT)  		//11
				...
				aDetalle:=FatDet ((NextArea)->F2_DOC,(NextArea)->F2_SERIE,(NextArea)->F2_FILIAL)                	   	//Datos detalle de factura
	...
	ELSE
		MSGINFO("La dosificacion no es vigente")
	endif
...
static function FatImp(cTitulo,bImprimir,aMaestro,aDetalle,nLinInicial,cTipo,cDireccion,cTelefono)
	...
	For nI:=1 to nDim
		...
		oPrn:Say(_nInterLin,100,FmtoValor(aDetalle[nI][4],10,2),oFont10)  //Cantidad
...
Static Function CabFact(nLinInicial,aMaestro,cTipo,cDireccion,cTelefono)
	...
	oPrn:Say( nLinInicial + 120, 1100,"FACTURA"  ,oFont14N )
	...
	oPrn:Say( nLinInicial + 330, 1550,"Actividad Economica: Construccion de Edificios completos"  ,oFont10N )
	oPrn:Say( nLinInicial + 370, 1550,"o de partes de edificios, Obras de Ingenieria Civil."  ,oFont11N )
	...
	oPrn:Say(nLinInicial + 645, 400, "CONCEPTO" ,oFont11N )
...
//	alert(aMaestro[1])
	//	if(aMaestro[1]=="0101")
	//		cFLogoMarkas := GetSrvProfString("Startpath","") + "lgrl01.bmp"
	//		oPrn:SayBitmap(090,80, cFLogoMarkas,400,200)
	//	end if
	cFLogo := GetSrvProfString("Startpath","") + "lgmid" + CEMPANT + aMaestro[1] + ".png"
	oPrn:SayBitmap(090,80, cFLogo,400,200)				//Logo de la empresa
	// if(aMaestro[1]=="0101")
	// 	cFLogoMarkas := GetSrvProfString("Startpath","") + "lgrl01.bmp"
	// 	oPrn:SayBitmap(090,80, cFLogoMarkas,400,200)
	// end if
	// if(aMaestro[1]=="0102")
	// 	cFLogoMarkas := GetSrvProfString("Startpath","") + "logo aaae.png"
	// 	oPrn:SayBitmap(090,80, cFLogoMarkas,400,200)
	// end if
	// if(aMaestro[1]=="0210")
	// 	cFLogoMarkas := GetSrvProfString("Startpath","") + "logo roquesanto.jpeg"
	// 	oPrn:SayBitmap(090,80, cFLogoMarkas,400,200)
	// end if
	// if(aMaestro[1]=="0211")
	// 	cFLogoMarkas := GetSrvProfString("Startpath","") + "logo_chimore.jpg"
	// 	oPrn:SayBitmap(090,80, cFLogoMarkas,400,200)
	// end if

	oPrn:Say(280, 80, "Ciabol Ltda." ,oFont11N )
...
static function FatDet (cDoc,cSerie,cFilialFat) // Datos detalle de factura
	Local 	aDupla		:= {}
	Local 	aDatos		:= {}
	Local 	cProducto	:= ""
	Local	cNombre		:= ""
	...
	// Local   cSql		:= "SELECT B1_COD,B1_DESC,D2_LOTECTL,D2_DTVALID,D2_QUANT,CASE WHEN D2_PRUNIT!=0 THEN D2_PRCVEN ELSE D2_PRCVEN END D2_PRUNIT,CASE WHEN D2_PRUNIT!=0 THEN D2_QUANT*D2_PRUNIT ELSE D2_QUANT*D2_PRCVEN END D2_TOTAL, D2_PEDIDO, D2_ITEMPV "
	Local   cSql		:= "SELECT B1_COD,B1_DESC,D2_LOTECTL,D2_DTVALID,D2_QUANT, round(D2_TOTAL / D2_QUANT,2) D2_PRUNIT,D2_TOTAL, D2_PEDIDO, D2_ITEMPV "
	cSql				:= cSql+"FROM " + RetSqlName("SD2") +" SD2 JOIN	" + RetSqlName("SB1") +" SB1 ON (D2_DOC='"+cDoc+"' AND D2_SERIE='"+cSerie+"' AND D2_FILIAL='"+cFilialFat+"' AND  D2_COD=B1_COD AND D2_ESPECIE='NF' AND SD2.D_E_L_E_T_=' ' AND SB1.D_E_L_E_T_=' ' ) "
	...
	While !(NextArea)->(Eof())
		...
		//	    End
		nTotal := Round((NextArea)->D2_TOTAL,2)
