...
cQuery += "C6_ENTREG,C6_ITEM,C6_PRODUTO,C5_ULOCAL,NNR_DESCRI,C6_UM, C6_QTDVEN,C6_PRCVEN,C6_VALOR, C6_LOTECTL,"
cQuery += "A1_NOME,A1_CGC,A1_END, CASE WHEN LEN(ISNULL(CONVERT(VARCHAR(2047), CONVERT(VARBINARY(2047), C6_UDESCLA)),''))=0 THEN B1_DESC " 
cQuery += "   ELSE ISNULL(CONVERT(VARCHAR(2047), CONVERT(VARBINARY(2047), C6_UDESCLA)),'') END C6_UESPECI, "
...
cQuery += "INNER JOIN " + RetSqlName("SE4") + " SE4 ON E4_CODIGO=C5_CONDPAG AND E4_FILIAL='"+ xfilial('SE4') +"'"
cQuery += "LEFT JOIN " + RetSqlName("NNR") + " NNR ON NNR_CODIGO=C5_ULOCAL AND NNR_FILIAL = C5_FILIAL AND NNR.D_E_L_E_T_ = ' '" 
...
oPrn:Say( Tpix(nLin+03.4), Tpix(01.0), "Deposito de Entrega: " + C5_ULOCAL + " - " + alltrim(NNR_DESCRI) 			, oFont10CN)
