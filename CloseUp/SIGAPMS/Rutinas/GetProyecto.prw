...
±±ºPrograma  ³GetProyecto  ºAutor  ³TdeP  º Data ³  05/03/17              º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Programa que carga automaticamente los datos para PMS en base º±±
±±º          ³al Item Contable a las rutinas de Factura de Entrada y Remitoº±±
...
*/
User Function GetProyecto(nPos)
	...
	nPosItemPMS   := aScan(aRatAFN,{|x| x[1] == aCols[nPos][nPosItem]}) ///ITEMS PARA INSERTAR
	...
	IF EMPTY(aCols[nPos][nPosItemCta])//SI NO TRAE  NADA DE ITEMS
		If nPosItemPMS > 0
			aDel(aRatAFN,1)
	...
	If nPosItemPMS > 0
		aColsPMS	:= aClone(aRatAFN[nPosItemPMS][2]) //COPIA EL PROYECTO

		//Aviso("nPosItem",u_zArrToTxt(aRatAFN, .T.),{'ok'},,,,,.t.) //
	Else
		nLenHeader := Len(aHeaderPMS)
		aadd(aColsPMS,Array(nLenHeader+1))
		For ny := 1 to nLenHeader
			If Trim(aHeaderPMS[ny,2]) == "AFN_ITEM"
				aColsPMS[1,ny] := "01"
	...
	//Aviso("acols",GetLastQuery()[2],{'ok'},,,,,.t.)

	//Aviso("acols",u_zArrToTxt(aColsPMS, .T.),{'ok'},,,,,.t.)
