...
Local aFilAux     := {}
Local cFilantAux  := ""
Local cTpValor	  := SuperGetMv("MV_TPVALOR",.F.,"D")
...
IF !EMPTY(MV_PAR05)
	oSection1:Cell("FILIAL_01"):SetBlock({|| ValorCTB(cArqTmp->COLUNA1,,,nTamValor,nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,cArqTmp->NORMAL,,,,,,lPrintZero,.F.)})	//	Movimento da Filial 01
ENDIF
IF !EMPTY(MV_PAR06)
	oSection1:Cell("FILIAL_02"):SetBlock({|| ValorCTB(cArqTmp->COLUNA2,,,nTamValor,nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,cArqTmp->NORMAL,,,,,,lPrintZero,.F.)})	//	Movimento da Filial 02
ENDIF
IF !EMPTY(MV_PAR07)
	oSection1:Cell("FILIAL_03"):SetBlock({|| ValorCTB(cArqTmp->COLUNA3,,,nTamValor,nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,cArqTmp->NORMAL,,,,,,lPrintZero,.F.)})	//	Movimento da Filial 03
ENDIF
IF !EMPTY(MV_PAR08)
	oSection1:Cell("FILIAL_04"):SetBlock({|| ValorCTB(cArqTmp->COLUNA4,,,nTamValor,nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,cArqTmp->NORMAL,,,,,,lPrintZero,.F.)})	//	Movimento da Filial 04
ENDIF
IF !EMPTY(MV_PAR09)
	oSection1:Cell("FILIAL_05"):SetBlock({|| ValorCTB(cArqTmp->COLUNA5,,,nTamValor,nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,cArqTmp->NORMAL,,,,,,lPrintZero,.F.)})	//	Movimento da Filial 05
ENDIF
IF !EMPTY(MV_PAR10)
	oSection1:Cell("FILIAL_06"):SetBlock({|| ValorCTB(cArqTmp->COLUNA6,,,nTamValor,nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,cArqTmp->NORMAL,,,,,,lPrintZero,.F.)})	//	Movimento da Filial 06
ENDIF
oSection1:Cell("TOTAL"):SetBlock({|| ValorCTB(cArqTmp->(IF(!EMPTY(MV_PAR05),COLUNA1,0)+IF(! EMPTY(MV_PAR06),COLUNA2,0)+IF(!EMPTY(MV_PAR07),COLUNA3,0)+IF(! EMPTY(MV_PAR08),COLUNA4,0)+IF(! EMPTY(MV_PAR09),COLUNA5,0)+IF( ! EMPTY(MV_PAR10),COLUNA6,0)),,,nTamValor,nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,cArqTmp->NORMAL,,,,,,lPrintZero,.F.)})	//	Total da Linha
...
IF !EMPTY(MV_PAR05)
	...
	Else
		TRFunction():New(oSection1:Cell("FILIAL_01"),nil,"ONPRINT"	,oBreak,/*Titulo*/,/*cPicture*/,;
			{ || (nTotFil1 := oTotFil1:GetValue(),ValorCTB(nTotFil1,,,nTamValor+Iif(nTotFil1==0,2,0),nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,"1",,,,,,lPrintZero,.F.)) },.F.,.F.,.F.,oSection1)
	Endif
	oTotFil1:Disable()
ENDIF
...
IF !EMPTY(MV_PAR07)
	...
	Else
		TRFunction():New(oSection1:Cell("FILIAL_03"),nil,"ONPRINT"	,oBreak,/*Titulo*/,/*cPicture*/,;
			{ || (nTotFil3 := oTotFil3:GetValue(),ValorCTB(nTotFil3,,,nTamValor+Iif(nTotFil3==0,2,0),nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,"2",,,,,,lPrintZero,.F.)) },.F.,.F.,.F.,oSection1)
	EndIF
	oTotFil3:Disable()
ENDIF
...
IF !EMPTY(MV_PAR08)
	...
	Else
		TRFunction():New(oSection1:Cell("FILIAL_04"),nil,"ONPRINT"	,oBreak,/*Titulo*/,/*cPicture*/,;
			{ || (nTotFil4 := oTotFil4:GetValue(),ValorCTB(nTotFil4,,,nTamValor+Iif(nTotFil4==0,2,0),nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,"2",,,,,,lPrintZero,.F.)) },.F.,.F.,.F.,oSection1)
	EndIF
	oTotFil4:Disable()
ENDIF
...
IF !EMPTY(MV_PAR09)
	...
	Else
		TRFunction():New(oSection1:Cell("FILIAL_05"),nil,"ONPRINT"	,oBreak,/*Titulo*/,/*cPicture*/,;
			{ || (nTotFil5 := oTotFil5:GetValue(),ValorCTB(nTotFil5,,,nTamValor+Iif(nTotFil5==0,2,0),nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,"2",,,,,,lPrintZero,.F.)) },.F.,.F.,.F.,oSection1)
	EndIF
	oTotFil5:Disable()
ENDIF
...
IF !EMPTY(MV_PAR10)
	...
	Else
		TRFunction():New(oSection1:Cell("FILIAL_06"),nil,"ONPRINT"	,oBreak,/*Titulo*/,/*cPicture*/,;
			{ || (nTotFil6 := oTotFil6:GetValue(),ValorCTB(nTotFil6,,,nTamValor+Iif(nTotFil6==0,2,0),nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,"2",,,,,,lPrintZero,.F.)) },.F.,.F.,.F.,oSection1)
	EndIF
	oTotFil6:Disable()
ENDIF
...
Else
	TRFunction():New(oSection1:Cell("TOTAL"),nil,"ONPRINT"	,oBreak,/*Titulo*/,/*cPicture*/,{ || (nTotGeral := oTotGeral:GetValue(),;
		ValorCTB(nTotGeral,,,nTamValor+Iif(nTotGeral==0,2,0),nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,"2",,,,,,lPrintZero,.F.)) },.F.,.F.,.F.,oSection1)
EndIF
...
IF !EMPTY(MV_PAR05)
	...
	Else
		TRFunction():New(oSection1:Cell("FILIAL_01"),nil,"ONPRINT"	,oBreakGrupo,/*Titulo*/,/*cPicture*/,;
			{ || (nTotGrp1 := iif(oTotGrp1:GetValue()==nil,0,oTotGrp1:GetValue()),ValorCTB(nTotGrp1,,,nTamValor+Iif(nTotGrp1==0,2,0),nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,"1",,,,,,lPrintZero,.F.)) },.F.,.F.,.F.,oSection1)
	EndIF
	oTotGrp1:Disable()
ENDIF
...
IF !EMPTY(MV_PAR06)
	...
	Else
		TRFunction():New(oSection1:Cell("FILIAL_02"),nil,"ONPRINT"	,oBreakGrupo,/*Titulo*/,/*cPicture*/,;
			{ || (nTotGrp2 := iif(oTotGrp2:GetValue()==nil,0,oTotGrp2:GetValue()),ValorCTB(nTotGrp2,,,nTamValor+Iif(nTotGrp2==0,2,0),nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,"2",,,,,,lPrintZero,.F.)) },.F.,.F.,.F.,oSection1)
	EndIF
	oTotGrp2:Disable()
ENDIF
...
IF !EMPTY(MV_PAR07)
	...
	Else
		TRFunction():New(oSection1:Cell("FILIAL_03"),nil,"ONPRINT"	,oBreakGrupo,/*Titulo*/,/*cPicture*/,;
			{ || (nTotGrp3 := iif(oTotGrp3:GetValue()==nil,0,oTotGrp3:GetValue()),ValorCTB(nTotGrp3,,,nTamValor+Iif(nTotGrp3==0,2,0),nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,"2",,,,,,lPrintZero,.F.)) },.F.,.F.,.F.,oSection1)
	EndIF
	oTotGrp3:Disable()
ENDIF
...
IF !EMPTY(MV_PAR08)
	...
	Else
		TRFunction():New(oSection1:Cell("FILIAL_04"),nil,"ONPRINT"	,oBreakGrupo,/*Titulo*/,/*cPicture*/,;
			{ || (nTotGrp4 := iif(oTotGrp4:GetValue()==nil,0,oTotGrp4:GetValue()),ValorCTB(nTotGrp4,,,nTamValor+Iif(nTotGrp4==0,2,0),nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,"2",,,,,,lPrintZero,.F.)) },.F.,.F.,.F.,oSection1)
	EndIF
	oTotGrp4:Disable()
ENDIF
...
IF !EMPTY(MV_PAR09)
	...
	Else
		TRFunction():New(oSection1:Cell("FILIAL_05"),nil,"ONPRINT"	,oBreakGrupo,/*Titulo*/,/*cPicture*/,;
			{ || (nTotGrp5 := iif(oTotGrp5:GetValue()==nil,0,oTotGrp5:GetValue()),ValorCTB(nTotGrp5,,,nTamValor+Iif(nTotGrp5==0,2,0),nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,"2",,,,,,lPrintZero,.F.)) },.F.,.F.,.F.,oSection1)
	EndIF
	oTotGrp5:Disable()
ENDIF
...
IF !EMPTY(MV_PAR10)
	...
	Else
		TRFunction():New(oSection1:Cell("FILIAL_06"),nil,"ONPRINT"	,oBreakGrupo,/*Titulo*/,/*cPicture*/,;
			{ || (nTotGrp6 := iif(oTotGrp6:GetValue()==nil,0,oTotGrp6:GetValue()),ValorCTB(nTotGrp6,,,nTamValor+Iif(nTotGrp6==0,2,0),nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,"2",,,,,,lPrintZero,.F.)) },.F.,.F.,.F.,oSection1)
	EndIF
	oTotGrp6:Disable()
ENDIF
...
Else
	TRFunction():New(oSection1:Cell("TOTAL"),nil,"ONPRINT"	,oBreakGrupo,/*Titulo*/,/*cPicture*/,;
		{ || (nTotGrpGeral := iif(oTotGrpGeral:GetValue()==nil,0,oTotGrpGeral:GetValue()),ValorCTB(nTotGrpGeral,,,nTamValor+Iif(nTotGrpGeral==0,2,0),nDecimais,Iif(cTpValor == 'S', .T., CtbSinalMov()),cPicture,"2",,,,,,lPrintZero,.F.)) },.F.,.F.,.F.,oSection1)
EndIF
