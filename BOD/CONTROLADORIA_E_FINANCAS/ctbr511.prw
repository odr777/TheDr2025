...
±±³Fun‡…o	 ³ CtbR511   ³ Autor³ PAULO AUGUSTO     	³ Data ³ 04/01/07 ³±±
...
±±³Jonathan Glz³26/06/15³PCREQ-4256³Se elimina la funcion CTR511SX1() la  ³±±
...
Function CtbR511(lExterno,aPerg,cReport,cTitulo,cDesc,nomeprog,cPerg,aDescCab)                           

Private dFinalA
Private dFinal
Default aDescCab:={}
Default CPERG	:= "CTR511"                                     
Default nomeprog:="CTBR511"    
...
Else
	If !CTG->( DbSeek( xFilial( "CTG" ) + mv_par01 ) )
		MsgAlert( STR0009 ) // "Exercicio Contabil nao encontrado verifique o parametro..."
		Return .F.		
	EndIf
EndIf
...
oReport:SetCustomText( {||		 CTBR511CAB(     ,      ,     ,      ,      ,dFinal,ctitulo,          ,     ,       ,    ,oReport,aDescCab,nomeprog) } )
...
If mv_par05 = 2
	...
	If cArqTmp->COLUNA < 2
	oSection1:Cell("COLUNA1" ):SetBlock( { || Iif( Ctb511Imp(cArqTmp->IDENTIFI),;
															  ValorCTB(cArqTmp->MOVIMENTO,,,aTamVal[1],nDecimais,.T.,cPicture,;
														     cArqTmp->NORMAL,cArqTmp->CONTA,,,cTpValor,,,.F.),"") } )	
	Else
	oSection1:Cell("COLUNA2" ):SetBlock( { || Iif( Ctb511Imp(cArqTmp->IDENTIFI),;
															  ValorCTB(cArqTmp->MOVIMENTO,,,aTamVal[1],nDecimais,.T.,cPicture,;
														     cArqTmp->NORMAL,cArqTmp->CONTA,,,cTpValor,,,.F.),"") } )		
	EndIf													     
Else
...
±±³ Fun‡…o    ³CTBR511CAB ³ Autor ³ Paulo Augusto        ³ Data ³ 22/01/07 ³±±
...
Function CTBR511CAB(lItem,lCusto,lCLVL,Cabec1,Cabec2,dDataFim,Titulo  ,lAnalitico,cTipo,Tamanho,aCab,oReport,aDescCab,nomeprog)
...
Static Function Ctb511Imp(cIdent)
