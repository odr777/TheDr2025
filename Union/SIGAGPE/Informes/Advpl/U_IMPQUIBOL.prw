...
User Function IMPQUIBOL()
	...
	oPrint:say ( 0729, 1410, /*STR0012*/"PERIODO PAGADO: " + DtoC(getFchInd()) + " - " + DtoC(DaySub(YearSum(getFchInd(), nQuiPg), 1)), oFont09 )    //"FECHA DE RETIRO"
	oPrint:line( 0774, 0062, 0774, nColMax-21 ) 		   		//Linha Horizontal

	cGlosa:= glosaQuinquenio()//ODialog para solicitar Glosa al usuario

	oPrint:say ( 0782, 0064, "PAGO DE INDEMNIZACION: "+cGlosa, oFont09 )    //STR0013 "PAGO DE INDEMNIZACION: " ## STR0075"PAGO "##"QUINQUENIO" STR0001
...
* @author: Denar Terrazas Parada
* @since: 12/02/2021
* @description: Funcion que presenta una pantalla para que el usuario digite una glosa para el quinquenio
*/
Static Function glosaQuinquenio()
	Local aArea		:= getArea()
	Local cMat		:= SRA->RA_MAT
	Local cNombre	:= If( !Empty(SRA->RA_NOMECMP), SRA->RA_NOMECMP, SRA->RA_NOME )
	Local cGlosa	:= "PAGO "+cCausa+". "+"QUINQUENIO" + SPACE(15)
	Local cAux		:= cGlosa
	Local cRet		:= ""
	Local oDlg
	Local oDlg, oFont, oSay

	DEFINE DIALOG oDlg TITLE "GLOSA QUINQUENIO" FROM 180,180 TO 315,670 PIXEL

	@ 10,10 SAY oSay PROMPT ("MATRÍCULA: " + cMat) SIZE 200,20 OF oDlg PIXEL
	@ 20,10 SAY oSay PROMPT ("NOMBRE: " + cNombre) SIZE 200,20 OF oDlg PIXEL

	@ 31,10 SAY oSay PROMPT "PAGO DE INDEMNIZACIÓN: " SIZE 200,20 OF oDlg PIXEL
	@ 30,80 MSGET cGlosa SIZE 160,008 OF oDlg PIXEL Picture "@!" //VALID !Vazio()

	@ 50,200 BUTTON "&Aceptar" SIZE 040,010 ACTION oDlg:End() OF oDlg PIXEL
