	...
	If !lRotinaEai 
		...
		SE2->( dbSetOrder(nIndex+1))
  		...
  		If dbSeek(cPrefix+RC1->RC1_NUMTIT, .F.)
			While !Eof() .And. SE2->E2_FILIAL == RC1->RC1_FILCEN .AND. SE2->E2_NUM == RC1->RC1_NUMTIT .And. cPrefix == SE2->E2_PREFIXO
				lRetBx	:= .T.
			...
			If lRetBx
				...
				lExclui	:=	.T.
				While  SE2-> ( !Eof() ) .AND. SE2->E2_FILIAL == RC1->RC1_FILCEN .And. SE2->E2_NUM == RC1->RC1_NUMTIT .And. cPrefix == SE2->E2_PREFIXO
					...
					Else
						...
						//³ Crear parámetro MV_PREASTO con el prefijo de los asientos que	³
						//³ han sido generados a partir de la integración GPE - FIN 		³
						//³ Ejemplo. MV_PREASTO = "RHR"	³
						//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
						
						If Alltrim(SE2->E2_ORIGEM) == "GPEM670" .AND. SE2->E2_PREFIXO == GetMV("MV_PREASTO")
							/*
							RecLock("SE2",.F.,.F.)
							dbDelete()
							MsUnLock()
							...
							*/
							...
							If lMsErroAuto
							    ...
							    MostraErro()
							Else
								FWAlertSuccess("Se procedió a revertir el asiento.", "Título excluído satisfactoriamente")
							Endif
						else
							RecLock("SE2",.F.,.F.)
							dbDelete()
							MsUnLock()
							SE2->( FKCOMMIT() )	
							FWAlertSuccess("No corresponde reversión contable.", "Título excluído satisfactoriamente")
						Endif
					EndIf
					SE2-> ( dbSkip() )
				Enddo
