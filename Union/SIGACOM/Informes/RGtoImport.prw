...
Static Function ReportDef()
	...
	oSection6 := TRSection():New(oReport,"TRANSFERENCIA DE SALIDA",,)
	...
	oSection7 := TRSection():New(oReport,"SALDOS POR TRANSFERIR",,)
...
Static Function ReportPrint(oReport,nTamVal)
	...
	While TMP0->(!Eof())
		...
		cQuery2:= " SELECT DISTINCT F1_FILIAL,F1_HAWB F1_UPOLIZA,F1_TIPODOC,F1_DOC,F1_SERIE,F1_FORNECE,F1_LOJA,A2_NOME,F1_EMISSAO,"
		...
		cQuery2+= " GROUP BY  F1_FILIAL, F1_HAWB, F1_TIPODOC, F1_DOC, F1_SERIE, F1_FORNECE, F1_LOJA, A2_NOME, F1_EMISSAO, F1_MOEDA, F1_VALBRUT, M2_MOEDA2, F1_TXMOEDA, F1_ESPECIE "
		cQuery2+= " UNION "
		...
		cQuery6+= " ON F2_FILIAL = D2_FILIAL AND F2_DOC = D2_DOC AND F2_SERIE = D2_SERIE AND SD2.D_E_L_E_T_!='*' "
		cQuery6+= " LEFT JOIN "+ RetSQLname('SA2') + " SA2 ON F2_CLIENT = A2_COD AND F2_LOJA = A2_LOJA AND SA2.D_E_L_E_T_!='*'"
		...
		cQuery7+= " AND F2_DOC = D2_DOC AND F2_SERIE = D2_SERIE WHERE F2_HAWB = SF1.F1_HAWB AND D2_COD = SD1.D1_COD AND F2_TIPODOC IN ('54') "
		...
		cQuery7+= " AND F1_SERIE = D1_SERIE AND SD1.D_E_L_E_T_!='*' LEFT JOIN " + RetSQLname('SA2') + " SA2 ON F1_FORNECE = A2_COD "
		cQuery7+= " AND F1_LOJA = A2_LOJA AND SA2.D_E_L_E_T_!='*' LEFT JOIN " + RetSQLname('SB1') + " SB1 ON D1_COD = B1_COD AND SB1.D_E_L_E_T_!='*'"
