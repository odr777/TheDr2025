...
User Function ConsListaP(_lSXB)
	...
	Private L110AUTO:=.F.
	DEFAULT _lSXB	:= .T.
	Private lastSelected := 0 // último NTP 23
	...
	AADD(_aGets,{"B1_UFABRIC",Space(15)})
...
//	alert("Nahim")
	...
	If lBrowse
		...
		@ 055,050 MSGET _aGets[4,2] SIZE 100,008 OF oDlg PIXEL  Picture PesqPict("SB1","B1_UFABRIC")
		...
		//±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
		oListProd := TWBrowse():New(  070,05,545,065,,{"Codigo","Descr.","Cod.Fabrica","Un.Medida","Desc.UM","2da.Medida","Desc.UM2","Descr. Especifica","Grupo","Marca"},{30,150,80,20,60,20,60,300,40,40},oDlg,,,,,{||IF(LEN(_aBuscaProd)>0,MostrarDatos(_aBuscaProd[oListProd:nAT,1],.T.),.t.)},,,,,,,.F.,,.T.,,.F.,,,)
	    ...
	    oListProd:bLDblClick := {||IF(LEN(_aBuscaProd)>0,MostrarDatos(_aBuscaProd[oListProd:nAT,1],.T.),.t.)}
	...
	IF BUSCAPROD->(!EOF()) .AND. BUSCAPROD->(!BOF())
		...
		WHILE BUSCAPROD->(!EOF())
			if _nCont==1
				...
				_aBuscaProd[1,3]:=alltrim(BUSCAPROD->B1_UFABRIC)
				...
				_aBuscaProd[1,8]:=alltrim(BUSCAPROD->B1_ESPECIF)
			...
			Else
				...
				AADD(_aBuscaProd,{BUSCAPROD->B1_COD,alltrim(BUSCAPROD->B1_DESC),alltrim(BUSCAPROD->B1_UFABRIC),alltrim(BUSCAPROD->B1_UM),ALLTRIM(Posicione("SAH", 1, XFilial("SAH") + BUSCAPROD->B1_UM , "AH_UMRES")),alltrim(BUSCAPROD->B1_SEGUM),ALLTRIM(Posicione("SAH", 1, XFilial("SAH") + BUSCAPROD->B1_SEGUM , "AH_UMRES")),alltrim(BUSCAPROD->B1_ESPECIF),alltrim(BUSCAPROD->B1_GRUPO),alltrim(BUSCAPROD->B1_UMARCA)})
			End
			BUSCAPROD->(DbSkip())
		END
	End
...
Static Function MostraLista(cProduto,_lSim,cArmazem,lBrowse,aArmazem)
	...
	If DbSeek(xFilial("SB1")+cProduto,.f.)
		...
		// _cNomeEmp := Alltrim(SM0->M0_NOME)+" / "+Alltrim(SM0->M0_FILIAL)
		// _cCodEmp  := SM0->(M0_CODIGO+M0_CODFIL)
		_cNomeEmp := ALLTRIM(FWCompanyName()) + " / " + ALLTRIM(FWFilialName()) // Alltrim(SM0->M0_NOME)+" / "+Alltrim(SM0->M0_FILIAL)
		_cCodEmp  := FWCodEmp()+fwCodFil()// SM0->(M0_CODIGO+M0_CODFIL)
		...
		// 	Endif
			...
			If nTC > 0
				...
				cQuery += "FROM DA1010 DA1 , DA0010 DA0 "
				cQuery += "WHERE DA1_FILIAL = '"+ xFilial("DA1") +"' and "
				cQuery += "      DA1_CODPRO = '"+_cProdSel+"' and "
