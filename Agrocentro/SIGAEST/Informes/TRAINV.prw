...
//"+RetSqlName("SYA")+" SYA On B1_UPROCE=YA_CODGI "
	...
	If _cTipoOp=='E'
		cQuery := "select F1_DOC,F1_EMISSAO,F1_FILORIG,B1_DESC,D1_CUSTO,(D1_QUANT*D1_CUSTO) as total, "
		cQuery += "D1_ITEM,D1_COD,D1_QUANT,D1_VUNIT,D1_TOTAL,D1_PEDIDO,D1_ITEMORI,D1_NFORI ,F1_MOEDA,NNR.NNR_DESCRI as almdes,D1_VUNIT,D1_TOTAL,F1_SERIE,D1_LOCAL,D1_LOTECTL,D1_LOCALIZ,D1_NUMSERI,D1_NFORI,D1_SERIORI,F1_USRREG,F1_FILIAL  "
		...
		cQuery += "left outer Join " + RetSqlName("SD2") + " SD2 on D2_doc=D1_doc and D2_filial=F1_FILORIG and D2_serie=D1_SERIORI and D2_TIPODOC='54' AND D1_COD = D2_COD and SD2.D_E_L_E_T_!='*' AND D1_ITEM = D2_ITEM AND D1_FORNECE = D2_CLIENTE "
	...
	Else
		cQuery := "select F2_FILIAL,F2_DOC,F2_EMISSAO,F2_FILDEST,B1_DESC,D2_CUSTO1,(D2_QUANT*D2_CUSTO1) as total,A2_NOME, max(A2_NOME) as cli1,min(A2_NOME) as cli2, "
		cQuery += "D2_ITEM,D2_COD,D2_QUANT,D2_PRCVEN,D2_TOTAL,D2_PEDIDO,D2_ITEMORI,D2_NFORI ,F2_MOEDA,NNR.NNR_DESCRI as almdes,D2_LOCDEST,F2_SERIE,D2_LOCAL,D2_LOTECTL,D2_LOCALIZ,D2_NUMSERI,F2_USRREG   "
		cQuery += "from " + RetSqlName("SF2") + " SF2 "
		cQuery += "inner Join " + RetSqlName("SD2") + " SD2 ON D2_ESPECIE=F2_ESPECIE AND F2_DOC=D2_DOC and F2_FILIAL=D2_FILIAL and F2_SERIE=D2_SERIE and D2_TIPODOC='54' "
		...
		cQuery += " Join " + RetSqlName("SA2") + " SA2 on A2_COD=F2_CLIENTE and F2_LOJA=A2_LOJA and A2_COD='"+trim(mv_par09)+"'"
		cQuery += " and sf2.d_e_l_e_t_=' ' and sd2.d_e_l_e_t_=' '  and sa2.d_e_l_e_t_=' ' "//and sb1.d_e_l_e_t_=' ' "
		cQuery += " group by F2_FILIAL,F2_DOC,F2_EMISSAO,F2_FILDEST,B1_DESC,D2_CUSTO1,D2_ITEM,D2_COD,D2_QUANT,D2_PRCVEN,D2_TOTAL,D2_PEDIDO,D2_ITEMORI,D2_NFORI,F2_MOEDA,NNR.NNR_DESCRI,D2_PRCVEN,D2_TOTAL,A2_NOME,D2_LOCDEST,F2_SERIE,D2_LOCAL,D2_LOTECTL,D2_LOCALIZ,D2_NUMSERI,F2_USRREG    "//,c6_num,
...
//MemoWrite("\TRAINV.sql",cQuery)  
	...
	While OrdenesCom->(!EOF())
		...
		If _cTipoOp=='E'
			...
			cNumPart := POSICIONE("SB1",1,XFILIAL("SB1")+OrdenesCom->D1_COD,"B1_NUMPART")

			@ li,02 PSAY OrdenesCom->D1_COD
			@ li,22 /*50*/ PSAY cNumPart

			@ li,42 /*50*/ PSAY SUBSTR(OrdenesCom->B1_DESC,1,40)

			@ li,84/*85*/ PSAY  OrdenesCom->D1_LOTECTL
			@ li,97/*85*/ PSAY  OrdenesCom->D1_LOCALIZ

			@ li,117/*118*/ PSAY OrdenesCom->D1_QUANT	Picture	 "@E 999,999.99"
		...
		Else
			cNumPart := POSICIONE("SB1",1,XFILIAL("SB1")+OrdenesCom->D2_COD,"B1_NUMPART")

			@ li,02 PSAY OrdenesCom->D2_COD

			@ li,22 PSay cNumPart

			@ li,42 PSay SubStr(OrdenesCom->B1_DESC,1,40/*len(OrdenesCom->B1_DESC)*/)

			@ li,84/*85*/ PSAY  OrdenesCom->D2_LOTECTL //erick
			@ li,97/*85*/ PSAY  OrdenesCom->D2_LOCALIZ
			@ li,117/*118*/ PSAY OrdenesCom->D2_QUANT
...
if OrdenesCom->cli1==OrdenesCom->cli2 
  ...
  iF _cTipoOp=='S'
  	@ 007,02 PSAY "SUCURSAL ORIGEN:  " + OrdenesCom->F2_FILIAL + "     DEPOSITO ORIGEN:  " + POSICIONE('NNR',1,XFILIAL('NNR')+ OrdenesCom->D2_LOCAL,'NNR_DESCRI')	
  ...
  ELSE
  @ 007,02 PSAY  "SUCURSAL ORIGEN:  " + OrdenesCom->F1_FILORIG + "     DEPOSITO ORIGEN:  " + POSICIONE('NNR',1,XFILIAL('NNR')+ Posicione("SD2",3,OrdenesCom->F1_FILORIG+OrdenesCom->(D1_NFORI+D1_SERIORI),"D2_LOCAL"),'NNR_DESCRI')	
  @ 008,02 psay "SUCURSAL DESTINO: "+ OrdenesCom->F1_FILIAL + "     DEPOSITO DESTINO: " + POSICIONE('NNR',1,XFILIAL('NNR')+ OrdenesCom->D1_LOCAL,'NNR_DESCRI')
...
*/   
	...
	@ 012,02 PSAY Chr(18)+ "Codigo"
	@ 012,20 PSAY "Nro Parte"
	@ 012,40 PSAY "Descripción"
	@ 012,82 PSAY "Lote"
	@ 012,95 PSAY "Ubicación"
	@ 012,115 PSAY "Cantidad"
...
Static Function ValidPerg(cPerg)
	...
	aAdd(aRegs,{"07","De Serie                :","mv_ch7","C",3,0,0,"G","mv_par07",""       ,""            ,""        ,""     ,"",""})
	aAdd(aRegs,{"08","A Serie                 :","mv_ch8","C",3,0,0,"G","mv_par08",""       ,""            ,""        ,""     ,"",""})
