...
static function FatMat(cDoc1,cDoc2,cSerie,cTitulo,bImprimir,nCopia,nLinInicial,cxTitulo)
	...
	local nI
	local i
	Local cSql	:= "SELECT A1_CGC,A1_NOME,F2_FILIAL,F2_REFTAXA,F2_REFMOED,F2_SERIE,F2_DOC,F2_NUMAUT,F2_EMISSAO,F2_UNOMCLI,F2_UNITCLI,F2_COND,F2_VALFAT,ROUND(F2_DESCONT,2) F2_DESCONT,F2_BASIMP1,F2_CODCTR,FP_DTAVAL,F2_USRREG,D2_PEDIDO,A1_MUN, A1_BAIRRO,A1_END"
	...
	if(! EMPTY(F2_NUMAUT))//No deja imprimir si el numero de autorizacion esta mal realizado
		While !Eof()
			...
			If ( bSw == .T.)
				...
				AADD(aDupla,IIF(!EMPTY(F2_UNOMCLI),F2_UNOMCLI,A1_NOME))         //6
...
Static Function CabFact(nLin,aMaestro,cTipo,cDireccion,cTelefono)
	...
	oPrn:Say( nLinInicial + 390,  160,"Telefono: 67894613 * Telf: 3258881"  ,oFont09 )
	oPrn:Say( nLinInicial + 430,  250,"Santa Cruz - Bolivia"  ,oFont09 )
