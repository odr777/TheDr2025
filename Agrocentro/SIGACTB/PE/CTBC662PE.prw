...
±±³Funcao    ³ CTBC662 ³ Autor ³ Erick Etcheverry       ³ Data ³ 19.09.22 ³±±
...
static function llamaAsiento(aCpoOri,__aDocOri,__aLanCT2)
	...
	If SX1->(DbSeek(cPregNah+Space(4)+'01'))
		RecLock('SX1',.F.)
		SX1->X1_CNT01 := DTOS(onModeloGrid:GetValue("CT2_DATA" )) //DTOS(onModeloGrid:GetValueByPos(2,1))
		SX1->(MsUnlock())
	End
	...
	If SX1->(DbSeek(cPregNah+Space(4)+'03'))
		RecLock('SX1',.F.)
		SX1->X1_CNT01 :=  onModeloGrid:GetValue("CT2_LOTE" )
		SX1->(MsUnlock())
	END
	...
	If SX1->(DbSeek(cPregNah+Space(4)+'05'))
		RecLock('SX1',.F.)
		SX1->X1_CNT01 :=  onModeloGrid:GetValue("CT2_DOC" )
		SX1->(MsUnlock())
	END
	If SX1->(DbSeek(cPregNah+Space(4)+'06'))
		...
		SX1->(MsUnlock())
	END
	If SX1->(DbSeek(cPregNah+Space(4)+'07'))///NUMERO LOTE
		RecLock('SX1',.F.)
		SX1->X1_CNT01 :=  "01"
		SX1->(MsUnlock())
	END
	If SX1->(DbSeek(cPregNah+Space(4)+'12'))  ///SUBLOTE
		RecLock('SX1',.F.)
		SX1->X1_CNT01 :=  '3'
		SX1->(MsUnlock())
	END
	If SX1->(DbSeek(cPregNah+Space(4)+'15'))
		RecLock('SX1',.F.)
		SX1->X1_CNT01 :=  onModeloGrid:GetValue("CT2_SBLOTE" ) //onModeloGrid:GetValueByPos(4,1)
		SX1->(MsUnlock())
	END
	If SX1->(DbSeek(cPregNah+Space(4)+'16'))/////DOCUMENTO
		RecLock('SX1',.F.)
		SX1->X1_CNT01 :=  onModeloGrid:GetValue("CT2_SBLOTE" )
		SX1->(MsUnlock())
	END
