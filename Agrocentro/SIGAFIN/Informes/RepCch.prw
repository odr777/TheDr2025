...
Static Function FA565Imp(lEnd,wnRel,cString)
	...
	cQuery += " AND EU_TIPO IN ('10','92','11')"
	...
	While !Eof()
		...
		li++
		IF (ALLTRIM(SEU->EU_TIPO))$'11'
			IF substr(ALLTRIM(SEU->EU_CAIXA),1,1)$'F'
				@ li,03 PSAY __PrtCenter("CIERRE DE FONDOS A RENDIR")	// Descri‡Æo do tipo de recibo
			ELSE
				@ li,03 PSAY __PrtCenter("CIERRE DE CAJA CHICA")	// Descri‡Æo do tipo de recibo
			END
		else
			IF substr(ALLTRIM(SEU->EU_CAIXA),1,1)$'F'
				@ li,03 PSAY __PrtCenter("FONDOS A RENDIR")	// Descri‡Æo do tipo de recibo
			else
		...
		//	END
		@ li,03 PSAY __PrtCenter("No.: " +EU_NUM)	// Nro do Recibo
		...
		cGlosa := ALLTRIM(SEU->EU_UGLOSA)

		li+=2
		IF (ALLTRIM(SEU->EU_TIPO))$'11'
			@ li ,03 PSAY "BANCO/CAJA PARA CIERRE" // + ALLTRIM(SET->ET_BANCO) + "   " + ALLTRIM(SET->ET_AGEBCO) + "   " + ALLTRIM(SET->ET_CTABCO)
		ELSE
			@ li ,03 PSAY "BANCO/CAJA" // + ALLTRIM(SET->ET_BANCO) + "   " + ALLTRIM(SET->ET_AGEBCO) + "   " + ALLTRIM(SET->ET_CTABCO)
		END
		li+=2
		IF (ALLTRIM(SEU->EU_TIPO))$'11'
			@ li ,03 PSAY "CÓDIGO: " + ALLTRIM(SEU->EU_BANCO) + "/" + ALLTRIM(SEU->EU_AGENCI) + "/" + ALLTRIM(SEU->EU_CONTARE)
		else	
		...
		IF (ALLTRIM(SEU->EU_TIPO))$'11'
			cBanco:= Posicione("SA6", 1, xFilial("SA6") + SEU->EU_BANCO, "SA6->A6_NOME")
		else
		...
		li+=2
		IF substr(ALLTRIM(SEU->EU_CAIXA),1,1)$'F'
			@ li ,03 PSAY "CÓDIGO FONDO A RENDIR: " + SEU->EU_CAIXA
		else
			@ li ,03 PSAY "CÓDIGO CAJA CHICA: " +  SEU->EU_CAIXA
		End
		li+=2
		//	IF SUBSTR(ALLTRIM(SEU->EU_TIPDEB),2,2)== 'CH'
		//		@ li ,03 PSAY "BANCO/CAJA: " + SET->ET_NOME
		//	else
		//		@ li ,03 PSAY "BANCO/CAJA: " + SET->ET_NOME
		//	End
		...
		cMoedBan:=GetAdvFVal("SA6","A6_MOEDA",xFilial("SA6")+SEU->EU_BANCO,1,"")   
		aAreaSEU := SEU->(GetArea())
		IF (ALLTRIM(SEU->EU_TIPO))$'11'
			cExtenso:=Extenso( SEU->EU_VALOR,.F.,cMoedBan)
		else
		...
		li++
		IF (ALLTRIM(SEU->EU_TIPO))$'11'
			@li,03 PSAY STR0010+ALLTRIM("Cierre") + " "+ ALLTRIM(SA6->A6_NREDUZ) +"." //"Este valor refere-se a "
		ELSE
			//@li,03 PSAY STR0010+ALLTRIM("Reposición de Banco") + " "+ ALLTRIM(SA6->A6_NREDUZ) +"." //"Este valor refere-se a "
		END
		li+= 5
		@li,03 PSAY __PrtCenter(Replicate("-",Len(SEU->EU_BENEF)))
		@li+1,03 PSAY __PrtCenter("SOLICITADO POR")
		li+= 6
		@li,05 PSAY (Replicate("-",25))
		@li,40 PSAY  (Replicate("-",25))
		@li+1,10 PSAY  ("ELABORADO POR")
		@li+1,45 PSAY  ("AUTORIZADO POR")
		li++
		@li,03 PSAY __PrtCenter(cBenef)
		@li+9,03 PSAY __PrtFatLine()
