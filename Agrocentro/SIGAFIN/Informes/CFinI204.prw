...
User Function CFinI204(cxRecibo,cxSerie) // u_CFinI204()
	...
	if funname() $ "FINA998"
		GraContPert(_cPergCD,cxRecibo,cxSerie)
	...
	// Invoca y establece los parametros en SX1 en el reporte
...
static function f4_ROfC3(cTitulo,bImprimir)
	...
	Local _cxSerie  := MV_PAR03  //SERIE DE COBRO
	Local _xfilial := XFILIAL("SEL")
	Local _xEmpresa:=FWCompany ("CTO")
	Local _csql  := ""
	Local _csql2 := ""
	...
	_csql := "SELECT "  + Chr(13)
	_csql := _csql + " CTO_SIMB, CTO_DESC, M2_MOEDA2,  EL_MOEDA,EL_CONTA,EL_NODIA, " + Chr(13)
	...
	_csql := _csql + " INNER JOIN " + RetSqlName("CTO") + " CTO ON CAST(CTO_MOEDA AS TinyInt) = EL_MOEDA AND CTO.D_E_L_E_T_ = ' ' AND CTO.CTO_FILIAL='" + _xEmpresa + "'" + Chr(13)
	...
	_csql := _csql + "AND  EL_CONTA = A6_NUMCON AND SA6.D_E_L_E_T_ = ' ' AND SA6.A6_FILIAL='" + _xEmpresa + "'" + CHR(13)
	...
	_csql := _csql + " WHERE (EL_TIPODOC IN ('CH ','TF ','EF ','CD ','CC ')" + CHR(13)
	_csql := _csql + " OR EL_TIPODOC IN ('TB ') AND EL_TIPO in ('RA ','NCC')   )" // Nahim aumentando cuando se hace una compensación

	_cCond := " AND SEL.D_E_L_E_T_ = ' '  AND EL_CANCEL='F'"  + Chr(13)
	_cCond := _cCond + " AND EL_RECIBO >= '" + _cRecIni + "' AND EL_RECIBO <= '" + _cRecFin + "'" + Chr(13)
	_cCond := _cCond + " AND EL_FILIAL = '" + _xfilial + "' AND EL_SERIE='"+ _cxSerie + "'"
	...
	_csql2 := _csql2 + " EL_RECIBO, EL_TIPODOC,EL_NODIA, EL_NUMERO,EL_BANCO, EL_SERIE ,EL_PARCELA, EL_TIPO, EL_TXMOEDA, EL_CLIENTE, EL_LOJORIG, " + Chr(13)
	...
	_csql2 := _csql2 + " INNER JOIN " + RetSqlName("CTO") + " CTO ON CAST(CTO_MOEDA AS TinyInt) = EL_MOEDA AND CTO.D_E_L_E_T_ = ' ' AND CTO.CTO_FILIAL='" + _xEmpresa + "'" + Chr(13)
...
//	_csql2 := _csql2 + " E1_PREFIXO + E1_NUM + E1_PARCELA + E1_TIPO = EL_PREFIXO + EL_NUMERO + EL_PARCELA + EL_TIPO AND EL_RECIBO=E1_RECIBO " + CHR(13)nahim quitando recibo 03/05/2019
	// _csql2 := _csql2 + " E1_PREFIXO + E1_PARCELA + E1_TIPO + E1_NUM = EL_PREFIXO + EL_PARCELA + EL_TIPO + EL_NUMERO " + CHR(13)
	_csql2 := _csql2 + " E1_PREFIXO = EL_PREFIXO AND E1_PARCELA = EL_PARCELA AND EL_TIPO = E1_TIPO AND EL_NUMERO = E1_NUM AND E1_CLIENTE = EL_CLIENTE " + CHR(13)
	...
	_csql2 := _csql2 + " AND EL_FILIAL = '" + _xfilial + "' AND EL_SERIE='"+ _cxSerie +"'"
	_csql2 := _csql2 + " AND EL_TIPODOC <> 'CH' " // Nahim no tomar cheque 05/03/2019

	_csql2 := _csql2 + _cCond + " ORDER BY EL_RECIBO, TIPOREC "
	//aviso("",_csql2,{'ok'},,,,,.t.)
	//cUArea2 := 'RCOF3B'
	...
	While !Eof()
		...
		if _cCobDiv == EL_RECIBO .And. !Eof()
			...
			_cxSerie = EL_SERIE
			_dFecha := EL_DTDIGIT // EL_EMISSAO
			_cCobDiv := EL_RECIBO
			_cCorCtb := EL_NODIA
			_nInterLin = _nInterLin - 40
			_nLin := _nLin + _nInterLin
//			oPrn:Say(_nLin, _nCol4, "Cliente: " + Alltrim(A1_COD)+" - " + IIF(!empty(EL_UNOMD),EL_UNOMD,Alltrim(A1_NOME)) ,oFont7)
			oPrn:Say(_nLin, _nCol4, "Cliente: " + Alltrim(A1_COD)+" - " + Alltrim(A1_NOME) ,oFont7)
			//			oPrn:Say(_nLin, _nCol4, "Dirección: " + Alltrim(A1_END),oFont7) Nt
			...
			if !Empty(EL_UOBSTAR)
				...
				// oPrn:Say(_nLin, _nCol4, "Cobrador: " + GetAdvFVal("SAQ","AQ_NOME",xFilial("SAQ")+EL_COBRAD,1,""),oFont7)
				oPrn:Say(_nLin, _nCol4, " Observaciones : " +AllTrim(EL_UOBSTAR) ,oFont7)
				// oPrn:Say(_nLin, _nCol3 + 500, AllTrim(EL_UOBSTAR) ,oFont2)
			endif
			//_nLin := _nLin + _nInterLin
			...
			if alltrim(EL_TIPODOC) != 'TB'// Nahim Adicionado para las compensaciones 03/05/19
				...
				oPrn:Say(_nLin , _nCol3 + 1290,  " Monto ",oFont1)
			...
			Do While EL_RECIBO == _cCobDiv .And. !Eof()
				if alltrim(EL_TIPODOC) != 'TB'// Nahim Adicionado para las compensaciones 03/05/19
					...
					if alltrim(EL_TIPODOC) == "CH" // si es cheque
						oPrn:Say(_nLin , _nCol3 , Tabela( "05", EL_TIPODOC, .F.) +  " (" + AllTrim(EL_NUMERO) +")" + " - " + Alltrim(EL_BANCO) + " / " +  ALLTRIM(EL_AGENCIA) + " / " + ALLTRIM(EL_CONTA) +" - " +  Alltrim(A6_NREDUZ) ,oFont2)
					else // no es cheque
						oPrn:Say(_nLin , _nCol3 , Tabela( "05", EL_TIPODOC, .F.) +  " (" + AllTrim(EL_NUMERO) +")" + " - " + Alltrim(EL_BANCO) + " / " +  ALLTRIM(EL_AGENCIA) + " / " + ALLTRIM(EL_CONTA) +" - " +  Alltrim(A6_NREDUZ) ,oFont2)
					endif

					oPrn:Say(_nLin , _nCol3 + 1290 , Transform(EL_VALOR, "@E 999,999,999.99") ,oFont2)
					oPrn:Say(_nLin , _nCol3 + 1600 , Capital(CTO_SIMB) ,oFont2)
					...
					if !Empty(EL_UOBSTAR)
						_nLin := _nLin + 50
						oPrn:Say(_nLin, _nCol3, " Observaciones : " + 	AllTrim(EL_UOBSTAR) ,oFont1)
						//oPrn:Say(_nLin, _nCol3 + 500, AllTrim(EL_UOBSTAR) ,oFont2)
					end if
		...
		oPrn:Say(nLin1, _nCol1 - 80, "RECIBO No.:" + _cCobDiv + " - " + _cxSerie, oFont4) //EL_RECIBO sería del siguiente...
		oPrn:Say(nLin2, _nCol1-80, "FECHA:" + (dtoC(stod(_dFecha))) ,oFont4)
		oPrn:Say(nLin3, _nCol1 -80, "TC." + AllTrim(CValToChar(_nCambio)) ,oFont4)
		oPrn:Say(nLin4, _nCol1 - 80, "CTB No.:" + _cCorCtb, oFont4) //EL_NODIA sería el Correlativo Contable
		...
		if TIPOREC == 1
			...
			oPrn:Say(_nLin, _nCol3 + 1290, "Valor", oFont1) // nahim 12/04/2019
			oPrn:Say(_nLin, _nCol3 + 1600, "Moneda", oFont1) // nahim 12/04/2019
			_nLin := _nLin + _nInterLin
		endif
		...
		Do While (EL_RECIBO == _cCobDiv2) .And. !Eof() .And. _nFinHoja>_nLin .and. TIPOREC == 1
			...
			ELSE
				if (alltrim(EL_TIPODOC) $ "RA") .or. (alltrim(EL_TIPODOC) $'TB' .and. alltrim(EL_TIPO) $ "RA") // Nahim modificado para incluir Compensaciones ()
					...
					oPrn:Say(_nLin , _nCol3 + 1600 , Capital(CTO_SIMB) ,oFont2)
					_nLin := _nLin + 50 //2*nInterLin
				elseif (alltrim(EL_TIPODOC) $'TB' .and. alltrim(EL_TIPO) $ "NF") // Nahim modificado para incluir Compensaciones ()
					oPrn:Say(_nLin, _nCol3, "Factura: " +ALLTRIM(E1_PREFIXO)+"-"+ Alltrim(E1_NUM) + iif(!empty(E1_PARCELA),"-" + E1_PARCELA,""),oFont2 )
					oPrn:Say(_nLin , _nCol3 + 1290 , Transform(EL_VALOR, "@E 999,999,999.99") ,oFont2)
					oPrn:Say(_nLin , _nCol3 + 1600 , Capital(CTO_SIMB) ,oFont2)
					_nLin := _nLin + 50 //2*nInterLin
				END
		...
		if _aSumas[1][nDolares] > 0
			oPrn:Say(nLin5, _nCol1 - 80, _aSumas[3][nDolares] + Transform(_aSumas[1][nDolares],"@E 9,999,999,999.99") ,oFont4 )
			if _nEspCab != 0
				// oPrn:Say(_nLin + 50 , _nCol4, "Ha cancelado la suma de: " ; Nahim Cambio a Son Sol. por
				oPrn:Say(_nLin + 50 , _nCol4, "SON: " ;
		...
		if _aSumas[1][nBolivianos] == 0 .and.  _aSumas[1][nDolares] == 0 // Nahim Adicionando 0 bs
			oPrn:Say(nLin5 , _nCol1 - 80, _aSumas[3][nBolivianos] ;
				+ Transform(_aSumas[1][nBolivianos],"@E 9,999,999,999.99") ,oFont4 )
			if _nEspCab != 0
				oPrn:Say(_nLin + 95 , _nCol4, "SON: 0 Bs",oFont4)
			end if
		end if
		...
		oPrn:Say(_nLin, _nCol3 ,"______________________",oFont1 )
		_nLin := _nLin + _nInterLin
		oPrn:Say(_nLin, _nCol3 + 65,"	Revisado por ",oFont1 )

		//oPrn:Say(_nLin, _nCol3 + 1345,FWLeUserlg("EL_USERLGA"),oFont1 )
...
Static Function Logo(cTitulo)
	...
	oPrn:Say( nLin4,0080, Capital(AllTrim(SM0->M0_ENDENT)), oFont4)
	...
	if alltrim(EL_TIPODOC) != 'TB'
		oPrn:Say( 0300 ,0780, "Comprobante de Cobros", oFont5)
	else // sólo es comensación
		oPrn:Say( 0300 ,0780,"Comprobante de Compensación", oFont5)
	endif
Return
...
Static Function GraContPert(cPerg,cxRecibo,cxSerie)

	SX1->(DbSetOrder(1))

	If SX1->(DbSeek(alltrim(cPerg)+Space(5)+"01"))
		Reclock("SX1",.F.)
		SX1->X1_CNT01 := altrim(cxRecibo)
		SX1->(MsUnlock())
	End
