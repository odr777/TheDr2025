...
User Function LOCXPE16()
	...
	IF alltrim(FunName()) $ "MATA101N"
		...
		If upper(M->F1_SERIE)=='FAC'
			IF EMPTY(F1_NUMAUT)
				_lRet = .F.
				Alert('Digite el Nro. de Autorización')
				Return _lRet
			END
		END
		...
		For ny := 1 To LEN(aCols)
			If !(upper(M->F1_SERIE)$'CCH|FAR') .AND. (POSICIONE('SF4',1,XFILIAL('SF4')+aCols[ny][nPosTes],'F4_XCCH') == "S") //NO DEJA USAR TES DE CCH
				_lRet = .f.
				Alert('Se esta usando una TES de caja chica')
				Exit
			End

			// F1_RECIBO<>'1' no es un recibo  .AND.   TES es de Retenciones
			If (M->F1_RECIBO<>'1' .AND. POSICIONE('SF4',1,XFILIAL('SF4')+aCols[ny][nPosTes],'F4_XRETEN') $ "2|3")
				_lRet = .f.
				Alert('Para usar TES de retenciones el documento debe ser un recibo.')
				Exit
			ENDIF
		Next ny
