...
Adición del campo F1_UQR - MIT044 Leer datos de la factura mediante QR desarollado para Agrocentro.
@type function
@version  1
...
User function LOCXPE33()
	...
	If nTipo == 60 .or. nTipo = 14 //Remito de Entrada o Conocimiento de Flete
		...
		//AADD(aDetalles,{"F1_XOBS "  ,  .T.      ,.t.      ,.F.   })  // adicionando obligatoriedad de glosa.
		AADD(aDetalles,{"F1_UCORREL"  ,  .T.      ,.F.      ,.T.   })
	...
	If nTipo == 10 .or. nTipo == 13 //Factura de Entrada
		AADD(aDetalles,{"F1_XOBS "  ,  .T.      ,.F.      ,.F.   }) // adicionando obligatoriedad de glosa.
		...
		AADD(aDetalles,{"F1_UPOLIZA"  ,  .T.      ,.F.      ,.F.   })
		//	AADD(aDetalles,{"F1_USRREG "  ,  .T.      ,.F.      ,.T.   })
        aAdd(aCposNF,{NIL,"F1_UQR",NIL,NIL,NIL,"",NIL,NIL,NIL,NIL,NIL,NIL,NIL,NIL,NIL,NIL,".T."})
	End
